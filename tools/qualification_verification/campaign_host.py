...
def install(root, manifest, profile_bytes):
    ...
    enrollment = {
        'schema': 'qualification_campaign_host/v1',
        'host_run_id': run_id,
        'scope': scope,
        'memory_bytes': memory_bytes,
        'profile_sha256': hashlib.sha256(profile_bytes).hexdigest(),
        'rule_path': str(path),
        'rule_sha256': hashlib.sha256(rule).hexdigest(),
    }
...
def restart(root, manifest, interpreter, bootstrap):
    ...
    subprocess.run(
        ['/usr/bin/systemctl', '--system', '--no-ask-password', 'stop', unit],
    ...
    host.run(
        [
            ...
            '--collect',
            '--unit=' + unit,
            '--slice=' + enrollment['scope'],
            '--uid=' + str(manifest['roles']['qexec']),
...
def cleanup(root, manifest, *, retire=False):
    ...
    result = subprocess.run(
        ['/usr/bin/systemctl', '--system', '--no-ask-password', 'stop', scope],
        stdin=subprocess.DEVNULL,
        capture_output=True,
        timeout=30,
        check=False,
    ...
    containers = host.run(
        [*docker, 'ps', '--all', '--quiet', '--no-trunc', '--filter=label=fp.s2.host=' + root.name]
    ).split()
    observations = []
    if containers:
        import sqlite3
        ...
        with sqlite3.connect(
            (root / 'data/journal.sqlite').as_uri() + '?mode=ro', uri=True
        ) as connection:
            enrollments = [
        ...
        for item in enrollments:
            if (
                item['schema'] != 'qualification_campaign_supervision/v1'
                or item['host_run_id'] != root.name
        ...
        image = json.loads((root / 'code/qualification-installation/release.json').read_bytes())[
            'worker_image_digest'
        ]
    ...
    host.save(
        root / 'evidence/campaign-cleanup.json',
        {
            'scope': scope,
            'populated': False,
            'stop_exit': result.returncode,
    ...
    if retire:
        ...
        if rule.exists():
            if (
                hashlib.sha256(host.protected(rule).read_bytes()).hexdigest()
                != enrollment['rule_sha256']
            ):
