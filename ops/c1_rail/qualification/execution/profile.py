...
CAMPAIGN_RESOURCE_SCOPE = MappingProxyType(
    dict(
        ...
        control_calls=3,
        control_cpu_seconds=1,
        control_wall_seconds=10,
        cpu_granularity_seconds=1,
        controller_environment=dict(
...
_DIAGNOSTIC_FIXED = MappingProxyType(
    dict(
        _FIXED,
        schema='qualification_execution_profile/v3',
        protocol_version=3,
        supported_checkpoints=[],
        capability='FULL_E1',
        dispatch_enabled=False,
        diagnostic_work_enabled=True,
...
_DISPATCH_FIXED = MappingProxyType(
    dict(
        _DIAGNOSTIC_FIXED,
        schema='qualification_execution_profile/v5',
        protocol_version=5,
        supported_checkpoints=['N1'],
...
def parse_profile(raw: bytes) -> ExecutionProfile:
    ...
    if doc.get('schema') == 'qualification_execution_profile/v2':
        fixed = dict(
            _FIXED,
            schema='qualification_execution_profile/v2',
            protocol_version=2,
            supported_checkpoints=[],
            capability='FULL_E1',
            dispatch_enabled=False,
    ...
    if doc.get('schema') == 'qualification_execution_profile/v4':
        fixed = dict(
            _DIAGNOSTIC_FIXED, schema='qualification_execution_profile/v4', protocol_version=4
        )
...
def parse_campaign_budget_profile(raw: bytes) -> dict:
    ...
    revised = type(doc) is dict and doc.get('schema') in (
    ...
    fields(
        ...
        | ({'funding_intents'} if funded else set()),
    )
    if doc['schema'] not in (
        'qualification_campaign_budget_profile/v1',
        'qualification_campaign_budget_profile/v2',
        'qualification_campaign_budget_profile/v3',
    ):
...
def diagnostic_budget_profile(profile_bytes):
    ...
    if profile.values['schema'] not in (
    ...
    result = dict(
        ...
        installed_profile_sha256=profile.sha256,
        record_byte_limit=131072,
        phases={
            phase: dict(_DIAGNOSTIC_PHASE, memory_bytes=profile.memory_bytes) for phase in PHASES
        },
