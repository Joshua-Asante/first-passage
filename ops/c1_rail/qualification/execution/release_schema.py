...
def parse_release(raw):
    ...
    identity(doc['release_id']); identity(doc['service_id'])
    ...
    for names in roles.values():
        if type(names) is not list or not names or any(type(name) is not str for name in names) or names != sorted(set(names)):
            raise ValueError('sorted nonempty signing role required')
        for name in names: identity(name)
        if enrolled.intersection(names): raise ValueError('signing role ID separation required')
        enrolled.update(names)
    fingerprints = fields(doc['trusted_key_sha256'], enrolled)
    for value in fingerprints.values(): digest(value)
    seen = set()
    for names in roles.values():
        current = {fingerprints[name] for name in names}
        if seen.intersection(current): raise ValueError('signing public key separation required')
        seen.update(current)
    ...
    for role,row in ordinary.items():
        ...
        _source_path(row['path']); digest(row['sha256'])
        if name in by_name or row['path'] in paths: raise ValueError('source inventory alias forbidden')
    ...
    for runtime in runtimes.values():
        ...
        for name,row in runtime['sources'].items():
            if by_name.get(name) != row: raise ValueError('runtime source differs from inventory')
        covered.update(runtime['sources'])
    if covered != set(by_name): raise ValueError('ordinary source absent from process closures')
    return doc
