...
def outcome_record(row):
    ...
    return dict(status=row.status, sessions_to_pass=row.sessions_to_pass,
                failure_reason=row.failure_reason, diagnostics=[list(pair) for pair in row.diagnostics])
...
def path_inventory(plan, populations):
    ...
    for population in populations:
        for index, row in enumerate(population['outcomes']):
            ...
            if seed is None or (seed['population'], seed['path_index']) != (population['population'], index):
                raise ValueError('worker depth/order differs from plan')
            records.append(dict(stage='N1', population=population['population'], path_index=index,
                panel_id=None, seed_input_sha256=sha256(encoded(seed)), outcome_sha256=sha256(encoded(row))))
    if next(seeds, None) is not None:
        raise ValueError('worker depth differs from plan')
    return dict(schema='qualification_path_inventory/v1', trust_domain_sha256=plan['trust_domain_sha256'], records=records)
...
def encode_worker_result(context, execution_id, plan_bytes, run, observations, *, admitted):
    ...
    populations = [dict(population=name, outcomes=[outcome_record(row) for row in rows])
                   for name, rows in run.populations]
    return encoded(dict(schema='qualification_worker_result/v1', execution_id=execution_id,
        plan_sha256=sha256(plan_bytes), source_admission=json.loads(admitted.source_admission_bytes),
        legality=json.loads(admitted.legality_bytes), populations=populations,
        runtime_load_manifest=[dict(role=role,sha256=digest) for role,_,digest in sorted(admitted.source.prepared.load_trace)],
...
def parse_worker_result(raw, *, context, execution_id, plan_bytes, campaign_limits=None):
    ...
    doc = fields(parse_canonical_json(raw, label='worker result'), {'schema', 'execution_id', 'plan_sha256',
        'source_admission','legality','runtime_load_manifest','populations', 'path_inventory', 'observations'})
    if (doc['schema'] != 'qualification_worker_result/v1' or doc['execution_id'] != execution_id
    ...
    admission = parse_source_admission(admission_bytes,contract_sha256=context.contract.contract_sha256,
        domain_sha256=context.domain.sha256,policy=context.policy)
    expected_roles = [dict(role=row.role,sha256=row.sha256) for row in sorted(context.contract.artifacts,key=lambda row:row.role)]
    if (admission['retained_roles'] != expected_roles
            ...
            or admission['population_sha256'] != {pop:sha256(encoded(list(context.contract.populations[pop]))) for pop in ('FULL','H1','H2')}
    ...
    expected_legality = build_legality_record(contract_sha256=context.contract.contract_sha256,domain_sha256=context.domain.sha256,
        policy=context.policy,geometry_bytes=context.retained_bytes[geometry],
        expected_geometry_sha256=next(row.sha256 for row in context.contract.artifacts if row.role==geometry),source_admission_bytes=admission_bytes)
    ...
    for population, expected in zip(doc['populations'], plan['depths']):
        ...
        if (population['population'] != expected['population'] or type(rows) is not list
        ...
        for row in rows:
            ...
            if (type(row['diagnostics']) is not list or any(type(pair) is not list or len(pair) != 2
                    or any(type(value) is not str for value in pair) for pair in row['diagnostics'])):
                raise ValueError('outcome string diagnostic pairs required')
            outcome = PathOutcome(row['status'], row['sessions_to_pass'], row['failure_reason'],
                                  tuple(tuple(pair) for pair in row['diagnostics']))
            if outcome.status == 'PASS' and outcome.sessions_to_pass > context.contract.replay.horizon_sessions:
    ...
    observations = fields(doc['observations'], {'worker_compute_wall_ns', 'worker_cpu_ns', 'worker_peak_memory_bytes'})
