...
from .campaign_funding import (
    FundingStoreMixin,
    SCHEMA as FUNDING_SCHEMA,
    PROFILE as FUNDED_PROFILE,
    _put as _put_funding,
)
...
TERMINAL_BODY_REFUSED = (
    'cancellation body already refused on the terminal campaign; no unmetered retry'
)
...
def void_authentication_bound():
    ...
    return {
        'cpu_ns': (policy['control_cpu_seconds'] + policy['cpu_granularity_seconds']) * 10**9,
...
def parse_void_authentication(raw):
    ...
    doc = fields(
        parse_canonical_json(raw, label='cancellation charge'),
    ...
    digest(doc['request_sha256'])
    integer(doc['sequence'], positive=True)
...
def parse_void_refusal(raw, *, schema=VOID_REFUSAL_SCHEMA):
    ...
    doc = fields(
        parse_canonical_json(raw, label='cancellation refusal'),
...
from ..evidence import (
    CHECKPOINT_RESULT_SCHEMA,
    CHECKPOINT_ATTESTATION_SCHEMA,
    CHECKPOINT_ASSESSMENT_SCHEMA,
    CHECKPOINT_RECEIPT_SCHEMA,
    CHECKPOINT_CUTOFF_SCHEMA,
    CHECKPOINT_INTENT_SCHEMA,
    parse_checkpoint_result,
    parse_checkpoint_attestation,
...
class CheckpointStoreMixin:
    ...
    def checkpoint_capture(self, attempt_id):
        ...
        with self.store.transaction() as connection:
            ...
            return {
                'work_id': row[0],
                'result_bytes': bytes(row[1]),
                'payload_bytes': bytes(row[2]),
        ...
        with self.store.transaction() as connection:
            ...
            for role, raw in sorted(self.objects(attempt_id).items()):
                if role.startswith('context_') or role == 'bundle_index':
                    members.append(
                        {'role': 'retained_' + role, 'sha256': sha256(raw), 'byte_length': len(raw)}
                    )
            ...
            if intent is not None:
                raw = bytes(intent[0])
                members.append(
                    {'role': 'candidate', 'sha256': sha256(raw), 'byte_length': len(raw)}
                )
            return members
    ...
    def fetch_checkpoint_member(self, attempt_id, object_sha256, offset, length):
        ...
        with self.store.transaction() as connection:
            ...
            raw = next(
                (value for value in sources.values() if sha256(value) == object_sha256), None
            )
            if raw is None:
    ...
    def checkpoint_snapshot(self, attempt_id, checkpoint='N1'):
        ...
        with self.store.transaction() as connection:
            ...
            return encode_campaign_checkpoint_snapshot(
                ...
                validity=self.row(attempt_id)['validity'],
    ...
    def _checkpoint_integrity(self, connection):
        ...
        for row in connection.execute(
            'SELECT attempt_id,checkpoint,work_id,result_bytes,payload_bytes,attestation_bytes '
            ...
            if (
                family is None
                or family['work_id'] != row[2]
                or family['payload_sha256'] != sha256(bytes(row[4]))
                or (
                    row[5] is not None and family.get('attestation_sha256') != sha256(bytes(row[5]))
                )
            ):
            ...
            if row[5] is not None:
                ...
                raise ValueError('checkpoint family state differs')
        for row in connection.execute(
            'SELECT attempt_id,checkpoint,work_id,snapshot_bytes,intent_bytes,candidate_bytes,cutoff_bytes,receipt_bytes '
            'FROM full_campaign_checkpoint_intents'
        ):
            attempt = row[0]
            state = self._budget(connection, attempt)
            family = (state.get('checkpoints') or {}).get(row[1])
            if family is None or family.get('assessment_sha256') != (
                None if row[7] is None else sha256(bytes(row[5]))
            ):
    ...
    def persist_checkpoint_intent(
        ...
        intent = fields(
            parse_canonical_json(intent_bytes, label='checkpoint signing intent'),
            {
                'schema',
                'attempt_id',
                'checkpoint',
        ...
        with self.store.transaction() as connection:
            ...
            if prior is not None:
                ...
                return self._negative_budget_response(
                    connection, self._budget(connection, attempt_id)
                )
            state = self._budget(connection, attempt_id)
            work = self._work(state, work_id)
            if work['phase'] != 'N1_G5':
            ...
            for raw, expected in (
                (capture_transition_bytes, 'CAPTURED'),
                (signing_transition_bytes, 'SIGNING_INTENT'),
            ):
    ...
    def commit_checkpoint_assessment(
        ...
        with self.store.transaction() as connection:
            ...
            if row[5] is not None:
                ...
                return encoded(
                    {
                        'receipt': parse_canonical_json(bytes(row[5]), label='receipt'),
                        'validity': self.row(attempt_id)['validity'],
                        'historical': True,
                    }
                )
            ...
            if (
                candidate['snapshot']['snapshot_sha256'] != sha256(bytes(row[1]))
                or candidate['snapshot']['campaign_revision']
                != persisted_snapshot['campaign_revision']
            ):
            ...
            while head != persisted_snapshot['event_head']:
                ...
                if (
                    interim['kind'] not in ('CAPTURED', 'SIGNING_INTENT', 'CHECKPOINT_INTENT')
                    and interim['authority']
                ):
                    raise ValueError('assessment snapshot identity differs')
                head = event[1]
            if (
                candidate['capture']['result_sha256'] != family.get('result_sha256')
            ...
            signed_transition = encoded(
                {
                    ...
                    'work_id': work_id,
                    'state': 'SIGNED',
                    'clock': parse_clock(clock_bytes),
                    'data': {'candidate_bytes_b64': self._b64(receipt)},
                }
            )
            ...
            return encoded(
                {
                    'receipt': parse_canonical_json(receipt, label='receipt'),
                    'validity': self.row(attempt_id)['validity'],
                    'historical': False,
                }
            )
...
class CampaignStore(FundingStoreMixin, CheckpointStoreMixin):
    ...
    def row(self, attempt):
        with self.store.transaction() as connection:
            row = connection.execute(
                'SELECT * FROM full_campaigns WHERE attempt_id=?', (attempt,)
            ).fetchone()
            if row is None:
    ...
    def status(self, attempt):
        ...
        if (
            parse_canonical_json(row['receipt_bytes'], label='receipt')['schema']
            == 'qualification_campaign_provisional_intent/v1'
        ):
            raise ValueError('private provisional intent is not dormant admission')
        return {
            'schema': 'qualification_campaign_status/v1',
    ...
    def admit(self, request_bytes, context, plan, *, now):
        ...
        with self.store.transaction() as connection:
            ...
            if connection.execute(
                'SELECT 1 FROM campaigns WHERE attempt_id=?', (attempt,)
            ).fetchone():
            ...
            if (
                ...
                or doc['source_bundle_sha256'] != context.bundle_sha256
            ...
            connection.execute(
                'INSERT INTO full_campaigns VALUES(?,?,?,?,?,?,?)',
                (attempt, campaign_id, request_bytes, receipt, 'VALID', None, None),
            )
            objects = {
                'plan': plan,
                'bundle_index': context.retained_bundle_index,
    ...
    def claim_supervision_control(
        self, attempt, work_id, slot, clock_bytes, *, recovery_owner_token=None
    ):
        ...
        with self.store.transaction() as connection:
            ...
            if spent:
                if slot == 'RECOVERY_OWNER':
                    recovery = next(
                        (r for r in state.get('recoveries', ()) if r['work_id'] == work_id), None
                    )
                    if (
                        recovery is not None
                        and recovery['completion_bytes_b64'] is not None
                        and not recovery['continuation_required']
                    ):
                        before = state['state']
                ...
                if slot != 'RECOVERY_OWNER':
                    ...
                    token = (
                        ...
                        if recovery_owner_token is None
                        else recovery_owner_token
                    )
                    ...
                    state['schema'] = (
                        ...
                        if state['profile']['schema'] == 'qualification_campaign_budget_profile/v3'
                        else 'qualification_campaign_budget_snapshot/v4'
                    )
                    state.setdefault('dispatches', [])
                    state.setdefault('recoveries', []).append(
                        {
                            'work_id': work_id,
                            'claim_sha256': sha256(raw),
                            'owner_sha256': sha256(token),
                            'observations_bytes_b64': None,
                            'completion_bytes_b64': None,
    ...
    def _dispatch_clock(self, connection, state, work, clock_source):
        ...
        reservation = parse_canonical_json(
            self._raw(work['reservation_bytes_b64']), label='reservation'
        )
        deadline = min(
    ...
    def launch_gate(self, attempt_id, work_id, clock_source, *, role='guardian'):
        ...
        with self.store.transaction() as connection:
            ...
            if any(
                r['work_id'] == work_id and r['role'] == role for r in state.get('dispatches', ())
            ):
                ...
                state.setdefault('dispatches', []).append(
                    {
                        ...
                        'role': role,
                        'owner_sha256': owner,
                        'started_clock': current,
                        'acknowledged_clock': None,
                    }
                )
        ...
        with self.store.transaction() as connection:
            ...
            row = next(
                (
                    ...
                    if r['work_id'] == work_id and r['role'] == role
                ),
                None,
            )
            ...
            return self._save_budget(
                connection, state, 'DISPATCH_ACKNOWLEDGED', authority=state['state'] != before
            )
    ...
    def _recovery_pending(self, state):
        ...
        with self.store.transaction() as connection:
            ...
            rows = connection.execute(
                "SELECT body,sha256 FROM full_campaign_objects WHERE attempt_id=? AND role GLOB 'supervision_control_*'",
                (state['attempt_id'],),
            )
    ...
    def complete_recovery(self, attempt_id, work_id, completion_bytes, *, recovery_owner_token):
        ...
        with self.store.transaction() as connection:
            ...
            if row['completion_bytes_b64'] is not None:
                ...
                return self._negative_budget_response(connection, state)
            if (
                row['observations_bytes_b64'] is None
                or doc['claim_sha256'] != row['claim_sha256']
            ...
            saved = connection.execute(
                ...
                (attempt_id, 'supervision_event_' + doc['cleanup_event_sha256']),
            ).fetchone()
            if saved is None:
                raise ValueError('durable cleanup absence required')
            cleanup = parse_supervision_event(bytes(saved[0]))
            if (
                cleanup['work_id'] != work_id
                or cleanup['attempt_id'] != attempt_id
                or cleanup['kind'] != 'CLEANUP'
                or cleanup['data']['status'] != 'ABSENT'
                or cleanup['clock'] != doc['clock']
            ):
    ...
    def retain_supervision_event(self, raw):
        ...
        with self.store.transaction() as connection:
            ...
            connection.execute(
                'INSERT OR IGNORE INTO full_campaign_objects VALUES(?,?,?,?,?)',
    ...
    def _payload_identity(self, connection, attempt, work_id):
        ...
        for (raw,) in connection.execute(
            ...
            if (
                type(event) is dict
                and event.get('kind') == 'PROCESS'
                and event.get('work_id') == work_id
                and type(event.get('data')) is dict
                and type(event['data'].get('cgroup')) is str
    ...
    def queue_diagnostic_void(self, raw):
        ...
        if (
            request['schema'] != 'qualification_campaign_request/v2'
            or request['operation'] != 'VOID'
        ):
        ...
        with self.store.transaction() as connection:
            ...
            if (
                ...
                or len(raw) > state['profile']['record_byte_limit']
            ):
                raise ValueError('bounded metered cancellation required')
            prior = connection.execute(
                "SELECT body FROM full_campaign_objects WHERE attempt_id=? AND role='pending_void'",
                (attempt,),
            ).fetchone()
            ...
            if prior is None:
                ...
                if connection.execute(
                    'SELECT 1 FROM full_campaign_objects WHERE attempt_id=? AND role=?',
                    (attempt, VOID_TERMINAL_REFUSAL_PREFIX + sha256(raw)),
                ).fetchone():
    ...
    def _cancellation_pending(self, connection, attempt):
        ...
        if (
            connection.execute(
                "SELECT 1 FROM full_campaigns WHERE attempt_id=? AND validity='VALID'", (attempt,)
            ).fetchone()
            is None
        ):
            return False
        return (
            connection.execute(
                "SELECT 1 FROM full_campaign_objects WHERE attempt_id=? AND role='pending_void'",
                (attempt,),
            ).fetchone()
            is not None
        )
    ...
    def _cancellation_barrier(self, connection, attempt, *, admitted_only):
        ...
        """
        if self._cancellation_pending(connection, attempt) and (
            not admitted_only or self._admitted(connection, attempt)
        ):
            raise ValueError(CANCELLATION_PENDING)

    def _void_authentications(self, connection, attempt):
        rows = connection.execute(
            'SELECT role, body FROM full_campaign_objects WHERE attempt_id=? AND role GLOB ? ORDER BY role',
            (attempt, VOID_AUTHENTICATION_PREFIX + '*'),
        )
        result = []
        for role, raw in rows:
            doc = parse_void_authentication(bytes(raw))
            if doc['attempt_id'] != attempt or role != _void_role(
                VOID_AUTHENTICATION_PREFIX, doc['sequence']
            ):
    ...
    def _void_refusals(self, connection, attempt):
        ...
        for role, raw in rows:
            ...
            if doc['attempt_id'] != attempt or role != _void_role(
                VOID_REFUSAL_PREFIX, doc['sequence']
            ):
    ...
    def _void_admission_refusals(self, connection, attempt):
        ...
        for role, raw in rows:
            ...
            if doc['attempt_id'] != attempt or role != _void_role(
                VOID_ADMISSION_REFUSAL_PREFIX, doc['sequence']
            ):
    ...
    def _void_terminal_refusals(self, connection, attempt):
        ...
        for role, raw in rows:
            ...
            if (
                doc['attempt_id'] != attempt
                or role != VOID_TERMINAL_REFUSAL_PREFIX + doc['request_sha256']
            ):
    ...
    def _refuse_admission_void(self, connection, attempt, request_bytes, reason, clock_doc):
        ...
        refusal = encoded(
            {
                'schema': VOID_ADMISSION_REFUSAL_SCHEMA,
                'attempt_id': attempt,
                'sequence': sequence,
                'request_sha256': sha256(request_bytes),
                'reason': (reason or 'refused')[:1024],
                'clock': clock_doc,
            }
        )
        ...
        connection.execute(
            ...
            (
                attempt,
                _void_role(VOID_ADMISSION_REFUSAL_PREFIX, sequence),
                sha256(refusal),
                len(refusal),
                refusal,
            ),
        ...
        connection.execute(
            "DELETE FROM full_campaign_objects WHERE attempt_id=? AND role='pending_void' AND body=?",
    ...
    def claim_void_authentication(self, request_bytes, clock_bytes):
        ...
        with self.store.transaction() as c:
            ...
            if not self._admitted(c, attempt):
                raise ValueError(
                    'post-admission cancellation authentication requires a retained receipt'
                )
            ...
            if (
                row['validity'] != 'VALID'
                or doc is None
                or doc['bootstrap_pending_work_id'] is not None
            ...
            if doc['state'] == 'BOUND' and doc['terminal_overlay'] is None:
                ...
                record = encoded(
                    {
                        'schema': VOID_AUTHENTICATION_SCHEMA,
                        'attempt_id': attempt,
                        'sequence': sequence,
                        'request_sha256': sha256(request_bytes),
                        'charge_cpu_ns': bound['cpu_ns'],
                        'wall_ns': bound['wall_ns'],
                        'clock': observed,
                    }
                )
                ...
                c.execute(
                    ...
                    (
                        attempt,
                        _void_role(VOID_AUTHENTICATION_PREFIX, sequence),
                        sha256(record),
                        len(record),
                        record,
                    ),
                ...
                doc.update(
                    settled_cpu_ns=doc['settled_cpu_ns'] + bound['cpu_ns'],
    ...
    def _charged_attempt(self, connection, attempt, sequence, request_bytes):
        claim = next(
            (
                r
                for r in self._void_authentications(connection, attempt)
    ...
    def refuse_void_authentication(self, request_bytes, sequence, reason, clock_bytes):
        ...
        with self.store.transaction() as c:
            if sequence == 0:
                ...
                refusal = encoded(
                    {
                        'schema': VOID_TERMINAL_REFUSAL_SCHEMA,
                        'attempt_id': attempt,
                ...
                c.execute(
                    ...
                    (
                        attempt,
                        VOID_TERMINAL_REFUSAL_PREFIX + sha256(request_bytes),
                        sha256(refusal),
                        len(refusal),
                        refusal,
                    ),
            ...
            c.execute(
                ...
                (
                    attempt,
                    _void_role(VOID_REFUSAL_PREFIX, sequence),
                    sha256(refusal),
                    len(refusal),
                    refusal,
                ),
    ...
    def inspect_unstarted_admission(self, attempt, clock_source):
        ...
        with self.store.transaction() as c:
            ...
            compact = (
                c.execute(
                    "SELECT substr(body,1,2049) FROM full_campaign_funding_works WHERE attempt_id=? AND work_id='admission'",
                    (attempt,),
                ).fetchone()
                if self._funding(c, attempt) is not None
                else None
            )
            ...
            if (
                ...
                return None
            roles = ['supervision_admission'] + [
                'supervision_control_' + sha256(encoded(['admission', slot]))
                for slot in ('START_OWNER', 'START_CLIENT', 'RECOVERY_OWNER')
            ]
            if c.execute(
                'SELECT 1 FROM full_campaign_objects WHERE attempt_id=? AND role IN (?,?,?,?)',
                (attempt, *roles),
            ).fetchone():
                return None
            ...
            resumable = (
                row['validity'] == 'VALID'
                and state['state'] == 'PROVISIONAL'
                and not expired
                and (
                    funding is None
                    or (
                        funding['bootstrap_pending_work_id'] is None
                        and funding['terminal_overlay'] is None
                    )
                )
                and not self._recovery_pending(state)
                and not dispatch_pending(state)
            ...
            return {
                ...
                and state['state'] in ('PROVISIONAL', 'BOUND'),
                'clock_bytes': clock_bytes,
                'reservation_bytes': self._raw(work['reservation_bytes_b64']),
            }
    ...
    def diagnostic_status(self, attempt):
        ...
        with self.store.transaction() as connection:
            row = self.row(attempt)
            saved = connection.execute(
                "SELECT body FROM full_campaign_objects WHERE attempt_id=? AND role='diagnostic_receipt'",
                (attempt,),
            ).fetchone()
            ...
            refusals = self._void_refusals(connection, attempt) or self._void_admission_refusals(
                connection, attempt
            )
            cancellation = {
                'void_pending': self._cancellation_pending(connection, attempt),
                'void_authentication_attempts': len(
                    self._void_authentications(connection, attempt)
                ),
                'void_refusal': None if not refusals else refusals[-1]['reason'],
            }
            if funding is not None:
                return dict(
                    schema='qualification_campaign_status/v2',
                    validity=funding['validity'],
                    state=(
                        'METERED_INSPECTION_REQUIRED'
                        if funding['bootstrap_pending_work_id'] is not None
                        or funding['terminal_overlay'] is not None
                        else funding['state']
                    ),
                    receipt=(
                        None
                        if saved is None
                        else parse_canonical_json(bytes(saved[0]), label='diagnostic receipt')
                    ),
                    settled_cpu_ns=funding['settled_cpu_ns'],
                    reserved_cpu_ns=funding['reserved_cpu_ns'],
                    remaining_cpu_ns=funding['remaining_cpu_ns'],
                    historical=True,
            ...
            budget = connection.execute(
                'SELECT substr(snapshot_bytes,1,?) FROM full_campaign_budgets WHERE attempt_id=?',
                (PLAN_CHUNK_LIMIT + 1, attempt),
            ).fetchone()
            if budget is None:
                raise ValueError('no diagnostic metered history')
            state = (
                parse_canonical_json(bytes(budget[0]), label='budget projection')
                if len(budget[0]) <= PLAN_CHUNK_LIMIT
                else {}
            )
            return dict(
                schema='qualification_campaign_status/v2',
                ...
                settled_cpu_ns=state.get('settled_cpu_ns'),
                reserved_cpu_ns=state.get('reserved_cpu_ns'),
                remaining_cpu_ns=state.get('remaining_cpu_ns'),
                historical=True,
                current_policy_eligible=False,
                dispatch_enabled=False,
                **cancellation,
    ...
    def finish_diagnostic_admission(
        self, request_bytes, context, plan, observations_bytes, *, now, trusted_keys=None
    ):
        ...
        if (
            request['schema'] != 'qualification_campaign_request/v2'
            or context.release.document['schema']
            not in (
            ...
            or document['budget']
            != parse_canonical_json(context.contract.canonical_bytes, label='frozen contract')[
                'replay'
            ]['budget']
        ...
        with self.store.transaction() as connection:
            ...
            state = self._budget(connection, attempt)
            if (
                state['profile'] != context.release.document['campaign_budget_profile']
            ...
            if pending is not None and owner['validity'] == 'VALID':
                ...
                domain = parse_canonical_json(
                    context.domain.canonical_bytes, label='enrolled domain'
                )
                ...
                try:
                    approval = verify_detached_approval(
                        decode_base64(cancel['operator_approval_bytes']),
                        trusted_keys=trusted_keys,
                        expected_scope='VOID_QUALIFICATION_ATTEMPT',
                        ...
                        expected_contract_sha256=context.contract.contract_sha256,
                        now=now,
                        allow_test_authority=True,
                    )
                    if (
                        approval.authority_class != 'TEST_ONLY'
                        or approval.key_id not in domain['freeze_key_ids']
                        or sha256(trusted_keys[approval.key_id].public_key)
                        != domain['trusted_key_sha256'][approval.key_id]
                    ):
                        raise ValueError('pending cancellation authority is not enrolled')
                except (ValueError, KeyError) as refusal:
                    self._refuse_admission_void(
                        connection,
                        attempt,
                        cancel_raw,
                        str(refusal)[:1024],
            ...
            if state['state'] == 'PROVISIONAL' and owner['validity'] == 'VALID':
                self.bind_budget(
                    attempt,
                    encoded(document['budget']),
                    expected_revision=state['authority_revision'],
                )
            state = parse_canonical_json(
                self.settle_work(attempt, 'admission', observations_bytes),
                label='settled admission',
            )
            ...
            receipt = encoded(
                {
                    'schema': 'qualification_campaign_admission_receipt/v2',
                    'campaign_id': owner['campaign_id'],
                    'attempt_id': attempt,
                    'state': 'ADMITTED_DIAGNOSTIC',
                    'authority_class': 'TEST_ONLY',
                    'dispatch_enabled': False,
                    'request_sha256': sha256(request_bytes),
                    'bundle_sha256': context.bundle_sha256,
                    ...
                    'profile_sha256': context.profile.sha256,
                    'policy_sha256': context.policy.sha256,
                    'budget_sha256': sha256(encoded(document['budget'])),
                    'budget_profile_sha256': sha256(encoded(state['profile'])),
            ...
            connection.execute(
                'UPDATE full_campaigns SET receipt_bytes=? WHERE attempt_id=?', (receipt, attempt)
            )
            return self.diagnostic_status(attempt)
    ...
    def context(self, attempt, installed_release, keys, *, now):
        ...
        retained = {
            role.removeprefix('context_'): raw
            for role, raw in objects.items()
            if role.startswith('context_')
        }
        context = verify_retained_bundle(
            objects['bundle_index'], retained, installed_release, keys, now
        )
    ...
    def chunk(self, request):
        with self.store.transaction() as connection:
            ...
            plan = connection.execute(
                "SELECT sha256,byte_length,substr(body,?,?) AS chunk FROM full_campaign_objects WHERE attempt_id=? AND role='plan'",
                (request['offset'] + 1, request['length'], request['attempt_id']),
            ).fetchone()
            if plan is None:
            ...
            return encoded(
                {
                    'schema': 'qualification_campaign_plan_chunk/v1',
                    'attempt_id': row['attempt_id'],
                    'object_sha256': plan['sha256'],
                    'offset': offset,
    ...
    def void(self, request_bytes, *, now):
        ...
        with self.store.transaction() as connection:
            ...
            receipt = encoded(
                {
                    'schema': 'qualification_campaign_void_receipt/v1',
                    'campaign_id': row['campaign_id'],
                    'attempt_id': row['attempt_id'],
                    'validity': 'VOID',
                    'request_sha256': sha256(request_bytes),
                    'voided_at_utc': instant(now),
                }
            )
            connection.execute(
    ...
    def _totals(state):
        ...
        settled = [
            w['charge_cpu_ns'] for w in state['works'] if w['observation_bytes_b64'] is not None
        ]
        reserved = [
            w['limits']['cpu_ns'] for w in state['works'] if w['observation_bytes_b64'] is None
        ]
        cap = (
            state['budget']['maximum_cpu_seconds'] * 10**9
            if state['budget'] is not None
            else state['profile']['phases']['ADMISSION']['cpu_ns']
        )
        state.update(
            settled_cpu_ns=sum(settled),
            reserved_cpu_ns=sum(reserved),
    ...
    def _save_budget(self, connection, state, kind, *, authority, funding_transfer=None):
        ...
        body = encoded({'kind': kind, 'authority': authority, 'snapshot': state})
        ...
        connection.execute(
            'INSERT INTO full_campaign_budgets VALUES(?,?) ON CONFLICT(attempt_id) DO UPDATE SET snapshot_bytes=excluded.snapshot_bytes',
            (state['attempt_id'], raw),
        )
        connection.execute(
            'INSERT INTO full_campaign_budget_events VALUES(?,?,?,?,?)',
            (state['attempt_id'], state['accounting_revision'], body, previous, head),
        )
        self._project_funding(
            connection, state, enroll=kind == 'BEGIN_ADMISSION', transfer=funding_transfer
        )
        return self._negative_budget_response(connection, state)
        ...
        with self.store.transaction() as connection:
            ...
            if existing is not None:
                ...
                if (
                    ...
                    or encoded(state['profile']) != profile_bytes
            ...
            if (
                profile['schema'] == 'qualification_campaign_budget_profile/v3'
            ...
            receipt = encoded(
                {
                    'schema': 'qualification_campaign_provisional_intent/v1',
                    'attempt_id': attempt,
                    'request_sha256': sha256(request_bytes),
            ...
            reservation = encoded(
                {'limits': limits, 'clock': started, 'input_sha256': sha256(request_bytes)}
            )
            work = {
                'work_id': 'admission',
                'phase': 'ADMISSION',
                'limits': limits,
                'input_sha256': sha256(request_bytes),
                'reservation_bytes_b64': self._b64(reservation),
                'state': 'RESERVED',
                'transitions': [],
            ...
            state = {
                'schema': 'qualification_campaign_budget_snapshot/' + snapshot_version,
                'attempt_id': attempt,
                'request_sha256': sha256(request_bytes),
                'profile': profile,
                'budget': None,
            ...
            if profile['schema'] == 'qualification_campaign_budget_profile/v3':
                state.update(
                    schema='qualification_campaign_budget_snapshot/v5', recoveries=[], dispatches=[]
                )
            return self._save_budget(connection, state, 'BEGIN_ADMISSION', authority=True)

    def bind_budget(
        self, attempt_id, contract_budget_bytes, *, expected_revision, clock_bytes=None
    ) -> bytes:
        ...
        with self.store.transaction() as connection:
            state = self._budget(connection, attempt_id)
            if fresh_clock is not None and state['profile']['schema'] not in (
                'qualification_campaign_budget_profile/v2',
                'qualification_campaign_budget_profile/v3',
            ):
            ...
            if (
                sum(p['cpu_ns'] for p in ceilings) > cap
                or sum(p['wall_ns'] for p in ceilings) > binding['maximum_wall_seconds'] * 10**9
                or max(p['memory_bytes'] for p in ceilings) > binding['maximum_memory_bytes']
            ):
                self._terminal(state, 'BUDGET_EXHAUSTED')
            state['budget'] = binding
            state['deadline_boottime_ns'] = integer(
                state['start_clock']['boottime_ns'] + binding['maximum_wall_seconds'] * 10**9
            )
            ...
            if (
                state['settled_cpu_ns']
                + state['reserved_cpu_ns']
                + self.void_authentication_charge(connection, attempt_id)
                > cap
            ):
                self._terminal(state, 'BUDGET_EXHAUSTED')
    ...
    def _retry_parent(cls, work):
        reservation = parse_canonical_json(
            cls._raw(work['reservation_bytes_b64']), label='reservation'
        )
    ...
    def _open_retries(cls, state, parent_id):
        return [
            ...
            if cls._retry_parent(w) == parent_id
            and (w['state'] not in ('COMPLETED', 'ABORTED') or w['observation_bytes_b64'] is None)
        ]
    ...
    def _signing_intent(cls, work):
        return next(
            (
                t
                for raw in work['transitions']
                if (t := parse_canonical_json(cls._raw(raw), label='transition'))['state']
                == 'SIGNING_INTENT'
            ),
            None,
        )
        ...
        with self.store.transaction() as connection:
            ...
            if existing is not None:
                if (
                    ...
                    or self._raw(existing['reservation_bytes_b64']) != limits_bytes
                ):
            ...
            if retry_of is not None:
                ...
                if (
                    parent['state'] != 'SIGNING_INTENT'
                    or parent['observation_bytes_b64'] is None
                    or self._retry_parent(parent) is not None
                    or phase != parent['phase']
                    or phase in ('N1', 'N2', 'PART_A')
                    or intent is None
                    or specification['input_sha256']
                    != sha256(self._raw(intent['data']['payload_bytes_b64']))
                    or self._open_retries(state, retry_of)
                ):
                    raise ValueError(
                        'signing retry requires a settled fixed intent and no open retry'
                    )
            if (
                retry_of is None
                and phase in ('ADMISSION', 'N1', 'N2', 'PART_A')
                and any(w['phase'] == phase for w in state['works'])
            ):
            ...
            if state['state'] == 'BOUND':
                ...
                if (
                    ...
                    and state['schema'] == 'qualification_campaign_budget_snapshot/v1'
                ):
                ...
                state['works'].append(
                    {
                        ...
                        'reservation_bytes_b64': self._b64(limits_bytes),
                        'state': 'RESERVED',
                        'transitions': [],
                        'observation_bytes_b64': None,
                        'charge_cpu_ns': 0,
    ...
    def _observation(self, state, work, raw):
        ...
        intents = [
            parse_canonical_json(self._raw(t), label='transition') for t in work['transitions']
        ]
        start = next((t for t in intents if t['state'] == 'START_INTENT'), None)
        if start is None or any(
            doc[k] != start['data'][k] for k in ('campaign_scope_id', 'work_scope_id')
        ):
    ...
    def settle_work(self, attempt_id, work_id, observations_bytes) -> bytes:
        with self.store.transaction() as connection:
            ...
            intent = next(
                ...
                if parse_canonical_json(self._raw(t), label='start')['state'] == 'START_INTENT'
            )
            if (
                doc['clock']['boottime_ns'] is not None
                and doc['clock']['boot_id'] == state['start_clock']['boot_id']
    ...
    def record_work_transition(
        self, attempt_id, work_id, transition_bytes, *, expected_revision
    ) -> bytes:
        ...
        with self.store.transaction() as connection:
            ...
            if retry_of is not None:
                parent = self._work(state, retry_of)
                if parent['state'] != 'SIGNING_INTENT' or target not in (
                    'START_INTENT',
                    'RUNNING',
                    'COMPLETED',
                ):
            ...
            if target in ('SIGNED', 'COMPLETED') and self._open_retries(state, work_id):
                raise ValueError(
                    'signing retry must finish and settle before candidate finalization'
                )
            if any(
                w['work_id'] not in (work_id, retry_of)
                and w['state'] in ('SIGNING_INTENT', 'SIGNED')
                for w in state['works']
            ):
                raise ValueError('authority-changing work is serialized during signing')
            self._observe_clock(state, doc['clock'])
            ...
            if (
                intent is not None
                and work['observation_bytes_b64'] is None
                and doc['clock']['boottime_ns'] is not None
                and doc['clock']['boot_id'] == state['start_clock']['boot_id']
                and doc['clock']['boottime_ns'] - intent['clock']['boottime_ns']
                >= work['limits']['wall_ns']
            ):
            ...
            if (
                target in ('CAPTURED', 'SIGNING_INTENT')
                and work['observation_bytes_b64'] is not None
            ):
                raise ValueError('settled work cannot acquire credit')
            if (
                target == 'COMPLETED'
                and work['state'] not in ('CAPTURED', 'SIGNED')
                and work['phase'] != 'ADMISSION'
            ...
            return self._save_budget(
                connection, state, target, authority=retry_of is None and target != 'SIGNED'
            )

    def recover_work(
        self, attempt_id, work_id, observations_bytes, *, recovery_owner_token=None
    ) -> bytes:
        ...
        with self.store.transaction() as connection:
            ...
            return self._save_budget(
                ...
                state,
                'RECOVER_WORK',
                authority=state['state'] != old_state
                or (changed_work and self._retry_parent(work) is None),
            )
    ...
    def _remaining_after_charges(self, connection, state):
        ...
        return max(
            0,
            state['remaining_cpu_ns']
            - self.void_authentication_charge(connection, state['attempt_id']),
        )
    ...
    def _budget_integrity(self, connection):
        ...
        for row in connection.execute('SELECT * FROM full_campaign_budgets'):
            ...
            if state['profile'][
                'schema'
            ] == 'qualification_campaign_budget_profile/v3' and connection.execute(
                'PRAGMA user_version'
            ).fetchone()[
                0
            ] not in (
            ...
            if receipt['schema'] == 'qualification_campaign_admission_receipt/v2':
                ...
                expected_receipt = encoded(
                    {
                        'schema': 'qualification_campaign_provisional_intent/v1',
                        'attempt_id': state['attempt_id'],
                        'request_sha256': state['request_sha256'],
                        'profile_sha256': sha256(encoded(state['profile'])),
                        'start_clock': state['start_clock'],
                        'dispatch_enabled': False,
                    }
                )
            ...
            profile_binding = (
                receipt['budget_profile_sha256']
                if receipt['schema'] == 'qualification_campaign_admission_receipt/v2'
                else receipt['profile_sha256']
            )
            if (
                state['attempt_id'] != owner['attempt_id']
                or state['validity'] != owner['validity']
                or state['request_sha256'] != sha256(owner['request_bytes'])
                or profile_binding != sha256(encoded(state['profile']))
            ...
            clock(encoded(state['start_clock']))
            clock(encoded(state['last_clock']))
            if state['budget'] is not None:
            ...
            for recovery in state.get('recoveries', ()):
                role = 'supervision_control_' + sha256(
                    encoded([recovery['work_id'], 'RECOVERY_OWNER'])
                )
                claim = connection.execute(
                    'SELECT body FROM full_campaign_objects WHERE attempt_id=? AND role=?',
                    (state['attempt_id'], role),
                ).fetchone()
                ...
                if recovery['completion_bytes_b64'] is not None:
                    completion = recovery_completion(
                        self._raw(recovery['completion_bytes_b64']),
                        attempt_id=state['attempt_id'],
                    ...
                    cleanup = connection.execute(
                        ...
                        (
                            state['attempt_id'],
                            'supervision_event_' + completion['cleanup_event_sha256'],
                        ),
                    ).fetchone()
                    if cleanup is None:
                    ...
                    if (
                        event['work_id'] != recovery['work_id']
                        or event['kind'] != 'CLEANUP'
                        or event['data']['status'] != 'ABSENT'
                        or event['clock'] != completion['clock']
                    ):
            ...
            for event in connection.execute(
                'SELECT * FROM full_campaign_budget_events WHERE attempt_id=? ORDER BY sequence',
                (row['attempt_id'],),
            ):
                ...
                if (
                    ...
                    or saved['event_head'] != previous
                    or saved['authority_head'] != authority_head
                    or saved['accounting_revision'] != revision
                    or saved['authority_revision'] != authority_revision
                ):
            ...
            totals = {
                k: state[k] for k in ('settled_cpu_ns', 'reserved_cpu_ns', 'remaining_cpu_ns')
            }
            self._totals(state)
    ...
    def _diagnostic_integrity(self, owner, state, objects):
        ...
        if (
            receipt['schema'] != 'qualification_campaign_admission_receipt/v2'
            or receipt['state'] != 'ADMITTED_DIAGNOSTIC'
            or receipt['dispatch_enabled'] is not False
            or receipt['attempt_id'] != owner['attempt_id']
            or receipt['campaign_id'] != owner['campaign_id']
            ...
            or plan['budget']
            != parse_canonical_json(objects['context_contract'], label='retained contract')[
                'replay'
            ]['budget']
            ...
            or work['observation_bytes_b64'] is None
        ):
            raise ValueError('diagnostic admission integrity differs')
        if set(objects) != {'diagnostic_receipt', 'plan', 'bundle_index'} | {
            'context_' + entry['role'] for entry in index['entries']
        }:
        ...
        for row in connection.execute('SELECT * FROM full_campaigns'):
            ...
            if receipt.get('schema') in (
                ...
                if 'pending_void' in retained:
                    ...
                    if (
                        pending['schema'] != 'qualification_campaign_request/v2'
                        or pending['operation'] != 'VOID'
                        or pending['attempt_id'] != row['attempt_id']
                        or state['profile']['schema']
                        not in (
                            'qualification_campaign_budget_profile/v2',
                            'qualification_campaign_budget_profile/v3',
                        )
                ...
                for name in tuple(retained):
                    if name.startswith(VOID_AUTHENTICATION_PREFIX):
                        ...
                        if (
                            charge['attempt_id'] != row['attempt_id']
                            or name != _void_role(VOID_AUTHENTICATION_PREFIX, charge['sequence'])
                            or state['profile']['schema'] != FUNDED_PROFILE
                ...
                for name in tuple(retained):
                    if name.startswith(VOID_REFUSAL_PREFIX):
                        ...
                        if (
                            refusal['attempt_id'] != row['attempt_id']
                            or name != _void_role(VOID_REFUSAL_PREFIX, refusal['sequence'])
                            or charge is None
                            or charge['request_sha256'] != refusal['request_sha256']
                        ):
                            raise ValueError('cancellation refusal binding differs')
                ...
                for name in tuple(retained):
                    if name.startswith(VOID_ADMISSION_REFUSAL_PREFIX):
                        refusal = parse_void_refusal(
                            retained.pop(name), schema=VOID_ADMISSION_REFUSAL_SCHEMA
                        )
                        if refusal['attempt_id'] != row['attempt_id'] or name != _void_role(
                            VOID_ADMISSION_REFUSAL_PREFIX, refusal['sequence']
                        ):
                            raise ValueError('admission cancellation refusal binding differs')
                ...
                for name in tuple(retained):
                    if name.startswith(VOID_TERMINAL_REFUSAL_PREFIX):
                        refusal = parse_void_refusal(
                            retained.pop(name), schema=VOID_TERMINAL_REFUSAL_SCHEMA
                        )
                        if (
                            refusal['attempt_id'] != row['attempt_id']
                ...
                for name in tuple(retained):
                    if name.startswith('supervision_control_'):
                        ...
                        if (
                            ...
                            or name
                            != 'supervision_control_'
                            + sha256(encoded([event['work_id'], event['data']['slot']]))
                        ):
                        ...
                        if event['attempt_id'] != row[
                            'attempt_id'
                        ] or name != 'supervision_event_' + sha256(raw_event):
                        ...
                        if (
                            enrollment['attempt_id'] != row['attempt_id']
                            or name != 'supervision_' + enrollment['work_id']
                        ):
                            raise ValueError('supervision enrollment projection differs')
                        self._work(state, enrollment['work_id'])
                        payload_slices[enrollment['work_id']] = enrollment['scopes'][
                            'payload_slice'
                        ]
                ...
                for work in state['works']:
                    if work['phase'] == 'ADMISSION' or work['state'] not in (
                        'CAPTURED',
                        'SIGNING_INTENT',
                        'SIGNED',
                        'COMPLETED',
                    ...
                    if not any(
                        ...
                        and self._under_payload_slice(e['data']['cgroup'], payload_slice)
                ...
                if retained:
                    ...
                    self._diagnostic_integrity(
                        row,
                        state,
                        (
                            dict(retained, diagnostic_receipt=row['receipt_bytes'])
                ...
                if row['validity'] == 'VOID':
                    if (
                        type(row['void_request']) is not bytes
                        or type(row['void_receipt']) is not bytes
                    ):
                    ...
                    if void['attempt_id'] != row['attempt_id'] or saved['request_sha256'] != sha256(
                        row['void_request']
                    ):
            ...
            if (
                request['operation'] != 'SUBMIT_E1'
                or request['attempt_id'] != row['attempt_id']
            ...
            if set(objects) != {'plan', 'bundle_index'} | {
                'context_' + entry['role'] for entry in index['entries']
            }:
            ...
            if row['validity'] == 'VALID':
                ...
                if (
                    void['operation'] != 'VOID'
                    or void['attempt_id'] != row['attempt_id']
                    or saved['request_sha256'] != sha256(row['void_request'])
                    or saved['campaign_id'] != row['campaign_id']
                    or saved['validity'] != 'VOID'
                ):
