...
from ..contract import (
    canonical_json_bytes as encoded,
...
class ExecutionService:
    def __init__(self, config):
        ...
        if (
            ...
            or info.st_uid != config['service_uid']
        ...
        if (
            not stat.S_ISDIR(info.st_mode)
            or info.st_uid != config['service_uid']
            or info.st_gid != config['socket_gid']
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix='qualification-execution'
        )

    def keys(self):
        return load_keys(
            read_regular(self.installation, 'keys.json', limit=1024 * 1024),
            authority_class=self.authority,
        )

    def _context(self, bundle_digest, *, at):
        context = verify_bundle(
            self.root / 'bundles' / bundle_digest, self.release, self.keys(), at
        )
    ...
    def handle_request(self, peer_uid, request_bytes):
        ...
        if operation == 'SUBMIT_N1':
            ...
            with self.dispatch_lock:
                with self.store.transaction():
                    ...
                    self.store.archive_object(
                        record.execution_id, 'preflight', build_binding(context, record, plan)
                    )
                    self.store.archive_object(
                        record.execution_id, 'bundle_index', context.retained_bundle_index
                    )
        ...
        if operation == 'STORE_ARTIFACT':
            ...
            return encoded(
                {
                    'artifact_sha256': self.store.store_proposed_artifact(
                ...
                with self.store.transaction() as connection:
                    roles = {
                        ...
                        for row in connection.execute(
                            'SELECT role FROM execution_objects WHERE execution_id=? AND sha256=?',
        ...
        if operation == 'STORE_RESULT':
            ...
            return encoded({'envelope_sha256': result_digest})
        ...
        if operation == 'VOID':
            with self.store.transaction():
                    ...
                    encoded(
                        {
                            ...
                            'contract_sha256': enrollment['contract_sha256'],
                        }
                    )
                )
                approval = verify_detached_approval(
                    ...
                    expected_contract_sha256=enrollment['contract_sha256'],
                    now=current,
                    allow_test_authority=enrollment['authority_class'] == 'TEST_ONLY',
                )
                if (
                    approval.key_id not in enrollment['freeze_key_ids']
                    or approval.authority_class != enrollment['authority_class']
                    or sha256(keys[approval.key_id].public_key)
                    != enrollment['trusted_key_sha256'][approval.key_id]
                ):
    ...
    def _campaign_request(self, peer_uid, raw):
        ...
        if release['schema'] in (
            ...
            if (
                operation == 'SUBMIT_E1'
                and request['schema'] != 'qualification_campaign_request/v2'
            ):
        ...
        with self.store.transaction():
            if operation == 'SUBMIT_E1':
                ...
                domain = parse_canonical_json(
                    objects['context_trust_domain'], label='original enrollment'
                )
                contract_sha = campaigns.status(attempt)['receipt']['contract_sha256']
                keys = self.keys()
                approval = verify_detached_approval(
                    decode_base64(request['operator_approval_bytes']),
                    trusted_keys=keys,
                    expected_scope='VOID_QUALIFICATION_ATTEMPT',
        ...
        if operation == 'SUBMIT_E1':
            with self.dispatch_lock, campaign_supervisor.controller_cpu_guard():
                ...
                if existing is not None:
                    ...
                    unstarted = campaigns.inspect_unstarted_admission(
                        attempt, campaign_supervisor.observe_campaign_clock
                    )
                    ...
                    state = parse_canonical_json(
                        campaigns.begin_admission(
                            raw,
                            encoded(release['campaign_budget_profile']),
        ...
        if operation == 'VOID':
            ...
            with self.dispatch_lock:
                ...
                with campaign_supervisor.controller_cpu_guard():
                    sequence, status = campaigns.claim_void_authentication(
                        raw, campaign_supervisor.observe_campaign_clock()
                    )
                    if sequence is None:
                        return status
                    try:
                        domain = parse_canonical_json(
                            campaigns.retained_object(attempt, 'context_trust_domain'),
                            label='original enrollment',
                        )
                        contract_sha = campaigns.diagnostic_status(attempt)['receipt'][
                            'contract_sha256'
                        ]
                        keys = self.keys()
                        approval = verify_detached_approval(
                        ...
                        campaigns.refuse_void_authentication(
                            raw,
                            sequence,
                            str(refusal)[:1024],
                            campaign_supervisor.observe_campaign_clock(),
    ...
    def _commit_checkpoint(self, campaigns, attempt, request):
        ...
        with self.dispatch_lock:
            ...
            if row is None:
                ...
                capture = encoded(
                    {
                        ...
                        'candidate_sha256': sha256(candidate_bytes),
                        'staged': [
                            {'role': row2['role'], 'sha256': row2['sha256']}
                            for row2 in request['artifacts']
                        ],
                    }
                )
                capture_transition = encoded(
                    {
                        ...
                        'work_id': work_id,
                        'state': 'CAPTURED',
                        'clock': parse_canonical_json(
                            campaign_supervisor.observe_campaign_clock(), label='clock'
                        ),
                ...
                signing_transition = encoded(
                    {
                        ...
                        'data': {
                            'intent_id': work_id + '-intent',
                            'payload_bytes_b64': base64.b64encode(candidate_bytes).decode('ascii'),
                            'key_id': candidate['signature']['key_id'],
                            'signing_at_utc': signing_at,
                        },
                    }
                ...
                with self.store.transaction() as connection:
                    saved = connection.execute(
                        'SELECT request_bytes FROM full_campaign_bootstraps '
                        'WHERE attempt_id=? AND work_id=?',
                        (attempt, work_id),
                    ).fetchone()
                    bootstrap = (
                        None if saved is None else campaign_funding.parse_request(bytes(saved[0]))
                    )
                if bootstrap is not None and bootstrap.get('fault') == 'hold_after_intent':
                    return encoded(
                        {'intent_persisted': True, 'candidate_sha256': sha256(candidate_bytes)}
                    )
            ...
            compare_checkpoint_evidence(
                InspectedEvidence(evidence.assessment_bytes, evidence.output_bytes_by_role),
                expected=InspectedEvidence(
                    evidence.assessment_bytes, evidence.output_bytes_by_role
                ),
            )
    ...
    def _schedule_request(self, peer_uid, raw):
        ...
        campaigns = campaign_store.CampaignStore(self.store)
        with self.dispatch_lock, campaign_supervisor.controller_cpu_guard():
            token, status = campaigns.claim_scheduler_bootstrap(
                raw, campaign_supervisor.observe_campaign_clock()
            )
            if token is None:
                return status
    ...
    def _commit(self, attempt, execution_id, request):
        ...
        with self.store.transaction():
            ...
            auth = fields(
                parse_canonical_json(authentication, label='authentication'),
            ...
            artifacts = {
                item['sha256']: self.store.fetch(attempt, item['sha256'])
                for item in payload['artifacts']
            }
            outputs = {
                item['role']: self.store.fetch_candidate(attempt, item['sha256'])
                for item in doc['outputs']
            }
            snapshot = self.store.snapshot_for_assessment(attempt)
            evidence = validate_result_envelope_v2(
                context,
                result,
            ...
            verify_role_signature(auth, context=context, current_keys=keys, role='result')
            response = parse_canonical_json(
                self.store.commit_assessment(evidence, authentication, now=committed_at),
    ...
    def _execute(self, execution_id):
        ...
        try:
            ...
            container = create_worker(
                context,
                execution_id=execution_id,
                input_dir=daemon_input,
                profile=self.profile,
                host_run_id=self.config['host_run_id'],
            )
            inspected = parse_canonical_json(
                inspect_worker(
                    container,
                    context=context,
                    profile=self.profile,
            ...
            if inspected['execution_id'] != execution_id or inspected['input_source'] != str(
                daemon_input
            ):
                raise ValueError('created container binding differs')
            record = self.store.record_container(
                execution_id, container, expected_revision=status['revision']
            )
            authorized = now()
            with self.store.transaction():
                context, _ = self.store.context(execution_id, now=authorized)
                record = self.store.record_start_intent(
                    execution_id, expected_revision=record.revision, now=authorized
                )
            with ThreadPoolExecutor(max_workers=1) as capture_executor:
                capture_future = capture_executor.submit(
                    start_and_capture,
                    container,
                    spool_dir=self.root / 'spool' / execution_id,
                    profile=self.profile,
                    maximum_wall_seconds=context.contract.replay.budget.maximum_wall_seconds,
                )
                while True:
                    inspected = parse_canonical_json(
                        inspect_worker(
                ...
                record = self.store.record_running(
                    execution_id, expected_revision=record.revision, now=now()
                )
            ...
            parsed = parse_worker_result(
                output, context=context, execution_id=execution_id, plan_bytes=plan
            )
            ...
            facts = encoded(
                {
                    'schema': 'qualification_capture/v1',
                    'container_id': container,
                    'service_id': context.domain.execution_service_id,
                    'profile_sha256': self.profile.sha256,
                    'runtime_manifest_sha256': context.release.runtime_sha256('worker'),
                    'worker_image_digest': inspected['image'],
                    'authorized_at_utc': authorized.isoformat().replace('+00:00', 'Z'),
                    'started_utc': state['StartedAt'],
                    'completed_utc': state['FinishedAt'],
                    'artifacts': [
                        {'role': role, 'sha256': sha256(raw), 'byte_length': len(raw)}
                        for role, raw in [('plan', plan), ('worker_result', output)]
                    ],
                    'observations': dict(
                        exit_code=capture.exit_code,
                        oom_killed=capture.oom_killed,
                        supervisor_wall_ns=capture.supervisor_wall_ns,
                        **parsed.document['observations'],
                    ),
                }
            )
            ...
            sign_captured(
                execution_id,
                store=self.store,
                credential_reference=self.config['execution_credential'],
            )
        except Exception as exc:
            row = next(
                row for row in self.store.execution_rows() if row['execution_id'] == execution_id
            )
            if row['state'] in ('DISPATCHED', 'START_INTENT', 'RUNNING'):
                self.store.record_abort(
                    execution_id, type(exc).__name__ + ': ' + str(exc)[:500], uncertain=True
                )
            if container:
    ...
    def recover_service(self):
        self.recovery_issues = {}
        if self.profile.values['schema'] in (
            ...
            with self.store.transaction() as connection:
                exists = connection.execute(
                    "SELECT 1 FROM sqlite_master WHERE name='full_campaign_budgets'"
                ).fetchone()
                attempts = (
                    []
                    if exists is None
                        ...
                        row[0]
                        for row in connection.execute(
                            'SELECT attempt_id FROM full_campaign_budgets'
                        )
                    ]
                )
            for attempt in attempts:
                ...
                if funding is not None:
                    ...
                    pending = (
                        funding['bootstrap_pending_work_id'] is not None
                    ...
                    if self.profile.values['schema'] == 'qualification_execution_profile/v3':
                        if not pending:
                            self.recovery_issues[attempt + ':funding'] = (
                                'FUNDING_RUNTIME_NOT_ENABLED'
                            )
                    ...
                    state = parse_canonical_json(
                        campaigns.budget_snapshot(attempt), label='recovery budget'
                    )
                ...
                for work in state['works']:
                    if (
                        work['work_id'] == 'admission'
                        and work['state'] == 'RESERVED'
                        and not work['transitions']
                    ):
                        ...
                        if unstarted is not None:
                            if unstarted['expired']:
                                ...
                                campaigns.recover_work(
                                    attempt, 'admission', unstarted['clock_bytes']
                                )
                            self.recovery_issues[attempt + ':admission'] = (
                                'ADMISSION_RESUMABLE'
                                if unstarted['resumable']
                                else 'ADMISSION_UNSTARTED'
                            )
                            continue
                    try:
                        campaign_supervisor.recover_campaign_work(
                            self,
                            decode_base64(work['reservation_bytes_b64']),
                            attempt_id=attempt,
        ...
        for row in self.store.execution_rows():
            if row['state'] in ('DISPATCHED', 'START_INTENT', 'RUNNING'):
                self.store.record_abort(
                    row['execution_id'], 'uncertain execution after service restart', uncertain=True
                )
            if row['validity'] == 'VOID' or row['state'] in (
                'DISPATCHED',
                'START_INTENT',
                'RUNNING',
                'IN_DOUBT',
                ...
                try:
                    ...
                    if container is None:
                        release = parse_canonical_json(self.release, label='installed release')
                        container = find_owned_worker(
                            row['execution_id'],
                            host_run_id=self.config['host_run_id'],
                            image_id=release['worker_image_digest'],
    ...
    def serve(self):
        ...
        with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as server:
            ...
            info = path.lstat()
            if (
                info.st_uid != self.config['service_uid']
                or info.st_gid != self.config['socket_gid']
            ):
    ...
    def _serve_connection(self, connection):
        with connection:
            ...
            try:
                ...
                limit = (
                    SCHEDULE_BYTE_LIMIT
                    if uid == self.config['service_uid']
                    else self.profile.rpc_byte_limit
                )
                raw = receive(connection, limit=limit)
                result = self.handle_request(uid, raw)
                response = encoded(
                    {'ok': True, 'data_b64': base64.b64encode(result).decode('ascii')}
                )
                ...
                response = encoded({'ok': False, 'error': str(exc)[:1024]})
            if len(response) > self.profile.rpc_byte_limit:
                response = encoded({'ok': False, 'error': 'RESPONSE_EXCEEDS_PROFILE_LIMIT'})
            try:
