...
def _campaign_inputs(contract, policy, execution_release_sha256):
    ...
    if (contract.policy_sha256 != policy.sha256 or domain.policy_sha256 != policy.sha256
...
def _campaign_seed_count(contract):
    ...
    return (budget.n1_paths + budget.n2_paths + budget.part_a_expanded_paths
            + contract.replay.part_a.expanded_panels + 5)
...
def derive_campaign_plan(contract, *, policy, execution_release_sha256: str,
    ...
    for label, value in (('release', execution_release_sha256),
    ...
    if (len(contract.canonical_bytes) + count * (namespace_bytes + 1024)
            > _CAMPAIGN_MAX_BYTES):
    ...
    def seed(stage, population, index, *, panel=None, purpose='path'):
        return parse_canonical_json(seed_input(contract, stage=stage, population=population,
            panel_index=panel, path_index=index, synthetic=True,
            purpose=purpose).canonical_bytes, label='campaign seed')

    from .policy import ATTESTED_CHECKPOINTS
    groups = [{'checkpoint': name, 'stages': resolved['checkpoint_groups'][name]}
              for name in ATTESTED_CHECKPOINTS]
    n1 = parse_canonical_json(derive_n1_plan(contract, policy=policy,
        execution_release_sha256=execution_release_sha256, attempt_id=attempt_id,
        exact_depth_approval_sha256=exact_depth_approval_sha256), label='N1 plan')
    depths, seeds, thresholds = [], [], []
    for stage in resolved['checkpoint_groups']['N2']:
        spec = contract.stage_specs[stage]
        thresholds.append({'stage': stage, 'exact_depth': spec.exact_depth,
    ...
    panels = [{'panel_index': panel,
               ...
               'path_seeds': [seed('n2', 'FULL', index, panel=panel)
                              for index in range(part.paths_per_population_per_panel)]}
              for panel in range(part.expanded_panels)]
    replay = original['replay']
    doc = dict(schema='qualification_campaign_plan/v1', purpose='PLANNING_ONLY',
        target_capability='FULL_E1', authorizes_dispatch=False, authority_class='TEST_ONLY',
        attempt_id=attempt_id, contract_sha256=contract.contract_sha256,
        trust_domain_sha256=contract.trust_domain_sha256, policy_sha256=policy.sha256,
        execution_release_sha256=execution_release_sha256, source_bundle_sha256=source_bundle_sha256,
        exact_depth_approval_sha256=exact_depth_approval_sha256,
        initial_state_sha256=sha(original['initial_state']), replay_sha256=sha(replay),
        root_rng_namespace=replay['root_rng_namespace'], mechanics_version=n1['mechanics_version'],
        budget=replay['budget'], budget_sha256=sha(replay['budget']), checkpoint_groups=groups,
        cutoff={'checkpoint': 'CUTOFF', 'stages': resolved['checkpoint_groups']['CUTOFF'],
                'thresholds': thresholds, 'seed_inputs': []},
        n1=n1, n2={'depths': depths, 'seed_inputs': seeds},
        part_a={'parameters': replay['part_a'], 'initial_panel_range': [0, part.initial_panels],
                'potential_appended_panel_range': [part.initial_panels, part.expanded_panels],
                'potential_panels': panels},
        probes=[{'checkpoint': 'N1', 'seed_inputs': [n1['probe']]},
                {'checkpoint': 'N2', 'seed_inputs': [seed('probe', 'FULL', 0)]},
                {'checkpoint': 'PART_A', 'seed_inputs': [
...
def validate_campaign_plan(raw: bytes, *, contract, policy,
    ...
    expected = derive_campaign_plan(contract, policy=policy,
        execution_release_sha256=execution_release_sha256,
        source_bundle_sha256=source_bundle_sha256, attempt_id=attempt_id,
        exact_depth_approval_sha256=exact_depth_approval_sha256)
