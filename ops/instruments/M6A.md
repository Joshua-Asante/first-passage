...
**Contract facts in committed record:** `tick_value` **$1.00** (via Stage-1 RT connecting arithmetic: RT 1t **$2.82** = 2×**$0.91**/side commission + $1.00 — ⚠ commission input disputed: `core/firm_rules.py` places M6A in the cheaper M6E/M6A $1.60-RT group, not the $1.82-RT index-micros group this figure derives from; correction disclosed but not yet applied, see [DL-2 prereg §0](../../docs/briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md)); Stage-2 derived point value **$10,000.00**/pt (`tick_value_usd / tick_size`). Do not invent tick size or multiplier beyond those citations.
**Status:** **Research/discovery only — geometry-documented, no mechanism cell, no candidate.** Stage-1 Currencies **SURVIVOR** (`FLAG-COSTBIND`); Stage-2 ex-FOMC surface measured; Q-TNEC-ENV-1 envelope **NON-EMPTY** at 20/40/80/160-tick cells. Census produced **no M6A entry**. Not a live leg; no allocation. K spend: DL-2 (2026-08-22, K_intrinsic≈10) — **ABANDONMENT**, construction retired for M6A; see [DL-2 RESULTS](../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md) and SESSION LOG.
...
family: []
venue_tradable: true
venue_note: "Tradeify Currencies Product Group lists 6E/M6E/6B/6J/6A/M6A/6C/6S (see MJY.md venue_note). Withdrawn Striker legs no longer reserve cap headroom for new non-Striker research (MSL B8 occupancy release 2026-08-12); Striker redeploy still barred; LEG_MAP cap_alloc dimension separately released 2026-08-26 (0/0, was 69/11) — see docs/adr/2026-08-26-striker-legmap-cap-release.md."
...
- **W4 — Envelope OPEN ≠ edge.** Every OPEN cell in the ENV-1 ladder states a **requirement** only; harvest Req 1–5, DSR-at-K, N-SURV MC, and the regime gate remain independent and unweakened ([`RESULTS.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/RESULTS.md) §4).
...
|---|---|---|---|
| **A1** | **Stage-1 Currencies SURVIVOR, `FLAG-COSTBIND`.** Own-panel N **484**, Clause-N power floor **0.0891**; RT **1t $2.82 / 2t $3.82**; cost-tax 1t r=1 **0.0902** (> floor ⇒ COST binds first on the pooled panel). Inputs pin commission **$0.91**/side. Connecting arithmetic for tick value: RT 1t $2.82 = 2×$0.91 + **$1.00** ⇒ `tick_value` **$1.00**. ⚠ Commission input disputed — `core/firm_rules.py` places M6A in the cheaper M6E/M6A $1.60-RT group, not the $1.82-RT index-micros group this figure derives from; correction disclosed but not yet applied, see [DL-2 prereg §0](../../docs/briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md). | [`RESULTS.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/analysis/c1/c1_thirdleg_instrument_map_2026-07-27/RESULTS.md) (Currencies row + Inputs meta) | **HIGH on Stage-1 provenance, DISPUTED on the commission input — see note.** |
| **A2** | **Stage-2 ex-FOMC surface: N 450 / floor 0.0924; binding-constraint FLIP COST→POWER.** Cost-tax 0.0902 sits above Stage-1 floor 0.0891 but below the ex-FOMC floor 0.0924 — so a leg that skips announcement days is power-bound, not cost-bound. Stage-1's `FLAG-COSTBIND` remains correct for the pooled panel it describes. | [`RESULTS_stage2.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/analysis/c1/c1_thirdleg_instrument_map_2026-07-27/RESULTS_stage2.md) joint table + Cost 1 prose | **HIGH** (committed Stage-2). |
| **A3** | **Primary `tau_max` (wed_thu ex-FOMC, σ ≤ $125.00):** 09:30/10:00/10:30 **360**; 11:00/12:00 **240**; 13:00 **180**; 14:00 **120**; 15:00 **90** (min). Exclusion does **not** change any start's `tau_max` vs pooled (costs power only on this instrument). Joint-table headline cell: largest measured tau_max ties include **360min @ 09:30** (with MYM and others). Point value derived **$10,000.00**/pt; IS cache coverage **56/56** chunks for its own window. | [`RESULTS_stage2.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/analysis/c1/c1_thirdleg_instrument_map_2026-07-27/RESULTS_stage2.md) §M6A + joint table | **HIGH** (measured surface). |
| **A4** | **Q-TNEC-ENV-1: envelope NON-EMPTY; census produced no M6A entry; FX-fix δ transplant REFUSED.** Ladder: 8-tick `DEAD(cost)`; **20/40/80/160-tick `OPEN`** against power floor 0.0891 (H_A NON-EMPTY). Across the 8-class × 7-instrument grid, every M6A cell is **no entry** (unconsultable at pass time because this ledger was missing). The daily-auction/settlement M6A row discloses only that the hurdle is **11.6 ticks = 11.6 pips** against a ~2 bp-scale cited FX-fix effect (**>7× short** at any plausible AUD level) and re-scores **P3-4 MULTI-FIX-FX** `UNSCREENABLE(δ)` — cross-instrument transplant of that δ is **REFUSED** under strategy_harvest Requirement 2 (same refusal class as the MCL entry's explicit Req-2 bar). | [`RESULTS.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/RESULTS.md) §2 M6A · §6 grid · §6.4.4 · [`prior-p3-4-multi-fix-fx-m6a.json`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/entries/prior-p3-4-multi-fix-fx-m6a.json) · [`mcl-tas-settlement-window-replication.json`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/entries/mcl-tas-settlement-window-replication.json) (Req-2 refusal wording) · [closure §4](../../docs/briefs/closures/Q-TNEC-ENV-1-closure.md) | **HIGH** on envelope + no-entry accounting; **disclosure-only** on the FX-fix magnitude comparison (not a scored M6A entry). |

## DEAD / REJECTED (instrument-specific)

| Rejection | Discriminator | Source |
|---|---|---|
| P3-4 MULTI-FIX-FX (Tokyo + London WMR + NY fix cluster) as an M6A seed | Re-score `UNSCREENABLE(δ)`; prior census forbade quoting δ; ENV-1 authored **no entry**. FX-fix effect cited only as Req-2-inadmissible cross-instrument disclosure (>7× short of the 11.6-tick hurdle). | [`RESULTS.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/RESULTS.md) §6 daily-auction/settlement × M6A · [`prior-p3-4-multi-fix-fx-m6a.json`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/tnec_envelope_compile_2026-08/entries/prior-p3-4-multi-fix-fx-m6a.json) |
...
- **No candidate, no election.** Envelope NON-EMPTY admits nothing (A4 / RESULTS §4). K spend: DL-2 (2026-08-22) — see SESSION LOG.
- **Parent 6A remains un-ledgered** (W1).

## SESSION LOG

- **2026-08-22** — **DL-2 campaign — ABANDONMENT.** Mechanism id `prior-session-breakout-continuation` (`NEW`) scored against M6A: all 10 frozen variants net-negative on 6A-parent TRAIN; nominee V9 failed 3/4 nomination gates (cost-law, SPA p=0.9755, M-16 slip-stress); confirm partition never read. Dated verdict **AMBIGUOUS — ABANDONMENT** (no strike against the lane's falsification budget, per prereg §4). K_intrinsic ≈ 10. [`DL-2 RESULTS`](../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md) · [`DL-2 prereg`](../../docs/briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md). No mechanism×instrument `cells` row added to this ledger's PROFILE YAML in that session — forward work.
