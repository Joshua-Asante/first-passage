...
- **[`overnight-gap-magnitude-range-conditioning` HELD until 2027-03-01](../../docs/notes/notice/N-2026-08-29-mym-gap-magnitude-rth-range.md) (2026-08-29, corrected same day).** Same stratified-design correction as the overnight-range sibling — the original marginal "clean kill" (diff −0.1039) inverted sign entirely: within-stratum lift +14.0pp / +6.7pp, bootstrap CI [−4.2pp, +14.8pp] straddling 0, p(lift≤0)=0.1247 / null-calibrated min-lift p=0.00860 (within-stratum enumerated; n_null=1304 vs original n=1307). **AMBIGUOUS / HOLD unchanged** — the CI still straddles 0; the corrected p would clear 0.05 and is flagged for a separate notice addendum, not flipped here. Moved DEAD → CONTINGENT-FORWARD (was never a graduate-worthy pass, but no longer a kill either). Re-check on the grown panel. **Now also nested under `Q-RANGEXFER-1`'s 2026-08-30 amendment as `H-RANGEXFER-1.a-MYM`**, mirroring MNQ's own gap-magnitude sub-question — that amendment discloses the same CI-vs-null-p disagreement flagged here, rather than resolving it; the joint-gate replication (`N-2026-08-29-mym-overnight-gap-joint-gate.md`) confirms this candidate's own weak result nests inside the overnight-range candidate's own gate the same way it does on MNQ (gap adds lift only when overnight range is calm). The HELD-until-2027-03-01 trigger on this cell stands independent of the Q-brief's own Phase 1 gating and of the CI-vs-null-p operator ruling still owed.
