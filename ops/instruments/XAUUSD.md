...
**Canonical feed:** **TV CSV export — Pepperstone** (per [`docs/adr/2026-06-12-tv-csv-canonical-feed-policy.md`](../../docs/adr/2026-06-12-tv-csv-canonical-feed-policy.md)). ⚠ Correction 2026-08-31: Pepperstone was retired outright 2026-08-02 — **no canonical CFD feed exists** for XAUUSD today ([Pepperstone feed retirement ADR](../../docs/adr/2026-08-02-pepperstone-feed-retirement.md); Tier-2 data bytes deleted from checkout, 35 files). CME futures TV exports are canonical for everything live, but XAUUSD (CFD spot) has no CME venue edition — see [`MGC.md`](MGC.md) for the separate, non-live gold-futures research line. Programmatic bar feeds (Dukascopy / broker REST) and the **Alchemy** TV feed were **staging-only** under the (now-retired) policy above; any staging result was TV-Pepperstone-verified before it gated anything. ⚠ Correction 2026-08-31: Dukascopy is not staging-only — it is **retired entirely** as a bar source (adapter deleted) — [Dukascopy retirement ADR](../../docs/adr/2026-06-17-dukascopy-retirement.md); the proposed FX/metals Dukascopy carve-out ADR ([`docs/adr/2026-06-12-rnd-feed-instrument-class-split.md`](../../docs/adr/2026-06-12-rnd-feed-instrument-class-split.md)) is **superseded/moot with it, not merely un-ratified** (tombstoned — [`docs/adr/TOMBSTONES.md`](../../docs/adr/TOMBSTONES.md)). Alchemy remains staging-only, now with no canonical feed left to verify staging results against.
...
**Last updated:** 2026-07-28 · **Canonical panel (Guardian baseline — ⚠ historical: bytes deleted from checkout 2026-08-02 per the [Pepperstone feed retirement ADR](../../docs/adr/2026-08-02-pepperstone-feed-retirement.md), offline-only):** `core/data/tv_exports/pepperstone/Guardian_Gold_v5.5_PEPPERSTONE_XAUUSD_2026-05-24_1bb97.csv` (N=203, 2022-01-11 → 2026-05-14; the LOCK-of-record baseline panel, gitignored vendor bytes).
...
- **W1 — XAUUSD is Guardian's LOCKED instrument.** ⚠ Correction 2026-08-31: not "LIVE" — dormant on the venue-binding axis, no live venue since the FXIFY/DXTrade CFD closure 2026-07-10 (see line 5; [CFD retirement ADR](../../docs/adr/2026-06-30-no-manual-trading-cfd-retirement.md) + its 2026-07-10 Addendum). Guardian Gold v5.5 (long-only trend-rider) is the instrument's `LOCKED · AUTHORIZED @1.00×` owner even with no current execution. Any XAUUSD R&D (a) must NOT touch the locked strategy/params/allocation (Rule 0 / Key Principle), and (b) must clear a **book-correlation gate** vs Guardian (and the rest of Constellation) before admission — a 5th leg that re-couples to Guardian destroys the diversification rationale. Guardian's config of record (EMA/SL/TP/maxHold/session/risk) and baseline metrics (N/PF/WR/Net/DD/1R) are **not restated here** — per this ledger's own "links out, never restates" rule (line 10), see [`core/strategies/_archive/guardian/LOCK.md`](../../core/strategies/_archive/guardian/LOCK.md) (Pine config) and [`.claude/skills/trade-csv-reconcile/references/baselines.md`](../../.claude/skills/trade-csv-reconcile/references/baselines.md) (canonical baseline cache).
- **W2 — Feed discipline.** ⚠ Correction 2026-08-31: canonical feed retired outright 2026-08-02 — see header pointer ([Pepperstone feed retirement ADR](../../docs/adr/2026-08-02-pepperstone-feed-retirement.md)). Of the former staging feeds, Alchemy remains staging-only; **Dukascopy is retired entirely, not a staging option** — [Dukascopy retirement ADR](../../docs/adr/2026-06-17-dukascopy-retirement.md). (Historical divergence record below predates both retirements.) Pepperstone↔Alchemy XAUUSD **diverge** ([`docs/notes/notice/N-2026-05-29-pepperstone-alchemy-feed-divergence.md`](../../docs/notes/notice/N-2026-05-29-pepperstone-alchemy-feed-divergence.md)); XAUUSD also has feed-specific stop-proximity drift on the order of ~16–29% (trade-csv-reconcile Rule-0 sub-rule). **A non-canonical-feed result does NOT transfer** — this file's own methodology required reproducing on Pepperstone TV before any disposition (the USOIL-RDM `FX_USOIL` precedent). ⚠ **Correction 2026-08-31:** that reproduction step is no longer executable — Pepperstone is retired and its bytes deleted (see header pointer). Until a successor canonical feed is admitted for XAUUSD, the non-transfer *rule* still holds (a non-canonical-feed result is still not disposition-grade), but there is currently no canonical feed to reproduce against — treat any such result as blocked pending a feed decision, not as clearable by the old Pepperstone step. Path-independence means an independent price *path/period*, not a feed-swap (OANDA↔Pepperstone entry-date Jaccard ≈0.96 = same path).
...
- **Canonical feed:** retired 2026-08-02 (see header pointer — [Pepperstone feed retirement ADR](../../docs/adr/2026-08-02-pepperstone-feed-retirement.md)); was Pepperstone TV CSV under the now-superseded policy (W2). Guardian XAUUSD panels present historically: Pepperstone (`…1bb97` LOCK-of-record baseline, bytes deleted from checkout 2026-08-02 + dated predecessors) and OANDA cross-feed. **Alchemy is staging-only and divergent** — the brief's §2.1 "precomputation" was on a non-existent-in-repo Alchemy export and is treated as unverified; the canonical kill-test (2026-06-15) supersedes it.
