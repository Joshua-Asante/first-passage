...
Placebo null (hourly-clock family, for apples-to-apples comparability with the ROBUSTNESS real
statistic): 1000 replicates; each replicate draws, INDEPENDENTLY PER TRADING DAY, a random
"impulse-hour" offset from a 6-candidate menu of ET hours structurally clear of the real fix
window (06:00,07:00,08:00,09:00,12:00,13:00 ET -- 14:00 was dropped: its +3h outcome-end needs
h17, which is absent for ALL 624 calendar dates in the panel = the CME Globex daily
trading-halt hour, 16:00-17:00 CT / 17:00-18:00 ET, confirmed empirically, not a data gap).
...
CONSTRUCTION -- the strongest possible form of matching. The verification step checks this is not
silently broken by differential NaN-dropping (a placebo offset missing data on a day the real
construction has, or vice versa).
...
the B2.1 ruling scope -- one could survive while the other dies; both died, see RESULTS.md).
"""
...
def build_daily_panel(hourly: pd.DataFrame, minute: pd.DataFrame, symbol: str) -> pd.DataFrame:
    ...
    # Minute wide: open price at the 4 clock-minutes needed for the precise impulse/target.
    need_hm = ["10:58", "11:04", "11:10", "13:00"]
...
def run_symbol(hourly, minute, symbol):
    ...
    dies = reversal_kill or placebo_kill
    ...
    print(f"  placebo leg       : {'FAIL -> kill-eligible' if placebo_kill else 'PASS'} "
          f"(real rank pct={rank_pct:.1f}, need > 60)")
    ...
    return dict(
        ...
        placebo_kill=placebo_kill, match_ok=match_ok, dies=dies,
    )
...
def main():
    ...
    for sym, r in results.items():
        print(f"{sym}: n={r['n_valid']}  imp_sign t(orthogonal)={r['modelC_imp_t']:+.2f} "
              f"coef={r['modelC_imp_coef']:+.6f}  placebo_rank={r['placebo_rank_pct']:.1f}pct  "
