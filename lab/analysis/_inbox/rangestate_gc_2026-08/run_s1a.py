"""H-RANGESTATE-GC-1 (S1a) -- daily range-state persistence, GC train era.

Frozen design: PREREG_S1A.md (same directory), committed and pull-recorded before this ran.
...
Panel: GC.v.0 continuous, ohlcv-1d, 2010-06-06..2019-01-01 (train era; MGC micro-era reserved).
"""
...
def block_bootstrap_ci_conditional(bias: np.ndarray, y: np.ndarray, block: int,
    ...
    Adversarial-review fix (2026-08-18): the prereg declares this bootstrap
    "circular"; the prior implementation truncated block starts to [0, N-block]
    (a non-wrapping moving-block / Kunsch bootstrap), a genuine procedure mismatch
    caught before this run was trusted. Measured effect on the real S1a draw: LB
    shifts +0.0044 (0.4676 -> 0.4720 at the old CI_BLOCK=10); verdict-preserving.
    """
    rng = np.random.default_rng(seed)
    N = len(bias)
...
def main():
    ...
    # Persist the ACTUAL roll-day dates (adversarial-review fix: prior prereg prose hand-typed
    # 5 example dates, 2 of which were off by one calendar day vs a live re-run).
    roll_dates = [str(d.date()) for d in tr_full.index[tr_full.isna()][1:]]  # [1:] drops row 0

    # Disclosure: full trading-day completeness census (gap-day audit, not blocking --
    # bounded materiality per adversarial review, but the Step-0 battery should name it).
    ...
    # Operate on the FILTERED valid-TR sequence only (drop NaN days entirely --
    # windows advance over valid observations, not raw calendar offsets).
    ...
    # also require TR_{d+1} itself valid (already guaranteed: tr[] has no NaN by construction
    # of `valid`, but the d+1 SLOT may not exist -- handled by the [:-1] assignment above,
    # last row's y is NaN since there is no d+1 in this valid-TR sequence).
    ...
    # ---- disclosure: unconditional base rate ----
    ...
    (HERE / "s1a_results.json").write_text(json.dumps(out, indent=1, default=str))
