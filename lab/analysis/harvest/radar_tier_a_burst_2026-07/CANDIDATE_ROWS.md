...
### Rank-1 axis-2 traversal (Koijen et al. 2018 *Carry*) — **BLOCKED-AT-SOURCE** (2026-08-16; not "0 survivors")

The never-run second rank-1 axis (forward-citation funnel from Koijen, Moskowitz, Pedersen & Vrugt, *Carry*, *JFE* 127(2) 2018, DOI `10.1016/j.jfineco.2017.11.002` — DOI verified against IDEAS/SSRN/CBS/NBER, not guessed) is **not executable on the prescribed instrument**: Semantic Scholar's graph has **no resolvable record for the paper at all**. Verified as a genuine index gap, not throttling: the DOI and its NBER edition both return real 404s; S2 title-match, a 4-author relevance search, a 57,162-record bulk "carry" sweep, and full author-profile enumeration (14 profile fragments — *Carry* is Vrugt's most-cited work everywhere else and absent from his S2 profile) all come back empty, while axis 1's TSMOM DOI resolves fine on the same endpoint (health control) and **OpenAlex resolves the same DOI instantly** (`W4234492972`, JFE, cited_by_count **299**). The S2 404 and the OpenAlex 299 were both independently re-verified in the recording session, not just taken from the traversal agent.
