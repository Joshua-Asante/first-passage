...
def main_default():
    ...
    print(f"Pass:             {pass_rate:>7.2%}  (baseline 99.83% under C2, 2026-05-23 allocation refresh-2)")
    ...
    # Halt-condition check (baseline updated 2026-05-23 to C2 allocation-refresh-2 anchor 99.83%)
    delta_pp = abs(pass_rate - 0.9983) * 100
    print(f"Deviation from 99.83% C2 baseline: {delta_pp:.2f} pp")
    if delta_pp > 2.0:
...
def regime_check():
    ...
    if both_below:
        ...
        print("     compare to the C2 allocation-refresh-2 anchor 99.83 / 0.17 / 4.37.")
        print("  3. Author archive/docs/briefs/Q-DDP-2/ if the live data confirms the")
