#!/usr/bin/env python3
"""Q-R2AGRUN-1 Stage-G (G2) — score C1 on EXPLORATION TBBO only (OFCHAN cache reuse).

PREREG_G0.md §6 step 2. CONFIRM window is never opened.
Operator explore GO: cache reuse only — no new Databento pull.
Must pass test_agrun_lib.py before this reads cache bytes.

Usage:
  .venv-research/Scripts/python.exe lab/analysis/c1/mnq_r2agrun_routeb_2026-08/run_agrun_g2.py
  .venv-research/Scripts/python.exe …/run_agrun_g2.py --from-checkpoint   # reuse day scan
"""
...
import agrun_lib as al  # noqa: E402
...
def load_day_trade_arrays(
    ...
    """Return (ts_ns, side, bid_px, ask_px) for trade-tagged TBBO in RTH pad.

    Pad 09:28–16:02 ET so 15:59+60s mid exists. Run walk further restricts to RTH.
    """
    ...
    return ts_ns[ok], side[ok], bid_px[ok], ask_px[ok]
...
def rth_quote_span_ok(ts_ns: np.ndarray, session_day: date) -> bool:
    ...
    if len(ts_ns) == 0:
        return False
    open_ns = int(
        datetime.combine(session_day, al.RTH_OPEN, tzinfo=ET)
        .astimezone(UTC)
        .timestamp()
        * 1e9
    )
...
def _process_one_day(path_str: str) -> dict:
    ...
    ts_ns, side, bid_px, ask_px = load_day_trade_arrays(path)
    ...
    a, r, n_ret, n_elig = al.sample_pairs_for_day(ts_ns, side, bid_px, ask_px, d)
    if n_elig == 0:
        return {
            "kind": "skip",
...
def scan_days(workers: int) -> tuple[
    ...
    # Deterministic order by session date (as_completed is nondeterministic).
...
def score_and_write(
    ...
    if sessions:
        ...
        rho = al.pearson_rho(a_all, r_all)
    else:
        a_all = np.array([])
        r_all = np.array([])
        rho = float("nan")
    ...
    ci = al.session_block_bootstrap_rho(sessions, n=al.N_BOOT, seed=al.SEED)
    print(f"rho={rho} ci95={ci}", flush=True)

    # PREREG §3 precedence: CI limb fires before placebo. Skip the O(N×n_placebo)
    # shuffle when CI already includes 0 (or non-finite) — same verdict path.
    lo, hi = ci
    ...
    if ci_fails:
        ...
        placebo = np.array([0.0])  # p95=0; unused once CI fails in verdict_g2
        ...
        placebo = al.within_session_r_shuffle_placebo_parallel(
            sessions, n=al.N_PLACEBO, seed=al.SEED, workers=placebo_workers
        )
        placebo_skipped = False
    h1, h2 = al.halves_rho(sessions, session_dates)
    placebo_p95 = (
        float(np.percentile(np.abs(placebo), 95)) if len(placebo) else float("nan")
    )

    verdict = al.verdict_g2(ci, rho, placebo, coverage, n_retained_total, h1, h2)
    candidates = al.candidate_list(verdict)
    ...
    payload = {
        "cell": "C1",
        "campaign": "Q-R2AGRUN-1",
        "stage": "G2",
        "run_n_min": al.RUN_N_MIN,
        "window": {
            "role": "EXPLORATION",
        ...
        "seed": al.SEED,
        "n_boot": al.N_BOOT,
        "n_placebo": al.N_PLACEBO,
        "mag_floor": al.MAG_FLOOR,
        ...
        "excluded_session_dates": sorted(str(x) for x in al.EXCLUDED_SESSION_DATES),
        "skipped": skipped,
        "census": census,
