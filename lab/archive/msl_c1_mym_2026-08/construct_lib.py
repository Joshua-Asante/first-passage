"""MSL-C1 Stage G0 — PDH/PDL failed-break reclaim (MYM).
...
Frozen geometry: lab/archive/msl_c1_mym_2026-08/PREREG_G0.md
"""
...
TICK = 1.0  # MYM mintick = 1.0 pt
POINT_VALUE = 0.50  # USD per 1.0 price point
RT_USD = 2.82
FOUR_X_RT_USD = 11.28
...
OVERNIGHT_CLOSE_MIN = 9 * 60 + 29  # 09:29 — DELETE sham clock window end
...
MYM_PANEL_REL = "core/data/bar_data/MYM_M15.csv"
MYM_SHA256 = "24e169528f7ea6693b75c71c3195edf6a04f3a26c6b6dff0f2e9c623fd597a58"
...
def cheap_falsifier_freeze_ok() -> CheapFalsifierResult:
    """Record-only: Stage-1 already PASSed cost-law / occupancy / delete unpaid."""
    return CheapFalsifierResult(
        verdict="CHEAP_FALSIFIER_OK",
        detail=(
            "STAGE1 PASS: cost-law $160 ≫ $11.28; B8 occupancy CLEAR; route ①; "
...
def overnight_hl(high: np.ndarray, low: np.ndarray, minutes: np.ndarray) -> tuple[float, float]:
    """DELETE sham — same Globex-day overnight clock window [00:00, 09:29] ET."""
    mask = minutes <= OVERNIGHT_CLOSE_MIN
    if not np.any(mask):
        return float("nan"), float("nan")
    return float(np.max(high[mask])), float(np.min(low[mask]))
...
def simulate_constrained(
    ...
    """Constrained: prior-day RTH PDH/PDL."""
...
def simulate_delete_sham(
    ...
    """DELETE: PDH/PDL replaced by same-day overnight clock H/L."""
