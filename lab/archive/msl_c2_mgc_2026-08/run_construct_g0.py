"""MSL-C2 Stage G0 runner — freeze recall always; path PnL GO-gated.

Usage:
  python run_construct_g0.py                 # freeze template + admission recall
  python run_construct_g0.py --explore-go    # requires EXPLORE_GO.md + MGC_M15 sha match
"""
...
PANEL_DEFAULT = _REPO / C.MGC_PANEL_REL
...
def _expected_mgc_sha() -> str:
    text = SHA256SUMS.read_text(encoding="utf-8")
    for line in text.splitlines():
        line = line.strip()
        if not line or line.startswith("#"):
            continue
        # format: <sha256> *MGC_M15.csv  OR  <sha256>  MGC_M15.csv
        parts = line.replace("*", " ").split()
        if len(parts) >= 2 and parts[-1].endswith("MGC_M15.csv"):
            return parts[0]
    return C.MGC_SHA256
...
def assert_panel_ok(panel: Path) -> None:
    ...
    want = _expected_mgc_sha()
    if got != want:
        raise SystemExit(
            f"REFUSE: MGC_M15 sha mismatch\n  got  {got}\n  want {want}"
        )
...
def run_explore(panel: Path) -> dict:
    ...
    # Prior Globex-day RTH H/L for DELETE sham (chronological)
    prior_rth: dict = {}
    prev_hi, prev_lo = float("nan"), float("nan")
    for s in sessions:
        prior_rth[s] = (prev_hi, prev_lo)
        grp = by_sess[s]
        hi, lo = C.rth_hl(
    ...
    for s in sessions:
        ...
        constrained_by_sess.append(C.simulate_constrained(o, h, lo_, c, m))
        phi, plo = prior_rth[s]
        delete_by_sess.append(
            C.simulate_delete_sham(o, h, lo_, c, m, phi, plo)
            if np.isfinite(phi) and np.isfinite(plo)
            else []
        )
        flip_by_sess.append(C.simulate_flip_join(o, h, lo_, c, m))
        sess_meta.append(s)
    ...
    def collect(trades_by_sess: list[list[dict]], side: int):
        ...
        for s, trades in zip(sess_meta, trades_by_sess):
            ...
            day_r = 0.0
            arr = np.asarray([t["R"] for t in arm], dtype=float) if arm else np.asarray([], dtype=float)
            if len(arr):
    ...
    # Flip sides: fade-short signals → flip long; fade-long → flip short
    # simulate_flip_join already flips side; collect by resulting side
    ...
    # Flip compare: fade short mean vs flip long mean (same upside-fail cohort);
    # fade long mean vs flip short mean. Approximate via arm means as declared.
    flip = {
    ...
    # Cost-law disclosure at realized mean stop
    def cost_law(a):
        sd = a.get("mean_stop_dist", float("nan"))
        if not np.isfinite(sd) or sd <= 0:
            return None
        one_r = sd * C.POINT_VALUE  # 1R gross at win; disclose vs 4×RT
        return {
    ...
    out = {
        ...
        "disclosures": {
            ...
            "confirm_reserved": "2025-09-01→2026-08-12",
...
def main() -> int:
    ...
    if not args.explore_go:
        print("Explore: BLOCKED (pass --explore-go after EXPLORE_GO.md + MGC_M15 restore)")
        return 0
