...
PANEL = CELL / "inputs" / (
    "Striker_NAS100_MYM_QTXG1_CBOT_MINI_MYM1!_2026-08-12_69f15.csv"
)
PANEL_SHA = "203480867dbcf4751c5cbfcdfdb6b28418c967fe3fe812884db19f91294a4ebf"
REQUIRED_NET_R = 0.06
MYM_POINT_VALUE = 0.50
COMMISSION_PER_SIDE = 0.91
INITIAL_CAPITAL = 100_000.0
BAR_PATH = Path(r"C:/Users/joshu/multi_firm_operations/core/data/bar_data/MYM_M15.csv")
BAR_SHA = "24e169528f7ea6693b75c71c3195edf6a04f3a26c6b6dff0f2e9c623fd597a58"
...
def static_equity_recompute(df: pd.DataFrame) -> dict:
    ...
    for _, ex in exits.iterrows():
        ...
        gross = qty * (float(ex["Price USD"]) - float(en["Price USD"])) * MYM_POINT_VALUE
        ...
        for ts_et, low in zip(ts_arr, low_arr):
            ...
            for entry_ts, exit_ts, entry_px, qty in day_legs:
                if entry_ts <= ts_et <= exit_ts:
                    mtm += qty * (float(low) - entry_px) * MYM_POINT_VALUE
            realized = 0.0
...
def main() -> int:
    ...
    df = load_csv(PANEL)
    m = compute_metrics(df, "striker_nas", account=INITIAL_CAPITAL)
