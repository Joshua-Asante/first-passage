"""DL-2 prereg Sec3 (frozen scoring conventions) + Sec6 step-2 (nomination
...
SPA/Hansen), per that module's own "do not reimplement" rule and DL-1's own
score.py precedent. Requires `lab` on PYTHONPATH for this import.
"""
...
SPA_SEED = 11                 # prereg Sec6 step 2b: RNG seed 11 (fresh, not DL-1's 7)
SPA_ALPHA = 0.10              # prereg Sec6 step 2b: p <= 0.10
COST_LAW_DESIGN_R = 0.40      # prereg Sec4: gross expectancy 0.40R at the design point
...
def daily_pnl_series(trades: list[Trade], trading_dates: list[Date]) -> pd.Series:
    """Daily net P&L over the CME calendar (trading_dates = every session
    date the stitched TRAIN series has bars on), flat days included as
    zeros -- prereg Sec3 item 1."""
...
def halves_split(daily_pnl: pd.Series, split_date: str) -> tuple[float, float]:
    ...
    exposed here for reuse, not invoked on TRAIN by this driver -- DL-1's
    own score.py precedent). This function touches no confirm data itself;
    it is dead code until a later, separately-authorized handoff calls it
    on the confirm partition."""
...
def cost_law_gate(trades: list[Trade]) -> dict:
    ...
    median_stop_ticks x $1/tick / $2.60 (design-point gross-R yardstick,
    stop width is the realized-train-geometry variable) >= 4x, equivalently
    median stop >= ~26.0 ticks (the prereg's own worked design-point
    number; the actual gate uses the nominee's realized geometry)."""
    if not trades:
...
def spa_universe(daily_pnl_by_variant: dict[int, pd.Series]) -> dict:
    ...
    block_size=20, reps=10000, seed=11 (pinned, NOT ACF-derived -- overrides
    universe_gate's own ACF default by passing block_size explicitly)."""
    idx = None
