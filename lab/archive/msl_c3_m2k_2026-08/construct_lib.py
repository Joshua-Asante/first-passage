"""MSL-C3-K2 Stage G0 — dual-axis MR-at-level (M2K).
...
Frozen geometry: lab/analysis/c1/msl_c3_m2k_2026-08/PREREG_G0.md
"""
...
TICK = 0.10  # M2K mintick = 0.10 pt
POINT_VALUE = 5.0  # USD per 1.0 price point
RT_USD = 2.82
FOUR_X_RT_USD = 11.28
...
OVERNIGHT_OPEN_MIN = 18 * 60  # 18:00 ET prior Globex day
OVERNIGHT_CLOSE_MIN = 9 * 60 + 29  # 09:29 ET
RTH_OPEN_MIN = 9 * 60 + 30  # 09:30
RTH_CLOSE_MIN = 15 * 60 + 59  # 15:59
PROBE_OPEN_MIN = RTH_OPEN_MIN
PROBE_CLOSE_MIN = RTH_CLOSE_MIN
...
CONFIRM_END = np.datetime64("2026-08-13")

M2K_PANEL_REL = "core/data/bar_data/M2K_M15.csv"
# Pin lives in core/data/bar_data/SHA256SUMS after land; runner reads it at explore.
M2K_SHA256: str | None = None

AXIS_A = "pdh-pdl-failed-break-reclaim"
AXIS_B = "overnight-range-failed-extension-fade"
AXES = (AXIS_A, AXIS_B)
...
def cheap_falsifier_freeze_ok() -> CheapFalsifierResult:
    """Record-only: STAGE1_K2 PASSed cost-law / door-check; delete unpaid."""
    return CheapFalsifierResult(
        verdict="CHEAP_FALSIFIER_OK",
        detail=(
            "STAGE1_K2 PASS: cost-law $160 ≫ $11.28; route ① both axes; "
            "K_intrinsic=2 floor 0.85; delete/flip unpaid until explore GO — "
            "see STAGE1_K2.md / PREREG_G0.md"
        ),
        rt_usd=RT_USD,
...
def find_failed_extension_signal(
    ...
    """First probe-window failed-break reclaim → entry at next open."""
...
def rth_hl(high: np.ndarray, low: np.ndarray, minutes: np.ndarray) -> tuple[float, float]:
    """Prior-day RTH H/L source — [09:30, 15:59] ET."""
...
def overnight_hl(high: np.ndarray, low: np.ndarray, minutes: np.ndarray) -> tuple[float, float]:
    """ONH/ONL — Globex overnight [18:00, 09:29] ET (wraps midnight)."""
    mask = (minutes >= OVERNIGHT_OPEN_MIN) | (minutes <= OVERNIGHT_CLOSE_MIN)
    if not np.any(mask):
        return float("nan"), float("nan")
    return float(np.max(high[mask])), float(np.min(low[mask]))
...
def simulate_session(
    ...
    *,
    ...
    """k=1: at most one trade per session (within an axis)."""
    sig = find_failed_extension_signal(
        open_, high, low, close, minutes, ref_hi, ref_lo
    )
    if sig is None:
        return []
    trade = build_trade_from_signal(open_, high, low, close, sig, mode=mode)
    return [trade] if trade is not None else []


def simulate_axis_a_constrained(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    """Axis A constrained: prior-day RTH PDH/PDL."""
    return simulate_session(
        open_, high, low, close, minutes, ref_hi=prior_pdh, ref_lo=prior_pdl, mode="fade"
    )


def simulate_axis_a_delete_sham(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    """Axis A DELETE: PDH/PDL replaced by same-session overnight ONH/ONL."""
    hi, lo = overnight_hl(high, low, minutes)
    return simulate_session(
        open_, high, low, close, minutes, ref_hi=hi, ref_lo=lo, mode="fade"
    )


def simulate_axis_a_flip(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    return simulate_session(
        open_, high, low, close, minutes, ref_hi=prior_pdh, ref_lo=prior_pdl, mode="flip"
    )


def simulate_axis_b_constrained(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    """Axis B constrained: same-session overnight ONH/ONL."""
    hi, lo = overnight_hl(high, low, minutes)
    return simulate_session(
        open_, high, low, close, minutes, ref_hi=hi, ref_lo=lo, mode="fade"
    )
...
def simulate_axis_b_delete_sham(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    ...
    """Axis B DELETE: ONH/ONL replaced by prior-day RTH PDH/PDL."""
    return simulate_session(
        open_, high, low, close, minutes, ref_hi=prior_pdh, ref_lo=prior_pdl, mode="fade"
    )


def simulate_axis_b_flip(
    open_: np.ndarray,
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
...
def pooled_mean_r(long_mean: float, short_mean: float, n_long: int, n_short: int) -> float:
    """Long/short pooled mean for promotion tie-break (n-weighted)."""
    if n_long + n_short == 0:
        return float("nan")
    total = 0.0
    w = 0
    if n_long and np.isfinite(long_mean):
        total += long_mean * n_long
        w += n_long
    if n_short and np.isfinite(short_mean):
        total += short_mean * n_short
        w += n_short
    if w == 0:
        return float("nan")
    return total / w
...
def promote_axis(gate_a: str, gate_b: str, pooled_a: float, pooled_b: float) -> str | None:
    """PREREG §4 promotion — at most one axis. Returns axis id or None if STOP."""
    a_ok = gate_a not in ("FALSIFIED", "VOID")
    b_ok = gate_b not in ("FALSIFIED", "VOID")
    if a_ok and not b_ok:
        return AXIS_A
    if b_ok and not a_ok:
        return AXIS_B
    if a_ok and b_ok:
        if not (np.isfinite(pooled_a) and np.isfinite(pooled_b)):
            return AXIS_A  # tie → A
        if pooled_b > pooled_a:
            return AXIS_B
        return AXIS_A  # higher or tie → A
    return None
