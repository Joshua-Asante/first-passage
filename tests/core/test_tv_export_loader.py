...
from tv_export_loader import (
    PRICE_COL_BY_INSTRUMENT,
    load_tv_export,
    pair_tv_export_dataframe,
)
...
def test_futures_price_columns_registered():
    for sym in ("NQ", "YM", "J7", "QO", "MNQ", "MYM", "MGC"):
        assert PRICE_COL_BY_INSTRUMENT[sym] == "Price USD"
...
def test_pair_wrapper_accepts_all_canonical_schema_vintages(fixture_name):
    fixture = Path(__file__).parents[1] / "fixtures" / fixture_name

    paired = pair_tv_export_dataframe(
        pd.read_csv(fixture),
        expected_symbol="NAS100",
        min_raw_rows=0,
        source_label=fixture_name,
    )

    assert paired["trade_num"].tolist() == [2, 1]
    assert paired["net_pnl_usd"].tolist() == [20, 10]
    assert paired["leg_type"].tolist() == ["pyramid_add", "base"]
