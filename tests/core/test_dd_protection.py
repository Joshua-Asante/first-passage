...
def test_dd_protection_rejects_non_finite_equity(value):
    from dd_protection import calculate_protection

    with pytest.raises(ValueError, match="equity must be finite"):
        calculate_protection(equity=value, peak=200_000.0)


@pytest.mark.parametrize("value", [float("nan"), float("inf"), float("-inf")])
def test_dd_protection_rejects_non_finite_peak(value):
    from dd_protection import calculate_protection

    with pytest.raises(ValueError, match="peak must be finite"):
        calculate_protection(equity=200_000.0, peak=value)
