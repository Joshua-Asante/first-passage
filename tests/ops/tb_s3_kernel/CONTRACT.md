...
### Review refinements after `ea8a5db`

- Admission, combined position/order decisions, restart reconciliation and retry
  require one fresh full acquisition matching the latest position cursor. Two
  individually fresh reads from different acquisitions cannot authorize those actions.
- A command publishes its owners and all required effects in one atomic store write.
  Nested planning cannot publish a prefix. This includes account flatten, takeover,
  scoped cancellation/close, entry reservations and orphan cancellation. Evidence
  and immediate outcome transitions likewise persist their required follow-up work
  together: a rejected attachment cannot durably record a gap without its recovery.
- A single effect driver drains committed work, including work generated by outcomes.
  Outcome recording itself never dispatches, so restart can reconstruct unknown
  attempts before resuming effects. Covering completion cancels an owner's obsolete
  planned effects durably; those effects cannot restore a completed operation's block.
- A known lot link does not establish order ownership during restart. Working refs
  must belong to recorded pending orders, validated expected protection, or owned
  cancellation. An unexpected attached order halts its leg and is not auto-cancelled.
...
passed at the starting head. The initial redesign suite had 232 passing tests, including
