...
def test_add_uses_remaining_current_base(tmp_path, reenter):
    account, route = filled_owner(tmp_path)
    account.dispatch(intent(), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:57"), now=NOW)
    ...
    account.dispatch(replace(intent("close", kind="exit", qty=close_qty),
        side=Side.SELL), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:62"), now=NOW)
    if reenter:
        ...
        account.dispatch(intent("new"), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:69"), now=NOW)
    result = account.dispatch(intent("add-current", kind="add"), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:70"), now=NOW)
...
def test_wrong_side_never_reserves_or_sends(tmp_path, kind, leg_id):
    ...
    result = account.dispatch(replace(intent(kind=kind), leg_id=leg_id, side=wrong), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:84"), now=NOW)
...
def test_invalid_cancel_target_never_sends(tmp_path, target):
    ...
    if target == "other-leg":
        ...
        account.dispatch(replace(intent("close", kind="exit", qty=1), side=Side.SELL), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:98"), now=NOW)
        target_id = "close"
    else:
        target_id = {"missing": "missing", "terminal": "base"}[target]
    count = len(route.commands)
    result = account.dispatch(Cancel("dj30_mym_p250", target_id), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:103"), now=NOW)
...
def test_cancel_all_expands_only_pending_targets_of_its_leg(tmp_path):
    ...
    count = len(route.commands)
    account.dispatch(Cancel("orb_mnq_v7"), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:112"), now=NOW)
    assert len(route.commands) == count
    account.observe(BrokerFact.terminal("orb", "cancelled", 0, NOW), now=NOW)
    account.dispatch(Cancel("orb_mnq_v7"), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:121"), now=NOW)
    assert len(route.commands) == count
...
def test_restart_does_not_resend_retained_scheduled_close(tmp_path):
    account, route = filled_owner(tmp_path)
    account.dispatch(intent(), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:192"), now=NOW)
...
def test_invalid_close_terminal_cannot_release_a_flatten_remainder(tmp_path, status, cumulative):
    account, route = filled_owner(tmp_path)
    account.dispatch(intent(), occurrence=account.make_occurrence("direct", "test_pr409_owner_lifecycle:222"), now=NOW)
    start = SESSION.flatten_start
    account.advance_schedule(now=start)
