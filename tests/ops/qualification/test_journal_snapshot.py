...
def test_fixed_empty_snapshot_vector():
    from c1_rail.qualification.journal_snapshot import (
        encode_assessment_snapshot,
        parse_assessment_snapshot,
    )

    raw = encode_assessment_snapshot(**snapshot_case())
    expected = (
        '{"attempt_id":"attempt-1","campaign_revision":0,"contract_sha256":"'
        + 'a' * 64
        + '","event_head":"'
        + 'd' * 64
        + '","executions":[],"policy_sha256":"'
        + 'c' * 64
        + '","schema":"qualification_journal_snapshot/v1","trust_domain_sha256":"'
        + 'b' * 64
        + '","validity":"VALID"}'
    ).encode()
    assert raw == expected
...
def test_invalid_snapshot_relationships(change):
    ...
    args['executions'] = (row,)
    if change == 'duplicate':
        args['executions'] = (row, row)
    elif change == 'missing_attestation':
        row['attestation_sha256'] = None
    elif change == 'premature_attestation':
        row['state'] = 'RUNNING'
    elif change == 'unknown_state':
        row['state'] = 'COMPLETE'
    elif change == 'bool_revision':
        args['campaign_revision'] = True
    elif change == 'bad_attempt':
        args['attempt_id'] = ''
...
def test_fixed_populated_snapshot_vector(validity):
    ...
    expected = (
        '{"attempt_id":"attempt-1","campaign_revision":3,"contract_sha256":"'
        + 'a' * 64
        + '","event_head":"'
        + 'd' * 64
        + '","executions":[{"attestation_sha256":"'
        + 'f' * 64
        + '","checkpoint":"N1","execution_id":"exec-1","execution_revision":2,"plan_sha256":"'
        + 'e' * 64
        + '","state":"ATTESTED"}],"policy_sha256":"'
        + 'c' * 64
        + '","schema":"qualification_journal_snapshot/v1","trust_domain_sha256":"'
        + 'b' * 64
        + '","validity":"'
...
def test_snapshot_checkpoint_order_is_canonical_and_unique(change):
    ...
    if change == 'reorder':
        doc['executions'].reverse()
    else:
        doc['executions'][1]['checkpoint'] = 'N1'
...
def funded_profile_v3():
    return {
        ...
        'installed_profile_sha256': 'a' * 64,
        'record_byte_limit': 131072,
...
def v5_snapshot():
    ...
    return {
        'schema': 'qualification_campaign_budget_snapshot/v5',
        'attempt_id': 'vector-attempt',
        'request_sha256': 'b' * 64,
        'profile': funded_profile_v3(),
        'budget': {
            'identity_sha256': '9' * 64,
            'maximum_cpu_seconds': 1500,
            'maximum_wall_seconds': 4000,
            'maximum_memory_bytes': 256_000_000,
            'n1_paths': 6,
            'n2_paths': 6,
            'part_a_initial_paths': 4,
            'part_a_expanded_paths': 8,
            'n3_paths': 6,
        },
        'start_clock': {
        ...
        'deadline_boottime_ns': 10_000_000_010,
        'state': 'BOUND',
        'validity': 'VALID',
        ...
        'works': [
            {
                ...
                'input_sha256': 'e' * 64,
                'reservation_bytes_b64': base64.b64encode(
                ...
                'observation_bytes_b64': base64.b64encode(
                    canonical_json_bytes(
                        {
                            ...
                            'campaign_scope_id': 'fpq-campaign',
                            'work_scope_id': 'fpq-payload',
                            'cpu_ns': 0,
                            'memory_peak_bytes': 0,
                            'oom_events': 0,
        ...
        'settled_cpu_ns': 20_000_000_000,
        'reserved_cpu_ns': 0,
        'remaining_cpu_ns': 1_480_000_000_000,
        'recoveries': [],
        'dispatches': [
            {
                'work_id': 'admission',
