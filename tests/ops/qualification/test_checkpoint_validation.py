...
def test_checkpoint_plan_is_the_retained_n1_subdocument(tmp_path):
    ...
    from c1_rail.qualification.checkpoint_plan import (derive_campaign_plan,
        derive_checkpoint_plan, derive_n1_plan)
    ...
    plan = derive_campaign_plan(context.contract, policy=context.policy,
        execution_release_sha256=context.domain.execution_release_sha256,
        source_bundle_sha256=_sha256_hex(case['index']), attempt_id=context.attempt_id,
        exact_depth_approval_sha256=context.exact_depth_approval.approval_sha256)
    sliced = derive_checkpoint_plan(plan, 'N1', None)
    direct = derive_n1_plan(context.contract, policy=context.policy,
        execution_release_sha256=context.domain.execution_release_sha256,
...
def test_checkpoint_operations_are_g5_only_with_closed_fields():
    ...
    base = dict(schema='qualification_campaign_request/v2', attempt_id='a1', checkpoint='N2')
    ...
    bad = dict(base, checkpoint='N1', operation='FETCH_CHECKPOINT_MEMBER',
               object_sha256='0' * 64, offset=-1, length=16)
    with pytest.raises(ValueError, match='bounded integer chunk offset'):
        protocol.parse_campaign_request(encoded(bad))
    bad = dict(base, checkpoint='N1', operation='STAGE_CHECKPOINT_ARTIFACT',
               role='r1', bytes_b64='')
    with pytest.raises(ValueError, match='bounded staged checkpoint artifact'):
        protocol.parse_campaign_request(encoded(bad))
    bad = dict(base, checkpoint='N1', operation='COMMIT_CHECKPOINT_ASSESSMENT',
               work_id='w1', candidate_bytes_b64='e30=', artifacts=[{'role': 'r'}])
...
def test_v1_builder_refuses_the_full_e1_family():
    ...
    with pytest.raises(ValueError):
        build_n1_evidence(contract=None, policy=None, worker_result_bytes=b'{}',
                          plan_bytes=b'{}', execution_attestation_bytes=b'{}',
                          journal_snapshot_bytes=b'{}', installed_release_bytes=b'{}')


def test_checkpoint_snapshot_parser_refuses_open_shapes():
    from c1_rail.qualification.journal_snapshot import (
        encode_campaign_checkpoint_snapshot, parse_campaign_checkpoint_snapshot)
    from c1_rail.qualification.contract import canonical_json_bytes as encoded
    good = dict(attempt_id='a1', checkpoint='N1', contract_sha256='1' * 64,
        trust_domain_sha256='2' * 64, policy_sha256='3' * 64, validity='VALID',
        campaign_revision=4, authority_head='4' * 64, event_head='5' * 64,
        campaign_state='BOUND', works=[dict(work_id='admission', phase='ADMISSION',
            state='COMPLETED', settled=True)],
        capture=dict(work_id='n1work', result_sha256='6' * 64, payload_sha256='7' * 64,
                     attestation_sha256='8' * 64),
        intent=dict(work_id='g5work', candidate_sha256=None),
        members=[dict(role='plan', sha256='9' * 64, byte_length=10),
                 dict(role='result', sha256='a' * 64, byte_length=10),
                 dict(role='payload', sha256='b' * 64, byte_length=10),
                 dict(role='attestation', sha256='c' * 64, byte_length=10),
                 dict(role='retained_bundle_index', sha256='d' * 64, byte_length=10)])
