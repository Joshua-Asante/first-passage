...
def test_fresh_profile_enrolls_exact_v7_projection(tmp_path):
    ...
    assert (
        json.loads(store.scheduler_status(ATTEMPT))['remaining_cpu_ns']
        == snap(store)['remaining_cpu_ns']
    )
    assert CampaignStore(ExecutionStore(store.store.path)).budget_snapshot(
        ATTEMPT
    ) == store.budget_snapshot(ATTEMPT)


def test_claim_funds_once_without_snapshot_decode_then_transfer_does_not_double_charge(
    ...
    reservation, enrollment = store.materialize_scheduler_bootstrap(
        ATTEMPT, 'worker', token, 'host1'
    )
...
@pytest.mark.parametrize(
    'change', [{'probe': 'cpu'}, {'role': 'probe_g5'}, {'signing_retry_of': 'parent'}]
)
...
@pytest.mark.parametrize(
    ...
    [
        ('extra', 1),
        ('reserved_cpu_ns', True),
        ('recovery_pending', 0),
        ('signing_work_ids', ['a', 'b']),
        ('state', 'FAKE'),
    ],
)
...
def test_insufficient_allowance_refuses_without_history_decode(tmp_path, monkeypatch):
    ...
    for index in range(11):
        ...
        store.reserve_work(
            ...
            'N1_G5',
            encoded(
                {'limits': config['phases']['N1_G5'], 'clock': clock(11), 'input_sha256': 'd' * 64}
            ),
...
def test_pending_negative_facts_commit_without_refund_or_current_export(tmp_path, action):
    ...
    if action == 'settle':
        ...
        with store.store.transaction() as c:
            revision = store._budget(c, ATTEMPT)['authority_revision']
...
@pytest.mark.parametrize(
    ...
    [
        ('input_sha256', 'f' * 64),
        ('claimed_event_head', 'f' * 64),
        ('claimed_authority_head', 'f' * 64),
    ],
)
...
def test_actual_recovery_adapter_cleans_materialized_work_while_bootstrap_pending(
    tmp_path, monkeypatch
):
    import base64
    ...
    enrollment = sup.prepare_campaign_work(
        store,
        ATTEMPT,
        'admission',
        host_run_id='host1',
        manifest_bytes=manifest,
        clock_bytes=encoded(clock()),
    )
    ...
    class Runtime:
            ...
            return encoded(
                {
                    ...
                    'work_id': 'admission',
                    'clock': clock(12),
                    'campaign_scope_id': enrolled['scopes']['campaign_slice'],
                    'work_scope_id': enrolled['scopes']['payload_slice'],
                    'cpu_ns': 20,
        ...
        def cleanup(self, enrolled):
            cleaned.append(enrolled['work_id'])
...
def test_concurrent_duplicates_allocate_one_live_owner(tmp_path):
    ...
    def contender():
        local = CampaignStore(ExecutionStore(store.store.path))
        barrier.wait()
        return claim(local)
...
def test_sqlite_ordered_void_race_matches_independent_funding_model(tmp_path, void_first):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        scheduled = pool.submit(scheduler)
        cancelled = pool.submit(cancel)
        token, _ = scheduled.result(timeout=30)
...
def uncertain_clocks():
    unavailable = clock(11)
    unavailable['boottime_ns'] = None
    return [
        pytest.param(clock(11, boot='boot-2'), id='changed_boot'),
        pytest.param(clock(9), id='backward'),
...
def test_uncertain_clock_refusal_survives_reopen_and_negative_recovery(tmp_path, observed):
    ...
    with store.store.transaction() as c:
        doc = store._funding(c, ATTEMPT)
        assert doc['terminal_overlay'] == {'state': 'BUDGET_UNCERTAIN', 'clock': observed}
...
def test_v7_upgrade_preserves_existing_history_and_rejects_downgrade(tmp_path):
    ...
    store = metered(tmp_path)
    historical = store.budget_snapshot(ATTEMPT)
    config = profile()
    config.update(
        schema='qualification_campaign_budget_profile/v3',
        funding_intents='qualification_campaign_funding/v1',
    ...
    fresh = json.loads(request())
    fresh['attempt_id'] = 'fresh'
...
@pytest.mark.parametrize(
    ...
    [('full_campaign_funding', 'attempt_id'), ('full_campaign_bootstraps', 'work_id')],
)
...
def test_claim_cannot_point_to_a_valid_but_ineligible_historical_head(tmp_path):
    ...
    with store.store.transaction() as c:
        first = c.execute(
            'SELECT sha256 FROM full_campaign_budget_events ORDER BY sequence LIMIT 1'
        ).fetchone()[0]
        doc = json.loads(c.execute('SELECT body FROM full_campaign_bootstraps').fetchone()[0])
...
def test_compact_fixed_signing_retry_preserves_parent_authority_and_own_charge(tmp_path):
    ...
    observed.update(
        ...
        orchestration_charge_cpu_ns=10,
        termination_known=True,
        campaign_scope_id=scope['campaign_slice'],
        work_scope_id=scope['payload_slice'],
    )
...
@pytest.mark.parametrize(
    'damage', ['missing_work', 'stale_work', 'stale_campaign', 'orphan_pending', 'missing_intent']
)
def test_negative_write_never_repairs_compact_predecessor(tmp_path, damage):
    store = enrolled(tmp_path)
    if damage in ('orphan_pending', 'missing_intent'):
        claim(store)
    with store.store.transaction() as c:
        before = bytes(c.execute('SELECT snapshot_bytes FROM full_campaign_budgets').fetchone()[0])
        if damage == 'missing_work':
            c.execute('DELETE FROM full_campaign_funding_works')
        elif damage == 'missing_intent':
            c.execute('DELETE FROM full_campaign_bootstraps')
        elif damage == 'orphan_pending':
            doc = store._funding(c, ATTEMPT)
            doc['bootstrap_pending_work_id'] = None
            c.execute('UPDATE full_campaign_funding SET body=?', (encoded(doc),))
        else:
            table = (
                'full_campaign_funding_works' if damage == 'stale_work' else 'full_campaign_funding'
            )
            doc = json.loads(c.execute('SELECT body FROM ' + table).fetchone()[0])
            if damage == 'stale_work':
                doc['settled'] = True
            else:
                doc['remaining_cpu_ns'] -= 1
            c.execute('UPDATE ' + table + ' SET body=?', (encoded(doc),))
    ...
    with store.store.transaction() as c:
        assert (
            bytes(c.execute('SELECT snapshot_bytes FROM full_campaign_budgets').fetchone()[0])
            == before
        )
        assert [
            tuple(row) for row in c.execute('SELECT body FROM full_campaign_funding')
        ] == damaged
...
def test_funded_profile_cannot_survive_removed_v7_enrollment(tmp_path):
    ...
    with store.store.transaction() as c:
        for table in (
            'full_campaign_bootstraps',
            'full_campaign_funding_works',
            'full_campaign_funding',
        ):
