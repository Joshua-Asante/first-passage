...
def campaign(tmp_path, monkeypatch, *, t=11):
    ...
    status = CampaignStore(instance.store).finish_diagnostic_admission(
        request_bytes, context, plan, admission_observation(case, state),
        now=NOW, trusted_keys=case['keys'])
...
def clock(state):
    return dict(schema='qualification_campaign_clock/v1', boot_id='boot-1',
                boottime_ns=state['t'], utc='2026-09-21T00:00:00Z')


def submit(instance, case):
    raw = encoded(dict(schema='qualification_campaign_request/v2', operation='SUBMIT_E1',
                       attempt_id=case['attempt_id'], bundle_sha256=sha256(case['index']),
...
def admission_observation(case, state):
    ...
    return encoded(dict(schema='qualification_campaign_observation/v2', attempt_id=case['attempt_id'],
        work_id='admission', clock=clock(state),
        campaign_scope_id=scopes['campaign_slice'], work_scope_id=scopes['payload_slice'],
        cpu_ns=0, memory_peak_bytes=0, oom_events=0, termination_known=True,
...
def claim(instance, raw=None):
    return store(instance).claim_scheduler_bootstrap(raw or schedule_document(instance),
                                                      supervisor.observe_campaign_clock())


def materialized(instance, raw=None):
    token, status = claim(instance, raw)
    assert token is not None, 'claim refused: ' + json.loads(status.decode() if isinstance(status, bytes) else status).get('state', '?')
    document = funding.parse_request(raw or schedule_document(instance))
    return store(instance).materialize_scheduler_bootstrap(instance.attempt, document['work_id'],
                                                            token, 'host1')


def transition(instance, work, state, data=None):
    return store(instance).record_work_transition(instance.attempt, work,
        encoded(dict(schema='qualification_campaign_work_transition/v1', attempt_id=instance.attempt,
                     work_id=work, state=state, clock=json.loads(supervisor.observe_campaign_clock()),
                     data=data or {})), expected_revision=snap(instance)['authority_revision'])
...
def settle(instance, work, cpu=20):
    ...
    observation = encoded(dict(schema='qualification_campaign_observation/v2', attempt_id=instance.attempt,
        work_id=work, clock=json.loads(supervisor.observe_campaign_clock()),
        campaign_scope_id=scopes['campaign_slice'], work_scope_id=scopes['payload_slice'],
...
def retained_identity(instance, work):
    ...
    scopes = supervisor.work_enrollment('host1', instance.attempt, work)
    event = encoded(dict(schema='qualification_campaign_supervision_event/v2',
        attempt_id=instance.attempt, work_id=work, kind='PROCESS',
...
def dispatched(instance, work):
    ...
    with campaigns.launch_gate(instance.attempt, work, supervisor.observe_campaign_clock, role='payload') as permit:
        pass
    campaigns.acknowledge_dispatch(instance.attempt, work, 'payload', permit['token'],
                                   supervisor.observe_campaign_clock)


def n1_plan(instance):
    from c1_rail.qualification.checkpoint_plan import derive_checkpoint_plan
    return derive_checkpoint_plan(store(instance).retained_object(instance.attempt, 'plan'), 'N1', None)


def result_document(instance, work='n1work', payload=b'archived-worker-frame'):
    scopes = supervisor.work_enrollment('host1', instance.attempt, work)
    state = snap(instance)
    return encoded(dict(schema='qualification_campaign_checkpoint_result/v1',
        attempt_id=instance.attempt, checkpoint='N1', work_id=work, campaign_id='campaign-1',
        plan_sha256=sha256(n1_plan(instance)), plan_byte_length=len(n1_plan(instance)),
        payload_sha256=sha256(payload), payload_byte_length=len(payload),
        worker_execution_id=work, container_id='9' * 64,
        worker_image_digest='sha256:' + '8' * 64, runtime_manifest_sha256='7' * 64,
        ...
        limits=dict(cpu_ns=state['profile']['phases']['N1']['cpu_ns'],
...
def enrolled_capture(tmp_path, monkeypatch):
    ...
    store(instance).retain_checkpoint_capture(instance.attempt, 'n1work', result, payload,
                                              capture_transition)
...
def attestation_document(instance, result, payload):
    ...
    return encoded(dict(schema='qualification_campaign_checkpoint_attestation/v1',
        payload=dict(schema='qualification_campaign_checkpoint_attestation_payload/v1',
            ...
            payload_byte_length=len(payload), plan_sha256=parsed['plan_sha256'],
            execution_release_sha256=sha256(instance.release), profile_sha256=instance.profile.sha256,
            service_id=release['service_id'], worker_image_digest=parsed['worker_image_digest'],
            runtime_manifest_sha256=parsed['runtime_manifest_sha256'], container_id=parsed['container_id'],
            capture=dict(exit_code=0, oom_killed=False,
...
def candidate_document(instance, decision='CONTINUE', revision=None):
    ...
    return encoded(dict(schema='qualification_campaign_checkpoint_assessment/v1',
        attempt_id=instance.attempt, checkpoint='N1', work_id='n1work',
        binding=dict(contract_sha256='1' * 64, trust_domain_sha256='2' * 64, policy_sha256='5' * 64,
                     execution_release_sha256='3' * 64),
        snapshot=dict(campaign_revision=revision or snapshot['campaign_revision'],
                      authority_head=snapshot['authority_head'], snapshot_sha256=sha256(raw)),
        capture=dict(result_sha256=capture['result_sha256'], payload_sha256=capture['payload_sha256'],
                     attestation_sha256=capture['attestation_sha256']),
        stages=[dict(stage='LEGALITY', status='PASS', input_sha256='b' * 64, output_sha256='c' * 64,
                     population_counts={}),
                dict(stage='N1', status='FAIL' if decision == 'FAILURE' else 'PASS',
                     input_sha256='d' * 64, output_sha256='e' * 64,
                     population_counts={'FULL': 2, 'H1': 2, 'H2': 2})],
        decision=decision, n1_decision='FAIL' if decision == 'FAILURE' else 'PASS',
        cutoff=dict(checkpoint='N1', n1_cutoffs={'FULL': 0, 'H1': 0, 'H2': 0}),
        n2_thresholds=dict(bound_to='1' * 64,
                           stages=[dict(stage='N2', exact_depth=2, max_failures_per_population=0)]),
        artifacts=[dict(role='attempt_journal', sha256='f' * 64, byte_length=10)],
        signature=dict(algorithm='Ed25519', key_id='test-producer',
...
def persisted_intent(instance, candidate=None):
    ...
    intent = encoded(dict(schema='qualification_campaign_checkpoint_intent/v1',
        attempt_id=instance.attempt, checkpoint='N1', work_id='g5work', key_id='test-producer',
        signing_at_utc='2026-09-21T01:00:00Z', candidate_sha256=sha256(candidate),
        snapshot_sha256=sha256(snapshot)))
    capture = encoded(dict(schema='qualification_campaign_g5_capture/v1',
        attempt_id=instance.attempt, checkpoint='N1', work_id='g5work',
        candidate_sha256=sha256(candidate), staged=[]))
    capture_transition = encoded(dict(
        schema='qualification_campaign_work_transition/v1', attempt_id=instance.attempt,
        work_id='g5work', state='CAPTURED', clock=json.loads(supervisor.observe_campaign_clock()),
        data=dict(capture_bytes_b64=base64.b64encode(capture).decode('ascii'))))
    signing_transition = encoded(dict(
        schema='qualification_campaign_work_transition/v1', attempt_id=instance.attempt,
        work_id='g5work', state='SIGNING_INTENT', clock=json.loads(supervisor.observe_campaign_clock()),
        data=dict(intent_id='g5work-intent',
                  payload_bytes_b64=base64.b64encode(candidate).decode('ascii'),
                  key_id='test-producer', signing_at_utc='2026-09-21T01:00:00Z')))
    campaigns.persist_checkpoint_intent(instance.attempt, 'g5work', snapshot, intent, candidate,
                                        capture_transition, signing_transition)
    return candidate


def cutoff_document(instance, candidate):
    return encoded(dict(schema='qualification_campaign_cutoff_receipt/v1',
        attempt_id=instance.attempt, checkpoint='N1', assessment_sha256=sha256(candidate),
        decision=json.loads(candidate)['decision'], n1_cutoffs={'FULL': 0, 'H1': 0, 'H2': 0},
        n2_thresholds=[dict(stage='N2', exact_depth=2, max_failures_per_population=0)],
        n2_bound_to=sha256(candidate), created_utc='2026-09-21T02:00:00Z'))


def commit(instance, candidate, cutoff=None):
    cutoff = cutoff or cutoff_document(instance, candidate)
    return store(instance).commit_checkpoint_assessment(instance.attempt, 'g5work', candidate,
        cutoff, now=datetime(2026, 9, 21, tzinfo=timezone.utc),
...
def test_dispatch_roles_require_completed_admission_and_noop_probe(tmp_path, monkeypatch):
    ...
    with instance.store.transaction() as connection:
        connection.execute("DELETE FROM full_campaign_objects WHERE attempt_id=? AND role='diagnostic_receipt'",
                           (instance.attempt,))
    ...
    store(instance2).materialize_scheduler_bootstrap(instance2.attempt, document['work_id'], token, 'host1')
...
def test_capture_retains_bytes_and_migrates_the_v6_family(tmp_path, monkeypatch):
    ...
    import re as _re
    with pytest.raises(ValueError, match=_re.compile('immutable checkpoint capture differs|archived payload differs')):
        reopened.retain_checkpoint_capture(instance.attempt, 'n1work', result, b'substituted',
            encoded(dict(schema='qualification_campaign_work_transition/v1', attempt_id=instance.attempt,
                         work_id='n1work', state='CAPTURED', clock=json.loads(supervisor.observe_campaign_clock()),
                         data=dict(capture_bytes_b64=base64.b64encode(result).decode('ascii')))))
...
def test_snapshot_serves_members_and_bounded_chunks(tmp_path, monkeypatch):
    ...
    whole = json.loads(store(instance).fetch_checkpoint_member(instance.attempt, members['payload'],
                                                               0, 1024 * 1024))
    assert whole['total_byte_length'] == len(payload)
    part = json.loads(store(instance).fetch_checkpoint_member(instance.attempt, members['payload'], 2, 4))
    ...
    with pytest.raises(ValueError, match='outside member'):
        store(instance).fetch_checkpoint_member(instance.attempt, members['payload'], len(payload) + 10, 4)
    staged = store(instance).stage_checkpoint_artifact(instance.attempt, 'attempt_journal', b'staged-bytes')
...
def test_void_before_commit_refuses_the_commit(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match='VOID campaign'):
        store(instance).commit_checkpoint_assessment(instance.attempt, 'g5work', candidate, cutoff,
...
def test_intent_survives_restart_and_exact_retry_completes(tmp_path, monkeypatch):
    ...
    candidate = persisted_intent(instance)
    reopened = CampaignStore(ExecutionStore(instance.store.path))
    assert json.loads(reopened.budget_snapshot(instance.attempt))['checkpoints']['N1']['state'] == 'ASSESSING'
    reopened.commit_checkpoint_assessment(instance.attempt, 'g5work', candidate,
