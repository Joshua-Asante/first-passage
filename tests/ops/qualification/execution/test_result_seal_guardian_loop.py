...
class Campaigns:
    """The CampaignStore surface the loops use outside the result store."""
    ...
    def _transaction(self):
        """A no-op store transaction."""
        yield None

    @contextmanager
    def launch_gate(self, *_, **__):
        """Grant the dispatch permit."""
        yield {'token': 'tok'}

    def acknowledge_dispatch(self, *_):
        """Accept the dispatch acknowledgement."""
        pass

    @staticmethod
    def _raw(value):
        """Decode a base64 retained value."""
        return base64.b64decode(value)

    @staticmethod
    def _work(state, work_id):
        """The work as the loop sees it after its commit."""
...
def fake_store(base, family):
    ...
    class Store:
        """The result/seal store surface; its campaign state is test-driven."""
        ...
        def _state(self):
            """The campaign snapshot for the test-driven state."""
            return {'attempt_id': 'att1', 'state': Store.state_name, 'validity': 'VALID',
                    'authority_revision': 7, 'last_clock': {}, 'works': [],
                    'phase': Store.phase}

        def result_state_bytes(self, *_):
            """The campaign snapshot bytes."""
            return encoded(self._state())

        def retain_result_supervision_event(self, *_):
            """Accept a PROCESS supervision event."""
            pass

        def settle_result_work(self, *args):
            """Record the settlement observation."""
            Store.settled.append(args[-1])
            return encoded(self._state())

        def record_result_transition(self, *args, **_):
            """Record a work transition."""
            Store.transitions.append(json.loads(args[2])['state'])
            return encoded(self._state())

        _completion_states = base._completion_states
        _commit_progressions = base._commit_progressions

        def _result_family(self, *_):
            """The committed result family."""
            return {'state': family}

        def _seal_family(self, *_):
            """The sealed family state."""
            return family
...
def run(tmp_path, monkeypatch, *, phase, start, committed=None, commit_after=1, exit_after=3):
    """Drive one RESULT or SEAL guardian loop to settlement."""
    ...
    class Runtime:
        """The Linux runtime's observation, reduced to the cgroup's population."""

        def __init__(self):
            self.parent = parent

        def observation(self, *_):
            """Sample the unit cgroup's population at settlement."""
    ...
    if committed is not None:
        def commit(sleeps):
            """Move the snapshot to the commit state after ``commit_after`` sleeps."""
...
def test_result_unit_is_supervised_from_its_statistical_predecessor(
    ...
    """Result unit is supervised from its statistical predecessor."""
...
def test_seal_unit_is_supervised_from_the_committed_pass(tmp_path, monkeypatch):
    """Seal unit is supervised from the committed pass."""
...
def test_commit_while_the_unit_runs_settles_only_after_the_cgroup_empties(
    ...
    """Commit while the unit runs settles only after the cgroup empties."""
...
def test_a_committed_unit_that_never_exits_is_observed_at_the_bound(tmp_path, monkeypatch):
    """A committed unit that never exits is observed at the bound."""
...
def test_an_authority_ended_campaign_still_stops_supervision(
    ...
    """An authority ended campaign still stops supervision."""
