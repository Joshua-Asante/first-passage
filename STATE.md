...
- **Deep-iteration lane — §4(c) supply-side audit DELIVERED 2026-08-23; verdict `AMBIGUOUS`; slot 3 recommended held, gated on a supply event.** The audit ([note](docs/notes/audits/programme-audit/2026-08-23-deep-lane-supply-audit.md)) discharged the tripped §4(c) report duty early (operator instruction) — process clean on all seven diagnostic questions, zero falsifier drift; the binding finding is supply: no candidate family is simultaneously genuinely-new-mechanism, reachable with current data, and venue-shape-compatible. Nearest satisfier: **`MNQFLOW-1-DEPTH` (order-flow modality) — FROZEN, ≤$125, K=0, one operator sign-off away** ([prereg](lab/analysis/c1/mnq_orb_flow_depth_2026-08-18/PREREG.md)); free alternate: a published MOC-imbalance cohort δ (auction-limb re-open); blind channel unsourced (1/3). Re-test: any supply event, else the 2026-11-08 slate escalates the lane to PARK/supersede. Background facts: DL-2's construction retired for M6A on the geometric-feasibility diagnostic (median R=1.0/0.687; 85–97% of fired trades never reach 1R); DL-1 is a separate candidate-level failure — no shared template defect. [`DL-2 Iterate block`](lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md) · [`charter`](docs/adr/2026-08-16-deep-iteration-lane-charter.md)
