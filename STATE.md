...
- **2026-08-15** — **Pre-G0 kills are NOT §4 strikes (blind channel).** Boundary is `register_search open`. Ratified alongside: **mandatory pre-G0 kill counting + disclosure** with every §4 reading (count now **1**), because the ruling makes §4 harder to fire. Consecutive-kill threshold later elected N=3 on the same ADR. $0/K=0. [`addendum`](docs/adr/2026-08-15-no-counterparty-statistical-sourcing-channel.md)
...
- **Harvest-intake §4 doctrine falsifier + idle guard** — limb 1 still 0-of-2; limb 2 `Accepted` (R10 GO); historical-kill pin unmarked (not fired). Idle = zero screen-PASS seeds beyond D5. [`harvest ADR §4`](docs/adr/2026-07-15-external-mechanism-harvest-intake.md)
