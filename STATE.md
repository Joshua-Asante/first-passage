...
- **2026-08-21** — **`REGIME-CANDIDATE` flag lane ratified `Accepted`** (operator override-free — its own §2 trigger is plain ratification, no evidentiary gate blocked it). ST-EH-1, the one existing instance, closed `operator-stopped` the same day it was drafted with zero of 80 grid reads executed; §4's forward falsifier has no evidence yet either way. §4 bootstrap check rides the 2026-11-08 audit. Blast-radius sweep found the ADR's own template target (`docs/ltm/briefs/rnd-pipeline/discovery-campaign-template.md`) absent from this tree despite being cited as canonical owner of Default #1–#6 by 8+ live briefs and one Accepted ADR — flagged separately, not fabricated. $0/K=0. [`ADR`](docs/adr/2026-07-26-regime-candidate-flag-lane.md).
...
- **2026-08-21** — **`Q-M1WIRE-1` closed `FALSIFIED`.** A2 (production write path for confirmed-base) and A5 (fixture-hash drift wired into an arm check or `gates.yml`) both directly confirmed absent — `require_resolved=True` is not a sufficient proxy for "every M1 capability is live-wired." A4 (alert reachability, unannounced, <10 min) stays untested, not load-bearing to the verdict. Live effect: none — rail stays disarmed, M1 stays not-`RESOLVED` (item 5 + `operator_signoff` remain the only bars, unchanged). $0/K=0. [`closure`](docs/briefs/closures/Q-M1WIRE-1-closure-falsified.md) · [`M1 ADR`](docs/adr/2026-07-22-c1-venue-native-monitoring-maturity.md).
