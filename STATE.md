...
⚠ **A `$0/K=0` tag below means governance, audit, or ADR-authoring work closed — not that P1
(`PIPELINES.md`) went idle.** Most rows below carry it because that is what most of this index *is*:
doctrine and process decisions, not discovery campaigns. Read it as the visible, named-exception
shape the (`Proposed`) three-speed design calls **Vet** — no fresh outcome-bearing look, §2.4 of
[`the design`](docs/superpowers/specs/2026-09-01-three-speed-alpha-research-design.md) — never as P1's
default operating mode. K-bearing exploration is tracked in `discovery_manifests/`, not here; an
empty run of `$0/K=0` rows here is not evidence P1 is inactive, and is not evidence it is active
either — check the manifests.
...
- **Four of the six 2026-08-30 ADRs — mechanical-enforcement implementation each owed its own STATE.md forward-board row, no tracked home until this row.** `2026-08-30-evaluation-order.md` §6, `2026-08-30-terminal-taxonomy.md` §6, `2026-08-30-tradeable-reachable-gate.md` §6, and `2026-08-30-operator-approvals-campaign-envelope.md` §6 each name a "new forward-board row" as an owed downstream artifact for their own mechanical enforcement — respectively: the selection-freeze hash-pinning tool + zero-K state-drift re-check script; the `N_expr` ordinal integrity check + `rejected_candidates.md`/`ops/instruments/<SYM>.md` schema fields; the `TRADEABLE-REACHABLE` delegating orchestrator; and the contract-integrity `α`/`M`/procedure check + probe-tranche citation bookkeeping — every one of the four **explicitly not licensed by its own ratification** (each ADR's own Ratification note). This row discharges all four §6 obligations at once, the same row-sharing convention `2026-08-30-channel-liveness-gate.md` and `2026-08-30-candidate-contract.md` already used below. All six ADRs read `Effective: immediately upon acceptance` — doctrine binds now; code may lag, per the HARV-lane precedent every one of the six cites (§0/§7). This row is that deferral, dated, not a claim that any of the four is built. Check due at the next quarterly programme audit, alongside the row below. [`evaluation-order`](docs/adr/2026-08-30-evaluation-order.md) · [`terminal-taxonomy`](docs/adr/2026-08-30-terminal-taxonomy.md) · [`tradeable-reachable-gate`](docs/adr/2026-08-30-tradeable-reachable-gate.md) · [`operator-approvals-campaign-envelope`](docs/adr/2026-08-30-operator-approvals-campaign-envelope.md)
