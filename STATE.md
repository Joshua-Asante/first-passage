...
- **Deep-iteration lane — §4(c) supply-side audit DELIVERED 2026-08-23 (`AMBIGUOUS`); its named nearest supply lead, `MNQFLOW-1-DEPTH`, is now BLOCKED AT P0 TWICE — operator decision owed with materially better cost information than either the audit or the original freeze had.** The audit ([note](docs/notes/audits/programme-audit/2026-08-23-deep-lane-supply-audit.md)) named `MNQFLOW-1-DEPTH` (order-flow) as the nearest reachable supply lead, "one sign-off away." Two independent, non-overlapping 30-day systematic samples of its own frozen 255-trigger population have now each been P0-priced and each blocked: original sample **$148.04**, redrawn sample (`PREREG_S2B.md`) **$154.73**, both against the **$125.00** ceiling. **This reads as structural, not unlucky draws** — both real-cost means (≈$4.93/day, ≈$5.16/day) run well above the $3.33–$3.97/day single-day extrapolation the original $125 ceiling was set from. Per the redraw's own pre-committed FM-9, no third resample follows automatically. **Named forward paths:** raise the ceiling to ≈$160–175 (now informed by two real draws, not one extrapolated day — still needs confirmation the Avenue-A credit extends that far); a smaller N (e.g. 20 of 255, ≈$100–103 at the measured pooled rate, its own fresh pre-registration, trading some of S8's already-disclosed power); or decline. $0 spent across both attempts. The free alternate (a published MOC-imbalance cohort δ) and the blind channel (unsourced, 1/3) remain the estate's other two named supply routes. Background: DL-2's construction retired for M6A (geometric-feasibility diagnostic, median R=1.0/0.687, 85–97% of fired trades never reach 1R); DL-1 is a separate candidate-level failure — no shared template defect. [`PREREG.md`§9.2](lab/analysis/c1/mnq_orb_flow_depth_2026-08-18/PREREG.md) · [`PREREG_S2B.md`§9.2](lab/analysis/c1/mnq_orb_flow_depth_2026-08-18/PREREG_S2B.md) · [`charter`](docs/adr/2026-08-16-deep-iteration-lane-charter.md)
