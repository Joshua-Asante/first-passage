...
- **Deep-iteration lane — geometric-feasibility-ratio audit RESOLVED 2026-08-22: DL-2's construction retired for M6A; operator decision owed on the lane's next move.** DL-2 abandoned 2026-08-22 (2nd consecutive after DL-1) on catastrophic force-flat/non-resolution dominance. The diagnostic named at closure confirmed it quantitatively: median full-session achievable range barely equals the reference-window width at 1-session lookback (R=1.0, only 7.8% of sessions ever have 2R-target room) and falls short at 2-session lookback (R=0.687); among trades that actually fired, 85–97% never complete a full 1R favorable move. The opposite-prior-extreme stop/target + single-session-hold construction is retired for M6A specifically — untested for generality elsewhere. Operator decision owed: whether any DL-3 targets a different instrument under this same construction (now requiring its own feasibility-ratio clearance first, per the entry packet) or a genuinely different construction, or whether the lane pauses for a broader template re-audit given 2/3 of its budget is now spent with zero survivors. DL-1's own abandonment remains a separate, candidate-level failure (reconciled 2026-08-22) and does NOT corroborate a shared template defect. [`DL-2 Iterate block`](lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md) · [`DL-1 Iterate block`](lab/archive/dl1_mgc_orc_2026-08-16/RESULTS.md) · [`charter`](docs/adr/2026-08-16-deep-iteration-lane-charter.md)
