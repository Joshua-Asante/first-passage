...
Exit codes:
    0 — check passed or regenerate completed (incl. public-clone / bare-worktree
        soft-degrade: a manifest dir whose SHA256SUMS lists files but ZERO of
        those files exist on disk is WARN-skipped — mirrors check_pine_manifest)
    1 — check failed (drift, missing, extra, parse error) or invalid CLI

Soft-degrade is per-directory and never applies to --regenerate. Partial
presence (≥1 listed file on disk) keeps full hard-fail semantics — that is
exactly the M-9 drift case the gate exists for.
"""
...
#
# Pepperstone (tv_exports/pepperstone + .../bar_export) left this contract
# 2026-08-02 with the feed itself — ADR 2026-08-02-pepperstone-feed-retirement.
# The bytes were deleted from the checkout after a verified offline copy; hashes
# survive in docs/ltm/notes/2026-08-02-pepperstone-data-tombstone.md (canonical;
# an independent 2026-08-03 record of the same retirement exists as a corrected
# stub at .../2026-08-03-pepperstone-data-tombstone.md). Five dirs became three.
#
# bar_data remains in the contract, narrowed 2026-08-03 (ADR
# 2026-08-03-bar-data-cfd-and-candidates-retirement, supersedes Pepperstone §2-F
# in part): KEEP only the three frozen CME micros (6J/MNQ/MYM_M15). CFD-era
# panels and tv_exports/candidates were deleted after a verified offline copy;
# hashes in docs/ltm/notes/2026-08-03-bar-data-cfd-candidates-tombstone.md.
# Producer (scripts/parse_bar_export.py → pepperstone/bar_export) is still dead —
# panels are usable, not regenerable without an offline restore.
MANIFEST_DIRS = [
    REPO_ROOT / "core/data/tv_exports/cme",
...
def manifest_relative_dirs() -> tuple[str, ...]:
    """Return machine-readable repository-relative manifest owners."""
    return tuple(path.relative_to(REPO_ROOT).as_posix() for path in MANIFEST_DIRS)
...
def check_all() -> int:
    ...
    for d in MANIFEST_DIRS:
        ...
        # Public-clone / bare-worktree soft-degrade: SHA256SUMS lists one or
        # more files but ZERO of those listed files exist on disk → WARN and
        # skip this dir. Partial presence keeps full hard-fail below.
        if manifest_names and not (manifest_names & set(on_disk)):
            print(
                f"WARN absent-tree skip {_rel_posix(d)} "
                f"(SHA256SUMS lists {len(manifest_names)} file(s); none on disk)",
                file=sys.stderr,
            )
            continue
...
def main() -> int:
    ...
    parser.add_argument(
        "--list-dirs",
        action="store_true",
        help="print the owned manifest directories as a JSON array",
    )
    args = parser.parse_args()

    if args.list_dirs:
        if args.check or args.regenerate or args.dry_run:
            print("--list-dirs cannot be combined with another mode", file=sys.stderr)
            return 1
        print(json.dumps(manifest_relative_dirs()))
        return 0
