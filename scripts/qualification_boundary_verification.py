...
def cases_refusal(args):
    ...
    A diagnostic subset exists for S3/S4 iteration only. A whitespace-only value is
        ...
        with RunRecord(ROOT, output, sys.argv if argv is None else argv) as record:
            ...
            try:
                ...
                with ownership_lock(manifest_path.parent):
                    ...
                    if args.test_only or args.s2 or args.s3 or args.s4:
                        selected_cases = S4_CASES if args.s4 else S3_CASES if args.s3 else S2_CASES
                        invariant_bytes = INVARIANT_MANIFEST.read_bytes()
                        all_required = _manifest(invariant_bytes)
                        if args.s3 or args.s4:
                            ...
                            # N1_ONLY (--test-only): every registered node outside the S4
                            # file set (S4_CASES includes the S3 and S2 files). Those nodes
                            # skip here by design, and the manifest validator refuses a
                            # required node that is skipped or never collected.
                            required = {node for node in all_required
                                        if not node.startswith(tuple(case+'::' for case in S4_CASES))}
                        record.data['metadata'].update(
                            acceptance_scope=('S4_JOINT_N2' if args.s4 else
                        ...
                        if args.cases is not None:
                            if not (args.s3 or args.s4):
                                raise ValueError('--cases is a diagnostic-subset selector for S3/S4 iteration only')
                    ...
                    if args.test_only or args.s2 or args.s3 or args.s4:
                        ...
                        selection = ([*selected_cases] if (args.s2 or args.s3 or args.s4) else
                            ['tests/integration/qualification_boundary', *('--ignore='+case for case in S4_CASES)] + sorted(
