...
- If routing is **Forward**: state the re-MC trigger that fired (Guardian safe band). Suggest the user review `dd_protection.py` and re-run the vendor-free engine regression (`tests/core/test_mc_synthetic_engine.py`) if MC inputs changed. Do not treat the Pepperstone `portfolio_mc` CLI as the re-MC.
