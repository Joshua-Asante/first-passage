...
Steps 4–5 are enforced by `scripts/guard_s2_runs.py` (Claude PreToolUse Bash hook + git pre-push): a push while a PR run is in flight is refused, and so is a full dispatch that adds no coverage on its SHA: a live or passed run whose mode covers the request (`[s3]` covers `[s2]`, never the reverse), or a failed run whose nodes the request would all re-run. Without `--ref` the guard checks the default branch, as `gh` does. Override only with a reason you can state: `FP_S2_GUARD=off`.
