...
Iterating on one failing case: `gh workflow run qualification-s2-supervision.yml --ref <branch> -f cases='<pytest -k expr>'`. The record is `acceptance_scope=DIAGNOSTIC_SUBSET`, the job fails its invariant gate by design, and `s2_run_evidence.py` exits 3 with a refusal — read the printed `junit` totals for the selected cases. It runs in its own concurrency group, so it does not cancel a full run on the same ref. Acceptance still needs a full run on the final head.
Still open (S3-era tooling, not for a worker to improvise): shard the suite across two fresh hosts (OOM case last on its shard); cache the provisioned venv/worker image; a labelled `workflow_dispatch` `-k` diagnostic mode (invariants fail by design, so it can never pass as acceptance); move Windows line 3 to a hosted runner once the source-bound recorder runs there.
