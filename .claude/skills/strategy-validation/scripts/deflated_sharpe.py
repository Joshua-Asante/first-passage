#!/usr/bin/env python3
"""Thin launcher — canonical module: lab/research_utils/deflated_sharpe.py.
...
_LAB = _REPO / "lab"
if not (_LAB / "research_utils" / "deflated_sharpe.py").is_file():
    sys.exit(
        "ABORT: lab/research_utils/deflated_sharpe.py not found under "
        f"{_REPO}. Skill launchers only work from a monorepo checkout. "
        "Use: PYTHONPATH=lab python -m research_utils.deflated_sharpe …"
    )
...
raise SystemExit(
    subprocess.call(
        [sys.executable, "-m", "research_utils.deflated_sharpe", *sys.argv[1:]],
        env=_ENV,
    )
)
