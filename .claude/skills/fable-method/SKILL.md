...
4. **Read narrow, never re-read.** Search to locate, then read the section. Input that will not fit one context — 10+ files, 50k+ tokens, or pairwise/multi-hop reasoning across scattered sources — runs `work-decomposition`'s input-shaped protocol (size, filter, batch, depth-1 wave, spot-check) instead of being read into this session.
