"""
Firm rules as config: per-product defaults composed with explicit per-tier
overrides (``_tiers``). Add a firm by adding both; every resolved row is pinned
by tests/core/fixtures/firm_rules_resolved.json.
...
def _tiers(product: dict, tiers: dict[str, dict]) -> dict[str, dict]:
    """Resolve one product line: shared defaults, then each tier's explicit overrides.

    Every row is a fresh dict (nothing aliases the product block), so a runtime
    monkey-patch of one tier -- the lab eval-lock studies do this -- never leaks
    into a sibling tier or into the defaults.
    """
    return {key: {**product, **overrides} for key, overrides in tiers.items()}
...
# Firm-wide EVALUATION facts shared by every Tradeify eval row (Select and Growth).
_TRADEIFY_EVAL = {
    "dd_type": "trailing_locking",  # fixed-$ EOD-ratcheting trail (NOT %-of-peak `trailing`)
    "dd_lock_offset_usd": 1_000_000.0,  # unreachable -- eval has no lock (fixed 2026-08-04; art. 10495897)
    "daily_loss_pct": None,         # Flex: no daily loss limit (Growth: soft breach, see its row)
    "profit_target_pct": 6.0,       # $1,500 / $3,000 / $6,000 / $9,000 at 25K / 50K / 100K / 150K
    "weekend_holds": False,         # NinjaTrader 8 / Rithmic force-flat (16:45 ET regular; art. 10495876)
...
# Select product line: the eval-only 40% consistency rule forces 3 trading days.
_TRADEIFY_SELECT_EVAL = {
    **_TRADEIFY_EVAL,
    "min_trading_days": 3,
...
_MFFU_RAPID_EVAL = {
    ...
    "daily_loss_pct": None,
    "profit_target_pct": 6.0,       # $3,000 / $6,000 at 50K / 100K
    "min_trading_days": 2,
    "weekend_holds": False,
...
_BLUSKY_PREMIUM_EVAL = {
    "dd_type": "trailing",
    "daily_loss_pct": None,         # eval: no standalone DLL in primary rules article
    "profit_target_pct": 6.0,       # $3,000 / $6,000 at 50K / 100K (6% standard Premium eval)
...
FIRM_RULES = {
    # Define when onboarding (product defaults above, tier overrides here):
    # "FundedNext": { ... },
    # "The5ers": { ... },
    # "BrightFunded": { ... },

    # Bulenox Option 1 (provenance: the block above _BULENOX_OPTION1) -- the
    # per-tier trailing-DD dollar table and micro-contract caps.
    **_tiers(_BULENOX_OPTION1, {
        "Bulenox_25K":  {"starting_balance": 25_000,  "max_dd_pct": 6.0, "micro_contract_cap": 30},   # $1,500 trailing DD / $25,000; Option 1 micro cap, this tier
        "Bulenox_50K":  {"starting_balance": 50_000,  "max_dd_pct": 5.0, "micro_contract_cap": 70},   # $2,500 / $50,000
        "Bulenox_100K": {"starting_balance": 100_000, "max_dd_pct": 3.0, "micro_contract_cap": 120},  # $3,000 / $100,000
        "Bulenox_150K": {"starting_balance": 150_000, "max_dd_pct": 3.0, "micro_contract_cap": 150},  # $4,500 / $150,000
        "Bulenox_250K": {"starting_balance": 250_000, "max_dd_pct": 2.2, "micro_contract_cap": 250},  # $5,500 / $250,000
    }),

    # Tradeify Select EVAL (provenance: the block above _TRADEIFY_EVAL) -- the
    # SELECT drawdown column, NOT Growth; eval micro caps 1/4/8/12 mini = 10/40/80/120.
    **_tiers(_TRADEIFY_SELECT_EVAL, {
        "Tradeify_Select_25K":  {"starting_balance": 25_000,  "max_dd_pct": 4.0, "micro_contract_cap": 10},   # $1,000 EOD trailing DD / $25,000
        "Tradeify_Select_50K":  {"starting_balance": 50_000,  "max_dd_pct": 4.0, "micro_contract_cap": 40},   # $2,000 / $50,000
        "Tradeify_Select_100K": {"starting_balance": 100_000, "max_dd_pct": 3.0, "micro_contract_cap": 80},   # $3,000 / $100,000 (Growth=$3,500)
        "Tradeify_Select_150K": {"starting_balance": 150_000, "max_dd_pct": 3.0, "micro_contract_cap": 120},  # $4,500 / $150,000 (Growth=$5,000)
    }),
    ...
    "Tradeify_Growth_100K": {
        **_TRADEIFY_EVAL,               # firm-wide eval facts: unreachable lock, 6% target ($6,000), 16:45 ET flatten, >=1 trade/week, $0.91/side
    ...
    # MFFU Rapid EVAL (provenance: the block above _MFFU_RAPID_EVAL).
    **_tiers(_MFFU_RAPID_EVAL, {
        "MFFU_Rapid_50K":  {"starting_balance": 50_000,  "max_dd_pct": 4.0, "micro_contract_cap": 50},  # $2,000 EOD MLL / $50,000; 5 mini / 50 micro
        "MFFU_Rapid_100K": {"starting_balance": 100_000, "max_dd_pct": 3.0, "micro_contract_cap": 80},  # $3,000 EOD MLL / $100,000; 8 mini / 80 micro
    }),

    # BluSky Premium EVAL (provenance: the block above _BLUSKY_PREMIUM_EVAL).
    **_tiers(_BLUSKY_PREMIUM_EVAL, {
        "BluSky_Premium_50K":  {"starting_balance": 50_000,  "max_dd_pct": 4.0, "micro_contract_cap": 50},   # $2,000 EOD trail -> $48,000 initial floor; 5 mini / 50 micro (Orbit/Premium class)
        "BluSky_Premium_100K": {"starting_balance": 100_000, "max_dd_pct": 3.0, "micro_contract_cap": 100},  # $3,000 EOD trail; 10 mini / 100 micro
    }),
}
