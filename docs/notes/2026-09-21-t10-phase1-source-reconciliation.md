...
## Executor disposition — corrected Striker port parity (post-rulings, 2026-09-21)

Responding to R1 hold item (i) ("… until a corrected-port parity run exists or the operator accepts Step 3 as sufficient at F1"), the source/freeze executor's determination: **the accepted Step 3 contract suffices as the parity evidence for the corrected Striker port `efd479b6…`, within its exact accepted domain; no fresh corrected-port parity run is owed by phase 1, and I recommend the operator record that acceptance at F1.** Basis, all by reference:

1. The corrected port's parity evidence exists and is directly on point: the accepted Step 3 contract/index/independent acceptance (`acfa7920…`/`95ac6dcb…`/`ebcb2efb…`) covered the corrected-ports generation — nine captured references, 3,632 trades, 851,011 ordered bar calls, full-origin cold replay — and the ledger re-checked every bundle's port/panel/export triple against it. Step 6 then replayed the seven bundles on those same corrected bytes (3,173 matched trades, zero exclusions), a second end-to-end match inside the same domain. PR #356's Striker row running `c81aa59c…` was never the corrected port's evidence; its absence removes nothing.
2. The port bytes are unchanged and cannot drift silently: the registry pins `efd479b6…` byte-identically from `d107ebd` through HEAD, and `load_port` refuses any other bytes **before execution** (`book_adapters.py:121-123`). The packet's own rule — re-run nothing that is unchanged — applies.
3. A fresh Track-B-style parity run today would not be Striker-port evidence at all: it would execute on the **moved** emulator (`2be77bab…` @`7ba7844`), i.e. on hold item (ii)'s surface — the on-changed-engine revalidation F11 already owes. Running it now would conflate the two hold items and produce evidence bound to a device identity whose own acceptance is pending.

What reopens this disposition, and nothing less: a registry re-pin of the Striker runtime hash (new generation), any movement of the accepted port/panel/export triples, or the F11/engine acceptance superseding the device — each demanding its own affected acceptance, per the provenance review's reuse decision ("new loader, effective settings … require separate affected acceptance").
