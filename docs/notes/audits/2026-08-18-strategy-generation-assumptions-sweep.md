...
4. **Sentinel Tier 2-3 (quarterly LLM probe) has never fired once, past its own first scheduled date.** → Already a named falsifiable gate ("Limb B") in `docs/briefs/programs/2026-07-27-hermes-agent-adoption-ruling.md` §6/§7, already logged as a `MISS ⇒ H-DELTA FALSIFIED` in `docs/briefs/programs/2026-07-17-0808-packet-delta-and-sequence.md`. Genuine residual (not itself novel enough to re-admit): Limb B has now missed two quarterly dates (08-08, approaching 11-08) un-dispositioned — a status-update question on a tracked item, not an unexamined assumption.
