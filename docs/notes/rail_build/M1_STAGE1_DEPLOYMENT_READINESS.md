...
| inject refused (any reason other than `publication uncertain`), window missed, ceremony `expired` / `target_missed` with no POST | confirm `/data/m1_upload_stage1-20260913-1.json` absent; 2.9 close in full; 2.10; record; `DONE_WITH_CONCERNS` — a second attempt is a **new session, new id** |
| `inject refused: publication uncertain` | the bar may already be published (`atomic_json` replaces the file before its directory fsync; the claim file is kept), so the daemon can consume it — **do not close yet**: run Step 2.7 as written (daemon log + journal until a terminal state, ≤ T+165 s), then branch on the journal — `RESPONSE_RECORDED` → continue at 2.8 (no receipt exists; the journal `bar_sha256` is recorded with that concern for the parent's §7 check); `CLOSED` with no POST → the row above; `TRANSPORT_UNKNOWN` → the row below. Never re-inject |
