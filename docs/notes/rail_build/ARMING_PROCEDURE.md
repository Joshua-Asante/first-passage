...
| E2 | Deployed build is the reviewed, sealed build | Operator | Locally, use the acceptance artifact retrieved from the image and a complete checkout of the sealed source revision (including tests, with the sealed line endings): `python scripts/validate_c1_monitoring_acceptance.py <image-acceptance.json> --require-tree-current --root <sealed-tree>`. The image omits test pins, so this full comparison cannot run in-container. Separately compare current release/image identity and in-container hashes of image-carried pinned files against the B7 execution fingerprint; never rewrite pins to force a pass | printed skew line; release + image tag | stop; a redeploy is a deploy (six pre-conditions), and a change after the seal re-seals and re-runs the live test before B10 |
...
| 5.14 Reconcile in-flight sends | Operator | CrossTrade Alert History for the window; ledger deviations / any `transport_unknown` since the risk-add stop; confirm no open reconciliation | every in-flight send accounted for (accepted, rejected, or reconciled); no unresolved `transport_unknown` | unresolved send → resolve before the flatness check; never disarm over unknown transport |
...
and the exported session directory are both retained evidence. No raw output enters a public record.
...
- **Restart-time unresolved-transport interlock** — TB-S3 (H) → TB-I3. Before any armed
  ...
  pre-arm operator check E4 supplies this protection. Missing interlock → no arm; do not
  implement it by changing this procedure's claimed runtime behavior.
