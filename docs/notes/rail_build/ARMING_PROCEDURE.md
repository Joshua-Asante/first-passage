...
| 5.14 Reconcile in-flight sends | Operator | CrossTrade Alert History for the window; ledger deviations / any `transport_unknown` since the risk-add stop; confirm no open reconciliation | every in-flight send has a broker-confirmed terminal outcome (filled, cancelled or rejected) or a completed reconciliation proving no outstanding order; HTTP `accepted` alone is not execution evidence. No unresolved `transport_unknown` or pending accepted send remains | unresolved send → resolve before the flatness check; never disarm over unknown transport |
...
- **Restart-time unresolved-transport interlock** — TB-S3 (H) → TB-I3. Before any armed
  ...
  until persisted unknown sends are reconciled against broker evidence, while preserving
  exit/flatten capability. Today `EventLedger.block_risk_add` is memory-only; startup validates
