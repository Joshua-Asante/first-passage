...
each holiday's halts, capture ids and CME trade date established explicitly. The current
authoring tool accepts halts and capture ids but still derives the CME trade date from the
account date; it has no explicit trade-date input. Adding that input and testing distinct
holiday trade dates remains required before a future holiday extension is qualified.
