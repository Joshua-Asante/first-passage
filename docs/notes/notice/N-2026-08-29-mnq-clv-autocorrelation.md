...
**Admission-route status — resolved by [`docs/adr/2026-08-29-clv-autocorrelation-admission-route-scope.md`](../../adr/2026-08-29-clv-autocorrelation-admission-route-scope.md) (Proposed, pending operator ratification), superseding the "genuinely unclear" framing this notice originally used.** The ruling: a bar-shape statistic with no entry rule attached does not trigger the raised bar's admission gate at all — that gate fires at Pre-Q admission for an actual directional-timing *candidate*, which this is not yet. If/when converted into an entry construct, Route 1 does not naturally fit (this finding is unconditional on session window, the opposite of Route 1's a-priori-named-moment shape) and Route 2 does not apply (same OHLCV modality) — **Route 3 (beat `ORB-MNQ-1` net-of-cost, not merely clear the cost floor) is the only route structurally open.** The cheap $0 cost-law pre-screen named in §5(b) below is the concrete next step before any Pre-Q, per that ADR's 2-C.
