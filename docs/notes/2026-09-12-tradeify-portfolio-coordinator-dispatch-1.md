...
| O-6 | Striker add law: floor(scale × normal add) vs floor(executed base × pyramid %) | **Striker executed-base floor**; retain Vanguard's `max(1, round(base × 80%))` | Combined with O-5 law B, the protected Striker ceiling is **22 base + 55 add = 77 micros**; 8 + 20 applies only if the protected base is capped at 8 under law A. Aegis's protected 3 full contracts consume 30 micro-equivalents, so protected takeover can be necessary; the old protected-capacity assurance must be re-derived in TB-S1 and explicitly included in the operator ruling. Vanguard normal base 1/2 keeps add 1/2. |
