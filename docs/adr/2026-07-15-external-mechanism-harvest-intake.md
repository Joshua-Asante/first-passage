...
> ⚠ **2026-09-01 reader-intercept:** Requirement 1 / Path 1a below ("who systematically loses money and why") predates [`2026-07-26-mechanism-counterparty-constraint-boundaries.md`](2026-07-26-mechanism-counterparty-constraint-boundaries.md) clause 2-A (`Accepted`), which sharpens Path 1a into a mandatory four-clause test -- WHO pays as a named constraint (not a preference), WHEN it must appear, WHY it survives (capacity/awkwardness), HOW it dies (a constraint observable) -- and excludes preference/behavioral stories from 1a entirely (they route to Path 1b's evidence-robustness test or die). Frozen §2 text immediately below is pre-sharpening and left unedited (dated-decision integrity); `docs/methodology/strategy_harvest.md` §1 carries the current, binding four-clause text.
...
> ⚠ **2026-08-17 reader-intercept:** Requirement 3 below ("…kills the seed") was demoted 2026-08-04 to **mandatory disclosure, not a gate** — it cannot fail a seed ([ADR](2026-08-04-family-k-bank-disclosure-not-gate.md), `Accepted`); frozen §2 text unedited.
...
**Cheap falsifier (PARENT-side, before the GO stamp):**

```
rg -n "Status: \`Proposed\`" docs/adr/2026-07-15-external-mechanism-harvest-intake.md
# present on origin/main @ 52f39979 (PR #15 merge) — the GO stamp flipped it
...
**§0 Rule 0 (verified 2026-08-15, pin-mark session):**
...
**Cheap falsifier (PARENT-side, before this mark):**

```
rg -n "Canonical count is unset" docs/adr/2026-07-15-external-mechanism-harvest-intake.md
# present on origin/main @ ce91a995 (PR #17) — this mark replaces it
```
...
**Pin marked `no` (JA, 2026-08-15):** already-closed D5 / H-OD-1 / H-TSMOM-1 do **not** increment. Only **post-mark** Stage-2 cost-law or Clause-N/power kills increment. This mark is not a fire. Starting the count at 0 is now authorized (it was forbidden only while the pin was unanswered).

### Counting machinery (without this, 0/2 is unbinding)
...
**(b) What increments.** A **distinct intake-class construct** that reaches a **dated kill** at Stage-2 cost-law or Clause-N/power **after this mark**, recorded on a dated surface. **Do not increment for:** the three already-closed kills named in the pin-mark §0 table (D5, H-OD-1, H-TSMOM-1); a variant, retune, transplant, or relabel of a prior construct; a naming-pass that never executes a screen.

**(c) Disposition when the count reaches 2.** Fundability-transfer is FALSIFIED. Harvest intake demotes to research-only / idle-named on the 2026-11-08 vehicle. Remedy is a superseding ADR + source-class post-mortem. Do not write that post-mortem until the count actually reaches 2.
...
**Forbidden under this addendum:** rewrite limb 1; treat the GO as a limb-2 fire; treat this `no` as a fire; increment the three already-closed kills; treat 0/2 as decorative or skip the 2026-11-08 disclosure; read a standing 0/2 as a limb-2 defect while the idle guard is the operative limb; treat a limb-2 fire as licence to raise Cap or open a third channel.

**Falsifier of this addendum:** the pin is marked. The 2026-11-08 harvest §4 reading must disclose the post-mark count and whether limb 2 fired. Treating 0/2 as decorative, or skipping that disclosure, is a decorative-threshold finding against this addendum. An unmarked pin is no longer the residual. Do not silently drop.
...
# R10 limb 2 Accepted; pin marked no; count 0/2; not fired
...
grep -n "Running count (canonical): 0 / 2" docs/adr/2026-07-15-external-mechanism-harvest-intake.md
# Expected: all three present. Limb 1 revert trigger above this addendum stays byte-unedited.
```
...
| 2026-08-15 | **Pin marked `no` (JA).** Already-closed D5 / H-OD-1 / H-TSMOM-1 do not increment. Counting machinery (a)–(d) written. Running count **0 / 2**. Not fired. Limb 1 unedited. | Joshua (mark) + Cursor (stamp) |
| 2026-08-16 | **Correction (no decision change).** PR #18 also edited the ratified `Decision (binding)` line in place — `**do** count` → `**do** count going forward` — undisclosed in its blast-radius table. Reverted: that line is again byte-identical to the operator-GO text at `ce91a995`. The timing rule the edit restated is already carried by the pin-mark paragraph, so nothing is lost. Count stays **0 / 2**; not fired. | Claude Code (adjudication) — operator-directed |
| 2026-08-16 | **Compensating control named (no decision change, no count change).** Limb 2's post-mark reachability disclosed: intake is parked (Q-INVENTORY-1 `FALSIFIED` 2026-07-17, accept-idle recorded), so 0/2 is expected to stand at 11-08; the idle case is carried by limb 1's §4 idle guard, which ranges over the same §Scope population and stays byte-unedited. Sharpens the decorative-threshold finding to fire only if intake **resumes** and post-mark kills still go unrecorded. **Also edits the `Forbidden` line in place** — one clause appended ("read a standing 0/2 as a limb-2 defect while the idle guard is the operative limb") — disclosed here rather than left silent. Count stays **0 / 2**; not fired. | Joshua (direction) + Claude Code |
| 2026-08-17 | Reader-intercept only (no decision change): §2 Requirement 3 still carries the pre-demotion "kills the seed" text; a one-line intercept now sits immediately above it pointing at [`2026-08-04-family-k-bank-disclosure-not-gate.md`](2026-08-04-family-k-bank-disclosure-not-gate.md) (`Accepted`). Frozen §2 body unedited. Companion living-doc fix: `strategy_harvest.md` relief-valve line no longer calls Req-3 "truly final". | Cursor (doc-skew) |
