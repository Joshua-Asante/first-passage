...
| **Invent a coded `CANDIDATE` rung + demote into WATCH-*** | Would silently extend `_LADDER_ORDER` / multipliers without a ladder-shape ADR; confuses admission standing with live AUTHORIZED→WATCH de-risk; Cap and decay RESULTS both forbade inventing mapping under pressure. The WATCH-1H ADR (`Withdrawn` 2026-08-22) was a different granularity question *inside* the live ladder — not a template for inventing below-AUTHORIZED rungs here. |
