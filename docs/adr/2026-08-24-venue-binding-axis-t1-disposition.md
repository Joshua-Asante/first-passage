...
**Status:** `Accepted` — operator GO 2026-08-24 ("Ratify it — flip to Accepted and run Phase 1"), given after the pre-ratification adversarial panel's confirmed BLOCKERs were fixed in place (see Change History). §7 Phase 1 executed same day.
...
| `docs/notes/audits/programme-audit/2026-08-14-f2-adr-corpus-disposition.md` | `git log -1` pending re-check at authoring; content read in full | L60: the prior, independent diagnosis this ADR formalizes — "Never ratified (Status: Proposed); its own T1 falsifier appears to have fired when F2/F3 were resolved by `2026-08-07-loop-s1-environment-ratification.md` without producing an edition-state transition. Needed a status-correction/dormancy addendum," resolved-by column citing PR #827 (the PR that produced the target ADR's own 2026-08-14 addendum). This is the single most on-topic prior-art hit in the corpus — engaged with here rather than left as a silent Phase-2 sweep gap (see §6). |

**Amendment-first (sub-rule 10, `docs/operational_rules.md` §8).** Dedup search executed before minting this file:

```
$ python scripts/check_advisor_dedup.py --keywords "venue binding T1 disposition bootstrap inapplicable"
[  4] docs/notes/audits/programme-audit/2026-08-14-f2-adr-corpus-disposition.md — F-2 disposition — full ADR-corpus classification + adversarial deletion test
      shared terms: ['binding', 'bootstrap', 'disposition', 'venue']
[  4] docs/notes/audits/docs-runtime-inventory.md ; 2026-07-01-methodology-belt-scoped-audit.md ; 2026-08-05-claim-alignment/{04-misleading,05-cosmetic,08-hooks}.md ; ops/instruments/{6J,SPX500}.md — remaining top-8, no on-point owner among them
```

Top hit is the F2-disposition audit note added to §0 above — not a live, unreconciled competitor: its content is the documented origin of the target ADR's own 2026-08-14 addendum (same PR, #827), which this ADR already cites and reasons from directly in §1. No candidate names an existing owner that could take this ruling as an addendum instead of a new file — the existing owner (`2026-08-05-strategy-venue-binding-axis.md`) is itself named throughout, and §3 row 3 argues why amending it in place is doctrinally forbidden (Trap #12), which is the sub-rule-10 exception that licenses a new file here.
...
T1 fired. The F2/F3 venue-scope decision (recorded via `2026-08-07-loop-s1-environment-ratification.md`) resolved in prose, with no edition-state transition, because the registry T1 needs did not exist — the ADR itself was still `Proposed`, unratified, until 2026-08-22. Two addenda concede this in writing: 2026-08-14 ("T1 likely fired... This ADR was never ratified; `ops/venue_editions/` was never created") and 2026-08-22 ("T1... is acknowledged as already fired... it does not claim T1 did not fire and does not rewrite S1"). Neither addendum executes §4's named revert action. Instead, the response was a Rule-14 status-framing addendum plus an operator GO that flips `Proposed → Accepted` directly. The target ADR's own header still reads `Supersedes: none` / `Superseded-by: none`; no supersession has ever been authored for this trigger. The 2026-08-14 addendum itself traces to an independent, prior diagnosis: `docs/notes/audits/programme-audit/2026-08-14-f2-adr-corpus-disposition.md` L60 already recorded, same day, "its own T1 falsifier appears to have fired... Needed a status-correction/dormancy addendum" (resolved-by PR #827, the PR that produced that same-day addendum) — so the gap this ADR closes was flagged in writing, in a dedicated corpus-wide disposition sweep, ten days before this ADR's own authoring, and sat un-actioned through both intervening addenda.
...
|---|---|
| **Revert the axis entirely** (T1's literal §4 default: "H is FALSIFIED... reverted by a superseding ADR"). | Ruled out on the bootstrap-timing argument alone (§1–§2): T1's firing tested whether an unaccepted, unbuilt axis was consulted, not whether the axis fails once live, so nothing in the actual evidence supports H being wrong. **Explicitly not** ruled out on cost grounds — the fact that §7 Phase 1–3 has since landed (2026-08-23) is not treated as a reason to spare the axis from a literal revert; doing so would let sunk cost accrued during the very window this ADR is dispositioning count as an argument against applying the falsifier's own named remedy, which is circular and is the same "gate nobody agreed to maintain" pattern this programme's audits flag elsewhere. If the bootstrap-timing argument in §1–§2 did not hold, this ADR would revert the axis regardless of what has been built on it since. |
...
- **Laundering risk:** a future non-consultation could be misclassified as "before acceptance" when it postdates 2026-08-22. T2' names this failure mode and anchors the carve-out to a single, already-recorded date rather than a re-litigated judgment call each time — but T2' is a self-report-dependent tripwire, not an active control, and by construction cannot fire on the one occurrence this ADR itself dispositions (2026-08-07/08, which unambiguously predates 2026-08-22). Its value is entirely prospective, against a future claim, not a mitigation already exercised here.

**Downstream artifacts that need updating (Phase-2 sweep, derived not recalled — raw output in §10, re-run 2026-08-24 to close the gap two adversarial-panel lenses found in the first draft's sweep-to-table mapping):**
...
**Explicitly ruled unaffected (with reason) — union of both §10 hook outputs, every hit dispositioned:**
...
- `docs/notes/audits/programme-audit/2026-08-14-f2-adr-corpus-disposition.md` — **the on-point hit**, already engaged with directly in §0/§1 above rather than only listed here: it is the origin of the 2026-08-14 addendum this ADR reasons from (same PR #827). Not edited — it is itself a dated audit record (same convention as the claim-alignment octet above) and this ADR's own ruling is the correct place for the disposition it called for, not a rewrite of the audit note's past-tense diagnosis.
- `docs/adr/2026-08-05b-blusky-inactivity-rule-sourced.md` — hook-2 hit; its header phrase "its §4 **T1 fired**" refers to a *different*, unrelated 2026-08-05 ADR's own T1 (the BluSky-inactivity-rule containment ADR, `2026-08-05-blusky-inactivity-unsourced-encoding.md`), not this ADR's T1 or the venue-binding axis at all — same coincidental-collision class as the third-leg-spec/08-04-descope/coldstore-phase-b rows above; unaffected.
...
- **Phase 2** — grep-sweep executed and raw output pasted in §10; disposition table in §6 is the union of both hooks' output, every hit dispositioned. Re-run at Phase 0 to confirm no new hits landed between authoring and acceptance.
...
```bash
# 1a. Phase-2 sweep, hook 1 (citation grep) — raw output, run 2026-08-24, 20 hits, all dispositioned in §6
...
./docs/adr/2026-08-14-repo-public-visibility-transition.md
./docs/adr/2026-08-23-strategy-coldstore-phase-b.md
./docs/adr/2026-08-24-venue-binding-axis-t1-disposition.md
./docs/adr/INDEX.md
./docs/ltm/notes/archive/sessions/SESSIONS-2026-Q3.md
./docs/ltm/notes/archive/state/STATE-decision-index-pre-2026-08-23.md
./docs/methodology/strategy_lifecycle.md
./docs/notes/audits/programme-audit/2026-08-05-claim-alignment/{01-diagnostics,02-blockers,04-misleading,05-cosmetic,06-operator-judgement,07-followups,08-hooks,README}.md
./docs/notes/audits/programme-audit/2026-08-14-f2-adr-corpus-disposition.md
./docs/spec/2026-07-27-third-leg-target-spec.md
./docs/superpowers/plans/2026-08-23-venue-binding-registry-implementation.md
./ops/venue_editions/Tradeify_Select_100K.md

# 1b. Phase-2 sweep, hook 2 (T1-phrase grep) — raw output, run 2026-08-24, 5 hits (incl. self), all dispositioned in §6
...
./docs/adr/2026-08-05b-blusky-inactivity-rule-sourced.md
./docs/adr/2026-08-24-venue-binding-axis-t1-disposition.md
./docs/adr/INDEX.md
./docs/ltm/notes/archive/sessions/SESSIONS-2026-Q3.md
./docs/superpowers/plans/2026-08-23-venue-binding-registry-implementation.md

# 1c. Expected on re-run: identical file sets (own filename excluded); any new hit needs a fresh §6 disposition
# before this ADR may flip Proposed -> Accepted (see Verification block below).
...
| 2026-08-24 | Pre-ratification adversarial panel (`pre-ratification-adversarial-panel` skill, 34 agents) returned `BLOCKED`: 2 confirmed BLOCKERs (§6/§10 Phase-2 sweep incomplete — missing the on-point `2026-08-14-f2-adr-corpus-disposition.md` hit and no pasted raw output; §3 row 1's rejection of a full revert relied circularly on the cost of work built during the firing's own unresolved window) + 1 CONCERN (a second sweep hit, `2026-08-05b-blusky-inactivity-rule-sourced.md`, also undispositioned). All three fixed in place (§0/§1/§3/§6/§10 revised, real grep output + dedup-search output pasted); disputed items (bootstrap-framing tension, date-labeling, dedup-formal-gap) left as-is per the panel's own split verdict, addressed where cheap (dedup output now pasted in §0). Still `Proposed`. | Claude Code |
