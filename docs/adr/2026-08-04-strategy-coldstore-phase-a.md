...
**Not decided here — operator call:** (a) whether to repair hooks #3/#4 to check the derived-source location (`firm_rules._BASE_RISK` / `LEG_MAP`) instead of literal `dd_protection.py` text; (b) whether T4's hot-pine scope should be narrowed to the original frozen-inventory filenames, or `candidates/` excluded from the T4 watch as an acknowledged-active directory, versus left exactly as worded. No disposition is made in this addendum.
