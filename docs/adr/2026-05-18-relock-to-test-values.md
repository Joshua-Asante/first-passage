...
**Supersedes:** `2026-05-18-pine-input-float-defaults-realignment.md` in part — the *direction* of that ADR (fix Pine to match lock); the validator/HARD-tier machinery from that ADR was later retired 2026-08-03 (see that file's Addendum 2026-08-29).
**Retain-until:** none
**Superseded-by:** `2026-05-14-allocation-refresh.md` - already on main (the re-lock proposed here was authored against the pre-2026-05-14 baseline of 1.00% / 0.40% / 350 pyramid / 1.00 maxDD, which had already been superseded by main's 0.75% / 0.45% / 500 pyramid refresh four days earlier; the comparison MC therefore did not speak to main's actual canonical).
