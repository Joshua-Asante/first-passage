...
> **Current four-leg incident authority:** the rev9 attended addendum at the end of this document supersedes the rev8 listener emergency-dispatch obligation. It does not alter completed M1 acceptance or grant emission/live authority.
...
### Amendment text — option A′ (NOT APPLIED — shortlisted; production-feed decision deferred 2026-09-11)

The afternoon ruling initially selected A′ after the A1 packet's pricing and the CrossTrade market-data exclusion. **Superseded later on 2026-09-11 by the operator's funding deferral:** no provider is selected; A′ is shortlisted only, and this amendment is not applied. The Codex advisory review (PR #344) also corrected the entitlement set to CME, CBOT and COMEX because MYM is a CBOT listing (`ops/instruments/MYM.md`), and identified the brokerage credential as order-capable unless the vendor enforces a data-only scope. Preserve the original amendment text as a scored candidate, not current doctrine:

```
| Live CME bar source | **Tradovate market-data API** (`live.tradovateapi.com`, `md/subscribeChart` or `md/getChart`, `MinuteBar`/1) on a **personal live Tradovate account used for data only** — never the Tradeify eval sub-account (FTA §6.6; eval accounts are API-ineligible). Credentials rest only on the daemon volume; the listener image stays stdlib-only; the daemon image may depend on a hash-pinned websocket client. Candidate text only; not selected or applied (O-4 deferred 2026-09-11). |
```
...
- **Q5 — RULED: later** (morning) **→ A′ selected** (afternoon) **→ DEFERRED 2026-09-11 (latest): provider unselected.** The afternoon ruling followed the CrossTrade market-data exclusion and declined Databento re-subscription, but the operator subsequently deferred funding and the provider decision. A′ remains shortlisted only. Provider-neutral preparation may proceed under the Track A plan §3.2; provider-specific implementation, signup, subscription and credential staging may not. M1 itself still needs no machine feed (option D).
...
| Date | Change | By |
|---|---|---|
| 2026-09-11 | **Production-feed decision deferred (latest):** provider unselected; A′ shortlisted only; removed the applied production-feed clause while preserving the afternoon selection as superseded history; provider-neutral preparation only until the operator returns after source-independent qualification | Joshua (deferral) · Codex (recorder) |
| 2026-09-11 | Amended on Codex advisory review (PR #344): MYM entitlement is CBOT (the first text named CME and COMEX only); A′ restated as the **selected leader** with funding and subscription gated on the Track A plan §3.2 readiness checkpoint after a written vendor verification (tastytrade, Ironbeam, Tradovate; IBKR docs); the verification record, six vendor questions, checkpoint and decision rule folded into that plan section | Claude (recorder) · Codex (review) |
...
## Addendum 2026-09-14 UTC — ratified four-leg halt and recovery contract

Joshua approved decision E as revised to TB-S3 rev8 at PR #380 merge `8101ba498aad812e79c3d80c45f963cd67b55de6`. [Approval record](../briefs/handoffs/2026-09-14-track-b-ratifications.md); [S3](../spec/2026-09-12-c1-multi-leg-rail-extension-spec.md); [rev8 halt/resume owner](../spec/2026-09-14-tb-s3-halt-resume-contract.md). This amends the Listener/Fail-closed rows and corresponding falsifier limbs for the fixed four-leg book only.

Retain R-B1/R-B3 optional B1 fields (`side`, `qty_normal`, `qty`, `stop_price`, `stop_loss`, `take_profit`, `trail_activation_ticks`, `trail_offset_ticks`, `order_ref`, `scope_refs`), cancel/amend types and five-field M1 compatibility. Unhealthy sources emit no ordinary strategy signal until valid bars return. Authenticated, ordered health reports request a listener-owned account halt; they carry no daemon B1 flat. The listener owns durable halt and qualified risk-reducing recovery under rev8 §§1–4. Missing reports never authorize risk-adds; watchdog and evidence-freshness checks apply. Neither source recovery, session rollover nor restart restores permission. Resume requires fresh bounded operator approval after reconciliation and all retained gates.

Emitting a risk-add under HALTED, an invalid/expired authorization, failed barrier or unhealthy required source falsifies this four-leg contract. So does treating an unhealthy-source strategy signal as the recovery authority or using an unsupported protective-order cancellation/close fallback. Retain separate apps/volumes, source selection, build GO, M1 and emission gates. Other B1 changes require their own authorization. Existing single-strategy operation is not migrated by this record. This addendum grants contract authority, not deployment, arming or trading permission.

## Addendum 2026-09-15 UTC — rev9 attended incident recovery

Joshua directed execution of Packet 0 after reviewing the [attended-release plan](../superpowers/plans/2026-09-14-tradeify-attended-release.md). Its contract is frozen in [TB-S3 rev9](../spec/2026-09-14-tb-s3-halt-resume-contract.md); [evidence/approval scope](../notes/2026-09-14-tradeify-attended-feasibility.md). This replaces the previous addendum's incident-triggered qualified risk-reducing dispatch obligation for the fixed four-leg release only.

The daemon supplies authenticated ordered source/session health and fault reports, never permission or an emergency B1 flat. The listener atomically owns incident scope and revokes all further runtime mutations under its account serializer, including late exits/amends. Transmitted or unknown requests survive fencing/restart. Joshua intervenes through the platform. Acknowledgment records attendance only; observations do not establish reconciliation. Missing E1–E3 or provider-side intervention evidence blocks resumption. Source recovery/day rollover/restart do not rearm.

Preserve R-B1/R-B3 fields, five-field M1 compatibility, detector thresholds, separate apps/volumes, normal qualified strategy/protection/takeover and non-incident scheduled flatten. An incident revokes the latter's narrow authority. No post-fence runtime mutation, permissive legacy exit bypass or acknowledgment-based resume is acceptable. Provider-managed effects must be qualified independently of the local fence. Implementation, actual route/source acceptance and operational GOs remain owed; no code or deployed behavior is changed by this addendum.
