...
**Outcome: H holds narrowly, on options A′ and B only** (the §2 feed row is the only amendment; $30–37/mo plus $1,000 parked, or $199/mo). Both require an operator signup outside this session's authority. The recommended option D needs the amendment the 2026-09-10 addenda already name — the brief's reject branch anticipates exactly this. **AMBIGUOUS on option A:** eval-account eligibility could not be established from a primary page; the repo's own 2026-07-22 verification says ineligible.
...
git diff --stat origin/main...HEAD                                                                           # the two paired ADRs: this file + docs/adr/2026-07-22-c1-venue-native-monitoring-maturity.md
```
