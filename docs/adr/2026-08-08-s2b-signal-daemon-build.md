...
### Amendment text — option A′

Replace this ADR §2 row "Live CME bar source" (applied by A1r on ratification):

```
| Live CME bar source | **Tradovate market-data API** (`live.tradovateapi.com`, `md/subscribeChart` or `md/getChart`, `MinuteBar`/1) on a **personal live Tradovate account used for data only** — never the Tradeify eval sub-account (FTA §6.6; eval accounts are API-ineligible). Credentials rest only on the daemon volume; the listener image stays stdlib-only; the daemon image may depend on a hash-pinned websocket client. Selected 2026-09-DD (Addendum 2026-09-11, ratified 2026-09-DD). |
```

No M1 ADR amendment: item 5 is discharged as written; `project_evidence` returns `qualifying_live_source: True`.
...
git diff --stat origin/main...HEAD -- docs/adr/2026-08-08-s2b-signal-daemon-build.md docs/adr/2026-07-22-c1-venue-native-monitoring-maturity.md   # the paired ADRs (companion edits in the same PR: STATE.md, the A7 handoff brief)
```
