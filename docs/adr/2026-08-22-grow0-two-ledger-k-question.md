...
**Status:** `Accepted` — operator GO (JA) 2026-08-22 ("GO on the ADR — ratify it as-is"), after
three rounds of adversarial review the same day (round 1: 6 lenses, invalid empirical inference;
round 2: 3 lenses, invalid replacement mechanism argument; round 3: 2 lenses, a misattributed
citation count/scope — all `BLOCKED`, all fully applied; see Change history). §7 Phases 0–3
executed the same session as ratification (charter edit landed; `burned_segments.py` extension
landed as its own dated commit; sweep run).
...
[GROW-0 closure](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/briefs/closures/GROW-0-closure-resolved.md) (`RESOLVED`, the licensing
...
[TNEC-1 application spec](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/spec/2026-08-11-tnec-application-unit-book-admission.md)
...
| [GROW-0 closure](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/briefs/closures/GROW-0-closure-resolved.md) | `70029e6` | The licensing precondition, discharged: real N=5,500/c=7 run, `RESOLVED`. Measured `nominal_p0 = 0.00059070` at `floor_at_k(10, 6.5) = 1.265` under argmax-of-10 nomination |
...
| [TNEC-1 application spec](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/spec/2026-08-11-tnec-application-unit-book-admission.md) | `027a729` | N-EDGE row: `DSR ≥ floor_at_k(K_intrinsic)` — `K_intrinsic`, not a read-count, confirmed by direct read |
...
| 2026-08-22 | **`Proposed` → `Accepted`** — operator GO (JA, "GO on the ADR — ratify it as-is"). §7 executed same session: **Phase 0** re-verified all §0 anchors current (no drift). **Phase 1** — charter §2.2(iv) text added to `2026-08-16-deep-iteration-lane-charter.md` (disclosure-only, does not join the §2.2(i)–(iii) refusal predicate), `Superseded-in-part-by` header line added, charter §7 item 6 + §10 audit hooks + charter's own change history row added. **Phase 2** (own dated commit, separate from Phase 1) — `lab/discovery/burned_segments.py` extended: `consultations` list field (migrated the seed MNQ entry from flat `read_date`/`source` to a one-element `consultations` list, no data loss), new channel-agnostic `consultation_count`/`consultation_history` functions; `tests/test_discovery_burned_segments.py` updated for the migrated schema plus 4 new tests (zero-consultation, seeded-entry count, cross-segment channel-agnostic summation, declared_k/floor_at_k exposure) — 11/11 green, `is_window_burned`'s existing binary behavior re-tested and unchanged. **Phase 3** — sweep (`rg -n "chargeable K is.*sealed consultation\|K = M \(GROW"`) found only historical/citation framing (GROW spec v2's own naming of the question, this ADR's own citations of it), no stale restatement of claim 1 as open or adopted. | Joshua (GO) + Claude Code |
