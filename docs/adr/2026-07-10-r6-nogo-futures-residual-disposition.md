...
| 2026-09-01 | Addendum: §10's "no rail" audit hook struck — non-recursive `ls ops/` check fails open now that `ops/c1_rail/crosstrade_payload.py` exists under the later-authorized c1 rail (2026-08-31 ADR-corpus audit §6, re-verified) | Claude Code (ADR-corpus reconciliation sweep) |

---

## Addendum 2026-09-01 — §10's "no rail" hook is stale and fails open; struck, not redirected

**Trigger:** `docs/notes/audits/programme-audit/2026-08-31-adr-corpus-audit.md` §6 flagged that §10's audit hook —
```bash
ls ops/ | grep -i "bulenox\|crosstrade\|rithmic" && echo "UNEXPECTED: rail artifact present" || echo "OK: no rail (NO-GO)"
```
— prints `OK: no rail (NO-GO)` even though `ops/c1_rail/crosstrade_payload.py` exists on disk today. Re-run verbatim 2026-09-01: confirmed, still prints `OK: no rail (NO-GO)`. Root cause: `ls ops/` lists only ops/'s immediate children; none of them contain "bulenox", "crosstrade", or "rithmic", so the nested `crosstrade_payload.py` is invisible to the check regardless of whether a rail exists.

**What changes:** the §10 hook (the "No live prop rail was built" check) is **struck** — retired from this ADR's audit-hooks battery. It is not repaired into a recursive search, because the premise it tested — "no futures-prop rail exists" — was itself superseded in part by `2026-07-17-c1-rail-build-account-registration-go.md`, which authorized building the c1 rail (dry-run, disarmed) at the ratified incumbent eval. A recursive fix would keep re-asserting a premise this ADR no longer controls.
