...
- `docs/spec/2026-08-09-dense1m-entry-mechanism-lane-spec.md` — `027a729` (2026-08-14, the public-transplant seed commit; content unchanged since the pre-transplant original). Step 6: "**Lane stop-rule:** 3 consecutive FALSIFIED mechanisms on this universe → a lane-review packet to the operator (SNAG discipline), never a 4th campaign by default." Counts only the literal `FALSIFIED` disposition.
- `docs/briefs/closures/Q-TNEC-CON-5-closure-ambiguous-hold.md` — `027a729` (2026-08-14, transplant seed; content unchanged). Confirms by direct evidence that the stop-rule above did **not** fire across CON-2 → CON-5 (four consecutive `AMBIGUOUS-HOLD` closures, 2026-08-10 → 2026-08-12): "Lane FALSIFIED counter **unchanged at 1/3**." The operator instead cited an informal, unshipped tally ("8 consecutive zero-yield closes... against SNAG anchor 3") to elect a manual STOP.
- `docs/adr/2026-07-15-external-mechanism-harvest-intake.md` — `8804ab1` (2026-08-16, pre-this-session). §4 verdict vocabulary (L91): "**RESOLVED** if ≥1 of the first two [intake-class closures] confirms OOS; **FALSIFIED** per the revert trigger [both of the first two close FALSIFIED]; **AMBIGUOUS** if no intake-class campaign closes by 2026-11-08." No branch covers a seed closing `AMBIGUOUS-HOLD` (or any non-RESOLVED/non-FALSIFIED disposition) among the first two — a real, currently-dormant gap (only D5, RESOLVED, has closed to date).
- `docs/adr/2026-08-15-no-counterparty-statistical-sourcing-channel.md` — `6e45aa8` (2026-08-15). Both addenda (K-cap; pre-G0-kill-is-not-a-strike) are the direct precedent for this ADR's shape: each names an "absorb unbounded cheap deaths without reaching strike one" defect and closes it with a **tightening-only**, dated addendum, never a silent §2/§4 edit. The pre-G0 addendum explicitly declines to generalize its own fix ("this addendum deliberately does not invent that threshold") — this ADR is the generalization for the AMBIGUOUS-HOLD-specific instance of the same shape, not a re-litigation of the pre-G0 question (which was separately resolved 2026-08-15, N=3 elected, and stays untouched here).
- `lab/archive/approach_scoreboard_2026-08/PREREG.md` — `027a729` (2026-08-14, transplant seed; content unchanged). F4 ("Streak + yield rules," L97–106): "WATCH at consecutive zero-yield streak ≥ 3... LEVEL-CHANGE-RECOMMENDED at streak ≥ 6... A close resets the streak ONLY if it yielded an admitted candidate... `AMBIGUOUS-HOLD` counts as zero-yield (design §12 item 4 — held is not yielded). VOID / NULL / SCREEN-FAIL / INTAKE-DRY / OPERATOR-STOPPED / ABORT count as zero-yield." This is the exact definition this ADR promotes — drafted once, correctly, but never ratified as standing doctrine because its host investigation (Q-SCORE-1) closed FALSIFIED on an unrelated axis before the runner shipped.
- `docs/briefs/closures/Q-SCORE-1-closure-falsified.md` — `027a729` (2026-08-14, transplant seed; content unchanged). Confirms F4 never went live: "Live effect: none — report-only; no lane closed; no runner shipped" (L8), and its own §3 disclaims licensing "closing any research lane by streak arithmetic" (L29) — i.e. F4's *definition* is sound and citable, but its *host investigation* conferred no authority to bind future lanes. This ADR supplies that authority directly, on F4's merits, rather than by inheritance from a failed campaign.
- `docs/rejected_candidates.md` — `027a729` (2026-08-14, transplant seed; content unchanged). Scope line: admits entries only at closure "`FALSIFIED` on strategy grounds, or... `SNAG`-budget-exhaustion grounds" — confirmed to carry no `AMBIGUOUS-HOLD` token anywhere in the file. Named in §6 as a known adjacent gap this ADR does **not** close (out of scope — see Forbidden move 4).
- Repo-wide sweep for `"consecutive FALSIFIED"` / `"lane stop-rule"` / `"SNAG discipline"` (2026-08-16, this session, re-verified against the correct public-lineage checkout after an initial pass on a stale pre-transplant worktree was caught and corrected) found exactly **one** currently-instantiated consecutive-counting stop-rule mechanism in the repo: the dense-1m lane spec above. No other lane/channel spec currently carries this shape, so §7's mechanical scope is complete as enumerated, not a partial sweep.
- **Correction note (this session):** this ADR's research and initial draft were produced against a worktree branch that, unknown at the time, descended from the *pre-2026-08-14* repo history (before the public-visibility fresh-repo transplant, [`2026-08-14-repo-public-visibility-transition.md`](2026-08-14-repo-public-visibility-transition.md)) rather than current `origin/main`. All §0 anchors above were re-verified against the correct post-transplant lineage before this ADR was accepted; the one substantive drift caught was `STATE.md`'s pre-G0-kill-threshold status (resolved N=3 on 2026-08-15, on the correct lineage, but still shown open in the stale worktree) — corrected throughout §5/§10 below. No other cited content differed between the two lineages.
...
- **Treating this ADR as also answering the "consecutive pre-G0-kill threshold" question** — a separate, already-resolved item (the no-counterparty channel ADR's own addendum elected N=3 on 2026-08-15, before this ADR was authored). That is a different disposition class (pre-`register_search open` kills, which are not battery-closures at all) from the one this ADR governs, and this ADR does not touch, revisit, or re-derive it.
...
# Pre-G0-kill threshold is a separate, already-resolved item (N=3, 2026-08-15) — Forbidden move 4
grep -n "consecutive-pre-G0-kill threshold elected N = 3" STATE.md
# Expected: present, unchanged by this ADR — confirms no overlap/re-derivation occurred
...
# Production-source verification (§0 anchors, pre-this-session-edit commits)
git log --skip=1 -1 --format='%h %cs' -- docs/spec/2026-08-09-dense1m-entry-mechanism-lane-spec.md  # 027a729 2026-08-14
git log --skip=1 -1 --format='%h %cs' -- docs/adr/2026-07-15-external-mechanism-harvest-intake.md   # 8804ab1 2026-08-16
git log -1 --format='%h %cs' -- docs/adr/2026-08-15-no-counterparty-statistical-sourcing-channel.md # 6e45aa8 2026-08-15
