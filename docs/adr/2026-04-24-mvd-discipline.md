...
**Status:** Accepted - generalizes and supersedes the standing Rule 0 (2026-04-17, audit-first for risk-control decisions). **AMENDED 2026-08-14:** code-level self-checks remain live; the doc-audit-table and MVD-attest one-liner are not enforced (Addendum 2026-08-14).
...
## Addendum 2026-08-14 — enforcement vs original ceremony

**Type:** dated correction under Rule 14. **Decision text is not edited.** The code-level MVD checks are **not** withdrawn and were not touched.

The 2026-04-24 design mandated three layers: (1) code-resident identity/cardinality/contract assertions, (2) a consumer-side **MVD-attest** one-liner on every brief/ADR/memory/CHANGELOG that cites a load-bearing number, and (3) a live audit table in `docs/methodology/mvd.md` updated on every post-MVD catch.

What is actually enforced today (verified 2026-08-14):

- **Live — code self-checks.** `core/lib/mvd.py` still ships the five helper families. `core/dd_protection.py` still runs `_validate_protection_rule()` at import (logic check + spec pin). The 2026-04-25 retrofit's `portfolio_mc.py` wiring still exists, relocated under `core/mc/modes.py` / `core/mc/ingest.py` (`assert_no_fallback`, `assert_min_rows`, `assert_window`). [`2026-04-25-mvd-retrofit.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/adr/2026-04-25-mvd-retrofit.md) wired only this half.
- **Dropped — doc-audit-table.** `docs/methodology/mvd.md` is gone (`ls` 2026-08-14: no such file; no `mvd.md` remains in-tree). The retrofit ADR already records it as deleted. The update-protocol half of this ADR has no live home.
- **Dropped — MVD-attest one-liner.** Repo-wide grep of `MVD-attest` hits only this ADR (the definition), the retrofit ADR's mention of the methodology doc, and three unused `docs/templates/` stubs (`lock_decision.md`, `calibration_brief.md`, `bust_analysis.md`). Zero hits in `docs/briefs/`. A sample of 2026-08 ADRs carries no attestation. The consumer-side promotion check is **not** practiced.

Do not read the original Decision as a current enforcement map. The code-level checks remain safety-relevant and are untouched by this addendum.

| Date | Change | By |
|---|---|---|
| 2026-08-14 | Addendum 2026-08-14 — ceremony vs practice: code checks live; audit table and MVD-attest dropped. Decision body byte-unchanged. | claim-alignment reconciliation |
