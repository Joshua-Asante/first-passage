...
6. **Adversarial mode, same session, same day.** A second operator directive (chat, same session as the retarget above) named the intended pipeline: Claude Code stays the orchestration surface; Cursor takes well-scoped implementation work; Cursor-scoped PRs get the Codex second look this addendum wires; and a Claude-authored PR here can also get an on-demand Codex look "if it is high stakes and an adversarial review could sharpen the output," using `fable-judge` (`.claude/skills/fable-judge/SKILL.md`) as the review lens. Landed as: `ADVERSARIAL_LABEL = "adversarial-review"` / `ADVERSARIAL_BODY_TOKEN = "codex-review: adversarial"` in `check_codex_judgment_review.py` (orthogonal opt-in, works on any branch including `claude/*` — the existing label/body-token paths already weren't `cursor/*`-restricted, so "our PRs shipped here" needed no new trigger surface, only the adversarial flag). When adversarial, `codex-judgment-review.yml` pipes the fable-judge skill body (frontmatter stripped) to `codex exec review -` as a custom prompt instead of Codex's default review pass; otherwise unchanged. Still review-only, still `--sandbox read-only` — fable-judge's own standing rule applies here too ("judging changes nothing... fixes happen only if the operator asks afterward").

**Write access, considered and declined:** the same directive asked, ambiguously, for "Codex can implement fixes and update the PR." Put to the operator directly (`AskUserQuestion`): give Codex its own push credential and a `workspace-write` sandbox so it can commit fixes itself, or keep Codex read-only and relay its findings to Cursor's Cloud Agent (which already has write access, already gets pinged via `notify-cursor.yml` on a landed review — extended by this same addendum's item 4 above to also match the Codex marker). **Ruled: relay to Cursor.** No second write-capable agent, no new push credential, reuses the existing governed implement-and-push path, and matches fable-judge's own judge/fix separation. Codex's only credential in this repo is the review-only `CODEX_ACCESS_TOKEN` / `OPENAI_API_KEY` from item 3 above; it never gets a write scope here without a superseding ADR.
