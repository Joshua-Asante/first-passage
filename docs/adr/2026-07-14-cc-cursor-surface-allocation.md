...
**What changes:** the worker surface named in §2 test 3 ("Above threshold and spec-frozen → Cursor") and in the handoff contract's return line (`cursor/*` branch) is retired. A frozen build above the handoff-overhead threshold now goes to a **worker session**: a Claude Code session opened on a `cc_handoff` brief, or a Codex task carrying the same §0 / §0.5 / §0.75 / §5 / §6 content — §0.75 (the test 0 local-dependency check) is required for every *cloud* worker, a Codex cloud task or a Claude Code on the web session alike, and reads `N/A` only for a worker on the operator's own machine; the template's wording is made environment-based in the same PR. Return branches are `claude/*` or `codex/*`. The 2026-08-29 proactive-dispatch ruling carries over to those surfaces: once tests 0–3 select a worker session, CC dispatches without a per-task go. The lightweight GitHub-issue format keeps its issue body and return loop but loses its launch action (step 2, the `@cursor` comment); the replacement launch step is explicit: the orchestrator opens the worker session against the issue itself — a Claude Code session pointed at the issue, or a Codex task created from it — and an issue with no session opened on it dispatches nothing. §4 limb 1's population ("merged Cursor-built PRs") and the 2026-08-29 addendum's proactive-dispatch revert trigger now count **worker-built PRs**: a PR whose branch is `claude/*` or `codex/*` and that was opened from a handoff brief or a dispatch issue; the thresholds are unchanged (two spec-interpretation defects in a rolling eight-week window; three consecutive overhead-negative handoffs), and pre-2026-09-15 `cursor/*` history stays in the population for any window that includes it. §10's hooks run over the same branches: the brief-mapping check greps `claude/|codex/` merges since 2026-09-15 against `docs/briefs/handoffs/` and the dispatch issues, the locked-surface check adds `--branches="claude/*" --branches="codex/*"`, and the limb-1 sweep greps `cursor|codex|worker` in `docs/SESSIONS.md`. `cursor-fleet`'s orchestration loop — umbrella brief, claim manifest, disjoint footprints, dispatch-time Phase-0, integration order — is surface-agnostic and stands; its frontmatter, title, routing table and return-contract lines are updated in PR #401, and its `cursor/*` naming and `cursor-agent` CLI mechanics are historical. The `cc_handoff` template's §0.5 "Cursor variant" (recommended defaults a worker applies unless Phase-0 contradicts them) is renamed the worker variant in the same PR; its content is unchanged.
