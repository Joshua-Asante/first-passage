...
the worker seat.
**Operator rulings 2026-09-26 on these four:** [Addendum 2026-09-26](#addendum-2026-09-26).
...
| How a card's seat is bound to the worker seat | "Let's consider how a card is bound to the worker seat" | **Not decided — under consideration.** No mechanism is adopted; `check_handoff_authority.py` behaviour is unchanged. The question stays open for the operator. |
...
target asks, a moved or wrapped directory fails closed, the M1-override arm asks and never
denies; `LIVE_FLY_APPS` pinned to the two fly.toml files), run test-first through the launcher.
