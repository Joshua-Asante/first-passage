...
**Running counts (canonical, this ADR):** campaigns completed **0** · survivors falsified **0 / 2** · campaigns abandoned **2** (consecutive **2 / 2** — **§4(c) audit-report duty DISCHARGED 2026-08-23**, report delivered early on operator instruction: [supply-side audit](../notes/audits/programme-audit/2026-08-23-deep-lane-supply-audit.md), verdict `AMBIGUOUS`, slot 3 held gated on a supply event — the quarterly vehicle still receives it) · preregs refused **0** · **active campaign: none**. Most recent completed: **DL-2 ABANDONED 2026-08-22** (train nominee V9 — 2-session lookback, drift-aligned, retest-limit, 3R; argmax train net annSR, no fallback per §6 step 2 — failed gates 2a/2b/2d [net annSR −0.6840 <0 (cost-law ratio 18.15× alone passes); SPA consistent p=0.9755 vs ≤0.10, full 10-variant universe; M-16 +1-tick slip annSR −0.9185]; gate 2c cadence passed (2.558/wk); confirm partition M6A.FUT never read per prereg §5 — [`docs/briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md`](../briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md); full per-variant TRAIN results: [`lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md`](../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md)). Prior: **DL-1 ABANDONED 2026-08-16** (GC/MGC opening-range-continuation, all 10 variants train-net-negative, train nominee V7 failed gates 2a/2b/2d, gate 2c passed, confirm MGC.FUT never read — [`docs/briefs/pre-registration/2026-08-16-deep-lane-dl1-mgc-orc-prereg.md`](../briefs/pre-registration/2026-08-16-deep-lane-dl1-mgc-orc-prereg.md); full per-variant TRAIN results: [`lab/archive/dl1_mgc_orc_2026-08-16/train_results.json`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/dl1_mgc_orc_2026-08-16/train_results.json)).
...
| 2026-08-22 | **DL-2 prereg FROZEN on operator GO** ("GO on freeze") — second lane campaign live, M6A × `prior-session-breakout-continuation` (declared `NEW` in `MECHANISMS.md` the same session, reversing the prereg's own first-draft election to reuse `pdh-pdl-breakout-rth`, after a sanitized ox-alpha second opinion). The GO adjudicated the one remaining gathered operator item (standing-pause reading, D2a-grounded) and doubles as the W4 SPA re-arm GO. §6 step 1 pulls fired same session; step 2 (train scoring) is the next work item. Running-count line updated: active campaign DL-2 | Joshua (GO) + Claude Code |
| 2026-08-22 | **DL-2 ABANDONED** — §6 steps 1–2 executed (train pulls confirmed landed; all 10 frozen variants scored on 6A.FUT TRAIN, 2010-06-06→2019-01-01, 2,200 Globex sessions, 2,815,486 stitched 1-minute bars, 35 UTC roll days/roll sessions excluded). Nominee = V9 (2-session lookback, drift-aligned, retest-limit, 3R; argmax train net annSR, no fallback). Failed gate 2a (net annSR −0.6840, not >0; cost-law ratio 18.15× alone passes), gate 2b (SPA consistent p=0.9755 vs the ≤0.10 bar, full 10-variant universe, block=20/B=10,000/seed=11), and gate 2d (M-16 +1-tick slip annSR −0.9185); passed gate 2c (cadence 2.558/wk). All 10 variants scored net-negative on TRAIN (range −0.68 to −1.62) — confirm partition (M6A.FUT) never read, per prereg §5. A pre-acceptance diagnostic check (win rate/direction-split/cost-arithmetic) ruled out a sign or fill-engine defect before this verdict was recorded — see the prereg's own Change-history row and [`lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md`](../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/RESULTS.md) for detail. Running-count line updated: abandoned 2 (consecutive 2/2), active campaign none. **§4(c) audit-report duty TRIPPED (2 consecutive abandonments) — owed at the next quarterly programme audit.** Engine + full per-variant results: [`lab/archive/dl2_m6a_pdhpdl_2026-08-22/`](../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/) | Claude Code (train-scoring session, per [handoff](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/briefs/rnd-pipeline/2026-08-22-cc-handoff-dl2-m6a-step2-train-scoring.md)) |
| 2026-08-23 | **§4(c) audit-report duty DISCHARGED early** — operator instruction ("run the supply-side audit") brought the report forward of the quarterly vehicle: [`2026-08-23-deep-lane-supply-audit.md`](../notes/audits/programme-audit/2026-08-23-deep-lane-supply-audit.md). Seven diagnostic questions run (hard core preserved; belt 3 adds/0 removes baseline; zero falsifier drift — §4 limb text byte-identical `f60959a`→HEAD; no degeneration signals; process clean). Supply census: no candidate family satisfies (i) genuinely-new-mechanism ∧ (ii) reachable-with-current-data ∧ (iii) venue-shape-compatible with data on hand; nearest satisfier is the order-flow route (`MNQFLOW-1-DEPTH`, FROZEN, ≤$125, K=0, one operator sign-off away on conjunct (ii)). Verdict **`AMBIGUOUS`** with named re-test conditions (a supply event, else the 2026-11-08 slate escalates to PARK/supersede). **Slot 3 recommended held, gated on a supply event, not a calendar date** — operator decision, not executed by the audit. Running-count line annotated. | Claude Code (audit session) |
