...
**Running counts (canonical, this ADR):** campaigns completed **0** · survivors falsified **0 / 2** · campaigns abandoned **1** (consecutive **1 / 2**) · preregs refused **0** · **active campaign: DL-2** — M6A × `prior-session-breakout-continuation` (`NEW` mechanism id, `MECHANISMS.md`), FROZEN 2026-08-22 on operator GO ("GO on freeze") — [`docs/briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md`](../briefs/pre-registration/2026-08-22-deep-lane-dl2-m6a-pdhpdl-prereg.md); K=10, `floor_at_k(10, 7.6386)=1.170`; §6 step 1 pulls fired same session, step 2 (train scoring) not yet started. Most recent completed: **DL-1 ABANDONED 2026-08-16** (train nominee V7 — argmax train net annSR, no fallback per §6 step 2 — failed gates 2a/2b/2d [net annSR −0.42 <0; SPA p=0.94 vs ≤0.10; M-16 slip annSR −0.66]; gate 2c cadence passed; confirm partition MGC.FUT never read per prereg §5 — [`docs/briefs/pre-registration/2026-08-16-deep-lane-dl1-mgc-orc-prereg.md`](../briefs/pre-registration/2026-08-16-deep-lane-dl1-mgc-orc-prereg.md); full per-variant TRAIN results: [`lab/archive/dl1_mgc_orc_2026-08-16/train_results.json`](../../lab/archive/dl1_mgc_orc_2026-08-16/train_results.json) — corrected this session; the path previously cited here (`lab/analysis/deep_lane/...`) never existed, caught while authoring DL-2's own step-2 handoff). Consecutive abandonments 1/2 — **if DL-2 also abandons at step 2, that is the 2nd consecutive and trips the §4(c) audit-report duty.**
