...
**Power calculator (separate implementation dependency):** [calculator PR #371](https://github.com/Joshua-Asante/first-passage/pull/371) carries `scripts/certification_power.py` and its tests; this document-only draft depends on that PR being reviewed and merged before TB-F1 uses the mode. Calculator acceptance does not freeze sample sizes, choose a policy or ratify this draft. The additive implementation provides `min_certifying_passes`, `speed_limb_power`, `joint_power_four` and `size_for_joint_four`, plus the CLI flags `--true-pass-rate` / `--pass-target`; the legacy output is byte-identical without them (test `test_cli_speed_mode_smoke_and_legacy_line_unchanged`); an exact-rational oracle pins `min_certifying_passes(100, 0.5, 0.05) == 59` and the small-binomial speed power. Speed-limb identity used: a lower bound on the pass proportion ≥ t is the upper bound on the failure proportion ≤ 1 − t.
...
Reading: at a true failure rate of 3 % the failure limbs bind and the speed limb is essentially non-binding once the true pass-by-200 rate is ≥ 0.60 (its minimum certifying count is 501 of 950); the dependence-valid Fréchet bound at 80 % joint power gives **n = 970 per full/H1/H2 population within each confirmation stream**. **Proposed (`OWED-BY: TB-F1`):** n2 = n3 = 970 at the (3 %, 0.65) design alternative under Fréchet; n1 = 200 as a screen with continuation cutoff "stage-1 point-estimate failure proportion ≤ 0.05 on full and both halves" (a failed cutoff ends the attempt; a passed cutoff certifies nothing). For this proposal, each n2 and n3 stage runs 970 full-panel, 970 H1-panel and 970 H2-panel evaluations; the speed limb reuses the 970 full-panel evaluations. n1 likewise specifies depth per population. TB-F1 freezes actual workload totals, per-population RNG namespaces and Part A's separate workload; these are not 970 trials split between halves. n3 is **never** sized from n1/n2 results.
...
The gate is mandatory for a new `POLICY_REGISTRY` instance and its three Part C criteria are the admission test; every deviation from [`docs/methodology/regime_robustness_gate.md`](../../methodology/regime_robustness_gate.md) is named. **Part A (criterion 1):** 100 alternate-history panels constructed by resampling 6-month contiguous blocks **with replacement**, concatenating to the **original venue-session length**, and **rebuilding the inner five-session MC blocks separately for each alternate panel**, each replayed continuously at the fixed instance on the **n2 stream inside TB-E1**; the 5th percentile of the per-panel pass-rate distribution must be ≥ 0.95 (the full-panel floor, one minus the 5 % ceiling; no separate regime floor); close-call rule n = 200 when p5 is within 1 pp of the floor; sanity checks retained (p5 ≤ the full-panel pass-rate; H1/H2 covered-session counts partition the full source panel under proposed B2 below). **Deviation A1 (proposed under TB-P2, not yet ratified):** TB-F1 freezes the block-boundary/index and final-block truncation rules, inner five-session block construction, RNG/seed allocation, percentile method and 200-panel close-call expansion, as well as the exact positive-integer per-panel path depth and its deterministic compute-budget calculation (`OWED-BY: TB-F1`). The first TB-P2 operator ratification is required before TB-F1; a second dated operator addendum ratifying that exact depth is required before TB-E1. A reduced depth is proposed, not removal of Part A, and calculator output cannot supply either ratification. **Part B (criteria 2–3):** the F-H1 / F-H2 limbs of §2 on each chronological half (§5), replayed independently as the gate requires. **Deviation B1 (stricter, recorded so the substitution is visible):** each half's limb is the one-sided 95 % upper failure bound ≤ 0.05, which implies the gate's point criterion (pass-rate ≥ 0.95) and never passes a half the gate would fail. **Deviation B2 (proposed, requires the first TB-P2 ratification):** replace the canonical H1+H2 MC-path-count sanity sentence with source-session coverage partitioning. Each half receives the full parent MC depth independently; MC counts do not sum to the full-panel depth. All decision bounds and Part B depth remain unchanged. **Part A is not rerun on n3:** TB-E2 evaluates only §3's four conditions; TB-E1's Part A result is retained admission evidence whose digest enters the fixed-book replay fingerprint (§6). A Part A failure ends the attempt exactly as a failed n2 bound does (§4).

**No hidden bootstrap substitution:** TB-S2's decision-bearing joint-flat path sampler and Part A's retained five-session inner construction must each be specified and validated before TB-F1. If their composition cannot preserve the qualified replay semantics, the owners must resolve it before freeze; neither a mock nor an undocumented block-length substitution supplies that proof.

## 4 — K = 1, one attempt, no runner-up, tie rule

D-B4 / D33: any failed screen, n1 cutoff, n2 bound, regime-gate Part A (§3a) or n3 bound ends the attempt with no qualifying configuration; no runner-up, extra sample or repeated n3. **Snapshot/fingerprint invalidation:** C10 voids the seal and dependent evidence and stops for the operator under governing plan S2 and TB-P2 R1c; it does not authorize an automatic replacement outcome-bearing draw. Any separately authorized continuation requires fresh bound evidence and dependent approvals; a failed criterion remains terminal. **Tie rule (written for a future K > 1 freeze to inherit; inert at K = 1):** rank by the frozen speed statistic (lower-bound P(T ≤ 200), higher is better), then by the full-sample failure upper bound (lower is better), then by the H1/H2 maximum failure upper bound, then by fewer executable legs, then by the lexical `leg_id` set.
...
The normative FBR/EF composition, normalization and EF1→EF2 GO-only reseal are [TB-P2 §2a–§2b](../../adr/2026-09-12-tradeify-book-protection-instance-admission.md). The summaries below cannot omit those requirements; the ADR remains proposed until its operator ratifications.

| Layer | Contents | Sealed by |
|---|---|---|
| **K = 1 confirmation contract** | adapter port hashes (`PORT_MANIFEST.sha256` via TB-A0), replay-engine commit, bar-panel digests (`SHA256SUMS`), warm-up boundaries (TB-W1: panel origin), D19 calendar digest, TB-C1 forward-calendar and closure-overlay digests, commission-schedule digest, capacity rules (TB-S1), fill model (§9), E1 initial state (§9), the fixed book and fixed policy, streams and sizes (§2), dates/halves/horizon (§5), full Part A construction (§3a), the shared fingerprint tool/runtime/dependency digests and literal byte/hash vectors prescribed by TB-P2 rev9, and frozen config serialization/initial-arm transition vectors | **TB-F1** (`OWED-BY: TB-F1` for every digest) |
| **Fixed-book replay fingerprint (FBR)** | the contract + exact book + canonical planned policy row excluding self-referential provenance + AST-normalized `dd_geometry.py` digest excluding only the registry assignment + n2/Part A result digests, with the shared components defined by TB-P2 §2a | **TB-E1** seal; unchanged through Phase 8, B7 and n3 |
| **Execution fingerprint (EF1, then EF2)** | complete rail/daemon commit, image/config manifest, dd-state seed, source identity and snapshot binding; every shared component equals FBR using TB-P2 canonical normalization. Only the verified T10 GO-only reseal may produce EF2 with the GO digest, artifact commit and new image; every non-GO layer and shared/config digest must remain equal | **TB-B7** seals EF1; **TB-D2 GO reseal** seals EF2 under TB-P2 T10–T11; host verifies complete EF2 and the exact permitted initial-arm config transition; effective activation after restart repeats boot-bound identity/no-activity/expiry checks and durably acknowledges before admitting risk under TB-P2 T11 |

**Fingerprint implementation gate:** TB-I1 supplies the single `ops/c1_rail/policy_fingerprint.py` implementation before TB-F1. Freeze its exact CPython patch version, serializer/version, tool/dependency digests and conformance vectors. TB-E1, TB-D0, B7, GO reseal and arm/boot checks all invoke that implementation; raw code hashes or independent normalizers do not substitute. Complete manifests retain full `book_policy.py` and all other shared inputs, with only the explicitly defined policy-provenance and registry-node exclusions; provenance/admission are independently verified.
...
|---|---|---|
| Leave-one-year-out (each frozen year/coverage partition dropped in turn) | report each variant's full-sample failure upper bound; severity WARN if any variant > 0.075 | operator review; no re-selection |
...
| Live time-to-pass predictive interval | quantiles q10 / q50 / q90 of unconditional T from all n3 paths, including T = infinity; TB-F1 freezes the quantile method and session-count clock anchored to the first venue session after verified effective activation. Lower-tail trigger: an observed completed pass before q10; an ongoing attempt before q10 is not a trigger. Upper-tail trigger: elapsed covered sessions exceed finite q90 without qualifying; infinite q90 has no finite upper-tail trigger | a live bust or predictive-band breach records the governing plan’s forward-model verdict: the model-fitted proposal is falsified; operator review with monotone cap: AUTHORIZED may move to WATCH-1, existing WATCH-1/WATCH-2/RETIRED never move upward. Bust/terminal failure invokes the governing account stop/recovery and fresh-GO requirements; it never authorizes continued reduced-size trading on a breached account |
...
**Battery freeze contract:** these are post-deployment diagnostic/monitoring proposals, not extra n3 acceptance tests or permission for another final sample. Before results, TB-F1 fixes every variant's exact partition definitions (including partial calendar years), replica count/budget, RNG namespace, perturbation unit and scope (including whether entry drops apply per path), missing/unresolved-output handling, statistic, cutoff, severity and down-only action or explicit descriptive-only disposition. These fields remain `OWED-BY: TB-F1`; choosing them after observation is forbidden. Existing terminal-account and operator-GO controls remain independent.
...
## 10a — Freeze readiness (all still owed at their owners)

A merge of this draft does not freeze its proposals. TB-F1 requires wave 2 and TB-C1 completion, all intake/parity evidence, the first TB-P2 ratification, merged calculator #371, the implemented fingerprint recipe and literal vectors, and every `OWED-BY` value. The exact-depth second ratification follows TB-F1 and precedes TB-E1.

The TB-S2/TB-S3/operator owners must settle one replay/operations schedule (regular and early-close cutoff, own-flat target and backstop), and the initial pre-reopen readiness predicate identified by TB-O1 must have its required approved contract before dependent qualification/activation. The generic 15:55 check/16:00 own-flat rule cannot silently be substituted for the replay scheduler. No missing capability is marked implemented here; later-session authorization remains separate.

The fixed-book chain is E1 seal → D0 admission and D1 ORB GO → V1 runtime binding → qualified dry-run image/B7 → retained D1 GO recheck → sole n3 → D2/GO reseal → verified initial activation. Missing ORB GO blocks V1 and the downstream chain; it does not remove ORB or turn withheld approval into a failed result.
...
**Extraction/review note (2026-09-13):** the calculator implementation and its tests are reviewed separately; this PR retains only the preregistration draft and ORB decision skeleton. All proposed sample sizes and freeze fields remain DRAFT—NOT FROZEN. The TB-P2 exact-depth ratification gate above is a dependency reconciliation, not a new ratification.
