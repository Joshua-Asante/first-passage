...
findings applied); given a sanitized ox-alpha second opinion ([`2026-08-22-ox-alpha-adversarial-lens-scope.md`](https://github.com/Joshua-Asante/first-passage/blob/07fecc6cceb8609fdd5feae78ae67779727ed5c6/docs/adr/2026-08-22-ox-alpha-adversarial-lens-scope.md)),
...
**Reversed after a sanitized ox-alpha second opinion** ([`2026-08-22-ox-alpha-adversarial-lens-scope.md`](https://github.com/Joshua-Asante/first-passage/blob/07fecc6cceb8609fdd5feae78ae67779727ed5c6/docs/adr/2026-08-22-ox-alpha-adversarial-lens-scope.md),
...
second opinion, [`2026-08-22-ox-alpha-adversarial-lens-scope.md`](https://github.com/Joshua-Asante/first-passage/blob/07fecc6cceb8609fdd5feae78ae67779727ed5c6/docs/adr/2026-08-22-ox-alpha-adversarial-lens-scope.md), reconciled against this table).**
...
**Ox-alpha sanitized-lens second opinion (per [`2026-08-22-ox-alpha-adversarial-lens-scope.md`](https://github.com/Joshua-Asante/first-passage/blob/07fecc6cceb8609fdd5feae78ae67779727ed5c6/docs/adr/2026-08-22-ox-alpha-adversarial-lens-scope.md),
...
| 2026-08-22 | **DL-2 ABANDONED** — §6 steps 1–2 executed (train pulls confirmed landed; all 10 frozen variants scored on 6A.FUT TRAIN, 2010-06-06→2019-01-01, 2,200 Globex sessions, 2,815,486 stitched 1-minute bars; roll-day detection derived from the TRAIN `ohlcv-1m` cache itself — no `ohlcv-1d` cache exists for 6A/M6A, so the per-UTC-day volume-leader signal was computed by summing 1-minute volume per (day, symbol), cross-validated as byte-identical to a native `ohlcv-1d` pull against DL-1's own real `ohlcv-1d`/`ohlcv-1m` cache pair, 0 mismatches across 83,165 rows — disclosed as a construction note, not a deviation from the frozen rule). Nominee = V9 (2-session lookback, drift-aligned, retest-limit, 3R target; argmax train net annSR, no fallback, no tie). Failed gate 2a (net annSR −0.6840, not >0; cost-law ratio 18.15× passes alone), gate 2b (SPA consistent p=0.9755 vs the ≤0.10 bar, full 10-variant universe, block=20/B=10,000/seed=11), and gate 2d (M-16 +1-tick slip annSR −0.9185); passed gate 2c (cadence 2.558/wk). All 10 variants scored net-negative on TRAIN (range −0.68 to −1.62) — confirm partition (`M6A.FUT`) never read, per §5. Diagnostic check before accepting the verdict (ruling out a sign/direction defect): win rates 42.6–46.6% across all variants, balanced long/short splits, `net = gross − n×$2.60` reproduces exactly; 85–90% of trades exit via force-flat rather than stop/target (nominee: 95.4%), a structural property of a prior-full-session-derived stop/target geometry against a 1-session holding window, not a code defect. **This is DL-2's own abandonment, the 2nd consecutive after DL-1 — charter §4(c)'s audit-report duty has now tripped**, owed at the next quarterly programme audit. Engine + full per-variant results: [`lab/archive/dl2_m6a_pdhpdl_2026-08-22/`](../../../lab/archive/dl2_m6a_pdhpdl_2026-08-22/). | Claude Code (train-scoring session, per [handoff](../rnd-pipeline/2026-08-22-cc-handoff-dl2-m6a-step2-train-scoring.md)) |
