...
- **Phase 1 — Design the joint-surrogation null (discharges D5's O1 `UNRESOLVED-NEEDS-DESIGN` item).** **Operator GO received 2026-08-30; design work ATTEMPTED this session, NOT YET RESOLVED — see full write-up at [`lab/analysis/_inbox/joint_surrogation_null_2026-08-30/RESULTS.md`](../../lab/analysis/_inbox/joint_surrogation_null_2026-08-30/RESULTS.md).** Three candidate constructions were built and empirically tested (a positive control against synthetic ground-truth data, plus direct diagnostic checks against the real cached `on_range`/`rth_range` frame): (1) a "linked-residual IAAFT" (lag-0 OLS decomposition + per-channel IAAFT + recombine) — failed its own positive control on every replicate (channel 2's own ACF badly distorted, med mismatch ≈0.14 vs a 0.04 tolerance); (2) a bivariate VAR(p) residual-bootstrap — preserves the target cross-correlation well but both channels' own ACF fails the same diagnostic gate (med ≈0.10–0.13), even at VAR order up to 90, because these specific MNQ range series are already only borderline-viable for plain univariate IAAFT (`rth_range` alone: med=0.0403, just over tolerance); (3) a shared-starting-permutation coupled IAAFT — the mirror-image failure: excellent own-ACF (tracking standalone univariate IAAFT quality) but cross-correlation decays immediately to ≈0.61 against a 0.77 target, regardless of iteration count. All three diagnostic-gate results are independently re-runnable (audit hooks in the RESULTS.md). **This triangulation is itself the Phase 1 output this session produced** — it rules out the "combine two well-behaved per-channel surrogates" family of constructions with evidence, not speculation, and names the two live next steps (a shared-latent-GARCH joint model, or an explicitly-disclosed VAR approximation used only for a conservative sensitivity check) for whichever future session takes this further. Phase 2/3 below do not proceed until a design clears its own positive control. Original candidate sketch, retained for context (superseded by the above, not discarded): model each day's shared regime as a common latent factor (e.g., a trailing joint EWMA of both overnight and RTH range), generate paired surrogates that preserve (a) each series' own linear ACF (as the existing IAAFT battery already does per-series) and (b) the lag-0 same-day cross-correlation attributable to that shared factor, while randomizing any residual cross-dependence beyond it. This is a coupled/joint IAAFT variant, not the existing per-series routine — genuinely new, and exactly why D5 named it unresolved rather than assuming the S1 machinery ports. **Amended 2026-08-30:** the design, once resolved and reviewed, is one design run twice — once against MNQ's cached `candidate24_joint_frame.csv`, once against MYM's cached `c24_joint_frame.csv` — not two separately-designed tests. A fresh K declaration is owed before Phase 3 execution for the MYM leg specifically (per H-RANGEXFER-1.a-MYM's own note in §4), since MYM's own joint gate was an unregistered look; MNQ's Phase 1 K-accounting is unaffected and unchanged by this amendment. **Second amendment, same day (H-RANGEXFER-1.b-MYM added):** that hypothesis rides the same Phase 1 design against MYM's own day-history-stratified frame (`c2_c4_stratified_rerun.py`'s cached scored set, `bprime=0` subset) rather than a third bespoke design — it needs its own fresh K declaration at Phase 3 execution time too, distinct from H-RANGEXFER-1.a-MYM's, since it is a separately-scoped stratum-conditional claim.
