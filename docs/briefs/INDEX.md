...
| **Q-GATESTACK-1** — does anything on GitHub actually require the gate stack to pass before `main`, and is CI-status doc current (assumption-sweep A1+D7) | **`OPEN — DRAFT (pre-lock)`** 2026-08-18 | [`Q-GATESTACK-1-gate-stack-enforcement.md`](Q-GATESTACK-1-gate-stack-enforcement.md) | Named, not opened. Operator GO → Phase 1 (`gh api` branch-protection/rulesets/Actions checks, $0/K=0). |
...
| **Q-ORBCUSH-1** — does a trailing edge/cost-fraction classifier explain ORB-MNQ-1's 2021-09-28 cushion-proportional-sizing regime break (triple-verified real, not boundary-luck; trailing-vol mechanism already REFUTED same discipline) | **`OPEN — DRAFT (pre-lock)`** 2026-08-20 | [`Q-ORBCUSH-1-regime-break-mechanism.md`](Q-ORBCUSH-1-regime-break-mechanism.md) · [pre-reg](pre-registration/Q-ORBCUSH-1-verdict-preregistration.md) · [probe](../../lab/analysis/c1/orbmnq1_cushion_sizing_probe_2026-08-20/) | Named, not opened. Operator GO → Phase 1 (reuse the already-verified intraday-honest harness, $0/K=0). Sibling of `Q-POLFRONT-1` (commissioned by Q-EVALSEQ-1, still unopened, c1-book-scoped, blocked on pyramided-leg data reconstruction) — this Q pivots to ORB-MNQ-1 where the same lever was actually testable. |
