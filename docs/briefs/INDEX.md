...
| **Q-NSURV-2** — can a magnitude-resampling second-uncertainty-layer be added to N-SURV reporting as a pure addition (headline-preserving), or does it require touching `run_partition_mc`/`blocks_from_daily_pnl` internals — the deferred `Q-NSURV-1` design question, opened one session early (operator: "go bigger") | **`OPEN — DRAFT (pre-lock)`** 2026-08-20 | [`Q-NSURV-2-second-uncertainty-layer-design.md`](Q-NSURV-2-second-uncertainty-layer-design.md) · [pre-reg](pre-registration/Q-NSURV-2-verdict-preregistration.md) | Pre-registration frozen; Phase 1 (wrapper build + bit-identical reproduction check) not yet run. Operator GO owed to execute. |
| **Q-ORBSURV-1** — does cushion-proportional sizing clear the frozen survivor-scoring gate at full-panel k=2 and the post-2021-09-28 sub-window in isolation (k=1/k=2) — the configurations today's informal cushion-sizing probes left unmeasured | **`OPEN — DRAFT (pre-lock)`** 2026-08-20 | [`Q-ORBSURV-1-cushion-sizing-gate-configurations.md`](Q-ORBSURV-1-cushion-sizing-gate-configurations.md) · [pre-reg](pre-registration/Q-ORBSURV-1-verdict-preregistration.md) | Pre-registration frozen; Phase 1/2 (full-panel k=2 + post-break isolation) not yet run. Operator GO owed to execute. Does **not** license unpark on `RESOLVED` (§5). |
