...
| **Q-TRADECAP-2** — which close is licensed for the confirmed unbounded per-trade realized loss on Tradeify_Select_100K | **`OPEN`** 2026-08-24 — Phase 0 geometry done: CFD 2.0% = $2,000 = 66.7% of the $3,000 trail; `daily_loss_pct=None`; listener omits `sl=`. Option 1-as-staged not startable | [`Q-TRADECAP-2-per-trade-bound-election.md`](Q-TRADECAP-2-per-trade-bound-election.md) · [pre-reg](pre-registration/Q-TRADECAP-2-verdict-preregistration.md) · [parent closure](closures/Q-TRADECAP-1-closure-resolved.md) | Operator elects exactly one of {**2** alert tripwire, **1-size** entry $ ceiling, **1-realized** hard-cap gated on disaster-stop Phase 0a}. Do not import 2.0%. |
...
- **Q-TRADECAP-1** — is there any per-trade dollar-loss bound anywhere in the sizing/arming path on
  ...
  **`RESOLVED` 2026-08-23** —   confirmed absent on all four checked limbs (sizing law, arming
  interlock, EM2, disaster-stop). Successor opened 2026-08-24:
