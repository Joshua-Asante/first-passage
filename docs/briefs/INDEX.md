...
| **Q-VOLREGIME-1** — does an M15 bar's own volume regime (above its own time-of-day-conditioned trailing median) carry next-bar range information beyond the trigger bar's own range state; joint MNQ+MYM from authoring, no taxonomy-merge review owed (single shared id from the start) | **`OPEN`** 2026-08-31 — both instruments' stratified stage-1 results already decisive (MNQ +22.3pp/+27.4pp post-ToD-fix; MYM +16.5pp/+24.5pp); bar-level by-year L4 now **PASS 7/7 both instruments** (2026-08-31); Phase 1 (1-bar-lag joint-surrogation null) is upstream-blocked — the day-level design it was to adapt closed `AMBIGUOUS-DESIGN` (Q-RANGEXFER-1, never certified) — assessed independently, not closed by inheritance; both binding failure mechanisms are materially weaker at this brief's own bar-level n | [`Q-VOLREGIME-1-intraday-bar-volume-regime.md`](Q-VOLREGIME-1-intraday-bar-volume-regime.md) · [pre-reg](pre-registration/Q-VOLREGIME-1-verdict-preregistration.md) · [L4 packet](../../lab/analysis/_inbox/volregime_byyear_l4_2026-08-31/RESULTS.md) | L3 PASS both instruments (2026-08-31) — do not re-run it. L4 PASS 7/7 both (2026-08-31) — do not re-run it. Still open: L5 attribution (C2–C5 / observed L5 unrun). A 2026-09-03 campaign-scoped waiver let the off-queue VOLREGIME translation campaign enter Packet T on L1–L4 presence; T0 then closed `PRE-CONTRACT DROP` (no surviving bridge). That waiver does not close or certify this Q. Any future joint-surrogation attempt must clear `BOUNDED_ROUND_PLAN.md`'s §3 certification requirements. |
...
- **Q-RANGECOND-1** — overnight-range-conditioned ORB-MNQ-1 payoff shape — **`FALSIFIED` 2026-08-31** (corrects a retracted 2026-08-30 `RESOLVED` verdict — a look-ahead defect in a shared upstream dependency inflated the original result; corrected, conditioned/unconditioned WR and mean-win are statistically indistinguishable). Addendum on [`b3-orb-mnq-payability-line.md`](../pursuits/b3-orb-mnq-payability-line.md) retracted same day; `ORB-MNQ-1` stays `PARKED`, no new evidence. [`closure`](closures/Q-RANGECOND-1-closure-falsified.md) · [`retracted closure`](closures/Q-RANGECOND-1-closure-resolved.md) · [`defect audit`](../notes/audits/2026-08-31-mnq-overnight-window-lookahead-defect.md)
