...
|---|---|---|---|
| **Q-XMEM-1** — cost of per-surface agent-memory invisibility; time-boxed Mem0 sidecar pilot (prefs/pointers only) | **`OPEN`** (architecture + §6 frozen 2026-07-16; v1.2 Limb B FTS landed 2026-08-15, **QUARANTINED same day** — recall regression; T0 not started) | [`Q-XMEM-1-cross-surface-memory-sidecar-pilot.md`](Q-XMEM-1-cross-surface-memory-sidecar-pilot.md) · [pre-reg](pre-registration/Q-XMEM-1-verdict-preregistration.md) | Limb B: `scripts/repo_retrieve.py` on hot surfaces — **do not paste its output as a sub-rule 8/10 attestation until re-measured** (see script docstring). Limb A Mem0 T0 unpaid — **operator GO only** (original §6). Do **not** store Rule-7 values. |
