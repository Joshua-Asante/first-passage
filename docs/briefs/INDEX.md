...
| **Q-RANGEXFER-1** — does overnight-range / gap-magnitude carry genuine same-day RTH-range information beyond a shared-regime confound (D5 "S2" cross-series shape); forks Q-RANGEXFER-1.a (gap, nested to the overnight-calm stratum); **amended 2026-08-30 to cover MYM's own replicated finding**, scored independently per instrument | **`OPEN`** 2026-08-29, amended 2026-08-30 — D5 stage-1 $0 falsifier + joint D-S-A gate both run and clear on MNQ; MYM's own stage-1 + joint-gate replication now folded in (§11); Phase 1 (joint-surrogation null design, D5's own O1 `UNRESOLVED-NEEDS-DESIGN` item) needs operator GO before it starts, on either instrument | [`Q-RANGEXFER-1-overnight-range-gap-magnitude-transfer.md`](Q-RANGEXFER-1-overnight-range-gap-magnitude-transfer.md) · [pre-reg](pre-registration/Q-RANGEXFER-1-verdict-preregistration.md) · [joint gate (MNQ)](../../lab/analysis/_inbox/mnq_dailygeom_notice_2026-08-29/candidate24_joint_gate.py) · [joint gate (MYM)](../../lab/analysis/_inbox/mym_mechanism_harvest_2026-08-29/c24_joint_gate.py) | Operator GO for Phase 1 design work (both instruments, one design), then adversarial review of that design (§7 Phase 2) before any execution; MYM leg needs a fresh K declaration first (§7). |
| **Q-VOLREGIME-1** — does an M15 bar's own volume regime (above its own time-of-day-conditioned trailing median) carry next-bar range information beyond the trigger bar's own range state; joint MNQ+MYM from authoring, no taxonomy-merge review owed (single shared id from the start) | **`OPEN`** 2026-08-30 — both instruments' stratified stage-1 results already decisive (MNQ +20.6pp/+25.6pp; MYM +16.5pp/+24.5pp); Phase 1 (1-bar-lag joint-surrogation null, adapted from Q-RANGEXFER-1's design) needs operator GO before it starts | [`Q-VOLREGIME-1-intraday-bar-volume-regime.md`](Q-VOLREGIME-1-intraday-bar-volume-regime.md) · [pre-reg](pre-registration/Q-VOLREGIME-1-verdict-preregistration.md) | Operator GO for Phase 1 design work, then adversarial review, then a K declaration before execution (§7). |
