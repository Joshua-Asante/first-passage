...
|---|---|---|---|
| **Q-EVALSEQ-1** — within-eval front-load sizing schedule (frozen K=4 family) | **`OPEN`** — un-dormed 2026-08-16 scoring-only (P2 mark; dormancy ground lapsed with S1); frozen §6 body byte-unedited | [`pre-reg`](pre-registration/2026-07-24-2leg-eval-frontload-schedule-preregistration.md) · [P2 closure](closures/STATE-POLICY-closure-resolved-p2.md) | Recover harness from `pre-prune-2026-08-08`, verify its own anchors BEFORE any policy read, then run the frozen K=4 MC + DSR/placebo + both-halves and close under its own §6. Lever measurement only — no deployment reading. |
| **Q-POLFRONT-1** — policy-augmented seed-target frontier (same 4 policy shapes on synthetic (w,b,r,k,d) cells) | **`COMMISSIONED`** — named by the P2 mark, **not opened**; brief owed before any run | [P2 closure §2.3](closures/STATE-POLICY-closure-resolved-p2.md) | Author the brief (measurement-only, $0, no manifest), then run only after its own gate is frozen. Feeds deep-lane GO-1. |
