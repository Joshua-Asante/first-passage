...
| **Q-VOLREGIME-1** — does an M15 bar's own volume regime (above its own time-of-day-conditioned trailing median) carry next-bar range information beyond the trigger bar's own range state; joint MNQ+MYM from authoring, no taxonomy-merge review owed (single shared id from the start) | **`OPEN`** 2026-08-30 — both instruments' stratified stage-1 results already decisive (MNQ +20.6pp/+25.6pp; MYM +16.5pp/+24.5pp); Phase 1 (1-bar-lag joint-surrogation null) is upstream-blocked — the day-level design it was to adapt closed `AMBIGUOUS-DESIGN` (Q-RANGEXFER-1, never certified) — assessed independently, not closed by inheritance; both binding failure mechanisms are materially weaker at this brief's own bar-level n | [`Q-VOLREGIME-1-intraday-bar-volume-regime.md`](Q-VOLREGIME-1-intraday-bar-volume-regime.md) · [pre-reg](pre-registration/Q-VOLREGIME-1-verdict-preregistration.md) | Cheap next step: this brief's own by-year L4 qualifying-year count from vendor bars (may PASS at bar-level n, unlike the day-level wall) — not yet computed. Any future joint-surrogation attempt must clear `BOUNDED_ROUND_PLAN.md`'s §3 certification requirements. |
