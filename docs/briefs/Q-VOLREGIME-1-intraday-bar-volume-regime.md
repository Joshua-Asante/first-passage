...
| 2026-08-31 | **Bar-level by-year L4 executed and PASS on both instruments.** MNQ and MYM each have `N_valid=7`, all 7 qualifying years positive, required=5. Vendor hashes match tracked `SHA256SUMS`; reconstructed pooled counts/lifts match the committed stage-1 frames. This discharges the only unresolved presence limb and removes panel-length `AMBIGUOUS-HOLD` as a forced route. It does not alter the frozen gate or close the Q: L5 joint-surrogation attribution and the distinct-WHO disclosure remain open. Full packet: [`volregime_byyear_l4_2026-08-31`](../../lab/analysis/_inbox/volregime_byyear_l4_2026-08-31/RESULTS.md). | Codex local execution |
