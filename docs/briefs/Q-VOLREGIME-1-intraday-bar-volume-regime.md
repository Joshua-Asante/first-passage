...
**Reject/Accept/Ambiguous-hold for H-VOLREGIME-MYM:** mirror H-VOLREGIME-MNQ's conditions verbatim, substituted for MYM's own panel (`core/data/bar_data/MYM_M15.csv`, 141,467 bars, 2020-07→2026-07) and its own by-year table. MYM's own precondition is CLEARED (2026-08-30), and its by-year floor is now also **L4 PASS, 7/7 qualifying years positive** (2026-08-31), independently matching MNQ. The attribution limb remains unresolved on both instruments.
...
- **Phase 0.75 — Compute the bar-level by-year L4 presence limb independently. COMPLETE, 2026-08-31.** Hash-verified both vendor panels, reconstructed the frozen scored frames, and applied the corrected per-stratum annual floor at [`volregime_byyear_l4_2026-08-31`](../../lab/analysis/_inbox/volregime_byyear_l4_2026-08-31/RESULTS.md). MNQ and MYM each return `N_valid=7`, `n_pass=7`, required=5 → **L4 PASS**. This removes panel-length `AMBIGUOUS-HOLD` as a forced outcome but does not score L5 or close the Q.
...
| 2026-08-31 | **Bar-level by-year L4 executed and PASS on both instruments.** MNQ and MYM each have `N_valid=7`, all 7 qualifying years positive, required=5. Vendor hashes match tracked `SHA256SUMS`; reconstructed pooled counts/lifts match the committed stage-1 frames. This discharges L4 specifically and removes panel-length `AMBIGUOUS-HOLD` as a forced route. It does not alter the frozen gate or close the Q: **L3 (both chronological halves of the conditional cases positive) has not yet been computed for this construct and remains open**, alongside L5 joint-surrogation attribution and the distinct-WHO disclosure. Full packet: [`volregime_byyear_l4_2026-08-31`](../../lab/analysis/_inbox/volregime_byyear_l4_2026-08-31/RESULTS.md). | Codex local execution |
