...
| 2026-08-30 | Upstream bounded-round plan filed (PROPOSED) at [`lab/analysis/_inbox/joint_surrogation_null_2026-08-30/BOUNDED_ROUND_PLAN.md`](../../lab/analysis/_inbox/joint_surrogation_null_2026-08-30/BOUNDED_ROUND_PLAN.md); two items touch this brief's reader directly. (1) **The day-level design's two failure mechanisms are both n-dependent and materially weaker at this brief's own bar level** (estimation noise in the long-memory `d` at n≈1.5k days vs n≈135k–140k bars; the ~6-qualifying-year L4 wall vs 7 spanned calendar years with enormous per-year n) — so a day-level certification failure must NOT close this brief by inheritance, consistent with the ratification's own scoped-to-`Q-RANGEXFER-1`-only clause; this brief's Phase 1 gets its own independent assessment either way. (2) The plan names a cheap, parallel, K-free diagnostic owed here: **this construct's own by-year L4 qualifying-year count from vendor bars** (mirroring the `rangexfer_byyear_l4_2026-08-30` convention including its per-stratum floor correction) — unlike the day-level frames, `N_valid ≥ 7` may PASS at bar level, which would make `RESOLVED`/`FALSIFIED` genuinely reachable here and materially raise the value of certifying the null. The plan also flags for a future gate pass that this brief's §6 has the same FALSIFIED/AMBIGUOUS-HOLD dual-fire precedence ambiguity as the sibling's (unstated which governs when L5 valid-fails AND L4<7 both hold) — to be frozen alongside the sibling's ⚖ P2 ruling, not separately. No hypothesis, precondition, or verdict changed; no Phase 1 execution. | Claude Code, cross-referencing the upstream bounded-round plan |
