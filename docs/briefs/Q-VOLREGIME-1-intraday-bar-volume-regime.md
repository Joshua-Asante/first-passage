...
- **Phase 1 — Adapt `Q-RANGEXFER-1`'s joint-surrogation null design to 1-bar lag (discharges the same class of D5-analogue unresolved-design gap for this construct).** **Operator GO received 2026-08-30; blocked upstream, not attempted at bar-level this session.** `Q-RANGEXFER-1`'s own Phase 1 (the day-level design this construct was meant to adapt) did not itself clear its own positive control this session — three candidate joint-surrogation constructions were tried and all failed, two in exactly opposite ways (own-series ACF fidelity vs. cross-correlation fidelity traded off against each other; see that brief's own §7 and the full write-up at [`lab/analysis/_inbox/joint_surrogation_null_2026-08-30/RESULTS.md`](../../lab/analysis/_inbox/joint_surrogation_null_2026-08-30/RESULTS.md)). Adapting an unresolved day-level design to 1-bar lag would compound rather than discharge the gap. This construct's own bar-level density (~135,000–140,000 bars vs ~1,300–1,500 days) and its own distinct-WHO three-way check remain to be addressed once a day-level design actually clears, not before.
