...
**Q-VOLREGIME-1:** Does an M15 bar's own volume, above its own time-of-day-conditioned trailing median, carry predictive information for the *next* bar's realized range being elevated (vs. its own time-of-day-conditioned trailing median), beyond what a null preserving the two series' shared same-bar volatility-regime structure would produce? **Run on MNQ and MYM, same design, same sign, comparable magnitude on the observed-series statistic — each instrument scored independently, not pooled into one statistic, and neither instrument's own within-stratum significance is yet null-calibrated (§4's precondition).**

Symptom-shaped (what is true about the data), not solution-shaped (what to build). This Pre-Q does not ask whether volume regime is the *same* mechanism as `daily-range-state-persistence` — that mechanism-attribution question is a named design requirement for Phase 1 (§7), not a separate falsifiable hypothesis with its own gate, since it is a question about *interpretation* of a result whose own statistical sufficiency is still pending (§4's precondition), not about whether an already-certified result survives correction.

---

## §4 — Falsifiable hypothesis (H-VOLREGIME-MNQ / H-VOLREGIME-MYM)

**Precondition common to both H below (added 2026-08-30 — see §1's second gap):** neither hypothesis may be scored RESOLVED/FALSIFIED/AMBIGUOUS-HOLD (§6) until that instrument's own within-stratum `circular_shift_null_p` (the function already built and reviewed in `c3_stratified_rerun.py` / `c2_c4_stratified_rerun.py`, per PR #207) has actually been computed against that instrument's own panel — not merely the observed-series `block_bootstrap_ci` either notice currently cites. This is a $0, cheap, vendor-bar-dependent precondition (Phase 0.5, §7), materially cheaper than Phase 1's own cross-series joint-surrogation design, and logically prior to it: there is no point running a joint-surrogation null on top of a within-stratum statistic that has not itself cleared its own Type-I bar.

**H-VOLREGIME-MNQ:** If the trigger bar's volume-regime incremental lift over matched own-range conditioning (observed-series figure: +20.6pp low-range stratum / +25.6pp high-range stratum, n=12,430/54,167 and 58,115/11,308 — **precondition above not yet cleared on MNQ; this is the number a within-stratum null must first be computed against**) survives, first, that within-stratum null and, second, a joint-surrogation null that preserves the volume/range same-bar cross-dependence (rather than deleting it via independent per-series surrogation), then bar-volume regime is a certified conditioner; otherwise the incremental lift is an artifact of the shared same-bar regime (or of the uncorrected bootstrap itself) and does not survive correction.
...
|---|---|---|
| `RESOLVED` | Presence limbs (L1–L4 analogues) pass AND attribution limb (L5 analogue) clears p_upper ≤ 0.05, for the relevant instrument's H. | `INTEGRATE — promote intraday-bar-volume-regime to a live conditioner-role class finding in MECHANISMS.md at CERTIFIED status for that instrument (conditioner-only; no entry construct authorized by this alone). Record the distinct-WHO design requirement's own outcome (§4) alongside the verdict, whichever way it lands — it is disclosed, not gating.` |
| `FALSIFIED` | Any presence limb fails outright (not merely AMBIGUOUS on L4), OR the attribution limb VOIDs after the full escalation ladder, for that instrument's own panel. | `STOP — re-proposal bar: a genuinely different joint-surrogation design (not a retuned tolerance on this one) or a longer panel, per whichever limb drove the FALSIFIED verdict, on that instrument. Does not falsify the same hypothesis on the other instrument.` |
| `AMBIGUOUS-HOLD` | Presence limbs pass but the by-year floor cannot resolve given panel length (N_valid < 7), on that instrument's own panel. | `ITERATE — return target: re-score when that instrument's panel extends to ≥7 full calendar years, or a fresh surrogate-class design is adopted for the by-year-independent limbs; no re-test date, panel-length-triggered, per instrument.` |
| `PRECONDITION-UNMET` (distinct from AMBIGUOUS-HOLD — added 2026-08-30) | The within-stratum `circular_shift_null_p` precondition (§4) cannot be computed: no vendor bars, no cached scored frame, on that instrument. **This is MYM's current disposition** (per `N-2026-08-29-mym-bar-volume-regime.md`'s own UNRESOLVED status) and, on re-review, may also turn out to be MNQ's. | `HOLD — not a verdict on H itself, a statement that H cannot yet be evaluated. Re-check trigger: vendor bars or a cached scored frame become available (a local-session, vendor-data-access task — see this Pre-Q's own delegation note, §7). Does not close the Q; does not license falling back to the observed-series CI as a substitute.` |
...
- **Phase 0.5 — Compute the within-stratum `circular_shift_null_p` precondition (§4) on BOTH instruments (added 2026-08-30).** NOT run this session — **this session's own environment has no `MNQ_M15.csv` or `MYM_M15.csv`** (vendor-licensed CSVs, gitignored per `CLAUDE.md`'s public-clone posture; confirmed absent by directory listing this session). This is a vendor-data-dependent task for a local session (Cursor / local Claude Code) that has those files checked out: on MNQ, run `circular_shift_null_p` (reusing PR #207's own construction from `c3_stratified_rerun.py`, ported the same way `candidate24_joint_gate.py` was ported to MYM) against `candidate3_volume_regime.py`'s own stratified frame; on MYM, re-run `c3_stratified_rerun.py` itself (it already contains the retrofit — it only needs the vendor bars or scored-frame cache PR #207 lacked). This is materially cheaper than Phase 1 and gates it: no joint-surrogation design should be built on top of a within-stratum statistic that has not itself cleared its own Type-I bar. $0; a fresh K declaration is not owed for re-running an already-declared, already-K-counted look with a corrected statistic (mirrors PR #207's own treatment — the retrofit did not re-charge K for MYM's own within-stratum null).
- **Phase 1 — Adapt `Q-RANGEXFER-1`'s joint-surrogation null design to 1-bar lag (discharges the same class of D5-analogue unresolved-design gap for this construct).** NOT run this session; requires operator GO before design work proceeds, same sequencing as `Q-RANGEXFER-1` (design → adversarial review → operator GO → execute). Candidate sketch: the same coupled/joint IAAFT-class surrogation `Q-RANGEXFER-1` §7 sketches (preserve each series' own linear ACF plus the lag-relevant same-window cross-correlation, randomize residual cross-dependence beyond it), re-parameterized for a 1-bar lag and bar-level (not day-level) panel density — genuinely adapted, not a blind copy, since the block/draw parameters that suit a ~1,300-1,500-day panel do not suit a ~135,000-140,000-bar one. Must also implement the distinct-WHO three-way check named in §4 as part of the same design, not a bolted-on afterthought.
- **Phase 2 — Adversarial review of the Phase 1 design.** Checks whether the 1-bar-lag adaptation actually controls for the shared same-bar regime confound (not merely inherits `Q-RANGEXFER-1`'s design by name without verifying it transfers), and whether the distinct-WHO three-way check is itself well-specified (e.g., does conditioning on daily-TR state at bar granularity introduce its own new confound?).
...
```bash
# Reproduce MNQ's own stage-1 result (deterministic, <2 min)
...
# Reproduce MYM's own stratified result (observed-series only; supersedes the naive marginal
# but is NOT the authoritative significance test — see §4's precondition)
...
# Expected VERDICT if run in an environment with vendor bars / cached scored frame:
# UNRESOLVED unless MYM_M15.csv or a cached scored frame is present (per PR #207's own gate)

# Confirm MNQ's own script has no null-calibrated function (the asymmetry this Pre-Q flags, §1/§5)
grep -n "def \|null_calibrated\|circular_shift" lab/analysis/_inbox/mnq_dailygeom_notice_2026-08-29/candidate3_volume_regime.py
# Expected: block_bootstrap_ci only — no circular_shift_null_p, confirming the precondition
# (§4) is equally unmet on MNQ, not merely on MYM
