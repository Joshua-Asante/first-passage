...
2. **Corrected on coordinator review (2026-09-24):** the R2 pending-only prefix is empty in execution order. Cancel before any open-price event, including a gap-triggered stop or queued market entry. The original build reading (open fills before cancellation) contradicted the ratified table and is withdrawn. Held-position and mixed position/pending legs retain the position-rule prefix.
