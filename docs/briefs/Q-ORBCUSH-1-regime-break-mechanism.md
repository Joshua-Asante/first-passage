...
- **Q-EVALSEQ-1** (`CLOSED-FALSIFIED` on its own headline question; `docs/briefs/closures/Q-EVALSEQ-1-closure-falsified.md`) — surviving finding: cushion-proportional sizing eliminated trailing-DD bust (20.18%→0.00%) on a 2-leg pyramided Striker DJ30/MYM + NAS100/MNQ book, on an EOD-clock-only simulator. Commissioned `Q-POLFRONT-1` as its successor, reframed around that book's own admissible-region widening. **`Q-POLFRONT-1` remains unopened.** This probe attempted to re-derive that finding intraday-honestly on the *same* 2-leg book first and found it `NOT-REACHABLE-AT-$0` — that book has no per-day intraday-excursion data and would need position-ladder reconstruction across two pyramided, cross-instrument legs, a real engineering task this probe correctly declined to fake. The probe pivoted to ORB-MNQ-1 instead, a *different* construct where the same lever could be tested honestly because ORB-MNQ-1 already has a validated intraday-honest engine. **This Q is not `Q-POLFRONT-1` under a new name — it is a sibling, narrower in scope, motivated by the same lever but targeting a construct where the test was actually reachable.** `Q-POLFRONT-1`'s own c1-book-scoped question stays exactly as commissioned, still blocked on the reconstruction problem, still needing its own operator GO if anyone wants to unblock it later.
