...
⚠ **But it sharpens §15h-3 for one leg.** **ORB-MNQ recon v7's first entry is `2022-09-01T15:00` — the
opening day of its own panel.** Any bar-derived quantity needing lookback (ATR, opening range, session
normals) is unavailable for its earliest trades. The Phase 3 freeze must either declare a warm-up window
*inside* the panel and start the evaluation after it, or state explicitly that the earliest ORB trades are
not bar-replayable and are scored on the fallback. Cheap to settle now; expensive to discover at Phase 5.
...
branch before merging so CI validates against current main. **The only freeze input still outstanding is
Codex's per-leg scaling-faithfulness read (§15b).**
