...
| 8 | ⚠ **NEW (§24): the 5.0% criterion has no valid decision rule** — the two-stage screen's fixed-sample Clopper–Pearson bound is not sequentially valid when stage-2 eligibility depends on the stage-1 outcome, so it is anti-conservative exactly for near-boundary configurations. Freeze one of: equal fixed total paths for all configs; the bound from independent stage-2 paths alone; or a sequentially valid procedure | **owed — freeze blocker** |
