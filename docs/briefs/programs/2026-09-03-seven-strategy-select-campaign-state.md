...
#      — reconciled (MATCH/BLOCKER at $0.01) where the walk is the true path, RECORDED + INFO where it is not,
...
5. §32 P1-C — keep the TradingView panel drawdown as a **separate recorded anchor**; key the DD limb's status on
   **measured** overlap (from the ledger's entry/exit timestamps): reconciled where the walk is the true path,
   recorded-with-INFO where it is not. Shares the overlap detector with item 2.
...
|---|---|---|
| **No overlapping trades** | the true intratrade equity path, given per-trade MAE (§30a's subsequence argument runs in the other direction here: with one position open, the walk *is* the path) | **Reconciled.** `walk == panel` at $0.01 is a genuine `MATCH`; a `MISMATCH` is a real defect signal — broken MAE ingestion, sign error, dropped rows — and stays a **BLOCKER** |
...
a **non-overlapping** leg's broken MAE path sail through: on Aegis, where the walk *should* equal TradingView to the
cent, a mismatch is exactly the defect G1.4 exists to catch. The overlap key gives each leg the strongest check its
