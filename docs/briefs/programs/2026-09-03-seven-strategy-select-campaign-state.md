...
6b. HISTORY SELF-AUDIT — CONTENT as well as paths, BEFORE EVERY PUSH (not only before opening the PR). A path check is necessary, not sufficient: an override value or an export row pasted into an allowed tracked file (phase1_config.json, a test, VERIFICATION.md) and removed in a later commit passes every path check and every base-to-head diff, yet stays fetchable from the PR's refs — nothing is rewritten here and a closed PR's commits remain fetchable — so the control is a byte-exact scan of every commit's FULL PATCH, run before the bytes leave this machine. (i) Build a needle file OUTSIDE the repo from the private bytes themselves: every key/value line of each lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json (whole lines, never bare scalars — a bare `20` matches everything) and every data row of each of the five frozen export CSVs (skip each header line: the repo's synthetic fixtures share it), keeping only needles of at least 12 characters — `NEEDLES="${TMPDIR:-/tmp}/needles.txt"`, outside the working tree, deleted after the scan. (ii) Content scan: `git log -p --no-color origin/main..HEAD | grep -F -c -f "$NEEDLES"` must print 0 — report the count only, never a match. (iii) Path scan, unchanged: `git rev-list origin/main..HEAD | while read c; do git ls-tree -r --name-only "$c" | grep -q '^lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/' && echo "PRIVATE FILE IN $c"; done` prints nothing, and `git log --diff-filter=A --name-only origin/main..HEAD -- lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/ '*.csv' '*.pine'` is empty. (iv) Run (ii) and (iii) before the FIRST push and before EVERY later push. A hit BEFORE a push is repaired locally — the offending commits are unpushed, so `git reset --soft <the last pushed tip, or origin/main before the first push>`, re-stage without the bytes, recommit, re-scan until 0. A hit AFTER a push STOPS the packet: no further push and no replacement PR from the worker — replacement purges nothing, the commits stay fetchable from the closed PR's refs — report the count and the commit to the orchestrator with no value quoted; the operator rules on closure, replacement and the ticket question (campaign-state §26 precedent). The PR body reports the needle count for every push (each must be 0).

7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: per source the two counts from step 0 (inputs captured, inputs differing from the body's defaults — counts only, never names or values), the binding result (fresh export SHA-256 equal to the pinned export_sha256: yes, per source) and the digest, the per-leg statuses from step 6, the new config hash, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0; live-account identifiers 0; live-account dollar figures 0; and the step 6b content-scan needle count, one per push, each 0.

NEVER: PUBLISH an override value — not in a commit, a PR body or comment, a tracked file, this artifact, or any surface that leaves this machine (reading the operator's screenshots and the pinned Pine bodies locally IS this job); force-add anything under inputs/; widen a tolerance; edit Pine, core/, STATE.md, docs/SESSIONS.md, ADRs or any campaign artifact; commit vendor bytes or copy them into any tracked surface (reading the ten frozen files locally is the job); merge.
```
...
| P7 | Publication by role — zero override values, zero live-account identifiers or figures — in tracked files, commit messages and the PR body; nothing under the study's private directory in ANY commit of the PR range, not only the final tree; and no private CONTENT in any commit's patch — a value pasted into an allowed file and removed later is the same disclosure (a file or a line added then deleted stays fetchable from PR history and cannot be purged under the no-rewrite rule) | full-diff role audit; `git ls-files lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` prints nothing; every commit in `origin/main..<head>` walked with `git ls-tree -r --name-only` for the private path, and `git log --diff-filter=A -- <private path> '*.csv' '*.pine'` over the range empty; then EVERY commit's patch read, not the base-to-head diff alone — `git log -p --no-color origin/main..<head>` — under a provenance rule on every added literal: a number, string or hash must trace to the §47b step 2 table, the SHA-256 of a named artifact, the base commit SHA, or an existing line of code or prose; an added literal with no source is FALSIFIED whether it is a private byte or an invented figure (the orchestrator cannot see the private bytes, so its content read is by provenance; the worker's byte-exact needle scan is the other half — the PR body must report a needle count of 0 for every push, and a body without it fails P7). Any hit is a disclosure: no replacement PR and no further push — replacement purges nothing, the commits stay fetchable whatever happens to the PR — and the operator rules (§26 precedent) (⚠ a repo-root `inputs/` pathspec inspects nothing relevant — four tracked files live under other `lab/**/inputs/` paths) |
...
§47b as amended eight times is canonical.
