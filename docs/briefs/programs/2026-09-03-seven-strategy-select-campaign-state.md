...
| D17 | **G1.4 names two dimensions the source cannot supply — amend the frozen gate, or leave it capped** | The G1.4 row in §4, frozen before any worker output was read, requires each reconciliation to cover *"trade count, net P&L, win rate, profit factor, drawdown, **commissions, monthly totals**"*. Two of those are now established as unobtainable from TradingView: **monthly net does not exist in the product** (operator-confirmed), and **total commissions** has no independent figure — the export's commission column is TradingView's own `rate × qty × 2`, which is precisely what the runner computes, so summing it reproduces the runner's arithmetic rather than checking it (§12e). ⚠ **This is a decision only the operator can take, and that is the point.** The orchestrator discovered the impossibility *after* the reports were inspected, which is exactly when a precommitted gate must not be quietly relaxed — doing so would let later phases proceed without the independent reconciliation G1.4 exists to demand (Codex on #291, P1, accepted; the orchestrator had written the weakening and it is reverted) | Three options. **(a)** Ratify an explicit amendment striking `monthly totals` and `commissions` from the frozen G1.4 row, dated and reasoned, with the verdict cap lifting only on those two limbs — cheapest, and honest so long as the amendment is recorded as an amendment rather than a finding. **(b)** Leave the row frozen as written and accept that **Phase 1 can never reach `COMPLETE`**; the campaign then proceeds under a permanent `NEEDS_CONTEXT` on those limbs, which is defensible but must be stated once rather than rediscovered each round. **(c)** Reconstruct monthly totals independently from the row-level ledger Codex already holds — that *would* satisfy the gate as frozen, since the requirement is a reconciliation, not a vendor screenshot, and it is the only option that closes the limb rather than excusing it. **Orchestrator recommends (c) for monthly totals** (the ledger has per-trade timestamps and P&L, so the monthly aggregation is a few lines and is genuinely independent of the TV panel) **and (a) for commissions**, where no independent figure exists at any price |
