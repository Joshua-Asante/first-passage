...
#   3. DJ30 +$287.00 -- CLOSED NON-BLOCKING 2026-09-04 (D30), attribution UNESTABLISHED: the 200K chart
#      states are unrecoverable, so the confound below is PERMANENT. Mechanism stands (SS19): one trade
#      (#170), the capital-anchored ddHit DAILY branch, not the day soft-stop. Historical text follows.
#      Was: CONDITIONALLY dispositioned. ⚠ NOT closed as capital-only: trade #127
...
# Until 1, 2, 3, 6, 7 AND 8 clear: phase1_verdict_cap stays NEEDS_CONTEXT, strategies stay BLOCKED_EXPLORATORY, and
...
| 2 | Fresh independent **TV Key-stats panels** for all five sources (G1.4) | ✅ **ROUTE RULED (D29)** — close by **policy ruling on the residual**, not by matching. Route B still built, acceptance becomes a **bracket** (§25b). Work owed |
...
is now RULED-with-work-owed except prerequisite 1** (Codex's scaling read), which still has no ruling because it
needs no decision — it needs the read.
...
configuration — so a saving remains. But the hard case costs **630** paths per survivor, not 340: my
...
for a frozen certification power, not at the expected bust count.** Under §25a's replacement table that is
~630 paths at a true 3% rate for 80% power.
...
2. **`n₃`, sized for a frozen certification power** — never at the expected bust count (§27a). At 80% power and
   a true 3% rate that is ~630 paths; the power itself is a number the freeze must state.
...
acceptance rule — **exactly what #301 already implements**: the excursion-bounded figure is compared to the panel
at $0.01; any mismatch is a `TV_SUMMARY_MISMATCH` **BLOCKER** carrying the basis note; a BLOCKER is adjudicated
by an **explicit operator `d17_policy` entry** naming the leg and the residual — never auto-accepted, never
tolerance-widened. The three figures (closed-trade, excursion-bounded, panel) are reported side by side with
