...
|---|---|---|---|
| **Orchestrator** | Astra coordinator (operator direction 2026-09-12; §57 supersedes D-B2 ownership) | decompose / freeze gates before outputs are read / review claims against artifacts / integrate / adjudicate; sole writer of this file, `STATE.md`, `docs/SESSIONS.md` for this campaign; owns specification, implementation and evidence assessment within each authorized packet | merge to `main` (operator retained this decision on 2026-09-05); arm or place trades |
...
an Astra evidence review: claims vs artifacts, re-run what is cheap, `VERIFIED / VERIFIED WITH
CAVEATS / REFUTED`.
...
**Consequence for the freeze.** Freeze prerequisites **8** and **9** (`n₂` / `n₃` sizing) are computed with this calculator, and (iii) is a silent wrong answer at alpha near 1, which is LOW confidence because confidence is `1 - alpha`; it does not demonstrate failure of the usual alpha=0.05 sizing pins. The other two defects remain real. The quantity returned is an admissible failure count, not the probability bound itself. **The calculator must not be used for the `n₂`/`n₃` freeze until the three items are fixed.** A2 can no longer be a fix round on #305 — the PR is merged and a merged PR cannot carry new work — so the remedy is a **fresh packet A3 off current `main`**, same frozen scope, or a CC-solo fix; the ordering is the operator's call and the pointer is prepared. This supersedes the "awaits the operator's dispatch; #305 waits on it" line above.
...
6b. PUBLICATION REVIEW — the private computation checkout is not pushed and its commits, if any, are not parents of a publication branch. Return a local-only review bundle containing the final intended patch, the exact file list, and the step 7 metadata. The publication branch starts in a clean worktree at the EXACT main commit recorded in step 2b before computation, and receives only the reviewed final patch; never merge, push, or transplant the private working history. Fetch main to check integration, but do not replace this validated base when main advances. Confirm the computation base remains an ancestor of current main; conflicting upstream changes require an explicit integration decision, not a silent rerun or pin rewrite. The allowed files are exactly those named above. Before any commit, the local orchestrator checks each changed field: config changes are the five digests only; anchors equal the step 2 table; manifest and RESULTS fields are the existing approved aggregate schema, statuses and digests; tests use synthetic data or approved pins; README and VERIFICATION contain only the prescribed provenance and reproducible verification metadata. New fields or statistics need explicit review before transfer. Chart titles/values, raw rows, screenshots, private strategy source, account identifiers/balances and machine paths have no publication slot. Keep the review bundle and logs local until this read is complete. Inspect the final staged patch, path list, commit identity/message and proposed PR body before the first push. Each subsequent commit receives the same read. This is a finite publication review, not a claim that string matching proves absence of all private information. Any ambiguous field stays local and blocks publication, without blocking other local research. A discovered post-push disclosure stops publication and is reported privately by location, never by repeating the content.

7. LOCAL RETURN, then reviewed PR BODY. First line: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Include per-source input/difference counts (no titles or values), export-binding yes/no and snapshot digest, per-leg statuses, new config hash, eight ignored-artifact digests in configuration order, pins changed, test commands/counts/runtime versions and the base SHA. Record that step 6b's final patch and metadata review passed and identify the reviewed commit. This metadata is proposed locally before publication, not posted as raw working output. No scanner output is required or accepted as a substitute for this review.
...
### 47c — Gate read (local review before publication; confirm on returned PR)
...
| P7 | Publication patch and metadata reviewed locally under step 6b; only approved fields in the named footprint; private computation history is not reachable from the publication branch; no unreviewed commit or PR-body addition | Compare the publication branch to its main base, verify its parents, inspect every staged/follow-up patch and the proposed body. Private files remain local. Scanner CLEAN is neither required nor sufficient. |
...
### 47d — Scanner split and review scope (operator instruction, 2026-09-05)

The scanner and its tests are preserved separately on
`codex/private-content-scanner-deferred`, from #302 head
`a6d281784db3006e8f3d8019bf2b9055eefdbcfe`. Its draft is not a campaign dependency.
The 22 review rounds and four panel runs are retained in
[that immutable revision](https://github.com/Joshua-Asante/first-passage/blob/a6d281784db3006e8f3d8019bf2b9055eefdbcfe/docs/briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md),
rather than replayed as current dispatch instructions. The operator had already
stood the panel down; no new scanner panel or repair loop is authorized here.
The former serialization-independent no-disclosure claim is withdrawn. Local
computation and public release are separate actions under step 6b.

### 47e — Deferred scanner findings

Historical findings and their dispositions live at the revision linked in §47d.
Passing the scanner's regression fixtures never established a universal disclosure
guarantee. Any future scanner work needs a finite supported contract; it cannot
delay this campaign's intake, engine work or plan review.

## §48 D26 private-capture checkpoint — local Codex, 2026-09-05 (02:30Z): **2 complete / 3 partial**, gate-read

Status report relayed by the operator; **no code action was requested at that checkpoint and none was taken.** Restored from `68fcbf4` after a merge dropped this section. Gaps are recorded by **position and count only** under §47b's publication boundary; private input titles and values stay local. These are reported capture statuses, not independently verified export bindings.
...
The operator approved design choices C1-C5 in the private account-feedback and
composition contract. The revised state/clock mapping has completed its focused
correctness reviews; final cursor and session-identity clarifications are
consolidated in the private specification. These are design approvals, not an
implementation or execution release. Required source, calendar, account and
coverage evidence remains open; contract disposition remains **HOLD**.
...
C1-C5 approval and this documentation commit do not release implementation,
...
Joshua directed on 2026-09-12 that Claude have no further role in bringing the Tradeify portfolio into live deployment. Astra owns active coordination, specifications, implementation, planning, review integration and evidence assessment across Track A and Track B. This campaign-specific direction supersedes D-B2 and earlier Claude-only surface assignments; it does not change routing for unrelated campaigns. Historical branch names and authorship remain intact; Claude/Fable and `fable-judge` are not active dependencies. Joshua retains risk/trading ratifications, merges and operational GOs.
...
## §58 — Campaign ownership: the current coordinating task (2026-09-20)

Operator direction 2026-09-20, given in-session and recorded here as the campaign's authority (this section supersedes §57's ownership assignment; §57's closeout routing for the TB-* packets is unchanged): **the current coordinating task — the Claude coordinator session for Protected Full E1 (the session that authored the [S2 acceptance continuation](https://github.com/Joshua-Asante/first-passage/blob/12d6a6f/docs/briefs/handoffs/2026-09-20-glm-s2-acceptance-continuation.md) and [T01](../handoffs/2026-09-20-glm-t01-s2-closeout-g3-worker-launch.md)) — is the campaign owner** for the Tradeify portfolio: roadmap, interfaces, serialized writers, review acceptance and combined acceptance of returns. Authorship of a record under another model identity is not evidence of reassignment; only this section and its successors are.

**Handoff obligation.** Ownership passes only by a recorded handoff entry in the [execution-slices ledger](../../superpowers/plans/2026-09-18-full-e1-execution-slices.md#progress-ledger-and-present-disposition) that names the successor task and the accepted state it inherits. A coordinating session that ends without that entry leaves the role with the operator, not with the last executor. Executors (currently GLM under the acceptance continuation) never inherit the role by default.

**Routing (Rule 7).** The [umbrella](../handoffs/2026-09-10-track-b-qualify-accepted-book-umbrella.md#ownership-routing--operator-ruling-2026-09-20-rule-7-one-owner-per-fact-not-one-document-for-everything) owns scope, authority and governing gates; the [deployment checklist](../../superpowers/plans/2026-09-20-tradeify-deployment-checklist.md) (adopted 2026-09-20 via [first-passage#440](https://github.com/Joshua-Asante/first-passage/pull/440)) owns the current cross-workstream sequence; the execution-slices plan owns qualification engineering requirements and the acceptance ledger; bounded handoffs own assignments and returns. [STATE](../../../STATE.md) points here.

**Standing S2 disposition at this ruling.** S1 accepted; S2 INCOMPLETE / NOT ACCEPTED with the enforcement closeout under [first-passage#436](https://github.com/Joshua-Asante/first-passage/pull/436): the existing G3/G4 assignment is finished under T01's G3 merge gate and acceptance follows verified integrated Linux evidence, written by the coordinator. S3 execution is gated on accepted S2 and verified worker-launch readiness. Joshua retains ratifications, merges and operational GOs. **No dedupe, incident-exception, ORB, feed-funding, production-qualification, deployment or arming GO is granted by this section.**

## §59 — Route-native expressions for the accepted book: three operator rulings (2026-09-25)

Operator direction 2026-09-25, given in-session against the T08 return and the [bounded-exposure scope](../../superpowers/specs/2026-09-24-bounded-exposure-unknown-request-amendment-scope.md) Q3 outcome (incident ADR [addendum §A4](../../adr/2026-09-17-bounded-platform-protection-incident-contract.md#a4--fit-with-the-book-scope-q3)). Recorded here because D-B4 (a) (§55) pins the accepted book's expressions and this section amends how those expressions are realized on the route. It is the owner of the three rulings; T08 §7.8, the scope note, addendum §A4, the vendor-question note, the venue-edition ledger and STATE carry dated pointers.

**Reads (Rule 0, `main@1f564ab`):** T08 §7.3–§7.8 (`docs/briefs/handoffs/2026-09-21-tradeify-t08-broker-protection-feasibility.md`); scope note §2 Q1–Q4 and the Q3 outcome; addendum §A1–§A4 (per-primitive and per-leg tables); rail extension spec S2/S3 and R-B3 L-2 items (a)–(g) (`docs/spec/2026-09-12-c1-multi-leg-rail-extension-spec.md:56-58, :132`); D-B4 (a) in §55; the venue-edition ledger (`ops/venue_editions/Tradeify_Select_100K.md`). The Vanguard and Aegis ports are private and were **not** read by the recording session; ruling 1 is the operator's attestation and is labelled so wherever it is restated.

### Ruling 1 — Vanguard MGC and Aegis 6J fit the narrowed shape (operator-attested)

**Current qualification (2026-09-26):** the [Vanguard source determination](../handoffs/2026-09-26-vanguard-mgc-trailing-determination.md#5-executor-return) returned TRAILING ACTIVE. It was accepted for the runtime binding at Gate A on 2026-09-26 ([REST §6.11](../handoffs/2026-09-25-crosstrade-rest-route-assessment.md#611-gate-a-factual-disposition) row A6), with a mandatory successor-binding check at freeze. The historical Vanguard attestation below is superseded for trailing and is not current evidence of narrowed-shape fit. This finding requires an operator edition-versus-rejection decision. Aegis is unaffected by this Vanguard finding.
...
The operator reports sending the [authorized question](../../notes/2026-09-24-t08-vendor-question-draft.md) to CrossTrade support on 2026-09-25 from his own account. Under T08 §7.8 part 2 only a written, retained and hashed reply counts; nothing is recorded as an answer yet. Option A (vendor-bounded release of a held reservation) remains available exactly as the scope note describes it and depends on that reply. The one permitted follow-up is unspent; the recording session's recommendation for it, if the reply is "no bound", is to ask whether durable recovery and OCO repair can be disabled or scoped for the account or key — a fence by removal rather than by time. That is a recommendation, not an instruction; the operator decides whether and how to spend the follow-up.

### Ruling 3 — adopt route-native expressions for ORB MNQ and Striker MYM ("go with the easier fix")

**Decision.** The accepted book stays the same four legs, allocations, protection policy (1% trigger / 40% scale / full ORB base / adds off under protection), capacity rule and takeover ordering. What changes is the **order expression** of the two public legs so that every exposure-creating request is in addendum §A1's narrowed shape and no leg depends on a route primitive documented unsupported (L2(e), L2(f), L2(g)):

| Leg | Declared expression (fails on this route) | Route-native expression (adopted for requalification) |
|---|---|---|
