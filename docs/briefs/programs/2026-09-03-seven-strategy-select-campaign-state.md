...
7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: per source the two counts from step 0 (inputs captured, inputs differing from the body's defaults — counts only, never names or values) and the digest, the per-leg statuses from step 6, the new config hash, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0; live-account identifiers 0; live-account dollar figures 0.
...
|---|---|---|
| P1 | Five `pine_input_overrides_sha256` present, 64 lowercase hex, five distinct; the runner's intake verified each (manifest `source_identity` carries the digest; exit 0) | config, manifest |
