...
| P2 | Anchors equal §18a on all five — count exact; net, win rate, PF, panel DD within the loader's tolerances; `coverage_status: COMPLETE`; `d17_policy.max_drawdown: OVERLAP_KEYED`; `OVERLAP_KEYED_D32` on DD rows, per-leg detail JSON and the report | anchors file, manifest, RESULTS |
...
| P5 | Every pin naming the old config hash moved in the same commit (two tests, `VERIFICATION.md`, `RESULTS.md`, manifest); the twenty source hashes unmoved; generation v4 | `grep` the old hash — only this file's historical mentions may remain |
| P6 | README's "D26 migration is pending" paragraph and the v3 generation string updated; `coverage_note` no longer says "pending operator discrimination" | README, anchors |
| P7 | Publication by role — zero override values, zero live-account identifiers or figures — in tracked files, commit messages and the PR body; nothing under `inputs/` tracked | full-diff role audit; `git ls-files inputs/` empty |
