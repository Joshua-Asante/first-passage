...
**Round 18 on `7533048` (02:09Z 09-05): 2 P1 + 2 P2, both P1s reproduced before fixing, all folded.** Both P1s are defects in the scanner's own merge and needle handling — the classes rounds 15–17 opened, found again one layer down.

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The per-commit content read matched patch-header paths against the raw NUL-delimited resolved set, but git C-quotes a header path containing a tab, newline, quote or backslash *regardless of* `core.quotePath` — so `b/odd\tname.txt` never equals the raw name and every added line of that file was discarded at the merge filter | reproduced: a merge resolution adding a VALUE needle to a tab-named file → `SCAN result=CLEAN` | no header is parsed at all any more: each path's diff is taken separately with a `:(literal)` pathspec, so the path is the one from the NUL listing by construction and cannot be re-encoded. The repro now reports `HIT class=VALUE … line=1`; regression test added |
| **P1-B** | A JSON number was round-tripped through `float` before becoming a needle, so a snapshot value written `1.2500` produced only the needle `1.25`; an added line carrying the value as the chart spells it fails the word boundary on the following digit and matches nothing | reproduced: snapshot `1.2500`, added line `1.2500` → `SCAN result=CLEAN` | numbers are parsed with a `str` subclass that keeps the source lexeme, and each contributes BOTH spellings — the lexeme exactly as written and the canonical JSON form. The repro now reports `HIT class=VALUE`; regression test added |
| P2-A | Round 17's fix promoted a whole PATH: when `main` and the branch edit *separate hunks* of one file, the cleanly merged file differs from both parents, so every line in it — including lines only `main` introduced — was reported as merge-authored | reproduced: a harmless feature append merged with a main-side `Aggressive` line → `HITS` for a line already on `main` | the path set still selects which files a merge may have authored, but within them only combined-diff lines that are added against **every** parent (all columns `+`) are scanned. Round 17's binary/path evidence stays unioned over the same path set. Two tests: a clean two-sided edit of one file is CLEAN; a conflict resolution carrying a needle is still caught |
| P2-B | The `04f` stub's live residue still told the operator to fire A2 and close #304 while the manifest in the same change recorded both as done — a pointer that invites a duplicate dispatch | own stub read against the manifest | residue refreshed to the manifest's state, and it is refreshed again in this commit for the #305 merge |
...
## §48 D26 private-capture checkpoint — local Codex, 2026-09-05 (02:30Z): **2 complete / 3 partial**, gate-read

Status report relayed by the operator; **no code action was requested and none was taken.** Gate-read here rather than accepted. ⚠ **How this section is written:** an input's *title* is a needle class in its own right (§47b step 6b TITLE — an input name never legitimately appears in the published footprint), so the gaps below are recorded by **position and count only**. The checkpoint located them by naming the controls that bracket each gap; those names are deliberately **not** carried into this tracked file. Counts, screenshot counts, coverage and digests are the publishable half, and the checkpoint reported nothing else — the discipline held on the worker's side.

### 48a — What was reported

| # | Source | Controls visible | Screenshots | Gap | Evidence digest (SHA-256) |
|---|---|---:|---:|---|---|
| 1 | `striker_dj30_mym_pyramid_250` | 31 | 4 | — **complete** | `472f97f100191b95ef4e2d66e4bd5c93750b6530c8d5df9e46ece893d2c47950` |
| 2 | `striker_nas100_mnq_dow_wed_excluded` | 36 | 5 | — **complete** | `c99708164c5e5447eaec77c5cbe46aaa1a8187e9cb5592c0c29e624ee68d277a` |
| 3 | `orb_mnq_recon_v7` | 36 | 5 | 9 declarations missing in one interior scroll region | `64fa48491da07dcdd97a3bc935cf7b9a0f88f181b65f9e9776fc0d193874f7d8` |
| 4 | `aegis_6j1` | 34 | 4 | 6 declarations missing in one interior scroll region, **plus 4 dropdowns visibly truncated** so their values are not established | `c7848f0242c49887d6c6f84c86462ead352c62575948e257359765fd26ef0482` |
| 5 | `vanguard_mgc_v04` | 40 | 5 | 2 declarations missing in one interior region **and at least 19 below the last captured control**; 21 confirmed missing | `e607ca65288965e074b82bc1828d5fc34eefdea07018ea9c6db7299af01e4be6` |

Also reported: the five artifacts and their screenshots are gitignored and show as `!!`, never `??`; `phase1_config.json` is unchanged and no D26 digest is populated; no Phase 1 regeneration or re-freeze has started; no override value has entered a tracked file, commit, PR comment or report; long calendar fields and truncated dropdowns were left image-bound rather than guessed.

### 48b — Gate read: five findings, none of them a defect in the capture work

| # | Finding | Basis |
|---|---|---|
| **1 — complete ≠ pinnable** | "Complete" here means every visible control was transcribed. §47b step 0(c) additionally requires a **binding check before anything is hashed**: SHA-256 of a fresh export taken *from the snapshot state* must equal that source's `export_sha256` in `phase1_config.json`. The checkpoint reports that "source identities and evidence-file hashes match the current config", which is a different statement. **No digest may enter `phase1_config.json` until its source's re-export is confirmed against its pin — the two complete captures included.** | §47b step 0(c) read against the checkpoint's wording |
| **2 — step 0(d) unreported, not passed** | Both complete captures are Strikers, and §47b step 0(d) requires `backtestMode` to be in the differing set **for both** — necessary, not sufficient. The checkpoint does not state it either way. | §19d; §47b step 0(d) |
| **3 — the three partial digests are not pins** | Completing a capture rewrites its artifact and therefore changes its digest. §47b step 0 requires the snapshot be written **once** and never edited, precisely because an edit forces a second re-freeze. The three partial digests are recorded above as **evidence of progress only** and are barred from `phase1_config.json`; the two complete ones are candidates subject to finding 1. | §47b step 0 |
| **4 — Aegis has a second, different obstacle** | Four truncated dropdowns lack established values. A **full** snapshot needs every input's value, and more scrolling cannot supply these — the remedy is a re-capture with each dropdown expanded, or, failing that, the §47a lost-chart fallback for that source. Leaving them image-bound was the correct call; guessing would have been the defect. | §47b step 0(e); §47a fallback |
| **5 — Vanguard's denominator is unknown** | "At least 19 declarations remain below the last captured control through the true bottom of the Inputs tab" means the source's total input count is not yet established, so its coverage cannot be stated as a fraction and "21 confirmed missing" is a floor, not a total. | checkpoint text |

**Baseline note (not a finding).** The checkpoint's baseline names `codex/tradeify-d26-d27` @ `512f7a3`. That work is **merged**: #301 landed as `5ab079c` (§46) and `main` has since moved twice more — `6b5bc96` (#303) and `2a89348` (#305). The capture work is branch-independent, but the next session should re-baseline on `main`. Its `--tier check` exit 0 claim **reproduces here on the merged tree**. Its per-interpreter suite totals are **not verifiable in this environment**: two test modules import `scipy` and `statsmodels`, neither of which is installed in this session, so collection stops before any count can be compared — the totals are recorded as reported, not as confirmed.

### 48c — Disposition (Codex's requested disposition, accepted with finding 1 attached)

* Recorded as **2 complete / 3 partial**.
* **D26 population and the shared Phase 1 regeneration stay BLOCKED** until the missing ORB, Aegis and Vanguard regions are captured — and, per finding 1, until each source's re-export binding check passes.
* When all five are complete **and bound**, the five digests and the downstream config pins land as **one input-side change**, followed by **exactly one** Phase 1 regeneration / re-freeze.
* No code action at this checkpoint.

### 48d — What the freeze is now waiting on

| Prerequisite | State |
|---|---|
| **2** (§18a panels populated as anchors under D32) · **6** (D26 digest binding populated) | **BLOCKED** on this checkpoint — 3 of 5 captures partial, 5 of 5 unbound |
| **1** (Codex's per-leg scaling read) | owed; its own dispatch |
| **7** (live-state MC engine change under `core/`) | owed; CC solo |
| **8** · **9** (`n₂` / `n₃` sizing) | **newly BLOCKED** — the certification-power calculator merged with all three A2 defects live on `main` (§45); at high confidence its certifying-bust bound saturates to `n`, which is a silent wrong answer in exactly the region these sizings use |
