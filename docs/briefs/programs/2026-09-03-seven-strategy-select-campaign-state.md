...
⚠ **Progress at `3323fc9` (§35): items 1, 2 and 5 DONE and verified; item 3 done to its first wording, the §34 refinement owed; item 4 NOT STARTED — the push predates the prompt that introduced it (relay lag).**
...
5. §32 P1-C — keep the TradingView panel drawdown as a **separate recorded anchor**; key the DD limb's status on
   **measured** overlap (from the ledger's entry/exit timestamps): a one-sided `walk ≤ panel` lower-bound check where no trades overlap (BLOCKER only if the walk EXCEEDS the panel;
   equality is INFO, never MATCH), recorded-with-INFO where they do. Shares the overlap detector with item 2.
...
not one it *equals*. The worker's label qualification (§30c item 3) should say so.
...
## §35 Gate read — #301 @ `3323fc9` (2026-09-04): three of five items land and verify; item 4 untouched by relay lag
