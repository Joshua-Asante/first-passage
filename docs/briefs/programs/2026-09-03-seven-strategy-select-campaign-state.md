...
#   2. DONE 2026-09-04 (SS18): TV Key-stats panels supplied for all five. Four reconcile TO THE CENT.
#   3. DJ30 +$287.00 reconciled or explicitly dispositioned. NOW PANEL-VS-PANEL, so NOT a capture artifact,
#      and the entire delta is on the LOSS side (gross profit flat, gross loss -$258..-393). Dispatch WITH 1:
#      an uncharacterised initial_capital path IS a scaling-faithfulness defect, and D15's resolution
#      ("sizing is static-100K; the soft-stop is the capital-dependent part") cannot be wholly true.
# Until 1 and 3 clear: phase1_verdict_cap stays NEEDS_CONTEXT, strategies stay BLOCKED_EXPLORATORY, and there is
# NO ELIGIBLE POPULATION to freeze a grammar over. Do not draft the freeze before then.
#
...
#
# Also open: #296 is `behind` main — operator updates the branch, then merges. R-STRIKER-EC is a Phase 8 gate
...
| 2 | Fresh independent **TV Key-stats panels** for all five sources (G1.4) | ✅ **DISCHARGED 2026-09-04 (§18)** — four of five reconcile to the cent; the venue-bound trio reproduces §13a's −1.86% from a separate artifact |
| 3 | **DJ30 +$287.00** reconciled or explicitly dispositioned | ✅ **DISPOSITIONED 2026-09-04 (§19)** — one trade (#170, 2025-12-02), exit one bar earlier, exactly +$287.00; mechanism is the capital-anchored `ddHit` daily branch, NOT the soft-stop. G1.4's DJ30 row may anchor on **$32,057.36** |
| 6 | ⚠ **NEW (§19d): the Striker exports are not reproducible from their pinned Pine** — `backtestMode` was overridden on the chart, unrecorded, and the file as written makes `ddHit` dead code. Scope beyond the two Strikers is **unmeasured**. **D26** | owed — **freeze blocker** |
| 4 | Four bar-panel hashes | ✅ accepted (this section) |
| 5 | §14e generation on `main` | ✅ merged (§16g) |

Until 1 and 6 clear, `phase1_verdict_cap` stays `NEEDS_CONTEXT`, every strategy stays `BLOCKED_EXPLORATORY`, and
...
| Field | Value | Note |
|---|---:|---|
| `Trailing Max Drawdown` | **$97,192.90** | displayed; *"One Rule: Do not go below your $97,192.90 trailing max drawdown"* |
| `Balance` | **$100,192.90** | +$193 on the day |
| **Peak (floor anchor)** | **$100,192.90** | **derived**: threshold + $3,000 (`max_dd_pct: 3.0` on a $100K tier, pure EOD fixed-$ trail, lock unreachable) |
| `Profit Target` | **$192.90 / $6,000.00** | $5,807.10 remaining |
| `Highest Profit Day` | **$205.60** | seeds `max_day_profit` |
...
⚠ **Peak = Balance, exactly.** $97,192.90 + $3,000 = $100,192.90, which equals the displayed balance to the cent.
...
|---|---:|---:|---|
| Equity | $100,192.90 | $100,000.00 | +$192.90 |
| Floor | $97,192.90 | $97,000.00 | — |
| **Headroom to floor** | **$3,000.00** | **$3,000.00** | **identical** — the trail carries the peak up with it |
| Remaining to target | $5,807.10 | $6,000.00 | **3.2% shorter** |
| `max_day_profit` seed | $205.60 | $0 | near-inert (§17c) |
| `trade_days` seed | **7** | 0 | **min-days gate already cleared** (§17d-1) |
...
`Consistency 100% / 40%` looks alarming and is not. Best day **$205.60** against total profit **$192.90** is 106.6%,
displayed capped at 100% — the ratio is large only because the denominator is tiny. The gate binds **at the pass
point**, where total profit is $6,000 and the 40% limit is therefore **$2,400**; $205.60 is **3.4%** of the eventual
total. And per [`ops/prop_envelope_default.md`](../../../ops/prop_envelope_default.md) the Select consistency rule is
an **eval-only SOFT at-pass gate** — *"cannot pass until best day ≤ 40%; big day delays, never breaches."* **It
cannot cause a bust**, so it touches median-days-to-pass and never the 5% criterion. `preflight` already threads
`consistency_frac`, so the engine models it; the snapshot only seeds the starting best day.
...
1. ~~**`trade_days`**~~ — **✅ RESOLVED 2026-09-04, same PRIMARY source.** The dashboard displays
   ...
   * **Seed:** `trade_days = 7`. In [`core/mc/simulation.py`](../../../core/mc/simulation.py) the pass test is
     ...
     pristine model must accrue 3 first. **Third favourable difference** — it can only shorten time-to-pass, never
     lengthen it, and like the consistency gate it is a *pass* condition, so it cannot touch the 5% bust criterion
     in either direction.
...
numbers. **G1.4's count, net, win-rate, profit-factor and max-DD limbs are now anchored on the
...
**The comparison itself is NOT confounded** — trade #127 proves the limiter was live in both runs, so the
override was constant across the pair and the +$287.00 is genuinely attributable to the capital change
alone. **This defect does not damage the disposition; it damages the freeze.**
...
**Freeze prerequisite 3 is DISPOSITIONED.** The delta has a named mechanism, an exact single-trade
attribution, a bracketed threshold, a satisfied max-DD constraint and an uncontaminated control.
**G1.4's DJ30 row may anchor on $32,057.36** — the 100K figure, which is the basis the campaign uses.

**But §19d is a new freeze blocker**, and it is cheap only if it is a metadata fix. **See D26.**
