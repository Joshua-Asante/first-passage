...
**Status:** `PHASE 0 SKIPPED (operator override) · PHASE 1 IN PROGRESS — Task 1/8 on origin (a51bc60), partial gate read · PLAN + THIS FILE ON MAIN (#272, #273 merged) · OLD VENDOR-BYTE REF DELETED, OBJECT PURGE PENDING (§6 D7)`
...
| G1.2 | **Folded Phase 0 inventory** per strategy: source hash, byte size, row count, first/last timestamp, instrument, direction, session, bar size, quantity convention, gross/net, exported commission/slippage settings, **export timezone resolved** (a `null` / `UNRESOLVED` timezone caps the Phase 1 verdict at `NEEDS_CONTEXT` — the plan's Phase 1 ledger is canonical UTC with exchange-local session dates, §6 D9), and whether synchronized intraday bars or timestamped intratrade paths exist — scalar MAE/MFE is inventory only. | `NEEDS_CONTEXT` |
...
| D8 | **Two instrument mismatches** in the supplied set | `striker_dj30_mnq_prototype` is declared MNQ but its export is a MYM chart; `striker_nas100_mym_prototype` is declared MYM but its export is an MNQ chart (spec §2, config `encoded_instrument`). The design rightly refuses to repair this, so both will sit `BLOCKED` (claim class `EXPLORATORY`; the spec's own per-strategy status word for this is `BLOCKED_EXPLORATORY`) | Re-export each prototype on its intended instrument (new hashes → config re-freeze) or re-declare the intent to match the export; Phase 2 cannot admit them otherwise |
| D9 | **TradingView chart timezone** of the seven exports — **blocks a Phase 1 `PASS`** | `source_timezone` is `null` for all seven, so `timestamp_utc` and exchange-local session dates stay null. The plan's Phase 1 deliverable is a canonical **UTC** ledger with exchange-local session dates, and Phase 2 audits session decomposition and venue legality on it; source-domain timestamps cannot carry that. One value — the chart timezone the exports were taken in — resolves it for all seven | State it and Codex re-freezes the config fingerprint. Until then the Phase 1 verdict is capped at `NEEDS_CONTEXT`; the only alternative is a formal plan amendment, not a deferral |
