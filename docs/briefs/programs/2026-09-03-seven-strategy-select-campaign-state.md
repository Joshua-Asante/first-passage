...
⚠ **D20 (2026-09-04) re-scopes rows 6–8 without editing them.** Under the operator's acceleration the deliverable is a deployed book with the live eval as its forward falsifier: Phases 6–7 become post-deployment monitoring (down-only), and Phase 8 runs **in parallel with** Phases 4–5 because it is the same work as M1 item 5. The rows above stay as the plan's record; **§15** is the live sequence.
...
```bash
git fetch origin && git branch -r | grep -i codex                         # Codex's fresh §14e branch off main ef8b7aa?
# 1. §14e remediation PR (Phase 1): gate-read against the six §14f conditions ONLY — five pins = §14a; calendar rows
#    set-equal to the 40 in-span venue_flat_dates (both directions); zero force-flat violations; manifest + RESULTS
#    regenerated; D17 implemented; tie fix under §14d's four conditions + at-cap 80 re-derived. Green light = all six.
# 2. D20 (§15): the moment §14e lands, draft the SHORT Phase 3 freeze (§15b item list) on this branch for operator
#    ratification. It cannot commit until §15d-a (Aegis / 6J bars) is answered; §15d-b/-c may follow the commit.
# 3. Hand the operator the Codex dispatch for Phase 4+5 (one dispatch, §15b) carrying the freeze hash.
# 4. In parallel, hand the operator the Phase 8 / M1-item-5 dispatch (c1-rail lane): dry-run replay of the leading
#    configuration through the daemon → listener at non-zero sizing; the event id is item 5's evidence.
# Still: never open vendor bytes; never merge a worker PR; never set dry_run=false — that is the operator's arm.
...
(`status: CODE_LANDED`, `operator_signoff: null`); `ops/c1_rail/c1_rail_arm.py` (fails closed without a
validator-passing `RESOLVED` artifact); `core/mc/simulation.py` (`run_seed` / `simulate_path`, `intraday_blocks`,
...
| "sizing works together" (3) | inactivity barrier **OFF** with the operator-placed weekly token trade assumed — the standing operational model (`CLAUDE.md`); the ON re-MC is degenerate and closed | `preflight.firm_kwargs(inactivity_off=True)` |
| "bust less than 5%" | `P(bust before pass)` with every path still open at `HORIZON_CAP` counted as a bust (contract item 5), on the **intraday clock**, at the live Part A ceiling **5.0%** (prereg v2), with the one-sided 95% Monte Carlo upper bound reported beside the point estimate. With item 5 in force this is **stricter** than prereg v2's pass floor: "< 5% bust" means ≥ 95% of paths pass within the horizon | `simulate_path` outcome tags; prereg v2 §3 |

**What the criterion does not contain, and D20 therefore waives:** the plan's contract item 3 (a qualifying
bound carrying source-sample, model and selection uncertainty), item 10–11 (multiplicity, `N_conf ≥ 59`), and
Phase 6's frozen challenge set. Under D20 the number that decides deployment is the **fitted-model point estimate
...
|---|---|
| 2 — Standalone audit | **Folded into what is already measured.** Venue legality: 545 → 0 force-flat violations (§13). Standalone geometry: ORB $6,794 / NAS100 $8,269 / DJ30 $4,569 TV drawdowns exceed the whole $3,000 barrier at exported size (§10), so those legs enter the search only at reduced integer sizes — a grammar fact, not an elimination |
| 3 — Freeze | **A short pre-registration, not the 14-item contract.** Frozen: the venue snapshot (item 1); the grammar `q ∈ {off, 1, …}` per leg with the cap, collision and tie rules (item 6); horizon cap and unresolved-as-bust (item 5); seeds, block family, integer-week joint-flat blocks (item 7); the halves split date; every Phase 4 cutoff as a number (item 9); the Rule 2 line (item 13); the five one-page template contracts (item 14, required by the [candidate-contract ADR](../../adr/2026-08-30-candidate-contract.md) for anything to enter the catalogue). **Declared not frozen, with the consequence named:** items 3, 10, 11, 12 — the book's terminal verdict is capped below `CONFIRMED` and its label is *model-fitted; unfalsified on the forward interval* for as long as it lives. Item 8: the live eval **is** the forward interval |
| 4 + 5 — Screen + joint MC | **One dispatch at the freeze commit.** Cap-feasible screen on the realized joint path and rolling starts; joint-flat MC on survivors on the intraday clock; full + H1 + H2; frontier kept; the deployable configuration is the first under the frozen lexicographic objective whose full, H1 and H2 point estimates and MC upper bound all clear 5.0% |
| 6 — Robustness | **Post-deployment monitoring, not a gate.** Runs after the book is chosen; its results can only demote through the lifecycle's down-only lane, never re-select |
...
| 8 — Shadow-operational | **Not skippable, and not the long pole it looks like** — see 15c-1. Runs in parallel with 4–5 on the leading configuration |

Rule 2: D3's partition stands — (i) Phase 1 at 3–4 of 8; (ii) Phases 2–3, now one freeze iteration; (iii) Phases
4–8 with Phase 8 carrying M1 item 5. No constituent self-extends.
...
**So "deploy at the Phase 3 commit" is not a date the repo can honour.** The earliest deployment is the day
item 5 closes — and the work that closes it is Phase 8's dry-run replay of the chosen book through the daemon →
listener path. Phase 8 is therefore on the critical path whatever the operator rules, and it costs nothing
extra: STATE queue `#2` (B7-REFIRE Stage 1 + M1) "does not wait on #1", and the test strategy for item 5 was
licensed 2026-08-24. The rail's own history is why this is not negotiable: the 2026-07-27 unintended second fill
...
recorded as of this session, with one business day left — operator call.
...
| # | Question | Options | Orchestrator recommendation |
|---|---|---|---|
| D20-a | **Aegis 6J1 in the deployable book?** | (i) drop it from the deployable set (research scoring continues); (ii) fresh `CME BAR EXPORT v0.2` of `6J1!` through 2026-09-02 and bar replay via the recovered-panel method, accepting the ~9-tick feed misalignment as disclosed; (iii) carry it on MAE and deploy on a `LOWER BOUND` label | **(ii) if the export can be supplied before the freeze commit, else (i).** Never (iii): a lower bound deployed as "< 5%" is the exact error `CLAUDE.md` bans. **Blocks the freeze** |
| D20-b | **Bar panels for the other three legs through 2026-09-02?** | (i) fresh exports of `MNQ1!`, `MYM1!`, `MGC1!` (three more captures, same route); (ii) bars where the frozen panels reach, timestamped MAE for the tail, whole result labelled `LOWER BOUND` | **(i)** — same session as D20-a, and it removes the label entirely |
| D20-c | **ORB-MNQ-1 recon v7 in the deployable book?** | (i) research-scored only, excluded from the deployable grammar from the start; (ii) admitted to the grammar; if it survives Phase 5, the orchestrator drafts the R2 superseding ADR on that result for the operator's fresh GO | **(ii)** — the search should measure it, and the ADR is cheap if the number earns it. Excluding it a priori would itself be a post-hoc choice |

D20-a blocks the Phase 3 freeze commit; D20-b and D20-c can follow it.
...
the clock it was measured on. If nothing clears 5.0% on full + H1 + H2, the outcome is **no qualifying
