...
#
# SIX FREEZE PREREQUISITES OPEN (1, 2, 6, 7, 8, 9) — every one RULED with work owed except 1, which needs a read,
# not a ruling. (⚠ Rewritten 2026-09-04 per §31 / Codex round-8 P1: this block still waited on 3 and omitted 8 and 9.)
...
#   2. RULED D29 (SS25b) — close by POLICY on the residual. ⚠ D32 OPEN (SS30b): the bracket MECHANISM is
#      falsified in its lower limb; recommended rule = equality-as-BLOCKER with explicit operator d17_policy
#      adjudication, which #301 already builds. Work owed: #301's four items (SS30c) + population + re-freeze.
...
per-limb value, so independence is the conservative middle, not the floor; (ii) the "true 3% in all three limbs" case
