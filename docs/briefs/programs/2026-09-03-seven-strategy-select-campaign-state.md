...
#   2. Fresh TV KEY-STATS panels for all five replacement exports (G1.4) — NOT the bar panels, different artifact.
#   3. DJ30 +$287.00 reconciled or explicitly dispositioned.
# Until 1-3 clear: phase1_verdict_cap stays NEEDS_CONTEXT, strategies stay BLOCKED_EXPLORATORY, and there is
...
| # | Prerequisite | State |
|---|---|---|
| 1 | Codex's per-leg **scaling-faithfulness read**, and one export per admitted size for any dollar-dependent leg — **each replacement export takes the full G1.1–G1.10 read, not a delta** | owed |
| 2 | Fresh independent **TV Key-stats panels** for all five sources (G1.4) | owed |
| 3 | **DJ30 +$287.00** reconciled or explicitly dispositioned | owed |
| 4 | Four bar-panel hashes | ✅ accepted (this section) |
| 5 | §14e generation on `main` | ✅ merged (§16g) |

Until 1–3 clear, `phase1_verdict_cap` stays `NEEDS_CONTEXT`, every strategy stays `BLOCKED_EXPLORATORY`, and
...
| `trade_days` | **NOT CAPTURED** | still owed — see §17d |
...
1. **`trade_days`** — the count against `min_trading_days: 3`, not present in the capture. Two canned-payload
   sessions plus weekly token trades since 2026-08-12 make it *likely* already satisfied, which would be one more
   favourable difference — **but it is not assumed.** One number from the dashboard.
...
primary-sourced input capture with a named method. The remaining freeze prerequisites are the scaling-faithfulness
read, the TV Key-stats panels, the DJ30 +$287 disposition, and this snapshot re-read at commit.
