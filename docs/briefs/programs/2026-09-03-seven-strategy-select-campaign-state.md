...
6b. HISTORY SELF-AUDIT before opening the PR: every commit on the branch, not only the final tree, must be free of the private directory — `git rev-list origin/main..HEAD | while read c; do git ls-tree -r --name-only "$c" | grep -q '^lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/' && echo "PRIVATE FILE IN $c"; done` must print nothing, and `git log --diff-filter=A --name-only origin/main..HEAD -- lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` must be empty. A hit means a capture was committed at some point: if not yet pushed, abandon the branch and rebuild from origin/main with clean commits only; if already pushed, STOP and report — that is a disclosure (the PR is closed and replaced, never rewritten; the operator decides the ticket question as under campaign-state §26).

7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: per source the two counts from step 0 (inputs captured, inputs differing from the body's defaults — counts only, never names or values), the binding result (fresh export SHA-256 equal to the pinned export_sha256: yes, per source) and the digest, the per-leg statuses from step 6, the new config hash, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0; live-account identifiers 0; live-account dollar figures 0.
...
| P7 | Publication by role — zero override values, zero live-account identifiers or figures — in tracked files, commit messages and the PR body; nothing under the study's private directory in ANY commit of the PR range, not only the final tree (a file added then deleted stays fetchable from PR history and cannot be purged under the no-rewrite rule) | full-diff role audit; `git ls-files lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` prints nothing; every commit in `origin/main..<head>` walked with `git ls-tree -r --name-only` for the private path, and `git log --diff-filter=A -- <private path>` over the range empty — any hit is a disclosure: the PR is closed and replaced, and the operator decides the ticket question (§26 precedent) (⚠ a repo-root `inputs/` pathspec inspects nothing relevant — four tracked files live under other `lab/**/inputs/` paths) |
...
§47b as amended seven times is canonical.
