...
`ops/sentinel/activity_week.py` reads — the operator's row there is what flips the session hook's `NOT RECORDED`. ⚠ **`STATE.md`'s weekly board now carries this statement, attributed, as of §39 (Codex round-13 P2) — it had still said "not claimed placed"; the private coverage row remains the operator's to write and the hook stays `NOT RECORDED` until it is.** **Operator, 2026-09-05 (restated):** *"I placed the weekly idle trade wednesday two days ago, this should be recorded."* ⚠ The two descriptors point to **different days** — Wednesday was 2026-09-02, and two days before 2026-09-05 is Thursday 2026-09-03 — and the board already carried **2026-09-03** from their 09-04 statement. This reads as the same placement restated, not a second one. **Coverage is unaffected either way: both candidate days fall inside the 08-31→09-04 bucket.** The exact day is deliberately **not reconciled here** — the authoritative record is still the operator's private coverage row, and guessing between their own two descriptors is precisely the kind of inference this artifact must not make. On the strength of the covered bucket, `STATE.md`'s rolling deadline is advanced to the next bucket (09-07→09-11, deadline 2026-09-11), which cleared the repo-wide `state-currency` gate failure of 2026-09-05.
...
## §42 Gate read — #301 @ `512f7a3` (2026-09-04): the last residual closed; GREEN LIGHT

`512f7a3` *Record D32 overlap-keyed drawdown policy*, on top of a merge of `main` (`8b44c4c`, carrying #295).
Worker-commit-only diff touches exactly: the study README, `tv_summary_reconciliation.py`,
`test_tradeify_drawdown_policy.py`, `test_tradeify_phase1_evidence_integration.py`,
`test_tv_summary_reconciliation.py`. Its commit body carries no currency or percentage token.

| Condition | Verified |
|---|---|
| `d17_policy.max_drawdown` requires `"OVERLAP_KEYED"`; null / missing / other fail closed | `raise ValueError("d17_policy.max_drawdown must be OVERLAP_KEYED")`; the placeholder test is now parametrized over `False`, `{}`, `"OVERLAP_KEYED"` and accepts only the last; `test_missing_d32_slot_fails_closed` kept |
| Status reports the ruling on every surface | `MAX_DRAWDOWN_POLICY_STATUS = "OVERLAP_KEYED_D32"`; asserted on DD rows, per-leg detail JSON and the human report |
| README no longer says pending | "policy slot requires `OVERLAP_KEYED` under D32. Reports carry `OVERLAP_KEYED_D32`; complete evidence coverage is not operator acceptance" |
| Tests here on 3.11 | **138 passed** across the policy, summary-reconciliation and runner modules |
| CI on `512f7a3` | `skills (3.12)` (the required check), `validation-controls`, semgrep **green**; `pytest (3.11)` and `build (3.11)` **in progress at the time of writing** — green light stands unless one of them fails, in which case it is withdrawn here |
| Threads | all three Codex threads replied to and resolved by the worker; a Codex round was requested by the worker and lands by event |
| `main` | merged at `8b44c4c`; `main` has since moved again (#297, `81b35d0`, touching only the gate runner / `gates.yml` / `Makefile` — no overlap). Behind, not conflicted; the operator may merge as-is or update the branch |

**Verdict: GREEN LIGHT on #301.** Every item of §30c is done and verified (1, 3, 4 at `3323fc9`; 2, 5 at
`ac36590`; the D32 policy value here). Codex returned zero findings at `ac36590`. **The operator merges.**

What the merge lands on `main`: D26's binding (exact-bytes digest verification, fail-closed before any
report) and D27/D32's mechanism (closed-interval overlap with ties, the panel DD as a separate anchor, the
one-sided lower-bound check, RECORDED on overlap, `OVERLAP_KEYED` declared). What it does **not** do: close
prerequisites 2 or 6 — both still owe **population** (the five current-input captures, §22a) and the **single
re-freeze**; the historical config still fails the loader by design until then.

## §43 Fleet returns gate-read (2026-09-04, 22:10Z): A resolved with two P2 fix rounds, B built though withdrawn, C falsified; #301 green light confirmed

**Dispatch-revision defect (process; no worker at fault).** The three workers were fired at ~21:50Z from the umbrella brief as [#302](https://github.com/Joshua-Asante/first-passage/pull/302) was opened (`af0203f`). Codex's pre-dispatch review of that revision (3 P1 + 4 P2, all accepted) re-froze the brief at `9914dd8e`…`70f1c47c` by ~22:03Z — after the builds had started (the packet PRs opened 21:55–21:57Z). One consequence per packet: B was built although withdrawn; C implemented the pre-re-freeze `_run` guard that the re-freeze turned into a forbidden move; A branched from `ba713ee` because the "`main` ≥ `81b35d0`" instruction did not yet exist. Same class as #301's D29 relay lag (§29). Rule: **a brief's Codex round is part of the freeze — dispatch waits for it**, and the dispatch pointer must carry the brief's commit SHA so a worker can detect a re-freeze. Proposed for the `cursor-fleet` skill's dispatch step; not applied here (CLAUDE.md §Continuous improvement, item 6).

| Packet | Head | §6 gate | Codex | Disposition |
|---|---|---|---|---|
| A — [#305](https://github.com/Joshua-Asante/first-passage/pull/305) | `7b1418b` | **RESOLVED** — footprint exact; 19 pinned tests pass here on 3.11; `check_repo_map_scripts_table.py --check` exit 0 on the head **and on a scratch merge with current `main`** (merge-tree clean: #297's 16 cell rewrites and A's row/caption do not collide); the three frozen CLI lines reproduce; `--tier check` exit 0; nothing wired into a gate | 2 × P2, both verified here: (i) out-of-domain arguments are accepted silently — a negative true rate sizes to `n=60 joint=1.000`, `--limbs 0` reports joint 1.000, `--alpha 2` certifies every count, a true rate above 1 reports power 0; (ii) `max_certifying_busts` rebuilds the whole lower tail for every candidate `k` and `size_for_power` repeats that at every grid `n` — 6.6 s at `n-max` 4000 for a rate near the ceiling, far longer at the default 8000 | **Fix round A1** (brief §6b): validation at API and CLI, incremental CDF, API and every §2 pin unchanged, `main` merged in. Not to be merged before A1 returns |
| B — [#304](https://github.com/Joshua-Asante/first-passage/pull/304) (draft) | `bb6d5d9` | n/a — WITHDRAWN before dispatch; built from `af0203f` regardless | none (draft) | Returned `NEEDS_CONTEXT` with exactly the §2-B contradiction (`2026-08-24b` above `i`, `a` above `h` at the base; `SESSIONS` off-limits to workers). Its three questions are the judgment call §2-B routes to CC/operator. **Close without merge; no re-anchor.** The `p2` slot stays burned |
| C — [#303](https://github.com/Joshua-Asante/first-passage/pull/303) | `0e7ab25` | **FALSIFIED** — footprint exact and its 2 tests pass, but `_run` is guarded (§5: "do not guard `_run` itself") and the second pinned test (git missing → `build_report()` raises) is absent, replaced by one asserting the forbidden rc-127 behaviour | 1 × P2, verified here: with `git` absent, `repo_hygiene.py --json` exits 0 with an empty report (`main`'s version raises) | **Fix round C1** (brief §6b): conform to §2-C as frozen — `_run` back to `main`'s bytes, the probe guarded in `build_report()`, the rc-127 test deleted, the git-raises test added. Not to be merged before C1 returns |

The fix-round prompts are handed to the operator in chat (the operator fires Cursor — `cursor-fleet` §5); their frozen scope and acceptance pins are the brief's §6b, so a later session can gate-read the returns without the chat.

**#302's own required check.** `skills (3.12)` went red at 22:06Z on `roll_sessions.py --check-append-only`: #297 merged the `2026-09-04e` stub above `04d`, so this branch's pointer edit to `04d` — legal while `04d` was the top entry — became a mutation of a prior entry in the CI merge tree. Repaired by merging `main`, restoring `04d` to `main`'s bytes and carrying the pointer refresh in a new top stub `2026-09-04f` (the SESSIONS template's own rule: a stub instead of editing the old top entry).

**#301 confirmation.** Codex's manual round at `512f7a3` returned zero findings (22:01Z); `pytest (3.11)`, `build (3.11)` and `validation-controls` are all green on that head. §42's conditional ("green light stands unless one of those fails") is discharged: **GREEN LIGHT CONFIRMED — the operator merges.**

**Head moved 22:23Z — `f4132a0`, a pure merge of `main`.** The operator merged `main` (`81b35d0`, #297) into the branch. Verified here: `f4132a0` has parents `512f7a3` and `81b35d04`, and its tree is byte-identical to `git merge-tree origin/main 512f7a3` — no content beyond the merge; the PR's diff against `main` is unchanged (14 files). The green light carries forward by construction. One caveat: the CI runs on `512f7a3` started six seconds before #297 merged, so `f4132a0` is the first CI pass of this PR against #297's gate-runner and `gates.yml` changes; all five jobs were in progress at the time of writing. The green light stands unless one of them fails on this head, in which case it is withdrawn here.

## §44 Fix round C1 gate read — #303 @ `94fa685` (2026-09-04, 22:33Z): RESOLVED

The operator fired C1 at 22:29Z; Cursor returned at 22:30Z with a merge of `main` (`4a85207`) and one fix commit (`94fa685`), the PR body restated as `DONE` with the revision line ("first build used brief revision `af0203f`; conformed to `1e16f4f4`"). Verified here on the head, per §10 and §6b:

| Pin | Result |
|---|---|
| Footprint exactly `scripts/repo_hygiene.py`, `tests/test_repo_hygiene.py` | ✅ |
| `_run` byte-identical to `origin/main`; the only hunk is the §2-C probe guard in `build_report()`, verbatim | ✅ |
| Test file: gh-absent test kept; `test_run_missing_binary_returns_127` deleted; `test_build_report_without_git_raises` added — seeded tmp repo, `REPO_ROOT` monkeypatched, `subprocess.run` raises for `git` only, `pytest.raises(FileNotFoundError)` | ✅ |
| `python -m pytest -q tests/test_repo_hygiene.py` on 3.11 | ✅ 2 passed |
| `repo_hygiene.py` on a host without `gh` | ✅ degrades (`gh available: False`) |
| Codex's scenario — `git` absent | ✅ raises `FileNotFoundError`, non-zero exit (was: exit 0, empty report) |
| `--tier check` exit 0; nothing wired into `gates.yml` / `Makefile` | ✅ |
| Codex thread replied to and resolved by the worker | ✅ |
| CI on `94fa685` | in progress at the time of writing (`skills (3.12)`, `pytest (3.11)`, `build (3.11)`, semgrep) |

**Verdict: C1 RESOLVED — #303 is mergeable by the operator once CI on `94fa685` is green.** A Codex round was requested on the fix head so the loop can confirm convergence; a new finding routes to Cursor per the operator's standing instruction. Packet A's fix round A1 (#305) is still owed.
...
**MERGED (2026-09-05 ~01:00Z).** #303 merged as `6b5bc96` — the operator's merge of the `2cc6e9a` head. Post-merge checklist on the merged tree (this branch merged `main` at `0fd2e39`): `python scripts/gate_manifest.py --tier check` exit 0; `python scripts/repo_hygiene.py` exit 0 on this host without `gh` (`gh available: False` — the very case Packet C repaired); the brief's §0.9-C probe on `main` now returns DONE (guard SCOPED, 2 tests pass); CATALOG n/a; STATE no change (tooling, not a forward item); the brief's §6 manifest row C → MERGED; the SESSIONS `04f` stub carries the residue. `main` is `6b5bc96`.
...
| Pin | Result |
|---|---|
| This round touches only `scripts/certification_power.py`, `tests/test_certification_power.py`; cumulative footprint vs `main` is those two plus the generated `REPO_MAP.md` row; `check_repo_map_scripts_table.py --check` exit 0 on the merged head (the row now sits on #297's cells) | ✅ |
| Every §2 pin unchanged — `max_certifying_busts` 60/160/340/630/950 → 0/3/10/22/36; per-limb 0.7403/0.6021/0.5577/0.8030; sizes 130/370/950 (independent), 130/390/970 (Fréchet), 630 single-limb; `(5)` → `-1` / `0.0`; three frozen CLI lines reproduce | ✅ |
| §6b (i) domain: `max_certifying_busts(0)`, `per_limb_power(60, −0.03 / 1.5 / nan)`, `joint_power(0.9, 0, …)`, `joint_power(1.5, 3, …)`, `size_for_power` with `ceiling=1.0` / `alpha=2` / `target=0` / `n_max=5` all raise `ValueError`; the three CLI cases exit 2 with empty stdout, a message on stderr and no traceback | ✅ |
| §6b (ii) incremental CDF: `max_certifying_busts` walks one peak-shifted log recurrence and stops at the first `k` whose CDF exceeds `alpha`; `per_limb_power` evaluates the CDF once; `size_for_power` keeps the linear grid scan (no binary search); no numpy/scipy; API unchanged | ✅ (read the recurrence: correct, including the `p = 0` and `p = 1` edges) |
| Regression pins at the grid's end: `max_certifying_busts(8000) == 367`; `per_limb_power(8000, 0.049) == 0.1013`; `size_for_power(0.049, 0.80, n_max=2000)` raises | ✅ |
| Wall time of the near-ceiling default-`n-max` run | ✅ 0.18 s here (worker: 0.091 s; was > 80 s at `7b1418b`), exit 2, empty stdout |
| `python -m pytest -q tests/test_certification_power.py` on 3.11 | ✅ 34 passed (worker: 34 on 3.11.16 and 3.12.3) |
| `--tier check` exit 0; nothing wired into `gates.yml` / `Makefile` | ✅ |
| Both Codex threads replied to and resolved by the worker | ✅ |
| CI on `b5b3fb4` | `skills (3.12)` and semgrep green; `pytest (3.11)`, `build (3.11)` in progress at the time of writing |

**Verdict: A1 RESOLVED — #305 is mergeable by the operator once CI on `b5b3fb4` is green.** A Codex round was requested on the fix head. With C1 (§44) and A1 both in, **nothing is owed to Cursor**; the fleet's remaining item is the operator closing #304. Post-merge, the orchestrator writes the fleet integration commit (manifest → MERGED, SESSIONS pointer).

**Not converged (22:41Z).** Codex's round on `b5b3fb4` returned **three P2**, all reproduced here: (i) `target == 1.0` is claimed attained on a rounded CDF — `size_for_power(0.001, 1.0, limbs=1)` returns 410 while the exact upper tail is positive; (ii) `--power` together with `--n` silently sizes instead of failing; (iii) the accumulated lower-tail sum ends a hair below 1 at `k = n`, so an alpha near 1 lets every bust certify. None touches a §2 pin; (i) was first recorded here as narrowing my A1 domain pin for `target` from `(0, 1]` to the open interval `(0, 1)` — SUPERSEDED (Codex round 3 on #302, 23:15Z): that blanket rejection wrongly refused the exact zero-rate case, so the domain stays `(0, 1]` with `target == 1.0` valid only when `true_rate == 0`; the brief's §6b A2 (i) carries the corrected rule and is the only statement to build from. **Fix round A2 is frozen in §6b, awaiting the operator's dispatch; #305 waits on it.** The "nothing is owed to Cursor" line above is superseded by this paragraph.

**Head moved 01:34Z 09-05 — `2c67b34`, a pure merge of `main`; NOT the A2 fix.** Parents `b5b3fb4` + `6b5bc96`; the head tree is byte-identical to `git merge-tree origin/main b5b3fb4`; `git diff b5b3fb4 HEAD` over the three packet files is empty. The A1 RESOLVED verdict therefore carries unchanged. The §10 Packet A block re-run in a detached worktree at `2c67b34`: footprint exactly the three files, `tests/test_certification_power.py` green, `check_repo_map_scripts_table.py --check` exit 0 (its caption reads 64 on this tree — `main`'s 63 plus `certification_power.py`), the three pinned CLI lines exact (`n=950`, `n=970`, `n=630 per_limb=0.803 joint_independent=0.518 joint_frechet=0.409`), `--tier check` exit 0, nothing wired into `gates.yml` or the `Makefile` — **AUDIT rc=0**.

**A2 re-verified as OWED on the merged head — all three items, probed with Codex's own cases:** (i) `size_for_power(0.001, 1.0, limbs=1)` returns **410** (must raise `ValueError`; `size_for_power(0.0, 1.0)` correctly stays **60**, so only the nonzero-rate case is wrong); (ii) `--true-rate 0.03 --power 0.8 --n 630` exits **0** with the `--power` answer (must exit 2 with empty stdout); (iii) `max_certifying_busts(8000, 0.9, 0.999999999)` returns **8000** and the last CDF term is **0.999999998848898** (pins: exactly `1.0`, and the quantile **7355**). ⚠ Probing (i) with a different rate is misleading — `size_for_power(0.03, 1.0)` already raises because the search exhausts `n_max`; Codex's rate of 0.001 is the case that attains spuriously. The §6b A2 scope is unchanged and still awaits the operator's dispatch; no Codex round was requested on this head — a pure merge with byte-identical packet files re-reviews nothing, and the three P2 findings on `b5b3fb4` stand.

**⚠ #305 MERGED 2026-09-05 as `2a89348` — at the pure-merge head, so A2 NEVER LANDED and all three items are now live on `main`.** The merged parent is `2c67b34`; `scripts/certification_power.py`, `tests/test_certification_power.py` and `REPO_MAP.md` are byte-identical to `b5b3fb4` on the merged tree, and the packet branch is gone from `origin`. Re-probed on `2a89348` in a detached worktree, with Codex's own cases: (i) `size_for_power(0.001, 1.0, limbs=1)` returns **410** (must raise); (ii) `--true-rate 0.03 --power 0.8 --n 630` exits **0** and prints the `--power` answer (must exit 2, empty stdout); (iii) `max_certifying_busts(8000, 0.9, 0.999999999)` returns **8000** against the pinned **7355**, and the lower-tail CDF ends at `0.999999998848898` rather than exactly `1.0`. `check_repo_map_scripts_table.py --check` exits 0 on the merged tree, so the caption is sound. ⚠ Probing (ii) **without** `--true-rate` shows exit 2 with empty stdout for the wrong reason — argparse rejecting a missing required argument — and looks fixed; the pin must be probed with a complete command line.

**Consequence for the freeze.** Freeze prerequisites **8** and **9** (`n₂` / `n₃` sizing) are computed with this calculator, and (iii) is a silent wrong answer in exactly the high-confidence region those sizings use: at an alpha near 1 every bust certifies, so the returned bound is `n` itself. **The calculator must not be used for the `n₂`/`n₃` freeze until the three items are fixed.** A2 can no longer be a fix round on #305 — the PR is merged and a merged PR cannot carry new work — so the remedy is a **fresh packet A3 off current `main`**, same frozen scope, or a CC-solo fix; the ordering is the operator's call and the pointer is prepared. This supersedes the "awaits the operator's dispatch; #305 waits on it" line above.

**Should the #305 merge be reverted? Orchestrator assessment: NO — fix forward with A3.** The deciding fact is that the calculator has **zero consumers**: nothing in the tree imports or calls `certification_power`, it appears in no `gates.yml` entry and no `Makefile` recipe, and the only references outside its own test are the two briefs. The three defects are therefore **latent, not active** — nothing computes a wrong answer today, and the one future consumer (`n₂`/`n₃` sizing, prerequisites 8 and 9) is already barred from using it by the paragraph above. Against that, a revert costs a revert commit on `main`, forces A3 to re-add the whole file rather than change two lines in it, requires a revert-of-the-revert or a fresh add whenever the work returns, and flips the `REPO_MAP` scripts-table caption a third time under the open PR. A revert would be the right call if a consumer existed, if the defect were a silent wrong answer somewhere already running, or if A3 were speculative — none holds. **The operator's ruling is owed; this is a recommendation, not a decision.**

## §46 [#301](https://github.com/Joshua-Asante/first-passage/pull/301) MERGED (2026-09-04, 22:35Z) — D26/D27 code readiness is on `main`

`5ab079c` *Merge pull request #301* (operator-merged; head `f4132a0`, the pure merge of `main` over `512f7a3` read in §43). Fourteen files, all under `lab/` and `tests/`; no governance surface touched; the check tier exits 0 on `main` here. Codex's last two rounds (`ac36590`, `512f7a3`) returned zero findings.

**On `main` now (from §42):** D26's binding — exact-bytes verification of the study-local private override artifact against the required digest, fail-closed before any report, digest-only serialization, with the `inputs/private_overrides/` ignore rule and real-Git regressions; D27/D32's mechanism — closed-interval per-leg overlap with ties, the TradingView panel DD as a separate anchor on every leg, the one-sided lower-bound check on non-overlapping legs, RECORDED on overlap, `d17_policy.max_drawdown: "OVERLAP_KEYED"` required and `OVERLAP_KEYED_D32` reported.

**Not closed by the merge:** prerequisites 2 and 6 still owe **population** — the five current-input captures (§22a; the two 200K halves are struck, D30) and the **single re-freeze**; the historical config fails the loader by design until then. Prerequisite 1 (Codex's per-leg scaling read, keyed on dollar columns) and prerequisite 7 (the live-state MC engine change under `core/`, CC solo) are unchanged. The next dispatch is the capture-and-populate job, drafted against the merged code in the section that follows.

**Post-merge checklist** (`post-merge` skill): gates on `main` — check tier exit 0 · CATALOG — #301's own two-line update passes the catalog check · STATE — queue row 1 refreshed in place with the merge and the population/re-freeze pointer · SESSIONS — the `2026-09-04f` stub's pointer refreshed (own entry, not yet on `main`) · sync-skills — skipped (cloud) · hygiene scout — `repo_hygiene.py` crashes on this `gh`-less container exactly as Packet C fixes (§44); re-run after #303 merges.

## §47 Dispatch packet — Phase 1 population + the single re-freeze (2026-09-04, 22:50Z): constituent (i) iteration 6 of ≤8

**What it closes, on the §47c gate read and not on the push:** freeze prerequisites **2** (G1.4 — the §18a panels populated as anchors under D32) and **6** (D26's digest binding populated). **What it leaves alone:** prerequisite 1 (Codex's per-leg scaling read — its own dispatch next), 7 (the live-state MC engine change under `core/`, CC solo), 8 and 9 (`n₂` / `n₃` sizing — computable with the certification-power calculator once [#305](https://github.com/Joshua-Asante/first-passage/pull/305) merges).

**Rule 2.** Constituent (i) iteration **6** of ≤8 under D3's unit (one dispatch → gate read → fold). Iterations 1–4 are on record (the failed Phase 0 return; the counts at §12/§13; SESSIONS `2026-09-04d` for the #294 remediation cycle); #301's cycle (§22 → §29/§35/§40/§42 → merged §46) is **5**; this is **6**. **Two remain for Phase 1 — a bounce here spends one.** $0 · K=0 · MC=none.

**Routing.** Test 0 fires: the ten frozen source files and the five private override maps are vendor/private bytes → **local** dispatch to the operator's Codex session, never cloud. Judgment content: none — the values come from §18a and the operator's captures, and every code path exists on `main` (§46). The orchestrator does not touch the study.

### 47a — Operator step first: snapshot the five Inputs tabs (no memory required; §22a as ruled, 200K halves struck)

**Why a Pine diff cannot do this (operator question, 22:58Z).** The bodies in the operator's `Downloads/strategies` are already hash-pinned as the five `pine_sha256` values, and their divergence from the locked editions is already recorded in `pin_divergence`. §19d's defect is different: **the pinned body's own `input()` defaults do not produce the export** — both Striker bodies default `backtestMode` to the value that makes the DD-limit dead code, yet both exports contain DD-Limit exits — so an input was changed on the chart's **Inputs tab**, which lives in no file. Diffing files cannot recover it; only the chart state can.

**What the operator does — screenshots plus one fresh export per chart.** For each of the five sources, open the chart that produced the pinned 2026-09-03 export (the §18a panel is that run's own; the current chart states are the recoverable ones, D30), open the strategy's Settings → **Inputs**, and screenshot the whole tab, scrolling so every input is captured once. Do not try to identify what changed; TradingView marks nothing as changed, and the local session computes that. Then, **with the chart in exactly that state**, export the Strategy Tester's trade list once more — the same export type as the pinned file — into the local session's private input area (vendor bytes: never the repo). Hand the screenshots and the fresh exports to the local Codex session only — they are read there, which is the job; they never go into a commit, a PR, a tracked file, or this file. **Why the fresh export:** a screenshot of today's Inputs tab proves nothing about which state produced the 2026-09-03 export (Codex, round 6 — accepted); the session hashes the fresh export and requires equality with the config's `export_sha256`, and only a snapshot whose re-export hashes to the pin is bound. A mismatch means the chart has drifted since the run: the snapshot is not bound, the row-level dollar-column diff (§19g) is reported as a finding, and the source is treated as lost-chart (fallback below).

**What the local session does (§47b step 0).** From the screenshots, the fresh exports and the pinned Pine body's `input.*()` calls, it writes the private file `lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json` as a **full snapshot** — every input's displayed title and value, plus the list of inputs whose value differs from the body's coded default — hashes it once, and never edits it again (an edit changes the digest and forces a second re-freeze). A full snapshot binds more tightly than a diff and removes the "which ones changed" judgment entirely; the runner hashes bytes without parsing, so the shape is free.

**Built-in validity check (§19d).** For both Strikers, `backtestMode` **must** appear in the differing set; if a Striker's snapshot shows it at the body's coded default, that chart is not the run that produced the export — STOP and report, do not proceed. For Aegis, ORB-MNQ and MGC the differing set may be empty; that is a finding to record, not a problem.

**Fallback if a chart state is gone — byte-identical re-export or nothing.** Five matching Key-stats aggregates do **not** establish the original input state: an override can be inert over this sample, and different input sets can yield the same count, net, win rate, PF and DD (Codex, round 5 — accepted). The only acceptable fallback is a **byte-identical re-export**: reset the strategy's inputs to the body's coded defaults, re-run over the same span with the same Deep / 100K / Default-detalization settings AND the same `Script execution` setting §18a records for that source (1 for Aegis and ORB-MNQ, 2 for DJ30, NAS100 and MGC — it changes calculation events, so a chart left at its default can hash differently and make a recoverable source look impossible), export, and compare the new export's SHA-256 with the config's `export_sha256` for that source. Identical → the snapshot records `provenance: "original chart state unrecoverable; the body's coded defaults reproduce the pinned export byte-for-byte"`, with `inputs` = the coded defaults and an empty differing set — that binds the export to a **declared** input state, which is what D26 needs, without claiming to recover the original one. Not identical, or only the aggregates match → the capture is impossible for that source: say so and stop (§22a); that is a ruling, not a worker call. (For the two Strikers this fallback cannot succeed — §19d proves their defaults make the DD limiter dead code — so a lost Striker chart state is unrecoverable outright.)
...
Preconditions — stop and report if any fails: origin/main at or after 5ab079cb (#301 merged) AND carrying #302's merge — `git ls-tree --name-only origin/main scripts/private_content_scan.py` prints the path and `python -m pytest -q tests/test_private_content_scan.py` passes on a checkout of it (the scanner step 6b requires lands with #302; a dispatch from the open review would reach the pre-push audit without it and spend the one iteration); the operator's Inputs-tab screenshots for the five charts that produced the pinned 2026-09-03 exports are available to this session; the five pinned Pine bodies (the config's pine_sha256 values) are on this machine; `git check-ignore -v lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/probe.json` resolves to the study's inputs/private_overrides/ rule. Shell: Git Bash or PowerShell 7+ — every command in this prompt is a plain `git` or `python` invocation; the only shell feature used is `&&` in step 5, which Windows PowerShell 5.1 lacks.

Branch codex/tradeify-phase1-population from current origin/main. One PR; no rebase, no force-push, no merge — the operator merges. Orchestrator surfaces are off-limits: STATE.md, docs/SESSIONS.md, ADRs, the plan, and the orchestrator-owned campaign documents (this campaign-state artifact and every brief under docs/briefs/); so are Pine, core/, and vendor bytes. The study files this packet names ARE the footprint and are edited exactly as instructed — phase1_config.json (the five digest keys only), tv_summary_anchors.json, reconciliation_manifest.json and RESULTS.md (regenerated by the run), VERIFICATION.md, README.md, run_phase1.py (the _BASE_COMMIT line only) and the three tests named in step 4 — and nothing else.

0. PRIVATE SNAPSHOTS (write once — bind FIRST, then one atomic write). For each of aegis_6j1, orb_mnq_recon_v7, striker_dj30_mym_pyramid_250, striker_nas100_mnq_dow_wed_excluded, vanguard_mgc_v04, in this order: (a) parse every input.*() call in that source's pinned Pine body (title and coded default); (b) transcribe every input's displayed title and value from the operator's screenshots and compute the differing set — all of this in memory, nothing written yet; (c) BINDING CHECK, before anything is written or hashed: SHA-256 of the operator's fresh export taken from the snapshot state must equal that source's export_sha256 in phase1_config.json. On a mismatch the snapshot is NOT bound: write NOTHING for that source, STOP for it, report the row-level diff on dollar columns (§19g — never percent columns) as a finding, and treat the source as lost-chart per §47a's fallback; never hash a snapshot the re-export did not confirm. (d) Validity check from campaign-state §19d: for BOTH Strikers, backtestMode must be in the differing set — necessary, not sufficient; (c) is the proof. For the other three the set may be empty; record it either way. (e) Only when (c) passed: assemble the FULL snapshot object {"strategy_id", "captured_at", "run": {"span": "Sep 1, 2022 - Sep 2, 2026", "mode": "Deep", "initial_capital": 100000, "detalization": "Default", "script_execution": <1|2>}, "inputs": {<title>: <value> for every input}, "differs_from_body_default": [<titles>], "reexport_sha256_matches_pin": true} and write it ONCE, atomically — to a temporary name in the same directory, then rename to lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json. After that write, never edit these files again (an edit changes the digest and forces a second re-freeze). `git status --porcelain` must not show them. Report only counts (inputs captured, inputs differing) and the binding result (re-export matched pin: yes/no) — never a title-value pair.

1. DIGESTS. For each of the five: SHA-256 of the EXACT BYTES of inputs/private_overrides/<strategy_id>.json → write it as `pine_input_overrides_sha256` in that strategy's record of phase1_config.json (required key, 64 lowercase hex). Change nothing else in the config — the ten export/Pine pins, byte counts and every other field stay byte-identical.

2. ANCHORS. Populate lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/tv_summary_anchors.json from campaign-state §18a (docs/briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md on main — the five TradingView Key-stats panels the operator captured for the pinned 2026-09-03 runs). One anchor per source with exactly the keys {strategy_id, export_sha256 (the config's pin for that source), source_note, metrics, missing_metrics: []}; metrics exactly {trade_count (integer), net_pnl_usd, win_rate_pct, profit_factor, tv_panel_max_drawdown_usd} as CANONICAL numeric strings — no `$`, no thousands separator, no `%` (the loader hands each string straight to Decimal, which rejects all three) — equal to §18a's displayed values after that normalization. The orchestrator's normalization of §18a is this table; cross-check it against §18a and STOP on any discrepancy instead of choosing:
  aegis_6j1: trade_count 121, net_pnl_usd "27996.05", win_rate_pct "63.64", profit_factor "3.422", tv_panel_max_drawdown_usd "1470.40"
...
source_note = operator provenance, e.g. "TradingView Key stats panel captured 2026-09-04 for the pinned 2026-09-03 run: Sep 1, 2022 – Sep 2, 2026, Deep, 100K USD, Default detalization, Script execution <1|2>; campaign-state §18a". coverage_status: COMPLETE. Rewrite coverage_note: the "+$287 pending operator discrimination" text is superseded — D30 (2026-09-04) recorded that attribution UNESTABLISHED; the panels are the pinned runs' own and are never inferred from exports. d17_policy: keep ruling_date, ruling_ref, monthly_totals (RECONSTRUCTED), commissions (AMENDED_OUT), reason, and ADD "max_drawdown": "OVERLAP_KEYED" (exact-key slot); extend ruling_ref/reason with the D32 reference (campaign-state §38, ruled 2026-09-04). Never use the retired anchor names max_drawdown_usd / max_drawdown_excursion_bounded_usd.

2b. PROVENANCE, BEFORE THE RUN: set run_phase1.py's _BASE_COMMIT to the origin/main commit this branch is cut from — the runner serializes that constant into the manifest as git_base_commit at run time, so it must be right before the one real run — and refresh the assertion near tests/test_tradeify_phase1_runner.py line 320. This is the only line of runner code this job touches; state the new value in the PR body.

3. RUN ONCE, FOR REAL. The command is exactly this line and nothing after the output directory (a stray token is an argparse error, exit 2):
python lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/run_phase1.py --config lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/phase1_config.json --source-dir <the operator's ten frozen files> --output-dir lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/local_artifacts/population_2026-09-04
Exit must be 0. Exit 3 is ANY intake failure, not only the override artifact: the runner returns 3 for every SourceIdentityError or TradeExportSchemaError — an unusable source directory, an export or Pine file that is missing, unreadable, or mismatched in length or SHA-256 against the config, a private override artifact that is missing, unreadable or mismatched, or a CSV with no header row. Report the emitted `intake failure:` diagnostic with every filesystem path replaced by `<path>` and stop — the diagnostics never quote file CONTENT, but two of them quote a PATH: the source-directory check (`source directory is not a directory: <resolved root>`, exit 3, tv_trade_ledger.py `_resolved_child`) and the configuration-read errors (`cannot read JSON configuration: <path>` / `invalid JSON configuration: <path>`, exit 2) — and a resolved local path carries the operator's username and the vendor-data location, so the raw text stays on this machine. Never synthesize a digest, a file, or a source. Exit 2 is a configuration failure, exit 4 an output failure — report those the same way. Publish the regenerated reconciliation_manifest.json and RESULTS.md (v4 generation).

4. THE SINGLE RE-FREEZE. The config's SHA-256 moves off df238cd7…; move every pin that names it IN THE SAME COMMIT: tests/test_tv_trade_ledger.py (the pinned config digest); tests/test_tradeify_phase1_runner.py — in test_committed_manifest_matches_frozen_five_strategy_acceptance refresh EVERY assertion the generation changes, each read from the generated artifacts and never guessed: runner_version (now tradeify-phase1-normalization-v4), phase1_verdict_cap (whatever the runner emits), the inputs hash set including tv_summary_anchors_sha256, the ledger hashes if their bytes moved, the five local_strategy_report_sha256 and local_monthly_reconciliation_sha256 entries, and the reconciliation_manifest.json / RESULTS.md digests; tests/test_tv_summary_reconciliation.py — replace test_checked_in_operator_anchors_reject_stale_panels_under_d17 (it pins the historical EMPTY inventory: a "d17_policy keys mismatch" failure, coverage_status NEEDS_CONTEXT, an empty strategies list and the "+$287" note) with a populated-state regression against the checked-in file: it loads cleanly with the six-key policy, coverage_status is COMPLETE, five anchors whose export_sha256 equal the config's pins, and no "+$287" in the note — keep the historical-empty expectations only against an inline fixture, never the checked-in file; lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/VERIFICATION.md — recompute EVERY row of its "Frozen hashes" table and the detail-hash line from this generation's bytes (config, tv_summary_anchors.json, reconciliation_manifest.json, RESULTS.md, the five strategy-detail reports in configuration order, and the three ignored ledgers — rows whose bytes did not change stay as they are; say which moved), and add this generation's invocation evidence: command, exit code, invocation-time SHA-256 of run_phase1.py, lab/research_utils/tv_trade_ledger.py, lab/research_utils/trade_reconciliation.py AND lab/research_utils/tv_summary_reconciliation.py — the module that produces every D32 status (the runner does not write that file). Refresh VERIFICATION.md's NARRATIVE as well, not only its hashes: the audit date and the "final tradeify-phase1-normalization-v3 generation" sentence, the "G1.4 remains NEEDS_CONTEXT: fresh scalar panels are missing … +$287" sentence, and the test-and-hygiene evidence section (new focused and full-suite counts on both runtimes, new HEAD hygiene facts) — every sentence that describes the superseded v3 state; leave every limitation that is still true. (_BASE_COMMIT was already set in step 2b, before the run.) The twenty source hashes do not move. Update the study README wherever population falsifies it: the "D26 migration is pending" paragraph (now the populated state); the Venue-boundaries sentence that the verdict cap remains NEEDS_CONTEXT because panels are missing; the Evidence-inputs paragraph that calls tv_summary_anchors.json an empty inventory with a five-key d17_policy and names the max-DD metric "max drawdown USD" (now six keys including max_drawdown, and tv_panel_max_drawdown_usd); the v3 generation string (now tradeify-phase1-normalization-v4). Leave every limitation that stays true (D13, D19 residuals, the overlap caveats) untouched.

5. VERIFY on 3.11 AND 3.12 — the same commands on each interpreter, using whatever launcher provides that version on this machine (`py -3.11` / `py -3.12` on Windows, `python3.11` / `python3.12` elsewhere; substitute it for <py>): FOCUSED — `<py> -m pytest -q tests/test_tv_trade_ledger.py tests/test_tradeify_phase1_runner.py tests/test_tradeify_phase1_evidence_integration.py tests/test_tv_summary_reconciliation.py tests/test_tradeify_drawdown_policy.py tests/test_tradeify_d27_excursion.py tests/test_trade_reconciliation.py`; then FULL SUITE — `<py> -m pytest -q -p no:cacheprovider --basetemp <a directory outside the working tree>`. Step 4's VERIFICATION.md evidence needs the focused AND full-suite totals on BOTH runtimes, and neither `--tier check` nor CI supplies the 3.12 full suite (`tests.yml` runs it on 3.11 only), so the four totals come from these four runs and nowhere else — never carried over from the previous generation. Then once: `python scripts/gate_manifest.py --tier check`. Chain each interpreter's commands with && — a failed test must stop the line; every command exits 0.

6. REPORT, DO NOT ADJUDICATE. Per leg: the four non-DD limb statuses (expected MATCH; a MISMATCH is a finding — never widen a tolerance, never edit an anchor to fit) and the DD limb status with its measured overlap flag (no overlap → WITHIN_BOUND / COINCIDENT / MISMATCH-BLOCKER; overlap or tie → RECORDED). A BLOCKER is reported as a finding, not repaired.

6b. HISTORY SELF-AUDIT — CONTENT as well as paths, BEFORE EVERY PUSH (not only before opening the PR). A path check is necessary, not sufficient: an override value or an export cell pasted into an allowed tracked file (phase1_config.json, a test, VERIFICATION.md) and removed in a later commit passes every path check and every base-to-head diff, yet stays fetchable from the PR's refs — nothing is rewritten here and a closed PR's commits remain fetchable — so the control is a serialization-independent scan of every commit's added lines and message, run before the bytes leave this machine. (i) THE SCANNER is tracked, tested code, not a pasted shell snippet — from the repo root, with step 5's <py>: `<py> scripts/private_content_scan.py --json lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/aegis_6j1.json --json lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/orb_mnq_recon_v7.json --json lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/striker_dj30_mym_pyramid_250.json --json lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/striker_nas100_mnq_dow_wed_excluded.json --json lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/vanguard_mgc_v04.json --csv <export 1> --csv <export 2> --csv <export 3> --csv <export 4> --csv <export 5> --exclude <a file listing the twenty-five canonical anchor strings of step 2 and the values already tracked in phase1_config.json on origin/main, one per line> --path-prefix lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/ --forbid-suffix .csv --forbid-suffix .pine --range origin/main..HEAD`. It runs under PowerShell or Bash alike (no shell logic to paste), builds the needles IN MEMORY from the private bytes and writes nothing to disk — there is no needle file to protect, race or clean up. Needle classes: TITLE (every input title, at ANY depth — the snapshot is walked recursively), PAIR (every title/value in four serializations `"k": v`, `"k":v`, `k: v`, `k=v`), VALUE (every distinctive value, plus its comma-grouped and currency spellings, its percent-encoded form, and its cp1252/latin-1 byte view so a paste into a non-UTF-8 file still matches) and CELL (every distinctive cell of every export, **header row included** — an identifier is often a column NAME, and the old decimal-or-date rule had no pattern for an identifier or a whole-dollar figure at all). Four further classes need no needles: PATH, NAME, BINARY (fail-closed: decided from the blob's own bytes, including armoured carriers, never from git's attribute-driven view) and REF (the branch name, and the name and message of any tag this push would publish). Trivial values (booleans, short integers, short strings) are covered by TITLE and PAIR only while the title travels with them — a DETACHED trivial scalar can be no needle (`true` or `3` matches every file) and is caught by (vi) instead. Matching is **case-insensitive SUBSTRING**, not whole-word: a word boundary counting `_` and digits as word characters hid every needle glued into a longer identifier and made the report print such a path verbatim. A coincidental hit is adjudicated under (iii); a missed one is published. The --exclude tokens are dropped from every class, and **any class that had needles and is emptied by exclusions exits 3**, never CLEAN. (ii) SCOPE and OUTPUT: ONLY the ADDED lines of every commit's patch (never context lines, deletions or headers) and, separately, every commit message in origin/main..HEAD. Each commit is read against EVERY parent, keeping only what differs from ALL of them: for an ordinary commit that is its diff against its parent, and for a merge it is what the merge itself introduced — the conflict resolution — so a merge of origin/main is audited without re-reporting everything main brought in (a first-parent read would stop the packet on the repository's own tracked images). Needles are matched against added lines, commit messages AND the paths a commit introduces — a private title or value in a FILENAME is published exactly like one in a blob, reported as class NAME. ANY binary file a commit adds or modifies is a hit of class BINARY — a screenshot or a zipped export cannot be text-scanned and the footprint has no legitimate binary, so it fails closed (deletions are not flagged). Git runs with `core.quotePath=false` and path lists are read NUL-delimited, so a non-ASCII filename cannot slip past a prefix or suffix test, and the `+++` file header is recognised by parser state, not by its prefix (an added line whose own text starts with `++ ` renders as `+++ ` inside a hunk). It prints `needle classes: TITLE=… PAIR=… VALUE=… CELL=…`, one `commit <sha> added=… message=… path=… name=… binary=…` line per commit, one `HIT class=… commit=… file=… line=…` line per hit — and where a needle matched the PATH itself the printed path is redacted and carries a per-run `path_index=` instead (a digest of the path was a brute-forceable oracle for a short token; `--show-paths` prints the mapping LOCALLY and is never pasted), so pasting the scanner's output into the PR body cannot publish a private filename, one `HIT class=… commit=… file=… line=…` line per hit — locations, NEVER a token — and a final `SCAN result=` line. Exit codes: 0 CLEAN is the ONLY pass; 2 = hits; 3 = the needles could not be built (a private file missing or unreadable, or a class empty) — the scan never runs "empty", so a 3 is a STOP, not a zero; 4 = git failure. A needle sitting in an unchanged context line is not a hit: it was already on origin/main. (iii) ADJUDICATION, fixed rule: a hit is BENIGN only when it sits in a runner-generated TRACKED artifact (reconciliation_manifest.json, RESULTS.md) whose bytes P5 attests are the runner's output — an aggregate can coincide with a cell (a single-trade month's total equals that trade's P&L) and those artifacts are the sanctioned publication; a hit anywhere else — config beyond the five digest keys, a test, VERIFICATION.md, README, a commit message — is a disclosure, and a TITLE, PAIR, NAME, PATH or BINARY hit is a disclosure anywhere. (iv) PATH walk: the same run walks every commit's tree for the private prefix and every path each commit INTRODUCES — added, copied, or RENAMED to (git classifies a rename `R`, which an added-only filter would miss) — for the vendor suffixes (`--path-prefix`, `--forbid-suffix`); its hits are class PATH — a capture force-added in one commit and deleted in the next is still a hit. (v) Run (i)–(iv) and (vi) before the FIRST push and before EVERY later push. The gate is exit 0 (CLEAN), OR exit 2 where EVERY hit is adjudicated benign under (iii) and recorded as such in the PR body with its file and line — a sanctioned coincidence inside a runner-generated artifact must not block the one-shot push, and a single un-adjudicated hit is a STOP. Exit 3 and exit 4 are always a STOP. A disclosure BEFORE a push is repaired locally — the offending commits are unpushed, so `git reset --soft <the last pushed tip, or origin/main before the first push>`, re-stage without the bytes, recommit, re-scan until exit 0. A disclosure AFTER a push STOPS the packet: no further push and no replacement PR from the worker — replacement purges nothing, the commits stay fetchable from the closed PR's refs — report class, commit, file and line to the orchestrator with no token quoted; the operator rules on closure, replacement and the ticket question (campaign-state §26 precedent). The PR body carries, per push, the scanner's `needle classes:` line, its per-commit lines and its `SCAN result=` line (CLEAN, or HITS with every hit adjudicated benign by file and line). (vi) PROVENANCE SELF-READ, before every push, for what no needle can carry: read every added line of every commit and every commit message; every literal — a detached `true` or `3` included — must trace to one of P7's sources (a)–(g); a literal with no source is removed before the push (the offending commits are unpushed, as in (v)), and the PR body states that the self-read was done for each push. This is the worker-side half of P7's rule: the needle scan is byte-exact for bindings, distinctive values and export cells; the self-read covers the rest.

7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: per source the two counts from step 0 (inputs captured, inputs differing from the body's defaults — counts only, never names or values), the binding result (fresh export SHA-256 equal to the pinned export_sha256: yes, per source) and the digest, the per-leg statuses from step 6, the new config hash, the `sha256sum` lines of the eight ignored artifacts (five detail reports, three ledgers) for P5's three-way check, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0 — by the needle scan for bindings, distinctive values and export cells, by the step 6b (vi) provenance self-read for every other literal, a detached trivial scalar included; live-account identifiers 0; live-account dollar figures 0; and the step 6b content-scan result per push — hits by class, each 0 or adjudicated benign with file and line, never a token. No local filesystem path anywhere in the body.

NEVER: PUBLISH an override value or a local filesystem path (the operator's username lives in it) — not in a commit, a PR body or comment, a tracked file, this artifact, or any surface that leaves this machine (reading the operator's screenshots and the pinned Pine bodies locally IS this job); force-add anything under inputs/; widen a tolerance; edit Pine, core/, STATE.md, docs/SESSIONS.md, ADRs, this campaign-state artifact or any brief under docs/briefs/ (the study files named in the preconditions are the footprint, not this class); commit vendor bytes or copy them into any tracked surface (reading the ten frozen files locally is the job); merge.
```

### 47c — Gate read (orchestrator, on the returned PR)

| # | Condition | Read against |
|---|---|---|
| P1 | Five `pine_input_overrides_sha256` present, 64 lowercase hex, five distinct; the runner's intake verified each (manifest `source_identity` carries the digest; exit 0); and per source the worker attests that a fresh export taken from the snapshot state hashed to the pinned `export_sha256` — the binding is the re-export, not the screenshot; a mismatch is a finding that stops that source, never a snapshot hashed anyway | config, manifest, PR body |
| P2 | Anchor VALUES equal §18a on all five after canonical normalization (the §47b table; a discrepancy between that table and §18a is an orchestrator transcription defect to fix here, never a value the worker "corrects") — a transcription check on all twenty-five values, including the panel DD as a value; the loader's tolerances apply only to the four non-DD limbs (P3), and the DD limb is judged under P4 alone; `coverage_status: COMPLETE`; `d17_policy.max_drawdown: OVERLAP_KEYED`; `OVERLAP_KEYED_D32` on DD rows, per-leg detail JSON and the report | anchors file, manifest, RESULTS |
| P3 | The four non-DD limbs `MATCH` on every leg — a `MISMATCH` is a finding recorded here, never a tolerance widened | manifest summary section |
| P4 | DD limb per D32 and nothing else: overlap measured and recorded per leg; non-overlap legs `WITHIN_BOUND` (any walk below the panel, whatever the gap) / `COINCIDENT`, or a `MISMATCH` BLOCKER (walk above panel by more than the inclusive cent) reported as a finding; overlap or tie legs `RECORDED`. No symmetric tolerance is ever applied to the panel DD | per-leg DD rows |
| P5 | Every pin naming the old config hash moved in the same commit (two tests, `VERIFICATION.md`, `RESULTS.md`, manifest); every generation-dependent assertion of `test_committed_manifest_matches_frozen_five_strategy_acceptance` refreshed from the artifacts (the test passes on both runtimes); **every row of `VERIFICATION.md`'s frozen-hash table plus its detail-hash line equals this generation's bytes**, and its invocation evidence carries all FOUR code hashes including `tv_summary_reconciliation.py` — the orchestrator recomputes the TRACKED ones (config, anchors, manifest, RESULTS); the eight ignored artifacts — the five detail reports under `local_artifacts/…/strategy_reports/` and the three ledgers — are not in the PR worktree, so for them the check is three-way consistency: the manifest's digests, VERIFICATION.md's rows and the `sha256sum` lines the worker pastes in the PR body must agree, with no private handoff (they derive from vendor rows and stay local); `git_base_commit` names the branch's real `main` base; the twenty source hashes unmoved; generation v4; and VERIFICATION.md's prose no longer calls the generation v3, no longer says G1.4 remains NEEDS_CONTEXT for missing panels, and no longer calls the +$287 delta unresolved (D30) | `grep` the old hash — only this file's historical mentions may remain; `sha256sum` on the tracked artifacts vs the table; the test's own run |
| P6 | Every README claim population falsifies is updated — the D26-pending paragraph, the verdict-cap-remains-NEEDS_CONTEXT sentence, the empty-inventory / five-key-policy / "max drawdown USD" paragraph, the v3 generation string — and no still-true limitation was touched; `coverage_note` no longer says "pending operator discrimination" | README diff read line by line, anchors |
| P7 | Publication by role — zero override values, zero live-account identifiers or figures, zero local filesystem paths (an operator username lives in one) — in tracked files, commit messages and the PR body; nothing under the study's private directory in ANY commit of the PR range, not only the final tree; and no private CONTENT in any commit's patch — a value pasted into an allowed file and removed later is the same disclosure (a file or a line added then deleted stays fetchable from PR history and cannot be purged under the no-rewrite rule) | full-diff role audit; `git ls-files lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` prints nothing; every commit in `origin/main..<head>` walked with `git ls-tree -r --name-only` for the private path, and `git log --diff-filter=A -- <private path> '*.csv' '*.pine'` over the range empty; then EVERY commit's patch read — `git log -p --no-color origin/main..<head>` — under a provenance rule on every added literal: a number, string or hash in a NEW line must trace to one of (a) the §47b step 2 table; (b) the SHA-256 of a named tracked artifact, recomputed here; (c) named-artifact metadata recomputed here — byte counts, line counts; (d) the base commit SHA; (e) reproducible command output stated WITH its command — test totals re-run here on the same files and matching, exit codes, hygiene facts — and the run's own timestamps and durations, accepted as reported when the command and its exit code are stated; (f) an unchanged or moved line; or (g) a value or sentence the frozen §47b instructions mandate — the `d17_policy` slot and its D32 ruling reference, `coverage_status: COMPLETE`, the anchor key names and `missing_metrics: []`, the `source_note` pattern (whose run facts — span, mode, capital, detalization, script execution — §18a already publishes), the runner version string, the `_BASE_COMMIT` line, and the README/VERIFICATION sentences step 4 prescribes — each checked against the instruction that mandates it, or prose whose every fact traces to §47b, §47a or (a)–(f). A token that merely occurs elsewhere in the repo is NOT a source: a common scalar in a new line needs one of (a)–(e) or (g) or it is FALSIFIED, whether it is a private byte or an invented figure (the orchestrator cannot see the private bytes, so its content read is by provenance; the worker's field-level needle scan plus its step 6b (vi) provenance self-read are the other half — the PR body must carry the scanner's `needle classes:`, per-commit and `SCAN result=` lines for every push and state the self-read, and a body without them fails P7). Any disclosure: no replacement PR and no further push — replacement purges nothing, the commits stay fetchable whatever happens to the PR — and the operator rules (§26 precedent) (⚠ a repo-root `inputs/` pathspec inspects nothing relevant — four tracked files live under other `lab/**/inputs/` paths) |
| P8 | CI green on 3.11 and 3.12 | checks |

**After a clean read:** prerequisites 2 and 6 close; the freeze then waits on 1, 7, 8, 9. Dispatch order after this: prerequisite 1's scaling read (Codex, local, the pinned Pine), then 7 (CC solo), then the `n₂`/`n₃` freeze with the calculator.

### 47d — Codex round on this packet @ `3ed227a` (22:52Z): 2 P1 + 5 P2, all verified, all folded before dispatch

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | "VERBATIM from §18a" would feed `$27,996.05` / `63.64%` / `1,470.40` to a loader that hands anchor strings straight to `Decimal` | `Decimal("$27,996.05")` → `InvalidOperation` here; `_decimal()` has no normalization | §47b step 2 now demands canonical numeric strings and carries the orchestrator's normalization table; P2 names a table-vs-§18a discrepancy as the orchestrator's defect |
| **P1-B** | P7's `git ls-files inputs/` is rooted at the repo, not the study | repo-root pathspec: 0 files; the study's private dir: 0 files; other `lab/**/inputs/`: 4 tracked files | P7 pathspec is the study's full private-overrides path |
| P2-A | README also asserts the verdict cap remains `NEEDS_CONTEXT`, the inventory is empty, a five-key policy, and "max drawdown USD" | README lines read on `main` | step 4 and P6 enumerate every falsified claim; still-true limitations stay |
| P2-B | `VERIFICATION.md` pins anchors, manifest, RESULTS, five detail reports and three ledgers, not only the config | its "Frozen hashes" table read on `main` | step 4 recomputes every row; P5 recomputes the tracked ones |
| P2-C | The `04f` stub's residue still told the operator to fire A1 and C1 | own unmerged entry | residue rewritten to the live state (C converged; A2 owed; #304 to close) |
| P2-D | The brief's status claimed no worker had started when the re-freeze landed — contradicting §43 | status line read | sentence corrected to the established timeline |
| P2-E | Verification commands joined with `;` | text | `&&`, stated as deliberate |

The chat copy of the prompt handed to the operator before this round is **superseded**; §47b as amended is canonical and was re-issued.

**Round 2 on `e58afe9` (23:02Z): 1 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | P2's "panel DD within the loader's tolerances" applies a symmetric tolerance D32 never has — `_drawdown_comparison` returns `WITHIN_BOUND` for any negative `walk − panel` and `RECORDED` under overlap — so a valid population could be rejected, contradicting P4 | function read | P2 is a transcription check on anchor VALUES; tolerances only on the four non-DD limbs (P3); DD under P4 alone, which now says so explicitly |
| P2-A | `test_committed_manifest_matches_frozen_five_strategy_acceptance` asserts the v3 runner version, the verdict cap, the anchors hash, ledger / detail / monthly hashes and the manifest and RESULTS digests — step 4 named only its config hash, so step 5 had to fail on a correct generation | test read (lines 805–869) | step 4 refreshes every generation-dependent assertion from the artifacts, never guessed; P5 checks the test runs |
| P2-B | `tv_summary_reconciliation.py` — the module producing every D32 status — was not among the pinned code hashes, and the runner's hard-coded `_BASE_COMMIT` (`git_base_commit` in the manifest, asserted near line 320) names an old baseline | runner lines 66 and 883; the test at 320 | fourth code hash added; `_BASE_COMMIT` set to the branch's real `main` base with the assertion refreshed — the one line of runner code the job touches |

The chat copy is superseded again; §47b as amended twice is canonical.

**Round 3 on `3ed1b9f` (23:12Z): 2 P1 + 4 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The NEVER line forbade override values "in screenshots" and "opening vendor bytes", while step 0 requires reading the screenshots and parsing the pinned Pine bodies — unfollowable | text | NEVER now forbids PUBLISHING (commit, PR, tracked file, this artifact, anything leaving the machine) and says reading locally is the job; §47a matched |
| **P1-B** | `_BASE_COMMIT` was edited in step 4, after the one real run in step 3 that serializes it into the manifest — P5 could only be met by editing a generated artifact or running twice | `run_campaign()` writes `git_base_commit: _BASE_COMMIT` | new step 2b sets it before the run; step 4 points there |
| P2-A | Manifest row A called A2 `DISPATCHED`; SESSIONS and the brief's status say owed/awaiting the operator | row text | row A now says FROZEN, awaiting the operator's dispatch |
| P2-B | The run command ended in a sentence period read as a stray positional `.` | argparse: "unrecognized arguments: ." exit 2 | command on its own line, nothing after the output directory |
| P2-C | A2's blanket `target ∈ (0, 1)` rejects the exact zero-rate case — `size_for_power(0.0, 1.0)` is exactly 60 through the `p ≤ 0` branch | computed on `b5b3fb4` | A2 keeps A1's `(0, 1]` and makes `target == 1.0` valid only when `true_rate == 0`; pin `size_for_power(0.0, 1.0) == 60` added |
| P2-D | The §10 hygiene hook pipes into `head`, hiding a crash | `exit 7 \| head` → 0; with `pipefail` → 7 | output captured to a file, rc echoed and required 0 |

§47b as amended three times is canonical; the chat copies are superseded.

**Round 4 on `e61a50e` (23:21Z): 1 P1 + 3 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | The brief's §0.9 Packet C no-op probe greps for `FileNotFoundError` anywhere in the file, so it also fires on the falsified first return `0e7ab25` (guard in `_run`) and would tell a worker to return DONE | the falsified head matches the grep | probe now distinguishes a guard in `_run` (forbidden — proceed per §2-C) from a guard inside `build_report()` (DONE), and runs the behaviour tests when present |
| P2-A | §0.9's open-PR loop diffs the hard-coded 297/301/302 and ignores the live query | text | the loop consumes `gh pr list` output, or a complete operator-supplied `OPEN_PRS` list when `gh` is absent |
| P2-B | §45 still said A2 was "frozen … and dispatched" | text | "frozen in §6b, awaiting the operator's dispatch" |
| P2-C | `test_checked_in_operator_anchors_reject_stale_panels_under_d17` pins the empty inventory and the `+$287` note; step 5 would fail after population | test read (289–305) | step 4 replaces it with a populated-state regression; historical expectations only against an inline fixture |

§47b as amended four times is canonical.

**Round 5 on `becd7f1` (23:30Z): 1 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | §47a's fallback inferred an empty override set from five matching aggregates — an inert override or a different input set can reproduce them, so hashing a default snapshot as the original state would falsely close D26 | reasoning accepted; the binding D26 needs is to a declared state | fallback is now byte-identical re-export from the coded defaults (SHA-256 equal to the pinned `export_sha256`) recorded as "defaults reproduce the export; original state unrecoverable" — or the capture is impossible and the worker stops; the Strikers cannot take it (§19d) |
| P2-A | The status header still said A2 was dispatched | text | "awaiting the operator's dispatch" |
| P2-B | Step 3 read exit 3 as "override artifact" only; the runner returns 3 for every intake failure (source dir, export/Pine read/length/hash, overrides, header-less CSV) | `main()` and `verify_source_pair` read | step 3 lists every exit-3 cause, has the worker report the emitted diagnostic verbatim, and names exits 2 and 4 |

§47b as amended five times is canonical.

**Round 6 on `0033226` (23:37Z): 2 P1 + 1 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The normal screenshot route accepted the chart's current Inputs state without proving it produced the pinned export; only the fallback re-exported, so a drifted chart would be hashed as the run's state | reasoning accepted — the §19d `backtestMode` check is necessary, not sufficient | every snapshot is now bound by a fresh export from that state hashing to the pinned `export_sha256` (operator exports once more per chart; the session verifies before hashing; mismatch → finding + lost-chart handling); P1 and the PR body carry the attestation |
| **P1-B** | The brief's §0.9 C probe matched any `except FileNotFoundError` inside `build_report()` and did not gate on the tests, so a partial repair could print DONE | text | exclusive flow: `_run` guard → FORBIDDEN; else `build_report` guard whose `try` body is the `gh --version` probe **and** the behaviour tests pass → DONE; else proceed — self-tested on `main`, `0e7ab25`, `94fa685` |
| P2 | `gh pr list` defaults to `--limit 30` | gh documentation | `--limit 1000` |

§47b as amended six times is canonical.

**Round 7 on `194938b` (23:49Z): 2 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The `# default --limit is 30` comment I appended sat before the line-continuation backslash, so the pasted §0.9 block fails syntax on the following `\|\|` | `bash -n` on the extracted block: "syntax error near unexpected token `\|\|`" | if-form; the block passes `bash -n` |
| **P1-B** | P7 inspected only the final tree and base-to-head diff; a capture force-added in one commit and deleted in the next stays fetchable from PR history and cannot be purged under the no-rewrite rule | by construction | worker step 6b walks every commit of the branch for the private path before opening the PR; P7 does the same over the PR range; any hit is a disclosure — the PR is closed and replaced, and the ticket question goes to the operator (§26) |
| P2-A | `VERIFICATION.md`'s prose still calls the generation "final v3", says G1.4 remains NEEDS_CONTEXT for missing panels and treats the +$287 delta as unresolved | file read (lines 3, 37, 73) | step 4 refreshes the narrative and the test-evidence section; P5 checks those sentences |
| P2-B | A2's "exactly 1.0 at `k == n`" leaves the recurrence under-normalized by ~1e-9, so the near-one quantile lands at 7999 and the `< 8000` pin passes on a wrong answer; the true value is 7355 | exact integer arithmetic here: largest k with Σ C(8000,i)·9^i ≤ 0.999999999·10^8000 is **7355** (the A1 head returns 8000) | A2 (iii) requires full normalization; the pin is `== 7355` |

**Round 8 on `0cf6de5` (23:59Z): 1 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | Step 6b and P7 audited every commit for the private PATH only; an override value or an export row pasted into an allowed tracked file and removed in a later commit passes both, yet stays fetchable from PR history and cannot be purged under the no-rewrite rule | step 6b (`ls-tree --name-only`, `--diff-filter=A --name-only`) and P7 read: filenames only | step 6b scans every commit's full patch (`git log -p origin/main..HEAD`) against a needle file built outside the repo from the private bytes themselves (override key/value lines, export data rows), before the FIRST and EVERY later push; a pre-push hit is repaired on the unpushed commits, a post-push hit stops the packet for the operator — replacing the PR purges nothing (Codex's "replace the PR" is not a remedy: the closed PR's commits stay fetchable); P7 reads every commit's patch under a provenance rule (every added literal traces to the canonical table, a digest, the base SHA or existing text) and requires the worker's needle count per push; step 7's body carries that count |
| P2-A | The brief's §0.9 A probe was symbol-only (`grep -rln "def max_certifying_busts"`): a stub or partial calculator on `main` would make a worker return DONE; and a FIX-ROUND worker finds the symbol on its own branch by construction | brief §0.9 read; the A2 worker runs §0.9 before touching anything | §0.9-A's no-op is the §10 Packet A acceptance run on the current checkout — the tests, `--check`, and the three pinned CLI lines (`n=950`, `n=970`, `per_limb=0.803 joint_independent=0.518 joint_frechet=0.409`) — with an explicit fix-round clause (on a §6b round §0.9-A is never a no-op); self-run: the A1 head `b5b3fb4` takes the DONE branch, this checkout (no calculator) takes the proceed branch |
| P2-B | §10's "run on its own head" was a comment: only the `git diff` named the remote ref; the pytest, CLI and gate lines ran in whatever checkout the orchestrator occupied | brief §10 read (the §43–§45 gate reads were in fact run in scratch worktrees, but the pasted block does not do that) | §10 creates a detached worktree per packet at the returned head, runs every line inside it, then removes it; self-run on `b5b3fb4` (34 passed, `--check` OK, `--tier check` rc 0, three CLI lines exact) and on `2cc6e9a` (2 passed, `repo_hygiene.py` rc 0 with `gh available: False`, `--tier check` rc 0) |

**Round 9 on `49f47e6` (00:12Z 09-05): 2 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | Round 8's needles were whole lines with a 12-character cutoff: a compact serialization, a copied scalar or key/value fragment, or a subset of a CSV row matches nothing, and short override values fall under the cutoff; step 0 leaves the snapshot's serialization free and P7's "existing text" could excuse a common scalar | step 6b (i) and §47a line "the shape is free" read | needles are FIELDS built by parsing the private bytes — TITLE, PAIR (four serializations), distinctive VALUE, export CELL — matched with `grep -F -w`, no cutoff; trivial values are caught through TITLE and PAIR (an input name never legitimately appears in the footprint); a fixed adjudication rule (benign only inside runner-generated artifacts P5 attests, since an aggregate can coincide with a cell); hits reported by class, commit, file and line, never a token; P7 says a token occurring elsewhere is not a source |
| **P1-B** | P7's provenance whitelist (table, digests, base SHA, existing literals) has no slot for the byte counts, test totals, exit codes, timestamps and durations step 4 requires VERIFICATION.md to carry, so a correct re-freeze would be FALSIFIED | VERIFICATION.md read: bytes column in both tables, `374 passed … in 11.29s`, `2495 passed … in 377.04s`, publication timestamp | P7 accepts (c) artifact metadata recomputed here and (e) reproducible command output stated with its command (totals re-run and matched; timestamps and durations as reported with command and exit code) |
| P2-A | Step 3 said the intake diagnostic "names no private path", yet `_resolved_child` formats the resolved source root into `source directory is not a directory: <root>` (exit 3) and the config-read errors format the config path (exit 2) — a resolved local path carries the operator's username and the vendor-data location | `lab/research_utils/tv_trade_ledger.py` lines 604–607 and 282–295 | step 3 reports every diagnostic with paths replaced by `<path>`, naming the two path-quoting ones; NEVER line and step 7 forbid a local filesystem path; P7 audits for paths as a third role |
| P2-B | The §10 C subshell `(… > rh.txt; echo "rc=$?"; head -5 rh.txt)` prints the script's status but returns `head`'s — an automated read approves the very crash Packet C repairs | self-test: a script exiting 7 → printed `rc=7`, subshell status 0 | `rc=$?` captured first, output shown, then `[ "$rc" -eq 0 ]` last so the subshell's status is the script's; self-test: failing script → status 1, passing → 0 |

**Round 10 on `ce12984` (00:25Z 09-05): 1 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | The preconditions and the NEVER line forbade editing "any campaign artifact" — in plain reading the study's config, anchors, manifest, RESULTS, VERIFICATION and README, exactly the files steps 1–4 require | lines read: both said "any campaign artifact" | the off-limits class is named (STATE, SESSIONS, ADRs, the plan, this artifact, every brief under docs/briefs/) and the study footprint is listed explicitly as the files to edit — config (five digest keys), anchors, manifest and RESULTS (regenerated), VERIFICATION, README, run_phase1.py (`_BASE_COMMIT` only), the three tests — and nothing else |
| P2-A | The brief's §0.9 C probe accepted `gh --version` within three lines above the handler: a `try` that also wraps a git call after the probe would pass it and pass the git-absent test (pre-probe git calls raise first) while swallowing a git disappearance after the probe | `ast` read of the converged head: the guarded body is the `_run(["gh", "--version"])` call plus one assignment from its result; a synthetic body with an added `_merged_via_git()` passed the old text probe | the probe parses `scripts/repo_hygiene.py` with `ast`: a `FileNotFoundError` handler in `_run` → FORBIDDEN; anywhere but `build_report` → proceed; in `build_report` DONE only when every call inside the guarded body is that one `_run` gh probe and the tests pass. Self-run: `main` → NONE/proceed, the first C return `0e7ab25` → RUN_GUARD/FORBIDDEN, the converged head → SCOPED/DONE, the synthetic partial → UNSCOPED/proceed |
| P2-B | The round-9 scan example (`git show … \| grep -v '^-'`) counted context lines, `+++` headers and metadata, so an unchanged neighbour line carrying a needle in a hand-written file would have stopped the one-shot job as a disclosure | self-test on this branch's head: the round-9 form counted a context-only needle (2), the added-only form counted 1 and the message 0 | step 6b (ii) scans `+` lines only (excluding `+++`) and commit messages separately, locates a hit by the `+++ b/<file>` header and a line number, and says a needle in a context line is not a hit — it was already on origin/main |

**Round 11 on `5c26d8b` (00:36Z 09-05): 2 P1 + 2 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | P7's sources (a)–(f) had no slot for strings the packet itself mandates — the `d17_policy` slot, `coverage_status: COMPLETE`, the D32 ruling reference, a new `source_note`, the runner version — so a compliant re-freeze would be FALSIFIED by construction | P7 read against steps 2 and 4 | source (g): a value or sentence the frozen §47b instructions mandate, checked against the instruction that mandates it, or prose whose every fact traces to §47b, §47a or (a)–(f) |
| **P1-B** | The needle builder excludes booleans, short integers and short strings from VALUE, and TITLE/PAIR help only while the title travels with the value; a bare private `true` or `3` pasted alone scores zero hits under a "zero override values" guarantee | step 6b (i) read; a bare `true` or `3` as a needle would match every file | the guarantee is stated by half: the needle scan is byte-exact for bindings, distinctive values and export cells; a new step 6b (vi) provenance self-read before every push covers every other literal — a detached private scalar has no source in a hand-written file and is removed before the push; step 7 and P7 say so |
| P2-A | The §10 fence neither fails fast nor chains its commands, so a failing test or CLI pin is followed by a successful `git worktree remove` and the pasted block returns 0 | fence read | every required command feeds `RC` (`\|\| RC=1`), the CLI pins are checked with `tee /dev/stderr \| grep -q`, the file lists are compared exactly, gate wiring is a negative check, and the last line returns `RC`; no `set -e` (a pasted `set -e` kills an interactive shell). Self-run: both heads `AUDIT rc=0`; the A block with its ref pointed at `origin/main` returns 1 |
| P2-B | The §0.9 open-PR loop checked every open PR including the one that owns the current branch, so an A2 worker on `cursor/scripts-side-2026-09-04-p1` would find #305 touching Packet A's footprint and stop as BLOCKED | loop read; #305's diff is Packet A's footprint by construction | the loop skips the PR whose head equals the pushed tip of the current branch (`origin/<branch>`; absent on an initial dispatch, so nothing is skipped there) and checks every other PR; self-run on this branch with the five open PRs: #302 skipped, the other four listed |

**Round 12 on `7529097` (00:49Z 09-05): 1 P1 + 5 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | Step 6b (i) deleted the needle file after the scan while (v) re-ran only (ii)–(iv) for later pushes, so the second push's `grep -f` would hit a missing file, fail inside an assignment, and print blank counts | (i) and (v) read | (v) runs (i)–(iv) and (vi) for every push; (i) says rebuild each time; (ii) checks `[ -s "$NEEDLES" ]` first and says a blank or missing count is a STOP, never a zero |
| P2-A | §45 still said A2 (i) narrowed `target` to the open interval `(0, 1)` and that §6b recorded it — the blanket rejection round 3 overturned | §45 line read against §6b A2 (i) | the sentence is marked SUPERSEDED in place with the corrected rule restated (`(0, 1]`, `1.0` valid only at a zero true rate) and §6b A2 (i) named as the only statement to build from |
| P2-B | Step 0 wrote the snapshot, then ran the binding check, then recorded the result in the file, then forbade editing it — a literal reading edits a write-once artifact | step 0 read | step 0 is reordered: parse and transcribe in memory, BINDING CHECK before anything is written, then one atomic write (temporary name, rename) that already carries `reexport_sha256_matches_pin: true`; a mismatch writes nothing |
| P2-C | The §0.9 open-PR loop ignored a failed pull-ref fetch and failures of `rev-parse`/`git diff`, so it could finish over stale or missing refs without establishing no-overlap | block read | `OK` flag: a failed fetch, a missing ref or a failed diff clears it; the loop reports each and the last line prints either "overlap probe complete" or STOP/BLOCKED. Self-run: the five real PRs → complete; a bogus PR number → STOP |
| P2-D | §47a's fallback re-export restored span, mode, capital, detalization and coded defaults but not the per-source `Script execution` setting, which changes calculation events; a default-setting chart could hash differently and make a recoverable source look impossible | §18a read: 1 for Aegis and ORB-MNQ, 2 for DJ30, NAS100 and MGC | the fallback names the §18a setting per source as a required condition of the re-export |
| P2-E | Step 4 requires focused and full-suite totals on both runtimes in VERIFICATION.md, but step 5 ran seven files plus `--tier check`; CI's full suite is 3.11 only, so the 3.12 full-suite total had no source | step 5 read; `tests.yml` matrix | step 5 runs the focused set and the full suite on each interpreter (`py -3.11`/`py -3.12` or `python3.11`/`python3.12`), then `--tier check` once; the four totals come from those runs and are never carried over |

**Round 13 on `3f67bfd` (01:01Z 09-05): 2 P1 + 4 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The round-12 guard only echoed when the needle file was missing, then entered the loop; each `grep -f` failed inside an assignment and the audit completed "clean" | (ii) read | the four needle-file findings of rounds 12–13 share one cause — a pasted shell snippet — so the scan is now tracked, tested code: `scripts/private_content_scan.py` (this PR adds it, `tests/test_private_content_scan.py` with eight tests, and the regenerated REPO_MAP row) exits 3 and prints `SCAN result=ERROR` when a needle source is missing or a class is empty; only exit 0 is a pass |
| **P1-B** | The needle file sat at a predictable path (`${TMPDIR:-/tmp}/needles.txt`) — readable under a permissive umask, overwritable by a concurrent job, left behind by an interrupted scan | (i) read | there is no needle file: the scanner builds the needles in memory from the private bytes and writes nothing to disk |
| P2-A | The §10 fence's `git fetch origin` was the one required command outside `RC`, so a failed fetch audited stale local refs and still returned 0 | fence read | `RC=0` moves first and the fetch feeds it (`git fetch origin \|\| RC=1`); self-run on both heads still `AUDIT rc=0` |
| P2-B | P5 said the orchestrator recomputes the five detail reports as tracked artifacts; `run_phase1.py` writes them under `local_artifacts/…/strategy_reports/` and the study's `.gitignore` excludes `local_artifacts/` | `git ls-files` of the study: config, anchors, manifest, RESULTS, VERIFICATION, README, calendar, commission, runner — no reports | P5 recomputes the four tracked artifacts; for the eight ignored ones the check is three-way consistency (manifest digests, VERIFICATION rows, the `sha256sum` lines the worker pastes in the PR body, now required by step 7), with no private handoff |
| P2-C | The §0.9 STOP branch was an `echo`, so the block exited 0 after an incomplete overlap probe | block read | `if/else` with `false` on the STOP branch; self-run: real PRs → status 0, a bogus PR number → status 1 |
| P2-D | The step 6b shell snippet (assignments, loops, `$TMPDIR`, `grep`/`cut`) could not be pasted into PowerShell on the Windows host step 5 explicitly supports | (i)–(iv) read | the Python scanner runs under PowerShell or Bash; the preconditions now name the shell (Git Bash or PowerShell 7+; the only shell feature in the prompt is step 5's `&&`) |

**Round 14 on `9b07855` (01:15Z 09-05): 1 P1 + 3 P2, all verified, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | A screenshot or zipped export committed as a binary and deleted later showed only `Binary files … differ` to the text scan, and the suffix check covered `.csv`/`.pine` only — CLEAN while the blob stays fetchable | scanner read: `git show` text parse; no binary branch | every binary file added or modified in the range is a hit of class BINARY (from `git diff --numstat`, the `-\t-` rows); new test: a PNG committed then removed → exit 2 |
| P2-A | `git diff-tree` on a merge commit prints nothing without a merge mode, so a forbidden file introduced while resolving a merge escaped the suffix audit | `git diff-tree` semantics; the packet merges origin/main by design | every commit is read against its FIRST PARENT (`git rev-list --parents`; the empty tree for a root), for the content scan, the numstat binary check and the added-suffix check alike; new test: a side branch's PAIR needle and `.csv` merged with `--no-ff` are attributed to the merge commit |
| P2-B | The preconditions accepted any `origin/main` at or after `5ab079cb`, which does not contain the scanner step 6b now requires — a dispatch from the open review would reach the pre-push audit without it | the scanner lands with #302 | the preconditions require `scripts/private_content_scan.py` present on `origin/main` and its tests passing there, and say why |
| P2-C | Packet A's frozen scope said the REPO_MAP caption moves `63 → 64`; `origin/main` has 63 today, #302 adds one, so after A2's merge of `main` the generator demands `64 → 65` | `git ls-tree origin/main scripts` counts 63 `.py`; this branch 64 | the caption is whatever `--check` demands on the worker's merged tree (one more than `origin/main`'s count at merge time), never a number copied from the brief |

**Round 15 on `c7bc5e8` (01:27Z 09-05): 3 P1 + 1 P2, all reproduced, all fixed.** Every finding was against the scanner this PR adds; each was reproduced in a scratch repository before the fix and is now a regression test (15 tests).

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | `ls-tree --name-only` C-quotes a non-ASCII path under git's default `core.quotePath`, so `startswith(prefix)` is false and a force-added capture with such a name evades the gate | reproduced: `inputs/private_overrides/é.json` force-added → `SCAN result=CLEAN` with the matching prefix | every git call runs with `core.quotePath=false` and every path list is read NUL-delimited (`-z`); the same repro now reports `HIT class=PATH` |
| **P1-B** | Needles were matched against blob content only, never against filenames — a private title or value in a FILENAME is published and retained exactly the same way | reproduced: a file named `Aggressive.txt` (a VALUE needle) → CLEAN | the paths a commit introduces are matched against every needle class; a hit is class NAME, a disclosure anywhere |
| **P1-C** | An added line whose own text starts with `++ ` renders as `+++ ` and was consumed by the unconditional header check, so its needles were never tested | reproduced: an added `++ Aggressive` line → `added=0`, CLEAN | the `+++` header is recognised by parser STATE (header block only, never inside a hunk); the repro now reports `HIT class=VALUE … line=1` |
| P2 | A rename to a forbidden suffix is classified `R`, which `--diff-filter=A` omits | reproduced: `git mv a.txt captured.csv` with `--forbid-suffix .csv` → CLEAN | introduced paths come from `--diff-filter=ACR` with rename and copy detection, taking the destination |
| **(own, found while verifying P1-A)** | Round 14's first-parent read made a merge of `origin/main` report everything main contributed since the fork, as if the merge had introduced it. `main` tracks images (`docs/analytics/**.png`), so a merge window containing one would raise BINARY and stop the packet on its one iteration — a latent false positive, not a fired one: this branch's own two merges of main happened to carry only text (`scripts/repo_hygiene.py`, `tests/test_repo_hygiene.py`) and the live self-run is CLEAN | `git ls-files` finds tracked PNGs; the first-parent diff of this branch's merge `0fd2e39` lists exactly what main contributed | every commit is read against EVERY parent, keeping what differs from ALL of them: an ordinary commit is unchanged, a merge yields only its conflict resolution. Two tests: content and a `.csv` introduced by a conflict resolution are caught at the merge commit; merging a mainline that carries a PNG and a `.csv` from outside the range stays CLEAN |

**Round 16 on `1d6034c` (01:41Z 09-05): 2 P1 + 2 P2, both P1s reproduced before fixing, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | `str.splitlines()` breaks on `\v`, `\f`, `\r`, `\x1c`–`\x1e`, `\x85`, `\u2028`, `\u2029`, but git delimits patch records with LF alone: a needle after such a byte lands in a fragment carrying no `+` and is never scanned | reproduced: an added `safe\vAggressive` line → git emits ONE record, Python yields `['+safe', 'Aggressive']`, scanner reported `added=0` CLEAN | git text output is split on `"\n"` only, for the patch and the commit message alike; the repro now reports `HIT class=VALUE … line=1` and is a regression test |
| **P1-B** | A `NAME` hit printed the offending FILENAME verbatim — which contains the private token — breaking the scanner's "never a token" contract; worse, step 7 pastes that output into the PR body, so the value would be published | reproduced: `Aggressive.txt` → `file=Aggressive.txt`, the token present in stdout | every printed path is passed through the needle patterns: matches become `<redacted>` and the line carries `path_sha256=` (first 12 hex of the true path) so the worker can still identify it locally. The repro now prints `file=<redacted>.txt path_sha256=…` with **zero** occurrences of the token; the test asserts the token is absent from stdout entirely |
| P2-A | §10's `cmd \| tee \| grep -q` takes the pipeline's status from `grep`, so a calculator that prints the pinned line and then crashes still passes | Codex reproduced it with a producer exiting 7; the shape is plain `sh` semantics | each CLI pin captures the calculator's own status into `OUT=$(…) \|\| RC=1` and then greps the captured text; no `set -o pipefail`, which would persist in a pasted interactive shell |
| P2-B | Clause (v) demanded exit 0 before every push while clause (iii) and the PR-body contract both allow an adjudicated-benign hit — the scanner necessarily exits 2 for one, so a sanctioned coincidence inside a runner-generated artifact would block the one-shot push | the two clauses read against each other | the gate is CLEAN **or** exit 2 with every hit adjudicated benign under (iii) and recorded with file and line; one un-adjudicated hit is a STOP, and exits 3 and 4 are always a STOP |

**Round 17 on `cdab0a9` (01:53Z 09-05): 1 P1 + 2 P2, the P1 reproduced before fixing, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1** | The multi-parent read intersected each SIGNAL independently, so a path binary in one parent and text in the other lost both: the content hit exists only against the text parent, the binary hit only against the binary parent, and each intersection discards the other's evidence | reproduced exactly: a mainline parent holding a NUL-bearing `f.dat`, a feature parent holding text at the same path, and a merge resolving it to a line carrying a needle → `SCAN result=CLEAN`; `numstat` shows `1\t1` against one parent and `-\t-` against the other | a merge's own contribution is decided as a PATH SET first — those differing from EVERY parent, its conflict resolution — and the evidence for those paths is then UNIONed across parents rather than intersected per signal. A one-parent commit is unfiltered by construction, so no ordinary commit can lose evidence to the filter. The repro now reports both `HIT class=VALUE … line=1` and `HIT class=BINARY`; regression test added (17 tests) |
| P2-A | §0.9's opening `git fetch origin && git log …` did not fail closed: the later pull-ref fetch carries an explicit refspec and so does NOT refresh `origin/main`, leaving the overlap checks free to pass against a stale main | `git fetch <refspec>` semantics; the fence read | the opening fetch is an `if !` that stops the fence with a STOP/BLOCKED line and a non-zero status |
| P2-B | §0.9's Packet A no-op probe still used `python … \| grep -q`, whose status is `grep`'s — a partial calculator that prints the pinned prefix and then crashes would satisfy all three pins and produce a false DONE (the §10 block had been fixed, this one had not) | the fence read | each pin captures the calculator's own status into `OUT=$(…) \|\| A_OK=0` and then greps the captured text. Self-run on three cases: crash-after-print → proceed; the same calculator exiting 0 → DONE; this repo with no calculator → proceed |

**Round 18 (02:08–02:09Z 09-05): 2 P1 + 2 P2, both P1s reproduced before fixing. ⚠ FOLDED TWICE, INDEPENDENTLY** — a Cursor agent on `7b07498` (`c81d65a1`) and the orchestrator on `7533048` (`81ce2ec7`), each unaware of the other, on the same branch. The merge keeps the **Cursor** design for all three code findings, because the adversarial panel (§47e) showed the orchestrator's P1-A fix was a regression; the panel's own root-cause fixes are re-applied on top. Both folds are recorded because the duplication is itself the defect worth remembering: **two workers held the same claim, which is exactly what the claim manifest exists to prevent, and neither dispatch went through it.**

| # | Finding | Verified | Fold as landed |
|---|---|---|---|
| **P1-A** | Diff headers C-quote a path containing a tab, newline, quote or backslash **even under `core.quotePath=false`**, while NUL-delimited `name-status` does not — so a merge filter comparing the header path to the resolved-path set silently discarded every hit on such a file | reproduced: a merge resolution adding a VALUE needle to a tab-named file → `SCAN result=CLEAN` | header paths are **C-unquoted** (`_unquote_path`, simple and octal escapes, `a/`–`b/` prefix stripped) before the comparison. ⚠ The orchestrator's alternative — take each path's diff separately with a `:(literal)` pathspec and parse no header at all — also closed this case but **introduced a wider hole**: the pathspec is built from a string git's output was decoded into, so any name that did not survive that decode matched no tree entry and the blob was never scanned (§47e). It is not the design that landed |
| **P1-B** | `json.loads`/`dumps` collapses a non-canonical numeric spelling, so a snapshot value written `1.2500` produced only the needle `1.25` and an added line carrying the chart's own spelling matched nothing | reproduced: snapshot `1.2500`, added line `1.2500` → `SCAN result=CLEAN` | the **raw value lexeme** for every key is taken from the source text by regex and added to that key's VALUE forms alongside the canonical spelling, so exponent and trailing-zero spellings both match |
| P2-A | Round 17's fix promoted a whole PATH: when `main` and the branch edit *separate hunks* of one file, the cleanly merged file differs from both parents, so lines only `main` introduced were reported as merge-authored | reproduced: a harmless feature append merged with a main-side needle line → `HITS` for a line already on `main` | the path set still selects which files a merge may have authored, but an added line is kept only when it appears against **every parent that has a text view** of that path; a parent for which the path is binary is skipped, so a binary/text resolution cannot lose its text hit. BINARY evidence stays UNIONed |
| P2-B | The live `Open / next` pointer still told the operator to fire A2 and close #304 while the manifest in the same change recorded both as done — a pointer that invites a duplicate dispatch | own stub read against the manifest | the residue is refreshed to the manifest's state, and refreshed again in this merge for the #305 merge and the §48 checkpoint |

**Standing from here — operator ruling 2026-09-05.** Before every push on this branch the diff goes through an **internal adversarial panel** whose prompt is Codex's own question — *what input makes this control silently pass?* Independent lenses hunt for an input that carries private content into a commit while the scanner exits 0; every candidate is then handed to a separate refuter who must **rebuild the repro from scratch** and observe the bypass, or the finding is dropped. The diagnosis this answers: verification against the reported case proves the reported case is fixed and says nothing about the next input shape, which is the only thing Codex has been asking. The panel's confirmed findings are folded before the push and recorded in the round they belong to.

**Round 19 on `c81d65a` (03:53Z 09-05): 1 P1 + 2 P2 — all three already closed on `dbe9eb26`, none needing a further change.** Codex reviewed the Cursor fold before the merge landed, so every finding was answered by work already in flight. Each was still reproduced on the reviewed commit and re-probed on the merged head before the threads were resolved.

| # | Finding | Verified | Disposition |
|---|---|---|---|
| **P1** | A needle beginning with punctuation was suppressed by the unconditional LEFT word boundary: CSV cell `-88.00` against an added line `x-88.00` — the lookbehind sees `x` before the minus and rejects an exact disclosure | reproduced on `c81d65a`; on `dbe9eb26` the same repro reports `HIT class=CELL … line=1`, exit 2 | already closed, and by a wider fix than the conditional boundary Codex proposes: the boundary was wrong at BOTH ends and also broke the report (§47e), so matching is substring with no boundary at all. Codex's asymmetric fix would have closed this case and left the glued-to-`_` case and the verbatim-token print |
| P2 | The scripts-table caption read 64 while the tree carries 65 — `2a89348` brought `certification_power.py` in from #305 and this branch adds the scanner; `--check` exits 1 and `gates.yml` does not run it | reproduced on the reviewed tree | already closed by `b5aeb9f8`, which regenerates the whole block with `--write` (`OK — 65 rows match sources`). ⚠ Second drift of the same caption for the same structural reason: the count is a function of `main` at merge time and `main` moved twice under this branch |
| P2 | The live pointer had not advanced past the #305 merge, so an operator could wait for or re-dispatch work already on `main` | reviewed commit's base read | already closed — **but Codex's inference from the base commit is wrong in a way worth keeping.** #305 merged at the pure-merge head, so A2 never landed; the pointer must not say the work is done. It now says #305 merged WITHOUT A2 and names the three live items |

**Round 20 on `dbe9eb2` (04:06Z 09-05): 2 P1 + 2 P2, both P1s reproduced before fixing, all folded.** Both P1s are the same class and one the panel had not reached: **the scan read git through configuration the machine controls and the push does not carry.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | A `refs/replace/<commit>` entry redirects every git read to a substitute object. A commit whose real blob carries a needle was scanned as its harmless replacement — and a push publishes the real object and no replacement ref | reproduced: real commit replaced locally with a harmless tree → `SCAN result=CLEAN`, exit 0 | every git call runs with **`--no-replace-objects`**. The repro now reports `HIT class=VALUE … line=1` |
| **P1-B** | The `a/`–`b/` patch prefixes are stripped when a header path is decoded, but `diff.noprefix=true` (or a renamed `diff.srcPrefix`/`dstPrefix`, or `mnemonicPrefix`) makes git omit them — so a real path that itself begins with `b/` was truncated to its tail, no longer matched the raw name, and a merge's hits on it were dropped | reproduced on a merge resolution under `diff.noprefix=true` at `b/f.txt` → CLEAN; on a plain commit the hit fired but reported the wrong path | the prefixes and `core.quotePath` are **pinned per invocation** (`_GIT_PINS`), so the patch format is a property of the scan rather than of the host. ⚠ Pinning `diff.external` to empty makes git exit 128; an external differ and attribute textconv are handled per command instead |
| P2-A | The tests create filenames containing a tab, a carriage return, a glob character and bytes that are not valid UTF-8. Those forms cannot exist on Windows, so the fixture fails before the scanner runs — and the step 6b preconditions name Windows/PowerShell as a supported worker host and require this file to pass there. CI is ubuntu-only and would never have surfaced it | the preconditions read against the fixtures | the five POSIX-only cases carry a `posix_only` skip marker; the rest of the file runs everywhere |
| P2-B | The fleet brief's own status header still said A2 was dispatched and awaiting return, while §6 in the same file recorded the merge and the owed A3 | header read against §6 | the header is rewritten to the post-merge state: MERGED and NOT CONVERGED, three live items, A3 owed |

**Round 21 on `be77e4b` (05:09Z 09-05): 2 P1 + 2 P2, all reproduced, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | The weak-value notice printed the JSON KEYS whose values produced no needle — and an input title is itself a `TITLE` needle, so the line disclosed exactly what that class exists to catch, on the CLEAN path, in the output the worker pastes into the PR body | already closed on the pushed head; the adversarial panel found the same defect independently in the same window | the notice prints a **count**; `--show-paths` prints the names locally |
| **P1-B** | The author and committer identities live in the commit object and are published by the push, but `%B` returns only the message body — so a private value in an identity field scanned CLEAN | reproduced: `GIT_AUTHOR_NAME=Aggressive` on a harmless patch and message → `SCAN result=CLEAN`, with the value visible in `cat-file -p HEAD` | every walked commit's `%an/%ae/%cn/%ce` are scanned; a hit reports `<commit identity>` |
| P2-A | `scan_refs` enumerated EVERY local tag, so one unrelated, already-published tag whose message matched turned every otherwise-clean branch into HITS — and it over-approximated `--follow-tags`, which sends only tags MISSING on the remote and reachable from what is pushed | reproduced: a pre-existing tag at the range's base → HITS on a clean branch | scope is now what the push could publish: skip a tag already on the remote (`ls-remote`, when reachable) **and** skip one whose target is an ancestor of the range's base. A tag on the pushed commit is still caught. ⚠ A control that cries wolf on a normal clone is one nobody runs — the same lesson as the data-URI false positive in §47g |
| P2-B | §47b's own step 6b described a scanner that no longer exists: headers skipped, whole-word matching, `path_sha256=`, one emptiness rule. §47b is the copy/paste job the worker executes, so a worker would misread new hits and reject the required PR-body evidence | the block read against the implementation | the block is synchronised: recursive TITLE, the widened VALUE and CELL rules, header row included, case-insensitive substring matching, `path_index=`, the per-class emptiness rule, and the four needle-free classes including REF |

**Operator ruling 2026-09-05 — the adversarial panel is STOOD DOWN, and the loop's stop condition changes.** The panel had run four times (§47e–§47h) and found 29, 5, 5 and 3 confirmed bypasses; it is not run again. The stop condition moves from *"a round returns zero findings"* to **"a Codex round returns no P1."** The reason is measured, not assumed: across eighteen recorded rounds the finding rate is FLAT — 3.7 per round and 13 P1s in the first half, 4.2 and 15 in the second — so a zero-finding round would have been a sampling artifact rather than convergence, and the old condition was unreachable. P2s no longer hold the loop open; they are folded when found and recorded either way.

**Round 22 on `e5fa5da` (05:37Z 09-05): 5 P1, every one reproduced before fixing, all folded.** The stop rule is "a round with no P1", so this round does **not** stop the loop. Four of the five are the same shape: **a private value has more than one spelling, and the scan knew only one of them.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | Round 21's tag fix was **wrong**. It skipped a tag whose target is an ancestor of the range's base, but a tag OBJECT created now on an already-published commit is missing on the remote and is exactly what `--follow-tags` sends | reproduced: a new tag on `origin/main` carrying a needle → CLEAN, then `push --follow-tags` published it | scope is keyed on **remote presence**, never on target ancestry; with no remote listing available the scan **fails closed** and reports the tag. ⚠ The ancestry test was a convenience for the offline case and it silently narrowed the guarantee — the previous round's test pinned the wrong behaviour and is rewritten |
| **P1-B** | A JSON string may spell any character with an optional `\uXXXX` escape, and `json.dumps` never recreates one, so a copy spelled `\u0041ggressive` matched no needle | reproduced: `added=0`, CLEAN | enumerating escaped needle forms is combinatorial, so the **haystack** is searched in an unescaped view as well. Quoted source lexemes are also kept now — dropping them as "duplicating the parsed form" was the assumption that hid this |
| **P1-C** | `re.IGNORECASE` is **simple** case mapping: it does not equate `ß` with `SS`, nor any other expanding fold | reproduced: value `StraßeBank`, added line `STRASSEBANK` → CLEAN | needles carry their `casefold()` form and the haystack is searched folded as well |
| **P1-D** | A private value can legally contain a newline — a quoted CSV cell does — so its needle spans lines and no single added record could ever match it | reproduced: a two-line cell copied verbatim → CLEAN | runs of consecutive added lines are scanned as a **block** as well as individually; the hit reports the block's first line |
| **P1-E** | The CSV was read with `errors="replace"`, which destroyed every non-UTF-8 byte **at the source**: the CELL class was populated but held `U+FFFD` while git's output held surrogate escapes, so an exact byte-for-byte copy matched nothing | reproduced: a cp1252 export and an identical cp1252 paste → CLEAN | the export is read byte-faithfully with `surrogateescape`. ⚠ This defeated the alt-encoding needle forms added two rounds earlier — the needle side was fixed while the reader that feeds it was still lossy |

**63 tests**, green with and without a global git identity; pre-commit tier exit 0.

§47b as amended eighteen times is canonical.

### 47e — First adversarial-panel run (2026-09-05, operator ruling): **31 candidate bypasses, 29 confirmed, 18 rated P1**

Run on the round-18 diff, prompt as ruled: *what input makes this control silently pass?* Six independent lenses (paths and encoding · diff and combined-diff parsing · needle construction · commit graph and merges · binary and introduced-path detection · the invocation contract and the report itself), each finding handed to a separate refuter told to **default to REFUTED** and required to rebuild the repro from scratch and observe the bypass. 38 agents, 31 candidates, **29 confirmed, 2 refuted**. Every confirmed finding is a demonstrated `SCAN result=CLEAN` (or a leaking report) on a commit that introduces private content.

**The result the panel exists for: the orchestrator's own round-18 P1-A fix was a REGRESSION, and the panel caught it before it could ship.** Replacing the patch-header read with a per-path `:(literal)` pathspec closed Codex's C-quoting hole and opened a wider one — the pathspec is built from a string git's output was decoded into, so any name that does not survive that decode names no tree entry, git exits 0 with an empty diff, and the blob is never scanned. The version it replaced scanned it. Eighteen Codex rounds did not find this; the first panel run did, because it asked what input defeats the new code rather than whether the reported case is fixed. **The regression never reaches `main`:** the independent Cursor fold of the same round used header C-unquoting instead, and the merge takes that design (§47d round 18).

**Two root causes, both fixed here and re-applied on top of the merged design, accounting for eleven confirmed findings:**

| Root cause | What it let through | Fix |
|---|---|---|
| `_git` ran with `text=True` | Universal-newline translation rewrote every CR in git's output: a CR in a **filename** mangled the pathspec so the file was never diffed, and a CR **inside an added line** split the record into fragments carrying no `+` column, hiding everything after it. `errors="replace"` additionally destroyed any filename byte that is not valid UTF-8 — the regression above | git output is captured as **bytes** and decoded here with `surrogateescape`, so a CR survives and an undecodable byte round-trips back through `os.fsencode` into the `:(literal)` pathspec. The report re-encodes for display, and `path_sha256` is taken over the **true** bytes |
| `_WORD` counted `_` and digits as word characters | A private value glued into a longer identifier, filename or commit message matched **nothing** — and because `_redact` used the same patterns, the report **printed such a path verbatim**, publishing the token into the PR body it is pasted into | matching is **substring**, not word-bounded. A disclosure control fails closed: a coincidental hit is adjudicated under the step 6b rule, a missed one is published. The test that pinned the old behaviour is inverted, and four regression tests added (25 total) |

**⚠ Fifteen confirmed findings remain OPEN. `scripts/private_content_scan.py` is therefore NOT yet fit to be §47b step 6b's gate,** and step 6b is **not satisfied** by the current scanner. Nothing regresses from this — the population dispatch is already on hold pending the Codex loop and §48's captures — but the artifact must not claim a control the panel has shown is porous. The open set, by class:

* **Needle construction.** JSON needles come from exactly one top-level key one level deep, so a nested or sibling override map yields none. CSV `CELL` skips the header row and every cell without a decimal point or date, so an identifier as a column header or a plain-integer cell yields none, and a headerless vendor CSV loses its first data row. Number spellings cover the JSON lexeme and the canonical float only — a thousands separator, a currency symbol, or **a percentage that inverts back to the value** all pass. String needles are case-sensitive, single-line and un-decoded, so a lowercased tag, a percent-encoded one, or a value reflowed across a line break all pass. Content that is not UTF-8 still misses a UTF-8 needle.
* **Fail-closed BINARY.** The class is decided by repo-local state that is never part of the push — a `.gitattributes` `diff` override, tracked or merely present, or `.git/info/attributes` — so a screenshot can be reclassified as text. A screenshot in any text-safe carrier (base64 sidecar, a notebook with an embedded image, a long ASCII preamble) is never BINARY. A git-lfs pointer scans CLEAN while the same push publishes the image to the LFS remote. A merge that introduces a binary file whose name contains a tab loses the class to the resolved-path intersection.
* **Path tests.** Case- and segment-sensitive: a differently-cased forbidden suffix, a differently-cased prefix, and a *directory* bearing a forbidden suffix all pass.
* **What the walk cannot see.** `origin/main` is a **local tracking ref**, so a stale one excludes commits the push actually publishes. An **empty walk reports CLEAN** — exit 0 does not distinguish "nothing to find" from "never looked". Ref names and annotated-tag messages are outside the range entirely, and `push --follow-tags` publishes them.
...
| # | Finding | Fold |
|---|---|---|
| **P1 — the comment that lied** | The previous commit added a comment asserting that an external differ and attribute textconv "are pinned off per command with `--no-ext-diff`/`--no-textconv`". A grep for those flags over the whole file returned **only that comment**. So `diff.external`, an attribute `textconv`, or an inherited `GIT_EXTERNAL_DIFF` replaced the patch text wholesale, `_content_vs` saw no `@@` and no `+` lines, and **every text class went silent at once, on every commit, for every file** — CLEAN on a plain-text paste. This is not exotic: difftastic and delta users set `diff.external` in `~/.gitconfig`, exactly the "property of the machine it runs on" the pins were added to eliminate | the four diff call sites pass `--no-ext-diff`/`--no-textconv`, and `_git_env` drops `GIT_EXTERNAL_DIFF` so the caller's shell cannot hand one in. **A test now asserts the flags are in the code**, because the comment drifted from it once already |
| **P2 — the one unpinned call** | `_blob_is_opaque` was the single git invocation not routed through `_git`, so it missed `--no-replace-objects` and the format pins the same commit had just introduced. A `refs/replace` entry on a **blob** re-opened the fail-closed binary check | every git call now goes through `_git_raw`, which owns the argv prefix; `_git` decodes, `_git_raw` returns bytes. A new call site cannot miss the pins |
| **P2 — wrapped base64** | The armour test required 512 **contiguous** base64 characters. Base64 is conventionally wrapped at 64 or 76 columns, which breaks every long run while leaving the payload intact | a density rule over the whole blob: eight or more base64-shaped lines that are at least half the non-empty lines |
| **P2 — the short token** | A 4–5 character all-alphabetic identifier cleared no distinctiveness bar, so it produced **no needle in any class, silently** — and when such a token appeared in a path flagged for another reason, the report printed it **verbatim**. The threat model names an account identifier as class one | values too plain to be needles are carried in a **redaction-only** set: never used to detect a hit, so no false positives, but they redact the report. The drop is reported **by key**, never by value |
| (refuted) | "A `--path-prefix` that can never match reports CLEAN" | refuted: the staged input contained no private content, so it demonstrated no disclosure |

**A false positive the run's own fix created, caught before the push:** requiring only the `data:…;base64,` marker made every file that *mentions* a data URI opaque — including this module's own tests, which turned the live self-run red. Each armour alternative now requires a real payload (256+ base64 characters). A control that fires on every mention is one nobody runs.

**49 tests**, green with and without a global git identity; live self-run `SCAN result=CLEAN commits=55`.
