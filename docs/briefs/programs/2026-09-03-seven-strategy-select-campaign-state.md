...
### 47a — Operator step first: the five current-input captures (§22a as ruled; 200K halves struck)

For each of the five sources, on the TradingView run that produced the pinned 2026-09-03 export (the §18a panel is that run's own), open Properties → **Inputs** and capture **every input that differs from the script's default** — entry, exit, risk, session, all of it; not only Properties' cost/detalization fields. Save each as a private file at `lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json`, with `<strategy_id>` one of `aegis_6j1`, `orb_mnq_recon_v7`, `striker_dj30_mym_pyramid_250`, `striker_nas100_mnq_dow_wed_excluded`, `vanguard_mgc_v04`. The directory is gitignored on `main` (verified in the real layout, §29); the runner hashes each file's **exact bytes** without parsing it, so the shape is free — a JSON object carrying the run identity (`Sep 1, 2022 — Sep 2, 2026`, Deep, 100K, Default detalization, the `Script execution` value) and an `inputs` map is enough. **Write each file once**: any later edit changes its digest and forces a second re-freeze. Values never go into chat, a commit, a PR, a screenshot committed anywhere, or this file. If a run's input state cannot be recovered, say so and stop — an unrecoverable capture must not become an assumed one (§22a).
...
Preconditions — stop and report if any fails: origin/main at or after 5ab079cb (#301 merged); the five operator-written files lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json exist for aegis_6j1, orb_mnq_recon_v7, striker_dj30_mym_pyramid_250, striker_nas100_mnq_dow_wed_excluded, vanguard_mgc_v04; `git check-ignore -v` resolves each to the study's inputs/private_overrides/ rule and `git status --porcelain` shows none of them.
...
7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: the five digests (digests only), the per-leg statuses from step 6, the new config hash, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0; live-account identifiers 0; live-account dollar figures 0.
