...
| 2 | Fresh independent **TV Key-stats panels** for all five sources (G1.4) | ⚠ **PARTIAL — NOT discharged** (corrected §20). Trade-count/net/win-rate/PF limbs anchor and the venue-bound trio reproduces §13a's −1.86% from a separate artifact. **But `max_drawdown_usd` mismatches on ALL FIVE** (closed-trade vs excursion-inclusive basis) and the runner emits `TV_SUMMARY_MISMATCH` **severity BLOCKER** at $0.01. Needs a D17-style policy ruling or a basis reconciliation |
...
Until 1, 2, 3 and 6 clear, `phase1_verdict_cap` stays `NEEDS_CONTEXT`, every strategy stays `BLOCKED_EXPLORATORY`, and
