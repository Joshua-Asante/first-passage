...
bites only when a leg shows a residual; then the runner BLOCKs where D29 would adjudicate. **Work owed under
D29, scoped as a follow-up:** the coincident limb, the per-leg ordering check (a leg whose bound falls *below*
the panel is a finding), and the `d17_policy` residual path.
