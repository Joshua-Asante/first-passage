...
| Phase 1 normalization (+ folded inventory) | Codex, [PR #283](https://github.com/Joshua-Asante/first-passage/pull/283) @ `809bbb4` (base `3522d63`) | **RETURNED — `NEEDS_CONTEXT`** | #283 open; not merged by the orchestrator | 19 files: spec + plan, study dir (config, fee + early-close captures, runner, manifest, RESULTS, VERIFICATION, README), three `lab/research_utils` modules, four test modules. Frozen hashes on the branch: config `8881a2af…`, events `03efac85…`, trades `900002b8…`, weekly `5bdcef07…`, calendar `a368dc61…` |
...
# Re-anchor round on #283 (verdict NEEDS_CONTEXT, 2026-09-03): re-read only the delta — renamed IDs + pin status (D10),
# early-close rows (D12), the _exposure_bounds causality fix + regenerated manifest hashes, the joint-flat deferral
# note, __init__.py, the iteration line — then re-verdict. Still: never open vendor bytes; never merge #283.
...
1. **Identity (G1.2, D10):** rename the four Striker IDs so no un-pinned body carries a locked name; add
   ...
   exports stand; re-freeze the config.
...
trades span the deadline; per-strategy peak exposure 80/80 (Aegis), 76–77/80 (each Striker), 4–6/80
(ORB-MNQ, MGC). The exported set is not a venue-legal book at these sizes.
