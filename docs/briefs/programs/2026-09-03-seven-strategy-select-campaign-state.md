...
| P2 | Anchors equal §18a on all five after canonical normalization (the §47b table; a discrepancy between that table and §18a is an orchestrator transcription defect to fix here, never a value the worker "corrects") — count exact; net, win rate, PF, panel DD within the loader's tolerances; `coverage_status: COMPLETE`; `d17_policy.max_drawdown: OVERLAP_KEYED`; `OVERLAP_KEYED_D32` on DD rows, per-leg detail JSON and the report | anchors file, manifest, RESULTS |
...
| P4 | DD limb per D32: overlap measured and recorded per leg; non-overlap legs `WITHIN_BOUND` / `COINCIDENT`, or a `MISMATCH` BLOCKER reported as a finding; overlap or tie legs `RECORDED` | per-leg DD rows |
| P5 | Every pin naming the old config hash moved in the same commit (two tests, `VERIFICATION.md`, `RESULTS.md`, manifest), **and every row of `VERIFICATION.md`'s frozen-hash table plus its detail-hash line equals this generation's bytes** — the orchestrator recomputes the tracked ones (config, anchors, manifest, RESULTS, five detail reports); the twenty source hashes unmoved; generation v4 | `grep` the old hash — only this file's historical mentions may remain; `sha256sum` on the tracked artifacts vs the table |
