...
| P7 | Publication by role — zero override values, zero live-account identifiers or figures — in tracked files, commit messages and the PR body; nothing tracked under the study's private directory | full-diff role audit; `git ls-files lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` prints nothing (⚠ a repo-root `inputs/` pathspec inspects nothing relevant — four tracked files live under other `lab/**/inputs/` paths) |
