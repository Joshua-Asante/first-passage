...
**The result the panel exists for: round 18's own P1-A fix was a REGRESSION.** Replacing the patch-header read with a per-path `:(literal)` pathspec closed Codex's C-quoting hole and opened a wider one — the pathspec is built from a string git's output was decoded into, so any name that does not survive that decode names no tree entry, git exits 0 with an empty diff, and the blob is never scanned. The previous version scanned it. Eighteen Codex rounds did not find this; the first panel run did, because it asked what input defeats the new code rather than whether the reported case is fixed.

**Two root causes, both fixed in this commit, that account for eleven confirmed findings:**
