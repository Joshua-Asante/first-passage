...
| 8 | **The 5.0% criterion's decision rule** | ✅ **RULED (ii) 2026-09-04 (D28)** — bound from the independent stage-2 paths alone; stage 1 is a pure screen. Work owed: freeze the disjoint seed stream, size n₂ for the bound, and record the unaddressed cross-configuration multiplicity |
| 4 | Four bar-panel hashes | ✅ accepted (this section) |
| 5 | §14e generation on `main` | ✅ merged (§16g) |

Until 1, 2, 3, 6, 7 and 8 clear, `phase1_verdict_cap` stays `NEEDS_CONTEXT`, every strategy stays `BLOCKED_EXPLORATORY`, and
...
grep whose pattern omitted `192.90`.** The pattern covered four of the five figures. A verification that does not
...
**Minimum `n₂` to certify ≤5.0% at a given true rate:** p=0% → **60**; p=0.5% → **60**; p=1% → **100**;
p=2% → **160**; p=3% → **340**.

**So discarding stage 1 from the bound costs very little.** Even a configuration sitting at a true 3% bust
rate — close to the 5.0% ceiling, i.e. the hard case — certifies on ~340 independent paths. At `n₂` in the
low thousands the bound has real headroom. **The compute saving that motivated two stages survives the
repair**, which is what makes (ii) the right call rather than merely the safe one; my earlier framing
treated the power loss as the cost to weigh, and the numbers say it is small.
...
1. **Disjoint seed stream.** Stage-2 paths must not reuse, extend, or derive from stage-1 seeds. If they do,
   ...
   wrong.
2. **`n₂` is sized for the bound, not for confirmation.** It now carries the entire decision. The table above
   is the sizing authority; pick `n₂` against the worst true rate worth certifying, not against a screen.
...
on. **Recommendation: `UNESTABLISHED`, prerequisite 3 closes NON-BLOCKING.**
