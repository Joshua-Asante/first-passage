...
**Round 18 on `7b07498` (02:08Z 09-05): 2 P1 + 2 P2, both P1s reproduced before fixing, all folded.**

| # | Finding | Verified | Fold |
|---|---|---|---|
| **P1-A** | Diff headers C-quote paths containing tab/newline/quote/backslash even under `core.quotePath=false`, while NUL-delimited name-status does not — so a merge filter comparing the header path to the resolved-path set dropped every content hit on such a file | reproduced: merge resolution adding a VALUE needle to `odd\tname.txt` → `SCAN result=CLEAN` | `_unquote_path` decodes C-escapes (and strips `a/`/`b/`) on every `+++` header before membership tests; the repro now reports `HIT class=VALUE`; regression test added |
| **P1-B** | `json.loads`/`dumps` collapses a non-canonical numeric spelling (`1.2500` → `1.25`), so a worker who copied the chart's lexeme into an added line missed the VALUE needle | reproduced: snapshot value `1.2500`, added line carrying `1.2500` → CLEAN | `_raw_lexemes_by_key` keeps every source numeric lexeme alongside the canonical form; the repro now hits; regression test added |
| P2-A | When main and the packet edit different hunks of the same file, the merged file differs from both parents, so path-level union of added lines attributed a main-only needle to the merge | reproduced: `main..HEAD` after a clean co-touch merge → `HIT class=VALUE` on the merge for a needle only main added | TEXT evidence on a merge is the intersection of `(class, file, line-text)` across every parent that has a text view of the path; a binary-view parent is skipped (preserving the round-17 binary/text case). The co-touch repro is now CLEAN; regression test added (20 tests) |
| P2-B | The live SESSIONS Open/next still told the operator to fire A2 and close #304, both already done on this branch | `04f` Open/next vs the fleet manifest / status header | new stub `2026-09-05c` advances the pointer: A2 dispatched, #304 closed without merge; remaining residue is A2's return and the §47 loop |
