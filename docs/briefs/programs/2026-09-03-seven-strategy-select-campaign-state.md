...
0. PRIVATE SNAPSHOTS (write once). For each of aegis_6j1, orb_mnq_recon_v7, striker_dj30_mym_pyramid_250, striker_nas100_mnq_dow_wed_excluded, vanguard_mgc_v04: parse every input.*() call in that source's pinned Pine body (title and coded default); transcribe every input's displayed title and value from the operator's screenshots; write lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/<strategy_id>.json as a FULL snapshot: {"strategy_id", "captured_at", "run": {"span": "Sep 1, 2022 - Sep 2, 2026", "mode": "Deep", "initial_capital": 100000, "detalization": "Default", "script_execution": <1|2>}, "inputs": {<title>: <value> for every input}, "differs_from_body_default": [<titles>]}. BINDING CHECK, per source, before anything is hashed: SHA-256 of the operator's fresh export taken from the snapshot state must equal that source's export_sha256 in phase1_config.json; record "reexport_sha256_matches_pin": true in the private file. On a mismatch the snapshot is NOT bound: STOP for that source, report the row-level diff on dollar columns (§19g — never percent columns) as a finding, and treat the source as lost-chart per §47a's fallback; never hash a snapshot the re-export did not confirm. Validity check from campaign-state §19d: for BOTH Strikers, backtestMode must be in differs_from_body_default — necessary, not sufficient; the binding check above is the proof. For the other three the list may be empty; record it either way. After writing, never edit these files again. `git status --porcelain` must not show them. Report only counts (inputs captured, inputs differing) and the binding result (re-export matched pin: yes/no) — never a title-value pair.
...
5. VERIFY on 3.11 AND 3.12: python -m pytest -q tests/test_tv_trade_ledger.py tests/test_tradeify_phase1_runner.py tests/test_tradeify_phase1_evidence_integration.py tests/test_tv_summary_reconciliation.py tests/test_tradeify_drawdown_policy.py tests/test_tradeify_d27_excursion.py tests/test_trade_reconciliation.py && python scripts/gate_manifest.py --tier check — the && is deliberate: a failed test must stop the line; both exit 0.
...
§47b as amended eleven times is canonical.
