...
**Fallback if a chart state is gone.** Reset the strategy's inputs to defaults and re-run the pinned body over the same span; if the Key-stats panel reproduces §18a to the cent, that source's override set is empty and the snapshot records exactly that. If it does not reproduce and the state is gone, the capture is impossible for that source — say so and stop (§22a: an unrecoverable capture must not become an assumed one); that is a ruling, not a worker call.
...
Exit must be 0. Exit 3 means an override artifact is missing, unreadable or mismatched — report it; never synthesize a digest or a file. Publish the regenerated reconciliation_manifest.json and RESULTS.md (v4 generation).
