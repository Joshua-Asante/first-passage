...
6b. HISTORY SELF-AUDIT — CONTENT as well as paths, BEFORE EVERY PUSH (not only before opening the PR). A path check is necessary, not sufficient: an override value or an export cell pasted into an allowed tracked file (phase1_config.json, a test, VERIFICATION.md) and removed in a later commit passes every path check and every base-to-head diff, yet stays fetchable from the PR's refs — nothing is rewritten here and a closed PR's commits remain fetchable — so the control is a serialization-independent scan of every commit's added lines and message, run before the bytes leave this machine. (i) NEEDLES ARE FIELDS, NEVER LINES — the private files' serialization is free (§47a) and a copied fragment matches no whole line. Build `NEEDLES="${TMPDIR:-/tmp}/needles.txt"` outside the working tree by PARSING the private bytes (never by hand), four classes: TITLE — every input title in each snapshot's `inputs` object; PAIR — every (title, value) of `inputs` in four serializations, `"title": value`, `"title":value`, `title: value`, `title=value`; VALUE — every `inputs` value that is distinctive (a string of at least 4 characters, or a number with a decimal point or at least 4 digits) as a bare token; CELL — every cell of every data row of the five frozen exports that carries a decimal point or a date, as a bare token. Trivial values (booleans, short integers, short strings) are covered by TITLE and PAIR only while the title travels with them: a Pine input's name never legitimately appears in this packet's footprint, so a TITLE hit is a hit whatever the value — but a DETACHED trivial scalar can be no needle (`true` or `3` matches every file) and is caught by (vi) instead. Drop from CELL any token equal to one of the twenty-five canonical anchor strings (step 2 — public on `main`) or to a value already tracked in phase1_config.json on origin/main. No length cutoff; matching is `grep -F -w` (fixed strings on word boundaries). Delete the file after each scan and REBUILD it from the private bytes for the next push — (i) is part of every per-push run, never a one-time step. (ii) SCOPE: ONLY the ADDED lines of every commit's patch (`+` lines, excluding the `+++` header — never context lines, deletions, `diff`/`index` headers) and, separately, every commit message in origin/main..HEAD — counts per commit, the needle file checked first: `[ -s "$NEEDLES" ] || echo "STOP: needle file missing or empty — rebuild (i) first"; for c in $(git rev-list origin/main..HEAD); do a=$(git show --format= --no-color "$c" | grep '^+' | grep -v '^+++' | cut -c2- | grep -F -w -c -f "$NEEDLES"); m=$(git log -1 --format=%B "$c" | grep -F -w -c -f "$NEEDLES"); echo "$c added=$a message=$m"; done` — and for each hit the class, commit, file (the `+++ b/<file>` header the added line falls under) and line (`grep -n -F -w -f "$NEEDLES" <(git show "$c:<file>") | cut -d: -f1`), NEVER the token. A needle sitting in an unchanged context line is not a hit: it was already on origin/main. A blank or missing count is a STOP, never a zero — `grep -f` against a missing file fails inside the assignment and the loop still completes. (iii) ADJUDICATION, fixed rule: a hit is BENIGN only when it sits in a runner-generated tracked artifact (reconciliation_manifest.json, RESULTS.md, the five detail reports) whose bytes P5 attests are the runner's output — an aggregate can coincide with a cell (a single-trade month's total equals that trade's P&L) and those artifacts are the sanctioned publication; a hit anywhere else — config beyond the five digest keys, a test, VERIFICATION.md, README, a commit message — is a disclosure, and a TITLE or PAIR hit is a disclosure anywhere. (iv) PATH scan, unchanged: `git rev-list origin/main..HEAD | while read c; do git ls-tree -r --name-only "$c" | grep -q '^lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/' && echo "PRIVATE FILE IN $c"; done` prints nothing, and `git log --diff-filter=A --name-only origin/main..HEAD -- lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/ '*.csv' '*.pine'` is empty. (v) Run (i)–(iv) and (vi) before the FIRST push and before EVERY later push — (i) rebuilds the needles each time. A disclosure BEFORE a push is repaired locally — the offending commits are unpushed, so `git reset --soft <the last pushed tip, or origin/main before the first push>`, re-stage without the bytes, recommit, re-scan until clean. A disclosure AFTER a push STOPS the packet: no further push and no replacement PR from the worker — replacement purges nothing, the commits stay fetchable from the closed PR's refs — report class, commit, file and line to the orchestrator with no token quoted; the operator rules on closure, replacement and the ticket question (campaign-state §26 precedent). The PR body reports, per push, the hits by class (each 0, or adjudicated benign with file and line) and the path-scan result. (vi) PROVENANCE SELF-READ, before every push, for what no needle can carry: read every added line of every commit and every commit message; every literal — a detached `true` or `3` included — must trace to one of P7's sources (a)–(g); a literal with no source is removed before the push (the offending commits are unpushed, as in (v)), and the PR body states that the self-read was done for each push. This is the worker-side half of P7's rule: the needle scan is byte-exact for bindings, distinctive values and export cells; the self-read covers the rest.

7. PR BODY, first line exactly: `PHASE1 population + re-freeze — $0 · K=0 · MC=none — constituent (i) iteration 6 of ≤8`. Then: per source the two counts from step 0 (inputs captured, inputs differing from the body's defaults — counts only, never names or values), the binding result (fresh export SHA-256 equal to the pinned export_sha256: yes, per source) and the digest, the per-leg statuses from step 6, the new config hash, the pins moved, test counts on both runtimes, and a publication-by-role sweep over every tracked line, every commit message and the body itself: chart-input override values 0 — by the needle scan for bindings, distinctive values and export cells, by the step 6b (vi) provenance self-read for every other literal, a detached trivial scalar included; live-account identifiers 0; live-account dollar figures 0; and the step 6b content-scan result per push — hits by class, each 0 or adjudicated benign with file and line, never a token. No local filesystem path anywhere in the body.
...
§47b as amended twelve times is canonical.
