...
1. **Identity (G1.2, D10):** rename the four Striker IDs so no un-pinned body carries a locked name; add
   ...
   (exclude Wednesday only vs the locked Mon + Tue) — name it as such, never as `_v1`. Final IDs ruled: `striker_dj30_mym_pyramid_250`, `striker_nas100_mnq_dow_wed_excluded`, `striker_dj30_qtxg1_port_on_mym`, `striker_nas100_qtxg1_port_on_mnq` (D10; the NAS100 ID made final 2026-09-03 — no `e.g.` remains); **D10 (ii) answered: the point value was not overridden — drop both swap-port entries** (`striker_dj30_qtxg1_port_on_mym`, `striker_nas100_qtxg1_port_on_mnq`) from the strategy list, record them under `dropped_sources` with reason `SWAP_PORT_BODY_POINT_VALUE_NOT_OVERRIDDEN` (hash, filename, pin ref, reason), remove their two rows from spec anchors §7.5–7.6, and re-freeze a five-strategy config; RESULTS and the manifest list five strategies plus the two dropped sources. **After #286 (D14, 2026-09-03):** `pine_filename` for the two modified bodies = the pinned candidate basenames `striker_dj30_v4.5_mym_pyramid_250.pine` / `striker_nas100_v1_mnq_dow_wed_excluded.pine` (the operator's `--source-dir` carries both), `pine_pin_status: PINNED_RESEARCH_VARIANT` with `pin_ref` to the `core/strategies/candidates/` line, `pin_divergence` as ruled ("pyramid 250% vs locked 750%" — full diff now confirmed pyramid-only; "day-of-week set {Mon,Tue,Thu,Fri} vs locked {Mon,Tue}"); the spec §7.5 table rows carry the ruled IDs and filenames; the reconciliation manifest's `pine_filename` echoes regenerate with the re-run. This supersedes the `pine_filename` hunk of #287, which is not merged separately.
...
3. **Early-close rows (G1.6, D12):** the committed `load_early_close_calendar` schema (`trade_reconciliation.py`)
   ...
   tests:** a top-level `sources` array of `{year, source_url, page_date, sha256}` — one per yearly CME calendar
   capture, hash of the captured file — and a `source_year` on every row that must resolve to one of them, exact-key
   checks kept. Then freeze the operator-supplied 2022–2026 early-close dates as rows and re-run (item 9 decides
