...
- **Routing:** ITERATE → two dated successor packets, per §6:
  1. **Branch-protection/ruleset authoring for `main`** — named, **not opened**. Scope this against the still-open W5 CI-derivation item (`docs/adr/2026-08-07-w5-governance-diet.md` §2) rather than duplicating it; needs its own operator GO — this is a repository security-setting change, handled separately from the doc-correction packet below.
