...
| 2026-08-20 | **U1 (ADMIT-ONE) exception granted, scoped to this construct only** — [`ADR`](../../adr/2026-08-20-dense1m-u1-operator-override-con4-reopen.md) (`Accepted`, operator override, no new evidence). Licenses scoring the already-reserved CONFIRM window (2025-09-01→2026-08-05) against this file's own frozen gate — no edit to any frozen constant, no new K. Not yet executed; CONFIRM remains unread until that run happens. Does not reopen CON-1/2/3/5 or mark U2. | Claude Code (operator-ratified) |
| 2026-08-20 | **CONFIRM scored → `AMBIGUOUS-HOLD`.** [`RESULTS_CONFIRM.md`](../../../lab/analysis/c1/mnq_tnec_con4_pdh_pdl_break_2026-08/RESULTS_CONFIRM.md): long n=113 mean +0.0067R CI[−0.0811,+0.0935]; short n=75 mean **−0.0611R** CI[−0.2006,+0.0806]; both placebo p ≫0.05, both halves disagree; gross/(4×RT) 0.35×. Neither the FALSIFIED nor live-pass trigger fires. Per the ADR's own §4 revert trigger: the **U1 exception is now spent — this cell reverts to `U0` (paused)**, same as `CON-1/2/3/5`. No further exception is auto-granted; a second attempt needs its own fresh ADR. This is the ninth consecutive zero-yield close in the short-horizon MNQ microstructure thread (now on out-of-sample data). | Claude Code (operator-ratified run) |
