...
| Approved source configured but disabled | **either** the A1b brief's config keys are already present on the volume (keys only; values never printed), **or** — the expected case, since no sub-track before A6 writes the daemon volume — a **reviewed pending write** is recorded here: the exact key names the A1b `load_config` requires, which of them are secrets, the non-secret values (`emit_enabled` false, `strategy` `null`, `m1_test.enabled` false, `bar_period_s`, `poll_interval_s`), and the operator's in-session confirmation that they hold the secret values and will perform the A6 Step 2.2b staging put themselves. Either form is GO; a key the A1b brief needs that neither form covers is NO-GO (owner: A1b). The expected disconnected `feed_mode` value is quoted from the A1b brief. | |
| Credentials at rest only on the daemon volume | for the present case: key names present in `/data/c1_signal_daemon_config.json`; for the pending-write case: the staging procedure in A6 Step 2.2b is the only path and the values never enter the repo, the listener volume, or an agent transcript. Either way: secret key names absent from the repo and from every log line sampled. | |
