...
At `target + 60 s` the bar that opened at `target` closes on the Tradovate chart of the manifest's `venue_contract`. The operator reads its O/H/L/C/V and, **between `target + 60 s` and `target + 120 s`**, writes the five values to a **private file on the daemon volume** in their own console (`/data/m1_bar_<ceremony_id>.json`, `{"open":…,"high":…,"low":…,"close":…,"volume":…}`; the values never appear in the session transcript, the readiness record or any public artifact — plan §1 item 10), then runs the command the agent pre-drafted before `target`: `python ops/c1_signal_daemon/m1_stage1_control.py inject --state … --config … --ceremony-id <id> --boot-id <boot_id> --contract <venue_contract> --time <target> --bar-file /data/m1_bar_<id>.json`. The CLI prints a value-free receipt (ceremony id, `bar_sha256`, UTC time) that the operator pastes into the record as actor proof, and deletes the private file after publishing. The CLI refuses (writing nothing) before `+60`, after `+120`, unless the journal **and** the config both show the ceremony enabled (a `READY` manifest after `prepare` but before `enable` is refused — the daemon would otherwise treat the ceremony as inactive and discard the sole injection), on a contract other than the manifest's, or on inconsistent OHLC; a refusal is not retried past the deadline — the ceremony expires and Step 2.7's no-POST branch applies. Injecting is the operator's act, like `enable`; the agent drafts, never runs it.
