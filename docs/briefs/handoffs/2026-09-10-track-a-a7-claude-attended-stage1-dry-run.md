...
### Step 2.1 — Pre-checks
Listener `--status` (every paste of it in this brief has `account=…` replaced by `account=<redacted>`; `_PRINTABLE` includes the account id) → `dry_run=True`, `armed_until=None`, `m1_gate … result=FAIL` (expected); anything else is the armed-host procedure (Track A plan §6: boot line, operator flatness attestation, `--disarm` + restart + verify, or alert without disarm if the operator cannot attest) and the ceremony does not run. Daemon `GET /` → `poll_interval_s:1` (**gate: any other value stops the session here**, before the listener allocation is touched in Step 2.2 — the running daemon caches the interval, so the remedy is the A6 operator re-stage + agent `fly deploy` + re-verify workflow, not a direct machine restart or a later `prepare`), `effective_emit:false`, `ceremony_state:"DISABLED"`; `status` CLI healthy; record daemon `boot_id`. Both `contract_sha256()` values (listener image, daemon image) printed in-container → **must be equal**. Listener ledger last `seq` recorded.
...
At `target + 60 s` the bar that opened at `target` closes on the Tradovate chart of the manifest's `venue_contract`. The operator reads its O/H/L/C/V and, **between `target + 60 s` and `target + 120 s`**, creates a private JSON file on their local workstation **outside the recorded console**, then transfers it with SFTP to the daemon volume at `/data/m1_upload_<ceremony_id>.json`. The raw values never appear in shell/editor input captured by the transcript, the readiness record, or any public artifact (plan §1 item 10). The operator then runs the command the agent pre-drafted before `target`: `python ops/c1_signal_daemon/m1_stage1_control.py inject --state … --config … --ceremony-id <id> --boot-id <boot_id> --contract <venue_contract> --time <target> --bar-file /data/m1_upload_<id>.json`. The CLI publishes to the **distinct** one-shot path `/data/m1_bar_<id>.json`, prints a value-free receipt (ceremony id, `bar_sha256`, UTC time) that the operator pastes into the record as actor proof, and removes the upload in a `finally` path on success or refusal. If the session aborts before invocation, the Step 2.9 close/teardown removes the upload; startup removes any orphan. The operator must not type or paste raw values in the recorded console. The CLI refuses (writing nothing) before `+60`, after `+120`, unless the journal **and** the config both show the ceremony enabled (a `READY` manifest after `prepare` but before `enable` is refused — the daemon would otherwise treat the ceremony as inactive and discard the sole injection), on a contract other than the manifest's, or on inconsistent OHLC; a refusal is not retried past the deadline — confirm the upload path is absent, then the ceremony expires and Step 2.7's no-POST branch applies. A stable per-ceremony injection claim serializes validation through publication, so concurrent or later attempts cannot replace the published bar. Injecting is the operator's act, like `enable`; the agent drafts, never runs it.

### Step 2.7 — The event
Watch `fly logs -a c1-signal-daemon` and the journal **until the attempt reaches a terminal state** — `RESPONSE_RECORDED`, `TRANSPORT_UNKNOWN`, or `CLOSED` (`expired` / `target_missed`) — bounded by `target + 120 s` (last permitted injection) + 1 s poll + the 30 s transport timeout + 10 s margin ≈ `target + 165 s`; never proceed to 2.8–2.11 while the journal reads `SEND_RESERVED` or `EMITTED`, because an injection near `+120` can leave the POST inside `default_transport`'s 30 s timeout when the manifest expires at `+150`. Expected: within ~1 s of the injection, the daemon consumes the one-shot bar and posts — exactly one `m1_b1_post status=200`; journal → `RESPONSE_RECORDED` with `response_kind:"dry_run_computed"`; `enabled` already false (flipped at reservation). If `target_missed` / `expired` closes the ceremony with no POST: record it, run the Step 2.9 close command in full (with `--state` and `--config`), disable the allocation (2.10), and return `DONE_WITH_CONCERNS` — a second attempt is a new session with a fresh ceremony id, never a retry here. If `TRANSPORT_UNKNOWN`: do not retry. Run the Step 2.9 close command in full (the journal keeps `previous_state: "TRANSPORT_UNKNOWN"`, which is exactly what blocks any fresh ceremony, and `close` also clears `emit_enabled` and `m1_test.enabled` in the config; a late response is recorded under the closed state without reopening it), then run Step 2.10 in full — the listener allocation back to cap 0 / `RETIRED` after flatness confirmation, and both posture checks — because the daemon's barrier does nothing for the listener row that Step 2.2 enabled. Record, then return `BLOCKED — plan-itself-wrong` (reconciliation is a separate review). Never edit the journal.
...
Pass 1 — spec compliance: one ceremony; `enable` **and `inject`** run by the operator (the transcript shows the agent drafting both commands and the operator running them — neither the journal nor the evidence projection records the CLI actor, so the transcript is the only evidence of operator authority); allocation back to zero; no acceptance edit. Pass 2 — quality: parent re-runs the projection from the retained host files and re-greps the ledger for the UUID (expects exactly three lines); parent confirms the manifest's `source` marker equals the ratified amendment's. The operator receipt's `bar_sha256` must equal both the journal and evidence projection `bar_sha256`; any mismatch rejects A7. Pass 3 — consolidated read of §A5–§A7.
