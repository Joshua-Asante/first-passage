...
The arming interlock reads the artifact **baked into the image**, and `m1_acceptance_reason()` validates it structurally without comparing its pins to the running bytes. After Step 2.3 the running image carries PR 1's artifact, whose `fixture_hashes` are the A5 values; if A1b moved any listener pin, that embedded record describes a different image than the one running while reading `PASS`. So, after PR 2 merges: the six pre-conditions again (host `--status` read; clean checkout of `main` at the PR 2 merge SHA; image-manifest test; pre-deploy in-container hashes of every **image-carried** pin equal PR 2's `fixture_hashes` for those keys exactly — the ops bytes did not change — and the tree-backed `tests/ops/test_m1_acceptance_drills.py` pin re-verified from the PR 2 merge-SHA tree); `fly deploy …`; boot line `dry_run=True armed_until=-`; health; in-container `--status` → `m1_gate: status='RESOLVED' result=PASS`; post-deploy in-container hashes of the image-carried pins **identical** to the pre-deploy ones (only the JSON changed) and the in-image JSON's own sha256 equals the repo file's. Record the release/machine/image in readiness §A8 via a notes-only PR 3 — no further deploy, because the JSON is not in its own pin set and the running record now describes its own bytes. If no pin moved in Step 2.3, skip this step: the baked pins already describe the running image.
