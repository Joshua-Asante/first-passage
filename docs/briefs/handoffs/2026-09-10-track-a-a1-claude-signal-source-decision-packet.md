...
2. For each option graded "viable", a one-page implementation sketch sufficient for the parent to freeze the A1b spec in one sitting: files touched (daemon side only), config keys, secret handling, Dockerfile/`.dockerignore` lines, tests, the `source` marker constant, expected time-to-first-event, cost.
   Ratification is not this session's act. After the operator rules, a follow-up docs-only edit (Track A task A1r, parent session) replaces the `PROPOSED` status line with `RATIFIED <date> — option <X>`, **rewrites the selection section to the operator's actual choice** when it differs from the packet's recommendation (the original recommendation stays as a dated historical line, never as the live selection), applies the chosen option's amendment text to the M1 ADR, and adds the change-history row; A6's Phase 0 hard-checks that edit, so an oral or PR-comment ruling alone does not unblock the daemon lane. Structure the addendum so A1r can do that cleanly: the status line in the exact `PROPOSED` form above; a distinct `### Selection` block holding the recommended option and its grounds; and each option's amendment text under its own `### Amendment text — option <X>` heading, so the ratified option's block is the one A1b freezes from and no rejected option's rationale survives as the live selection.
