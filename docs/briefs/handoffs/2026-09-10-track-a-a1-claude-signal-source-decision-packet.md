...
2. For each option graded "viable", a one-page implementation sketch sufficient for the parent to freeze the A1b spec in one sitting: files touched (daemon side only), config keys, secret handling, Dockerfile/`.dockerignore` lines, tests, the `source` marker constant, expected time-to-first-event, cost.
   Ratification is not this session's act. After the operator rules, a follow-up docs-only edit (Track A task A1r, parent session) replaces the `PROPOSED` status line with `RATIFIED <date> — option <X>`, adds the change-history row, and applies any amendment text to the M1 ADR; A6's Phase 0 hard-checks that edit, so an oral or PR-comment ruling alone does not unblock the daemon lane. Leave the status line in the exact `PROPOSED` form above so that edit is a one-line replacement.
