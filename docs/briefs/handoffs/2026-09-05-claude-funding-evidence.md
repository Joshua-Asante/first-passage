...
## 3.2 Parent clarification: deterministic arithmetic work budget

This clarification supersedes any reading of §3.1 that requires unbounded materialization. It establishes an explicit computation policy for this pure helper, not a strategy, account, sizing, or input-validity limit.

The default arithmetic work budget is **1,000,000 decimal coefficient digits**. Apply it to newly required coefficient construction/alignment work after context-independent, value-preserving trailing-zero normalization and zero, identity, and exact-equality cancellation shortcuts. Do not reject a fact merely because its exponent magnitude is large or an already supplied Decimal has many stored insignificant zeros. An exact identity result or exact zero must not require expanding a coefficient merely to preserve a preferred exponent.

Before constructing powers of ten for alignment, compare each normalized operand's actual required aligned span (coefficient digit count plus shift) with the budget. A true over-budget alignment may return OUTSIDE_PROVEN_DOMAIN / ARITHMETIC_LIMIT with all five witnesses None, even if nontrivial later cancellation could reduce the result. Exact-equality cancellation must shortcut first. This is an explicit bounded-work disposition; it is not INVALID_INPUT and must not alter the positive-funding decision for successfully computed witnesses.

Precision estimates can be one digit loose. Permit one bounded carry digit of scratch and decide borderline result size from the actual normalized result; do not reject a budget-sized result solely because a speculative carry or an identity factor adds to an upper estimate. Normalize insignificant result zeros before checking native Decimal exponent representability. Preserve exact subnormals and inclusive MAX_EMAX cases.

The budget must operate independently of sys.get_int_max_str_digits(), including when that setting is zero. Use coefficient conversion/reconstruction that does not require decimal-string conversion of large integers. Do not mutate interpreter-global settings. Catch genuine conversion, arithmetic, and reconstruction resource failures within the existing arithmetic-limit outcome, preserving validation/domain precedence and no partial witnesses.

Use one private/internal budget parameter or equivalent internal injection for inexpensive tests of the same algorithm; the public API remains unchanged. Cover under/at/over-budget work, one-digit carry headroom, identity factors, exact cancellation, normalization at MIN_ETINY, and disabled/restrictive string settings. The existing compact subnormal, MAX_EMAX, trillion-exponent zero-surplus, 1E-201, frozen cash503/cost3 and 5,000-digit identity cases must pass under the default budget. The earlier 999,999-digit identity case remains within policy; it must not acquire phantom work from multiplying by one. Do not use enormous allocations merely to exercise the budget.

This is the parent-approved resource policy for the remaining correction. No other handoff scope, financial semantics, campaign gates, production integration, or publication rules change.
