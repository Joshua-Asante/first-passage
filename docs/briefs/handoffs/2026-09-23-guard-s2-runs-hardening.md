...
**Date:** 2026-09-23
**Status:** FROZEN. Ready to dispatch. The operator launches GLM locally (`glm_agent` / Z Code runs only on the operator's Windows machine).
...
**Report before code:** (a) `sha256sum tests/test_guard_s2_runs_acceptance.py` equals `4bba3f47aa332c7fd27730671dd6482fd720fdaa9885d8c81659d9ae9322e041`. (b) On the base, the acceptance suite gives **123 failed / 7 passed**. The coordinator's record for this is `20260923T034523Z-42e8f2294188` on Linux, where most failures are the missing `cwd` keyword. Through a `cwd` shim the result is 85 failed / 45 passed, and every failure maps to a row in §3. (c) The pre-existing suites pass on the base: `tests/test_guard_s2_runs.py`, `tests/scripts/test_guard_shell_command.py`, `tests/test_guard_open_verification_record.py` and `tests/test_git_hooks.py` give **115 passed / 1 skipped** (record `20260923T034527Z-c7c3fb8ab47a`).
...
- **D3. A push is refused only when it would cancel a run.** A push to branch B is refused when both hold:
  ...
  Exceptions:
  - A tip commit carrying `[skip ci]`, `[ci skip]`, `[no ci]`, `[skip actions]`, `[actions skip]` or a `skip-checks: true` trailer is allowed, **unless an earlier segment of the same command runs `git commit`**. The new message is unknown at PreToolUse time.
...
- **D7. Default branch (F26).** With no `--ref`, try `gh repo view [-R X] --json defaultBranchRef`, then `git symbolic-ref refs/remotes/origin/HEAD` (short or long form), and otherwise allow. **Never** use the local branch.
- **D8. `-R/--repo` (F9).** Pass the flag to every `gh` call. `gh api` takes no `-R`, so its path must be `repos/<owner>/<repo>/…`. Do not call `git ls-remote` when `-R` names the repository; get the SHA through `gh api`.
...
- **D12. MCP tools (F20).** Route PreToolUse for `mcp__github__(push_files|create_or_update_file|delete_file|update_pull_request_branch|actions_run_trigger)` to `guard_s2_runs.py claude-hook`, and map each one's `tool_input`:
  ...
  `mcp__github__get_file_contents` must not match. Any other `tool_name` gets no output.
...
|---|---|---|---|---|
| 1 | P2 | `git push && gh workflow run --ref <same>` is judged against the pre-push remote SHA | D6 | `test_f1_dispatch_after_a_push_in_the_same_command_skips_the_stale_sha` |
...
| 15 | P3 | `cd`/`-C` are ignored | D14 | `test_f15_*` (two) |
...
| 19 | P3 | Wrappers hide git/gh | D14 | refused `env`, `command`, `timeout`, `/usr/bin/git`, `bash -c`, `sh -c`, `if`, subshell, `&`, `out=$(…)` cases |
| 20 | P3 | Server-side pushes and dispatches bypass both layers | D12; `gh pr update-branch` is a push to the PR head | `test_f20_*` (six) |
| 21 | P3 | A `[skip ci]` tip is refused | D3 | `test_f21_*` (three) |
...
- **Acceptance.** `./fp.ps1 python -m pytest tests/test_guard_s2_runs_acceptance.py` gives 130 passed / 0 failed / 0 skipped, and the file's SHA-256 is still `4bba3f47…e041`. *Falsified by* any failure or skip, a changed hash, or an `xfail`/`skip`/`importorskip` added anywhere that affects these tests.
