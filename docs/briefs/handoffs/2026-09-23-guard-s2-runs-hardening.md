...
**Report before code:** (a) `sha256sum tests/test_guard_s2_runs_acceptance.py` equals `f20687fcafc1eca96776c510461c5a3005323bc792224f8ca8bb3f2f50483c75`. (b) On the base, the acceptance suite gives **147 failed / 7 passed** of 154 (Revision 1; it was 138/7 of 145 before). The coordinator's record for this is `20260923T170003Z-0d1586db5746` on Linux, where most failures are the missing `cwd` keyword. Through a `cwd` shim the result is 92 failed / 53 passed, and every failure maps to a row in §3. (c) The pre-existing suites pass on the base: `tests/test_guard_s2_runs.py`, `tests/scripts/test_guard_shell_command.py`, `tests/test_guard_open_verification_record.py` and `tests/test_git_hooks.py` give **115 passed / 1 skipped** (record `20260923T150709Z-c0996fd36abb`).
...
**Revision 1, 2026-09-23 (operator review of #470 at `0171305`; coordinator verdict RESOLVED withdrawn).** The operator found four defects the frozen suite did not pin, and the coordinator reproduced all four:
- **F30 (P2).** An inherited repository flag before the subcommand bypasses every check: `gh -R owner/repo workflow run …`, `gh --repo … run rerun …`, `gh -R … pr update-branch …`. **Rule:** strip gh's inherited flags (`-R`/`--repo` with its value, `--repo=`) wherever they appear before identifying the subcommand, and carry the repository into D8.
- **F31 (P2).** Field precedence. `gh` applies typed `-F/--field` after raw `-f/--raw-field`, so `-F mode=s3 -f mode=s2` dispatches s3. The guard read s2 and allowed a re-dispatch over a passed s3 run. **Rule:** for each key, a typed field overrides a raw field **regardless of argument order**. Within one kind, the last occurrence wins.
- **F32 (P2).** A whitespace-only `cases` (`-f cases=' '`) was stripped and treated as a full dispatch. The workflow puts any non-empty value in the diagnostic group, so the dispatch could cancel a live diagnostic run. **Rule:** refuse a whitespace-only `cases` outright, with `cases` named in the reason.
- **P1, structural.** #470 carried the frozen suites for cards 2 and 3, adding 100 failing tests. Those two cards and their suites now live on `claude/guard-cards-2-3` and are removed from this card's lineage. **Merge `origin/claude/guard-s2-hardening-card` into `glm/guard-s2-hardening`** (a merge commit; no rebase) to drop them and pick up the revised suite. Cards 2 and 3 will be re-based on this card's accepted head before they are dispatched.

The suite gains nine cases: `test_f30_*` (×2), `test_f31_*` (×4) and `test_f32_*` (×3), for 154 in total. On `0171305`, 7 of them fail and the other 147 pass (coordinator record, Linux). **The earlier hash `c1d0509b…ab8e` is void.**
...
| 30 | P2 | Inherited `-R/--repo` before the subcommand bypasses all checks | Revision 1 | `test_f30_*` |
| 31 | P2 | Raw `-f` read over typed `-F` for the same key | Revision 1 | `test_f31_*` |
| 32 | P2 | Whitespace-only `cases` treated as a full run | Revision 1 | `test_f32_*` |
...
- **Acceptance.** `./fp.ps1 python -m pytest tests/test_guard_s2_runs_acceptance.py` gives 154 passed / 0 failed / 0 skipped, and the file's SHA-256 is still `f20687fc…3c75`. *Falsified by* any failure or skip, a changed hash, or an `xfail`/`skip`/`importorskip` added anywhere that affects these tests.
