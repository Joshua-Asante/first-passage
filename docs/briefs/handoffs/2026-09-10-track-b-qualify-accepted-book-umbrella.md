...
| **D-B11** | The 1% / 40% protection policy is **not admitted** to `core/dd_geometry.py::POLICY_REGISTRY` until the concept ADR's §4 chain completes, and a K=1 confirmation of the already-selected cell **does not satisfy step 2** (the pre-registered trigger × scale grid). Two legal routes: **(a) run the chain** — TB-P1 pre-registers the objective, the grid (a small frozen trigger × scale set that contains 1% / 40%), the selection rule and the both-halves gate; TB-E1 runs the grid; the winning row is frozen and admitted with its ADR in TB-D2. This reopens the protection cell of the accepted configuration (the book itself is unchanged), spends K equal to the grid size, and may select a cell other than 1% / 40%. **(b) supersede step 2 for this instance** — a full-tier ADR, superseding the concept ADR in part, records that the instance was operator-selected from unregistered exploratory evidence and is admitted on pre-registration (step 1) + the both-halves gate at the accepted cell (step 3 via TB-E1 H1/H2) + freeze (step 4), without a grid; authored by **TB-P2** and ratified by the operator **before TB-F1**, so the pre-registration cites it. Under route (a) TB-F1 freezes the **whole grid**, TB-E1 selects and **seals the winning cell** into the winner-specific replay fingerprint, and Phase 8 (TB-I3 live test), B7 and n3 all run on that sealed cell — never on the 1% / 40% candidate if a different cell won. Until admission the policy exists only as a **threaded candidate `ProtectionPolicy` value** in replay and offline rail tests, plus the **sealed dry-run binding** of TB-S3 (L) for the pre-n3 live test; the armed sizing path (`dry_run=false`) reads only the admitted registry row and refuses to arm if that row is absent or differs from the sealed binding. | No shortcut default. If the operator wants the accepted cell kept as is, (b) is the honest route and must be an explicit governance amendment, not a packet default; (a) is the methodologically stronger route at the cost of reopening the cell. State which. | TB-S1, TB-I1, TB-I3, TB-P1, TB-E1, TB-D2 |
...
D-B1..D-B12 answered ──► TB-G0 (orchestrator governance commit)   [D-B0 satisfied 2026-09-10]
        ...
        │        (dry-run only, under the sealed binding for the TB-E1 winner)
...
| TB-I3 | Codex local | `codex/tb-i3-rail-extension` | `ops/c1_signal_daemon/*` · `ops/c1_rail/{c1_rail_listener,c1_sizing_host_reference,crosstrade_payload,c1_rail_telemetry,c1_rail_arm}.py` · `deploy/*` · tests (incl. the bar-time barrier, the sealed dry-run policy binding and its arm-interlock check) | TB-S3 accepted, TB-I1 | STUB |
...
- **Operator** — D-B1..D-B12 (D-B0 done); OP-1 exports and TB-W1's pre-window bars; TB-P2 ratification if route (b); TB-C1 source attestation; every merge; the private-root backup after each wave; weekly preservation trade; calendar and closure-overlay upkeep; account snapshot with its evidence files; ORB GO; deployment GO; the arm.
...
- Wave 3 internal: TB-C1 merged and (if route (b)) TB-P2 ratified before TB-F1; TB-F1 before TB-E1; TB-E1's seal (winner-specific replay fingerprint) before TB-D1 and before the Phase 8 live test; TB-D1's operator GO recorded before TB-E2 (a withheld or pending GO leaves Track B `BLOCKED` at TB-D1, attempt not consumed); Track A RESOLVED + TB-B7 before TB-E2; TB-E2 pass before TB-D2; nothing after a failed TB-E1 or TB-E2 except the closure record.
...
# tokens (ISO dates, 7-64 hex digests, PR/issue numbers, packet/decision ids, ADR filenames). Every
...
python -c "import re,glob,sys;A=[r'\d{4}-\d{2}-\d{2}(-[a-z0-9-]+\.md)?',r'\b[0-9a-f]{7,64}\b',r'#\d+',r'\bD-B\d+\b',r'\bTB-[A-Z]\d*\b',r'\bOP-\d\b',r'\b(n1|n2|n3|H1|H2|M1|B\d{1,2}|S[1-7]|R[1-3]|D\d{1,2})\b'];bad=[(f,i,l.rstrip()) for f in glob.glob('docs/notes/*track-b-deployment-decision*.md') for i,l in enumerate(open(f,encoding='utf-8'),1) if re.search(r'\d',__import__('functools').reduce(lambda t,a:re.sub(a,'',t),A,l))];[print(*b) for b in bad];sys.exit(1 if bad else 0)"
...
- (F) Expression in the sizing host: normal quantities are fixed integers per leg (not risk-fraction derived); the spec must say how each leg's normal quantity is represented so `generate_constants` cannot silently size a leg differently (either a fixed-quantity leg type, or a `base_risk` chosen so that `floor()` yields the exact integer across the whole equity range that the eval can reach, with the binary64 hazard from #332 addressed). The spec also defines the **sealed dry-run policy binding** consumed by TB-S3 (L): a config artifact carrying the sealed winner policy value and the TB-E1 fingerprint digest, honoured by the sizing host only while `dry_run=true`; with `dry_run=false` the host reads only the admitted registry row, and `c1_rail_arm.py` refuses to arm unless that row exists and equals the sealed binding.
...
- (L) Sealed dry-run policy binding: the pre-n3 live test needs the sizing host to size with the winner's policy before the registry row is admitted. The binding is a config artifact carrying the sealed winner policy value (trigger, scale, reference mode) and the TB-E1 winner-fingerprint digest; the sizing host honours it **only while `dry_run=true`** and logs every use with the digest; with `dry_run=false` it reads only `POLICY_REGISTRY`; `c1_rail_arm.py --arm` refuses unless the admitted row exists, equals the sealed binding, and the execution fingerprint sealed at B7 names the same digest. Tests: binding ignored when armed; arm refused on missing or mismatched row; telemetry carries the digest.
- (K) Split every requirement into **offline-testable now** (registry, rows, capacity ledger, bar-time barrier, side mapping, kill switch, scheduler, restart, telemetry kinds, sealed dry-run binding and its arm-interlock check) vs **live-source-dependent** (feed health with a real source, the item-5-class dry-run signal, end-to-end SIM chain).
