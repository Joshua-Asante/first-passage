...
**Status:** PROPOSED — no packet may be dispatched until every still-open §0.5 decision is answered. D-B0 was satisfied 2026-09-10; D-B4 and D-B11 were ruled by the operator 2026-09-11. This file authorizes nothing by itself.
...
| **D-B5** | Track A / Track B interface: the daemon's replacement market-data source, its implementation, M1 item 5 discharge and `operator_signoff` are **Track A deliverables**. Track B builds everything that is offline-testable and consumes Track A's output for the live-feed integration test, B8's timing, B9 and B10. Three items sit on the seam and are owned here unless Track A lands them first: the arming/disarming procedure (**TB-O1** — the archive runbook stays private; a redacted public procedure is the B10 prerequisite), the PREREG-C1-DEDUPE-1 implementation (**TB-I4**, after M1 RESOLVED and the D-B13 GO), and the **feed-equivalence pre-flight** on the delivered source (**TB-I5**, per `docs/spec/feed_equivalence_discovery_test_LOCKED.md`, which `CLAUDE.md` §Firm Expansion already requires for any new execution feed; its source identity enters the execution fingerprint). | Yes. Track B never chooses the feed. | TB-I3 live part, TB-I4, TB-I5, TB-O1, TB-E2, TB-D2 |
...
| **D-B9** | Shared-symbol scope: the fixed book has exactly one strategy per order symbol (6J, MGC, MYM, MNQ). Implement one controller per symbol with single ownership. A multi-owner MNQ controller and Striker NAS100 are outside this attempt. | Yes — fixed by D-B4. | TB-S2, TB-S3 |
...
| **D-B13** | Separate operator GO for implementing PREREG-C1-DEDUPE-1 (its status line: "GATED on M1 RESOLVED + a separate operator GO. Not scheduled"). The pre-registration's §6 preconditions are M1 RESOLVED, this GO, and a host read confirming `dry_run=True`. Without it the Phase 8 live test has an unowned safety gap on the exact incident class (a re-POSTed payload filled a second position). | Grant the GO now, effective when Track A reports M1 RESOLVED; TB-I4 then executes the frozen implementation plan verbatim before the TB-I3 live test. | TB-I4, TB-I3 live part |
| **D-B14** | Down-only lifecycle tiers for size- or mode-dependent legs. Automation may move a leg to WATCH-1 (0.50×) and WATCH-2 (0.25×), which compound with the fixed protection policy and can produce additional executable quantities/modes. Options: (a) validate every distinct reachable lifecycle × fixed-policy mode through TB-R2/OP-1/TB-R3/parity; (b) restrict the automated ladder for this book to {AUTHORIZED, OFF} through a separate governance amendment. These are lifecycle execution states, never alternative protection-policy cells. | (a) — retain the ratified ladder and state the finite export count before results. | TB-R2, OP-1, TB-R3, TB-A1..A4, TB-P1 |
| **D-B15** | Operator emit GO for the Phase 8 dry-run test. The M1 addendum of 2026-08-24 states the daemon refuses `emit_enabled=true` without a strategy GO and grants one for the test strategy only; the accepted book's Phase 8 test needs the daemon to build and POST real-adapter payloads to the listener (`evaluate_loop.py` lines 99–112 suppress before the payload otherwise), with the listener locked to `dry_run=true`, which never calls the sender. | Grant a bounded emit GO for the Phase 8 window only: real adapters, `dry_run=true` verified by a host read before and after, `armed_until` unset, revoked at the window's end; never an arm. | TB-I3 live test, TB-B7 |
...
```
D-B1..D-B15 answered ──► TB-G0 (orchestrator governance commit)   [D-B0 satisfied 2026-09-10]
        │
        ...
        │   ── Track A delivers live source + M1 RESOLVED ──► TB-I5 CME feed-equivalence successor spec
        │        frozen, then applied to the delivered source → D-B13 GO → TB-I4 dedupe implementation
...
|---|---|---|---|---|---|
| TB-G0 | orchestrator | `claude/tb-g0-track-b-release` | plan status block · campaign-state §55 + §1 Roles · `STATE.md` queue row · `docs/SESSIONS.md` | D-B1..D-B15 (D-B0 satisfied) | QUEUED |
...
| TB-A1..A4 | Claude Code local ×4 | `claude/tb-a<n>-<strategy>-adapter` | private port under the primary checkout's ignored port root + one parity test per adapter under `tests/ops/` (spec'd in TB-S3); the port's SHA-256 line is **reported in the PR body**, not written to `PORT_MANIFEST.sha256` (TB-A0 lands all lines serially); parity at the captured size **and** at every admitted protected size / ORB adds-off mode against the **TB-R3-intaken** exports | TB-R1, TB-R2, OP-1, TB-R3, TB-W1, TB-S3 accepted | STUB |
...
| TB-V1 | Claude Code local (orchestrator-authored, operator-ratified) | `claude/tb-v1-fixed-book-runtime-binding` | `ops/venue_editions/Tradeify_Select_100K.md` (rows for all four fixed-book legs, with ORB gated on TB-D1 GO) · `ops/c1_rail/c1_sizing_host_reference.py` `LEG_MAP` (`cap_alloc` for all four legs from TB-S1 under the operator's first-deploy cap ruling) · the **active-leg configuration** artifact (fixed leg set + digest) consumed by the daemon registry and `c1_rail_arm.py` · tests | TB-E1 seal, TB-D0, TB-D1 GO, per-symbol verification for every fixed-book symbol; before TB-I3 live test and TB-B7 | STUB |
...
- **TB-I5 (feed-equivalence pre-flight, Claude Code local).** Entry: Track A reports the delivered source. First authors and freezes `docs/spec/2026-09-1x-cme-execution-feed-equivalence-test.md`, the CME-futures successor to the locked spec (which is scoped to XAUUSD on MT5-Pepperstone vs TV-Pepperstone for 2026-04-13→25 and recorded as inapplicable to CME futures by `Q-DATAFIDELITY-1`; the locked spec forbids changing its own scope, so a successor, not an edit, satisfies `CLAUDE.md` §Firm Expansion): same test shape, re-scoped to the four order symbols, a frozen overlap window, tolerances and a binary verdict rule, pre-registered before any comparison. Then applies it to the delivered source versus the TV panels per order symbol: OHLC aggregation, session boundary, timezone, continuous-roll handling and bar-close timing at frozen tolerances; FAIL blocks the live test (the source, not the adapters, is the defect); PASS records the source identity and configuration digest as a shared component of the B7 execution fingerprint.
