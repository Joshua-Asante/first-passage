...
| **D-B5** | Track A / Track B interface: the daemon's replacement market-data source, its implementation, M1 item 5 discharge and `operator_signoff` are **Track A deliverables**. Track B builds everything that is offline-testable and consumes Track A's output for the live-feed integration test, B8's timing, B9 and B10. Two items sit on the seam and are owned here unless Track A lands them first: the arming/disarming runbook (**TB-O1**, retrieved from the archive, a B10 prerequisite) and the PREREG-C1-DEDUPE-1 implementation (**TB-I4**, after M1 RESOLVED and the D-B13 GO). | Yes. Track B never chooses the feed. | TB-I3 live part, TB-I4, TB-O1, TB-E2, TB-D2 |
...
D-B1..D-B13 answered ──► TB-G0 (orchestrator governance commit)   [D-B0 satisfied 2026-09-10]
        ...
        │   TB-W1 warm-up coverage   TB-C1 forward calendar + overlay               │
        ...
        │   [TB-A5 only under D-B4 (b)] → TB-A0 serial PORT_MANIFEST integration
        ...
        │   ── Track A delivers live source + M1 RESOLVED ──► D-B13 GO → TB-I4 dedupe implementation
        │   → TB-I3 live-feed test (dry-run, admitted row in the image)
        │        + per-symbol CrossTrade ticket verification for 6J / MGC
        │   → TB-O1 runbook retrieved from the archive (B10 prerequisite)
...
| TB-A0 | orchestrator | integration commit | `core/strategies/PORT_MANIFEST.sha256` (all adapter lines in one commit, from the hash lines the TB-A PRs report) | TB-A1..A4 merged | STUB |
...
| TB-E2 | Codex local compute | `codex/tb-e2-n3` | study dir results + digests (numbers private per D-B12) | TB-B7 snapshot + execution fingerprint sealed, winner-specific replay fingerprint unchanged since TB-E1, Track A RESOLVED, TB-D1 operator GO recorded | STUB |
...
| TB-O1 | Claude Code local | `claude/tb-o1-rail-runbook` | `docs/notes/rail_build/RUNBOOK.md` retrieved verbatim from the archive (`git show pre-prune-2026-08-08:docs/notes/rail_build/RUNBOOK.md`, present there per the 2026-09-11 check) plus a dated addendum re-homing pointers; no reconstruction, no edit to the recovery procedure | before TB-B10; may be superseded if Track A lands it first | STUB |
...
- **TB-O1 (rail runbook, Claude Code local).** Entry: any time before TB-B10. Retrieves `docs/notes/rail_build/RUNBOOK.md` verbatim from the archive ref (`pre-prune-2026-08-08`, confirmed present 2026-09-11) into its original path with a dated addendum that re-homes the pointers the M1 ADR, the GO ADR, the dedupe pre-registration and the acceptance JSON make to it; no reconstruction and no edit to the private crash-loop recovery procedure (#332: "Do not reconstruct or waive that recovery procedure"). If the archive ref is unreadable, `BLOCKED — context-problem` for the operator. Superseded without conflict if Track A lands the same file first.
...
- **TB-B10 (operator).** Arm per the retrieved runbook (TB-O1 or Track A's equivalent, a hard prerequisite), start the frozen forward clock at actual deployment, apply only the frozen monitoring and down-only controls, continue the weekly preservation trade.
