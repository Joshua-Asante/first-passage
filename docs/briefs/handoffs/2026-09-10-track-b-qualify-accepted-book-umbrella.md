...
**Revision 6 (2026-09-11):** ten further Codex findings on revision 5 folded — see the PR threads. Substantive changes: wave 1 drawn as dependency-respecting subwaves; under D-B11 route (a) the export menu covers every distinct quantity/mode the whole grid produces; replay and rail consume the forward calendar as well as the D19 file and overlay; the Aegis takeover is an atomic cancel-confirm-release step for displaced legs' pending orders (fail closed if a cancel is unconfirmed); TB-A0 depends on TB-A5 under D-B4 (b); the registry audit compares each branch to its merge-base instead of classifying by name; new packet **TB-I5** runs the locked feed-equivalence pre-flight on Track A's delivered source and binds its identity into the execution fingerprint; the per-symbol gate no longer accepts a dry-run receipt (dry run never calls the sender); TB-D1 applies only when the sealed book contains ORB; TB-O1 keeps the archive runbook **private** and publishes only a redacted operating procedure (the runbook is on the public-visibility exclusion list).

**Revision 5 (2026-09-11):** seven further Codex findings on revision 4 folded — see the PR threads. Substantive changes: protection admission moves to a new packet **TB-D0** that lands the admitting ADR and the registry row after TB-E1's seal and **before** the Phase 8 live test and B7, so the sealed executable already contains the row (the dry-run binding of rev 4 is withdrawn as unnecessary); new packets **TB-R3** (full intake gate for OP-1 exports), **TB-I4** (PREREG-C1-DEDUPE-1 implementation, gated on M1 RESOLVED + new **D-B13** operator GO) and **TB-O1** (retrieve the arming/disarming runbook from the archive, a B10 prerequisite); TB-C1's overlay may override historical D19 rows (non-overlap applies to the forward extension only); the digest allowlist requires a hex letter; TB-R1's no-op covers regeneration only.

**Revision 4 (2026-09-11):** eleven further Codex findings on revision 3 folded — see the PR threads. Substantive changes: the freeze/seal sequence is split (TB-F1 freezes the search contract; TB-E1 seals the winner-specific replay fingerprint that Phase 8, B7 and n3 must match); a dry-run-only sealed candidate-policy binding lets Phase 8's live test run before admission while the arm still requires the admitted row; new packets TB-P2 (route-b ADR), TB-C1 (forward calendar + overlay), TB-W1 (warm-up coverage), TB-A0 (serial port-manifest integration); a frozen E1 initial state; bar-time barrier ordering for capacity admission; the D-B4(b) fifth leg propagated into every topology surface; the D-B4(b)+D-B11(a) combination frozen as one Cartesian catalogue; the publication hook replaced by an allowlist validator.
...
| **D-B4** | B4 mode. (a) **Single-candidate confirmation**: the catalogue is exactly the accepted book (K=1); the deterministic legality/cap screen, n1 screening and n2 bound run on that one cell; a failed screen or bound ends the attempt with "no qualifying configuration". (b) The full frozen catalogue search of plan Task 4 over all five retained expressions. **(b) is not free:** it adds a fifth adapter packet (TB-A5, `striker_nas100_mnq_dow_wed_excluded`), flips D-B9 so the multi-owner shared-MNQ controller is built and proven before any two-MNQ cell is replayed, and requires TB-R2's export menu to cover the fifth leg; the manifest carries those rows as CONDITIONAL on (b), and **every topology surface** (TB-S1 quantity table, TB-S3 registry/`leg_id`/`LEG_MAP`/lifecycle/side/symbol tables, TB-I1 keys, TB-I3 rows) gains the fifth leg `nas100_mnq_wedx` under (b). **Combination with D-B11 route (a):** allowed only as **one frozen Cartesian catalogue** — every legal book cell × every grid cell, total K = |books| × |grid|, evaluated in one pass under the plan's single S1 ranking and tie order; no book-then-policy or policy-then-book staging, no post-result choice. TB-F1 records the exact combined catalogue and K. | (a). (b) re-opens the configuration selection the operator closed on 2026-09-10, makes runner-up temptation structural, and adds the scope above. | TB-P1, TB-F1, TB-E1, TB-A5, TB-S2/TB-I2 multi-owner branch |
...
| **D-B11** | The 1% / 40% protection policy is **not admitted** to `core/dd_geometry.py::POLICY_REGISTRY` until the concept ADR's §4 chain completes, and a K=1 confirmation of the already-selected cell **does not satisfy step 2** (the pre-registered trigger × scale grid). Two legal routes: **(a) run the chain** — TB-P1 pre-registers the objective, the grid (a small frozen trigger × scale set that contains 1% / 40%), the selection rule and the both-halves gate; TB-E1 runs the grid; the winning row is frozen and admitted with its ADR in **TB-D0**. This reopens the protection cell of the accepted configuration (the book itself is unchanged), spends K equal to the grid size, and may select a cell other than 1% / 40%. **(b) supersede step 2 for this instance** — a full-tier ADR, superseding the concept ADR in part, records that the instance was operator-selected from unregistered exploratory evidence and is admitted on pre-registration (step 1) + the both-halves gate at the accepted cell (step 3 via TB-E1 H1/H2) + freeze (step 4), without a grid; authored by **TB-P2** and ratified by the operator **before TB-F1**, so the pre-registration cites it. Under route (a) TB-F1 freezes the **whole grid**, TB-E1 selects and **seals the winning cell** into the winner-specific replay fingerprint, and Phase 8 (TB-I3 live test), B7 and n3 all run on that sealed cell — never on the 1% / 40% candidate if a different cell won. **Admission happens at TB-D0, after TB-E1's seal and before the Phase 8 live test and B7**: the both-halves gate (step 3) is TB-E1's full/H1/H2 evidence for the sealed cell, the admitting ADR (route (a)) or the ratified TB-P2 ADR (route (b)) is named in `provenance`, and the row lands with it, so the executable sealed at B7 already contains the row and no code changes after n3. Until TB-D0 the policy exists only as a **threaded candidate `ProtectionPolicy` value** in replay and offline rail tests; the sizing host never reads a candidate. If n3 later fails, the row stays admitted as an instance record but the closure notes that no book is deployed under it. | No shortcut default. If the operator wants the accepted cell kept as is, (b) is the honest route and must be an explicit governance amendment, not a packet default; (a) is the methodologically stronger route at the cost of reopening the cell. State which. | TB-S1, TB-I1, TB-I3, TB-P1, TB-E1, TB-D0 |
...
```
D-B1..D-B13 answered ──► TB-G0 (orchestrator governance commit)   [D-B0 satisfied 2026-09-10]
        │
        ...
        │        (+ the policy grid proposal under D-B11 (a))                       │
        │   [TB-P2 route-b ADR, only under D-B11 (b), ratified before TB-F1]        │
        ├─ wave 1b (after TB-R1 2.1 + TB-P1) ───────────────────────────────────────
        │   TB-R2 scaling read + export menu (needs Pine/overrides by digest and,    │
        │        under route (a), TB-P1's grid → menu covers every grid quantity)    │
        ...
        │   [TB-A5 only under D-B4 (b)] → TB-A0 serial PORT_MANIFEST integration (waits for A5 under (b))
        ...
        │
        ├─ wave 3 (sequential) ─────────────────────────────────────────────────────
        │   TB-F1 freeze search contract (needs TB-C1; TB-P2 if route b)
        │   → TB-E1 screen + n1 + n2 [+ policy grid under D-B11 (a)] + seal winner fingerprint
        │   → TB-D1 ORB R2 superseding ADR on the book-level result → operator GO
        │        (only if the sealed book contains ORB; N/A otherwise)
        ...
        │   ── Track A delivers live source + M1 RESOLVED ──► TB-I5 feed-equivalence pre-flight
        │        on the delivered source (LOCKED spec) → D-B13 GO → TB-I4 dedupe implementation
        ...
        │        (winner-specific replay fingerprint must be unchanged; shared components equal;
...
|---|---|---|---|---|---|
| TB-G0 | orchestrator | `claude/tb-g0-track-b-release` | plan status block · campaign-state §55 + §1 Roles · `STATE.md` queue row · `docs/SESSIONS.md` | D-B1..D-B13 (D-B0 satisfied) | QUEUED |
...
| TB-P2 | Claude Code local | `claude/tb-p2-protection-instance-supersession` | `docs/adr/2026-09-1x-tradeify-book-protection-instance-admission.md` (new, full tier, supersedes-in-part the concept ADR §4 step 2 for this instance; PROPOSED until the operator ratifies) | **CONDITIONAL on D-B11 route (b)**; must be ratified before TB-F1 | CONDITIONAL |
...
| TB-A0 | orchestrator | integration commit | `core/strategies/PORT_MANIFEST.sha256` (all adapter lines in one commit, from the hash lines the TB-A PRs report) | TB-A1..A4 merged, **and TB-A5 under D-B4 (b)** | STUB |
...
| TB-F1 | Claude Code local | `claude/tb-f1-freeze` | the pre-registration file (freeze edit only: search contract, catalogue/grid, K, streams, sizes, E1 initial state, calendar/overlay digests, warm-up boundaries) | wave 2 merged, TB-C1 merged, TB-P2 ratified if route (b) | STUB |
| TB-E1 | Codex local compute | `codex/tb-e1-selection-evidence` | study dir results + digests + the sealed **winner-specific replay fingerprint** (frozen contract + selected book cell + selected policy cell) | TB-F1 | STUB |
| TB-D1 | Claude Code local | `claude/tb-d1-orb-r2-adr` | the ORB ADR (fill + status) | TB-E1; **applies only if the sealed book contains ORB** (otherwise recorded N/A and the skeleton stays PROPOSED) | STUB |
| TB-I5 | Claude Code local | `claude/tb-i5-feed-equivalence` | the feed-equivalence pre-flight per `docs/spec/feed_equivalence_discovery_test_LOCKED.md` applied to Track A's delivered source vs the TV panels over an overlap window (OHLC aggregation, session boundary, timezone, continuous-roll handling, bar-close timing; frozen tolerances); a RESULTS note with digests only; the source identity/config digest recorded for the execution fingerprint | Track A source delivered; before TB-I3 live test | STUB |
| TB-E2 | Codex local compute | `codex/tb-e2-n3` | study dir results + digests (numbers private per D-B12) | TB-B7 snapshot + execution fingerprint sealed, winner-specific replay fingerprint unchanged since TB-E1, Track A RESOLVED, TB-D1 operator GO recorded (when the sealed book contains ORB) | STUB |
| TB-D0 | Claude Code local | `claude/tb-d0-protection-admission` | the D-B11 admission artifacts: admitting ADR (route (a)) **or** a pointer to the ratified TB-P2 ADR (route (b)) · `core/dd_geometry.py` (the registry row for the sealed cell, `provenance` naming the ADR) · `tests/core/test_dd_geometry.py` (governance-chain check exercised non-vacuously) · `scripts/check_track_b_decision_record.py` + test (the D-B12 allowlist validator) | TB-E1 seal; before TB-I3 live test and TB-B7 | STUB |
...
- **Operator** — D-B1..D-B13 (D-B0 done); OP-1 exports and TB-W1's pre-window bars; TB-P2 ratification if route (b); TB-C1 source attestation; every merge; the private-root backup after each wave; weekly preservation trade; calendar and closure-overlay upkeep; account snapshot with its evidence files; ORB GO; deployment GO; the arm.
...
- (C) The export menu is one table: leg · captured size (already exported) · each admitted protected size · protected-mode variant (ORB adds-off; any other leg whose protected mode changes a branch) · required export (Y/N, with the chart-state attestation the operator must repeat). **Under D-B11 route (a)** the admitted sizes are not only D-B10's 40% table: the menu enumerates every distinct integer quantity per leg produced by **every scale value in TB-P1's frozen grid** under D-B10's rounding, so no grid cell can be scored on a size that was never exported; TB-P1 therefore proposes the grid before TB-R2 runs and keeps it small enough that the distinct-quantity set is finite and stated. One menu; if a later decision changes a size or the grid, the menu is re-issued as a replacement, not extended after results.
...
### §0.5 — recommended defaults
- (A) Topology: one daemon process, a **strategy registry** of adapters keyed by `leg_id` — four for the accepted book, **five under D-B4 (b)** (adding `nas100_mnq_wedx` for `striker_nas100_mnq_dow_wed_excluded`, sharing the MNQ source with `orb_mnq_v7` under the multi-owner controller), one `BarSource` per order symbol (6J, MGC, MYM, MNQ; all 15-minute), per-leg evaluate loops that hand every signal for a bar timestamp to a **bar-time barrier**, which releases them to one listener client in the frozen global order of TB-S2 (F) (D-B8 priority, frozen leg order, entry before add, `leg_id`); the listener's capacity ledger admits in that order and rejects out-of-order or late arrivals for a closed bar; the source implementation is a Track A deliverable behind the existing `BarSource` protocol, and satisfying the protocol is not equivalence: **TB-I5** runs the locked feed-equivalence pre-flight on the delivered source against the TV panels (OHLC aggregation, session boundary, timezone, continuous-roll handling, bar-close timing) before the live test, and the source's identity/config digest is a shared component of the B7 execution fingerprint.
- (B) New `leg_id`s: `aegis_6j`, `vanguard_mgc`, `dj30_mym_p250`, `orb_mnq_v7`, plus `nas100_mnq_wedx` **only under D-B4 (b)** (every table below — `LEG_MAP`, `INSTRUMENT_SYMBOLS`, lifecycle keys, `allowed_entry_side` = `buy`, sizing expression, MNQ symbol ownership — carries a conditional fifth row) — distinct from the retired `dj30_mym` / `nas100_mnq` rows, which stay at zero and are never reused. `INSTRUMENT_SYMBOLS` entries for 6J and MGC are **provisional strings** (the listener's own docstring, lines 25–28: continuous-contract notation must be verified against CrossTrade's accepted order-ticket format at a dry-fire; only `MYM1!` has live fill evidence). The spec adds a **per-symbol verification gate**, live-tagged: before TB-B10, each new symbol needs an operator-attested CrossTrade ticket-format confirmation (from CrossTrade's documentation or account-manager UI) **or** a separately authorized, operator-placed transmitted validation that actually reaches CrossTrade — never a dry-run receipt, because the listener's own docstring (lines 32–33) says dry run never calls the sender, so a receipt proves only local payload construction; recorded in telemetry; until then the leg's `cap_alloc` stays 0. `LEG_MAP` rows created at `cap_alloc=0`, lifecycle keys added, allocations expressed per TB-S1 (F).
...
- (D) Fields that depend on wave 2 are written as `OWED-BY: TB-F1` with the exact source that will fill them. Three layers, kept distinct: the **frozen search contract** (adapter port hashes from `PORT_MANIFEST.sha256` via TB-A0, replay-engine commit, bar-panel digests and warm-up boundaries, D19 calendar digest, TB-C1 forward-calendar and closure-overlay digests, commission schedule digest, capacity rules, fill model, the E1 initial state, the catalogue — book cells and, under D-B11 (a), the whole policy grid — with K, streams and sizes) is frozen at TB-F1; the **winner-specific replay fingerprint** = that contract + the selected book cell + the selected policy cell, sealed by TB-E1 and **unchanged through Phase 8, B7 and n3**; the **execution fingerprint** (rail/daemon commit, image hashes, deployed config digests) is sealed at B7 after Track A's source lands and the TB-I3 live packet merges, and the B7 seal must **prove equality of the shared components** (adapter port hashes, rule tables, the sealed policy cell, capacity rules, calendar and overlay digests) with the winner-specific replay fingerprint. The continuous-roll policy's two obligations enter the freeze verbatim: the back-adjustment seam limitation stated on every campaign claim (fills cannot be attributed to a contract month; a seam crossing is indistinguishable from a price move) and a seam-sensitivity check pre-registered with a frozen severity in the battery. Dates: propose full = the documented coverage (2022-09-01 → 2026-09-02, from every strategy report's `summary_source_note`, which records coverage only); propose the H1/H2 boundary from a **stated rule** — chronological midpoint of the weekday axis of that coverage, odd date to H2 — and show the derivation (the earlier 2024-08-30 figure came from the feasibility screen's own midpoint convention and is not a campaign authority; do not cite the ledger note as its source). The overall horizon is proposed, not inherited (the campaign record says it was never frozen). All marked for freeze in TB-F1.
...
- **TB-P2 (protection-instance supersession ADR, Claude Code local; CONDITIONAL on D-B11 route (b)).** Entry: the operator's route (b) election. Full-tier ADR superseding the concept ADR §4 step 2 for this instance only, recording the operator selection from unregistered exploratory evidence and the retained steps 1, 3 and 4; PROPOSED until ratified; must be ratified before TB-F1 so the pre-registration can cite it.
...
- **TB-F1 (freeze, Claude Code local).** Entry: wave 2 merged, consolidated read passed, TB-C1 merged, TB-P2 ratified if route (b). Fills every `OWED-BY` field, freezes the **search contract** (catalogue — book cells and, under route (a), the whole policy grid — K, streams, sizes, E1 initial state, calendar and overlay digests, warm-up boundaries), flips the pre-registration to FROZEN in one commit; any later change to a frozen component is a replacement freeze and a new operator decision. Execution-only components (rail/daemon commit, image hashes) are expected to change afterwards and are sealed at B7, not here.
- **TB-E1 (screen + n1 + n2 + seal, Codex local compute).** Entry: TB-F1 merged. Runs exactly the frozen sequence; a failed screen/n1/n2 returns the closure record and ends the attempt; a pass seals the provisional winner — the selected book cell and, under route (a), the selected policy cell — into the **winner-specific replay fingerprint** (contract digest + winner configuration digest) that Phase 8, B7 and n3 must match; under route (a) a winning cell other than 1% / 40% is the sealed policy from here on.
- **TB-D1 (ORB R2 ADR, Claude Code local).** Entry: TB-E1 sealed **and the sealed book contains ORB** (under D-B4 (b) a winner without ORB records TB-D1 as N/A, leaves the skeleton PROPOSED, and TB-E2 does not wait on it). Fills the Decision slot from the book-level result at integer size; requests the fresh operator GO. A withheld or pending GO is an authorization dependency, not evidence: Track B is **`BLOCKED — context-problem` at TB-D1**, the attempt is not consumed, and nothing downstream opens until the operator decides. ORB stays research-scored meanwhile; the book without ORB is not a substitute (that would be re-selection).
- **TB-B7 (operator).** Entry: Track A `RESOLVED`, TB-I5 PASS, TB-D0 row and TB-I4 merged and in the image, TB-I3 live test passed on that image, TB-O1 merged, account flat with no pending orders. Operator runs TB-T1's sealer with the evidence files; the orchestrator records the snapshot digest + timestamp and seals the **execution fingerprint** (rail/daemon commit, image hashes, deployed config digests) with a written proof that every shared component — adapter port hashes, rule tables, policy value, capacity rules, calendar and overlay digests — equals the replay/evidence fingerprint frozen at TB-F1.
- **TB-E2 (sole n3, Codex local compute).** Entry: TB-B7 sealed; winner-specific replay fingerprint unchanged since TB-E1; execution fingerprint sealed with shared-component equality proven (including TB-I5's source identity); TB-D1 GO recorded when the sealed book contains ORB. One run; four conditions; unresolved = failure/infinite; no extra sample; result adjudicated by the orchestrator under `fable-judge`; numerical outputs stay in the private root per D-B12.
- **TB-D0 (protection admission, Claude Code local).** Entry: TB-E1 sealed. Completes the D-B11 admission on the route the operator chose — route (a): the admitting ADR records the pre-registered grid, its winner, the both-halves gate (TB-E1 full/H1/H2 for the sealed cell) and the freeze; route (b): points to the ratified TB-P2 ADR and records steps 1, 3 and 4 — and lands the `POLICY_REGISTRY` row for the sealed cell with `provenance` naming that ADR, the `tests/core/test_dd_geometry.py` governance-chain check exercised non-vacuously, and the D-B12 allowlist validator script. Must merge before the TB-I3 live test so the tested and sealed executable contains the row.
