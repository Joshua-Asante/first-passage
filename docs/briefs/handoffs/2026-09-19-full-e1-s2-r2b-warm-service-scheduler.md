...
- (A) **Recover the 15 unpublished red R2b regressions before adapting them.** The former Codex checkout was emptied on 2026-09-19. The exact file is preserved in the local pre-dispatch recovery packet `C:/Users/joshu/multi_firm_operations/recovery/full-e1-r2b-predispatch-20260919/README.md` (untracked, not in Git), extracted from commit `1ef91befe0c1ea48f98258669d348e7c2c8f6846` (parent `966b9f6`), path `tests/ops/qualification/execution/test_campaign_scheduler.py`. SHA-256: `a49c7cd74f8670c18aae12f58691b59750c54a0ea9faf6f18031e786e37b9fab`. Copy the packet file byte-for-byte to that test path and verify the hash; alternatively extract that exact blob with `git show 1ef91be:tests/ops/qualification/execution/test_campaign_scheduler.py` using byte-preserving output. Transfer the packet with this handoff to a cloud executor: the snapshot is local-only. Only if both packet and Git object are unavailable, re-derive from the R2 contract and report `DONE_WITH_CONCERNS`. The original tests reference `prepare_scheduled_work`, `parse_schedule_request` and `scoped_boottime_deadline`, absent at `8aa5753`; adapting them to the accepted claim/materialize interface is expected Step 2.1 work, not loss of test identity. Preserve the recovered original and report the adaptation diff. The historical 15-failure record `20260919T100953Z-42977cf0e724` is not a retained complete verification record.
- (A2) **R2a evidence recovery boundary.** The packet preserves actual archived command outputs reading final records `20260919T110352Z-59d6fa6a497e` (47 passed), `20260919T110450Z-2380d5ed6980` (257 passed), and `20260919T110920Z-a2365379e666` (check exit 0), plus the original narrative return. These are historical excerpts, not restored record directories or a complete source inventory. The fingerprint `0ec013d2…` and IDs in the ledger remain historical provenance. Step 2.1 must retain a new source-bound R2a funding baseline; cite that new record and Step 2.7 final verification for this dispatch, never claim the old complete records are available.
...
- **Action:** restore and hash the original per §0.5 (A), retain it unchanged in the packet, then adapt the active tests to the accepted R2a claim/materialize API without weakening the contract; record the adaptation diff. Run `./fp.ps1 doctor` (or the Linux launcher) then `./fp.ps1 python -m pytest tests/ops/qualification/execution/test_campaign_scheduler.py tests/ops/qualification/execution/test_campaign_funding.py -q --tb=short` and retain the record: funding tests pass, scheduler tests fail (historically 15 cases; explain any count change after adaptation). Also run the funding file alone through the checkout launcher and retain its complete source-stable record as the new R2a baseline; do not use the expected-failing combined command as a passing R2a record. Confirm `./fp.ps1 python -m pytest tests/ops/qualification/execution --collect-only -q` reports zero collection errors.
