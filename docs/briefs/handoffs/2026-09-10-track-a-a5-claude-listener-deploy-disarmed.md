...
`sha256sum` in `/app` of the five deployed pins (`ops/c1_rail/c1_rail_arm.py`, `c1_rail_http_server.py`, `c1_rail_listener.py`, `c1_rail_telemetry.py`, `c1_sizing_host_reference.py`) plus `ops/c1_rail/m1_stage1_contract.py`, `ops/c1_rail/m1_stage1_control.py`, `scripts/validate_c1_monitoring_acceptance.py`. The sixth pin, `tests/ops/test_m1_acceptance_drills.py`, is **not in the image** (the Dockerfile copies no `tests/`; A2's L2 asserts that): hash it from the merge-SHA tree in both byte forms — LF blob via `git show <merge SHA>:tests/ops/test_m1_acceptance_drills.py | sha256sum` and CRLF via the same piped through `sed 's/$/\r/'` — and record both. The existing pin `bf91071b…` is the **CRLF form at `31fd642`**; the file has moved since (at authoring time `origin/main` gave LF `104dafa8…` / CRLF `786009cc…`, explained by commit `25711e2`, the 2026-08-26 cap release), so do not require a match. Apply the same rule as the image pins: a moved tree-backed pin must have an explaining commit in `git log 31fd642..<merge SHA> -- tests/ops/test_m1_acceptance_drills.py`; with one, its merge-SHA value goes into the refresh (Step 2.10) with the form stated; without one, stop and return `FALSIFIED`. `31fd642` is a public commit and an ancestor of `main` (verified via `gh api repos/Joshua-Asante/first-passage/commits/31fd642`), so this range is valid in any **full-history** clone; the Phase 0 currency check therefore includes `git cat-file -e 31fd642^{tree} || git fetch --unshallow origin`. A shallow-clone `unable to read tree` is a clone condition, never a missing explanation. Equivalent with no local history: `gh api "repos/Joshua-Asante/first-passage/commits?path=tests/ops/test_m1_acceptance_drills.py&sha=<merge SHA>" --jq '.[] | select(.commit.committer.date > "2026-08-19T17:30:26Z") | .sha[0:7] + " " + .commit.committer.date'`. Then `python -c "import sys;sys.path.insert(0,'ops/c1_rail');import m1_stage1_contract as m;print(m.contract_sha256())"` in-container. Paste all.

### Step 2.10 — Acceptance JSON update (repo, on a `claude/*` branch)
- `fixture_hashes`: the in-container values from 2.9 for every image-carried pin (extend the pin set with the two `m1_stage1_*` listener modules if A4 recommended it); for `tests/ops/test_m1_acceptance_drills.py`, the merge-SHA tree value from 2.9 with its byte form and explaining commit named in the note — the 08-19 entry's "re-verified against the local tree" is the precedent, and a moved value with an explaining commit is a refresh, not a match failure.
