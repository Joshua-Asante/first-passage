...
**Status:** ⚠ **Fleet fully dispositioned, but Packet A is MERGED and NOT CONVERGED.** #303 (C) MERGED `6b5bc96`; #304 (B, withdrawn) CLOSED without merge; **#305 (A) MERGED `2a89348` — at the pure-merge head `2c67b34`, so fix round A2 never returned and never landed.** All three A2 items are live defects in `scripts/certification_power.py` on `main` (campaign-state §45), and that calculator must not size the `n₂`/`n₃` freeze until they are fixed. A merged PR cannot carry a fix round, so A2's frozen §6b scope is re-issued as a fresh packet **A3** off current `main` (branch `cursor/scripts-side-2026-09-05-p1`), operator-fired; a CC-solo fix is the alternative and the choice is the operator's. Nothing is owed to Cursor on #305 itself. See §6 for the manifest rows.
...
`python scripts/check_repo_map_scripts_table.py --write` (new row; the caption is whatever `--check` demands on YOUR merged tree — the tracked script count on the resulting tree; an A3 repair adds no new script — never a number copied from this brief). Nothing else.
...
|---|---|---|---|
| A — certification-power calculator | `cursor/scripts-side-2026-09-04-p1` | `scripts/certification_power.py`, `tests/test_certification_power.py`, `REPO_MAP.md` (generated scripts block only) | **FIX ROUND A1 RETURNED @ `b5b3fb4` — RESOLVED** (campaign-state §45; every §2 and §6b pin verified; both Codex threads resolved by the worker; `main` merged in, `--check` exit 0 on the merged head). First return `7b1418b` was RESOLVED on the §2 gate with two Codex P2s (§43). **Codex's round on `b5b3fb4` returned 3 × P2 (22:41Z), all verified → fix round A2 FROZEN in §6b, awaiting the operator's dispatch. Not converged; not to be merged before A2 returns.** Head moved 01:34Z 09-05 to `2c67b34` — a PURE MERGE of `main` (parents `b5b3fb4` + `6b5bc96`; tree byte-identical to `merge-tree(main, b5b3fb4)`; the three packet files unchanged), **not** the A2 fix: the A1 verdict carries, the §10 A block re-runs `AUDIT rc=0` on it, and all three A2 items are re-verified as still owed (campaign-state §45). **A2 DISPATCHED 2026-09-05 ~02:00Z** on the operator's instruction, off head `2c67b34` (level with `main` `6b5bc96`). Dispatch-moment gate run at dispatch time, not reused from the freeze: `main` re-fetched; the branch behind by 0; test 0 clean (stdlib only, no vendor bytes — cloud-safe); the open-PR overlap probe returned ONE overlap, #302 on `REPO_MAP.md`, **adjudicated by the orchestrator as not a blocker** (the scripts table is generated and never hand-edited; whichever PR merges second regenerates it, and §2-A already takes the caption from `--check` on the worker's own merged tree) — the pointer says so explicitly, since §0.9 otherwise tells the worker to return BLOCKED on any overlap. The pointer is self-contained: this brief lives on #302's branch, not on `main`, so the worker's worktree cannot read it. A Codex round was in flight on `2c67b34` at dispatch; it **returned zero findings 01:58Z**, so there is no A3 and the dispatched A2 scope stands unchanged. ⚠ That clean round does NOT mean the three A2 items are fixed — Codex does not re-raise threads already answered on a PR, and the orchestrator re-probed all three as live on that same head at 01:35Z. **⚠ MERGED 2026-09-05 as `2a89348` — AT `2c67b34`, THE PURE-MERGE HEAD. A2 never returned and never landed.** On the merged tree the three packet files are byte-identical to `b5b3fb4` and the packet branch is gone from `origin`. All three A2 items re-probed as live defects on `main` (campaign-state §45), so the packet is **MERGED but NOT CONVERGED**: `size_for_power(0.001, 1.0, limbs=1)` returns 410 instead of raising; `--true-rate 0.03 --power 0.8 --n 630` exits 0 instead of 2; `max_certifying_busts(8000, 0.9, 0.999999999)` returns 8000 against the pinned 7355. A merged PR cannot carry a fix round, so **A2's scope is re-issued as a fresh packet A3 off current `main`** — same frozen §6b text, new branch `cursor/scripts-side-2026-09-05-p1`, operator-fired; the alternative is a CC-solo fix, and which one is the operator's call. Until it lands, the calculator must not size the `n₂`/`n₃` freeze. |
