...
| C — `repo_hygiene` without `gh` | `cursor/scripts-side-2026-09-04-p3` | `scripts/repo_hygiene.py`, `tests/test_repo_hygiene.py` | **RETURNED `DONE` @ `0e7ab25`** ([#303](https://github.com/Joshua-Asante/first-passage/pull/303)) — **FALSIFIED** (§43): guards `_run` (§5 forbidden move; built from the pre-re-freeze revision), pinned git-raises test absent; Codex 1 × P2 is that consequence → **fix round C1 DISPATCHED** (§6b) |
