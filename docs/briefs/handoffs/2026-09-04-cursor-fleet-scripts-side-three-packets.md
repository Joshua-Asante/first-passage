...
|---|---|---|---|
| A — certification-power calculator | `cursor/scripts-side-2026-09-04-p1` | `scripts/certification_power.py`, `tests/test_certification_power.py`, `REPO_MAP.md` (generated scripts block only) | **RETURNED `DONE` @ `7b1418b`** ([#305](https://github.com/Joshua-Asante/first-passage/pull/305)) — **RESOLVED** on the §6 gate (campaign-state §43; §10 hooks pass); Codex 2 × P2 verified → **fix round A1 DISPATCHED** (§6b). Cut from `ba713ee`, not `main` ≥ `81b35d0` (fired before that instruction existed): merge-tree with current `main` is clean and `--check` exits 0 on the merged tree; A1 merges `main` in |
...
| C — `repo_hygiene` without `gh` | `cursor/scripts-side-2026-09-04-p3` | `scripts/repo_hygiene.py`, `tests/test_repo_hygiene.py` | **FIX ROUND C1 RETURNED @ `94fa685` — RESOLVED** (campaign-state §44; every §6b pin verified; Codex thread resolved by the worker). First return `0e7ab25` was FALSIFIED (§43: guarded `_run`). **Mergeable by the operator once CI on `94fa685` is green.** |
