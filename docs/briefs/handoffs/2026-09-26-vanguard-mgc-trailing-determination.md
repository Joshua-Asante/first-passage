...
**Gate A caveat work (DRAFT, 2026-09-26, coordinating session; part of the [Gate A disposition](2026-09-25-crosstrade-rest-route-assessment.md#611-gate-a-factual-disposition-draft)):**
...
- **Caveat 2: refined; the qualification binding is not the runtime binding.** `load_qualification_adapters` (`ops/c1_signal_daemon/book_adapters.py:246-257`) reads a separate `effective_settings_successor` artifact bound by the frozen contract's `effective_settings_sha256`. `_qualification_snapshots` (`:205-243`) **refuses** the historical `EFFECTIVE_INPUTS_SHA256`, and requires ORB `qty` = 1. `build(**row['adapter'])` has the same semantics (`:310`). So the verdict binds the runtime identity (`9d4d4e1d…`). For qualification it holds exactly when the frozen successor's Vanguard row leaves the trail input unset. No successor exists yet. **Residual condition:** at freeze, the successor's Vanguard row must be checked against the operator's edition-versus-rejection decision ([pre-registration](../pre-registration/2026-09-26-tradeify-vanguard-fixed-stop-edition-prereg.md) effective-inputs row, owner). The pre-registration already names a successor file as the input-only realization path.
