...
3. **The scale-coupling question is pre-resolved — see §0.** No code change to `build_scaled_panel` or the `expect3` dict's `"scale"` guard is needed or wanted; `AEGIS_RISK` parameterization is orthogonal to it.
