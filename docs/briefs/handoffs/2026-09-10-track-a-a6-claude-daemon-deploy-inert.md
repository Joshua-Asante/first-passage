...
- `curl -sS https://c1-signal-daemon.fly.dev/` → `poll_interval_s:1` (the running value, reported by the A1b heartbeat field; any other value is a NO-GO for A7 — the config is re-staged **and the machine restarted** (`fly machine restart <id> -a c1-signal-daemon`; `run_daemon` caches the interval at startup, so re-staging alone changes nothing), then this health check is re-run before proceeding), `emit_enabled:false`, `effective_emit:false`, `strategy:"NullStrategy"`, `connected:false`, `feed_healthy:false`, `ceremony_state:"DISABLED"`, `feed_mode` = the A1b-defined disconnected value, `boot_id` new.
