...
Publication and effective-revision times are unknown. These are documents, not account observations. The eleven pins are registered in `docs/evidence/PRIVATE_EVIDENCE.sha256`. All 54 captured files are archived in `first-passage-archive` (M-41), and every committed blob hashes to its own name. The first attempt failed: the repository was archived on GitHub, and `core.autocrlf` changed one blob. The operator unarchived the repository on 2026-09-24, and [#481](https://github.com/Joshua-Asante/first-passage/pull/481) made the tool line-ending safe.
