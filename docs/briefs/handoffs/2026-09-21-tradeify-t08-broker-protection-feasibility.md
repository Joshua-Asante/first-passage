...
Executed 2026-09-24 by the coordinating Claude Code session at the operator's direction ("start with T08 step 1"). This is documentary work only. No login, account page, form, support contact, order action or drill took place. Vendor pages were retained as raw bytes under the primary checkout's gitignored `local_artifacts/t08-r3-2026-09-24/`, which holds 1,802 files and 72.2 MB. Every file is hashed in `EVIDENCE_INDEX.sha256` there; that index's own SHA-256 is `974ea080aaa3dc0a3d421dea7a1dcd15af2786922b2ca8d239513a40edba065a`. No vendor text is reproduced in this repository.
...
- **The CAP pages have changed since 2026-09-17.** All three CrossTrade pages CAP retained have different bytes now. The API overview is `f84d10c1…` (was `dd7f839a…`), the webhook advanced/internals page is `adaa513e…` (was `ebba0572…`), and cancel-replace is `21a8e8c7…` (was `a0000d0d…`). CrossTrade's docs were rebuilt on 2026-09-21. CAP's 09-17 statements about these pages should be re-read against the new bytes before anyone relies on them. The Tradovate order-status article is unchanged (`cedcc29c…`).
...
2. **T09 and the live route.** T09 stays unspecifiable, and CAP R3 stays blocked for live release. Moving forward needs an operator decision among contract or route options, for example:
   ...
   Any contract change is an ADR-level decision. This return proposes none.
