...
- (D) **Workflow triggers.** Default: `pull_request` and `push` filtered to `deploy/**`, `.dockerignore`, `ops/c1_rail/**`, `ops/c1_signal_daemon/**`, `scripts/validate_c1_monitoring_acceptance.py`, `docs/notes/rail_build/M1_MONITORING_ACCEPTANCE.json`, `tests/ops/**`, `scripts/c1_image_validation.sh`, the workflow file itself; plus `workflow_dispatch`. Not a required merge check (the ruleset owner is Q-GATESTACK-1's closure; do not edit it).
...
- **D2 contents:** file set equals the COPY sources (includes `ops/c1_rail/m1_stage1_contract.py`, `ops/c1_rail/__init__.py`); `python -c "import databento"` fails; no `requirements.txt` in the image.
