...
- **D2 contents:** file set equals the COPY sources (includes `ops/c1_rail/m1_stage1_contract.py`, `ops/c1_rail/__init__.py`); `python -c "import databento"` fails regardless of any later source decision; the third-party distributions installed in the image (`pip list --format=json` minus the base `python:3.12-slim` image's own set) equal exactly the set of distribution names in `deploy/c1_signal_daemon/requirements.txt` when that file exists on the merge SHA, and are empty when it does not. That file, when present, must be a **fully resolved, hash-pinned lock** naming every transitive distribution (the `pip-compile --generate-hashes` form this repo already uses for `requirements-ops.lock`, installed with `pip install --require-hashes`); a requirements file naming only top-level packages is itself a `FAIL D2`, because a hand-authored list cannot be compared to the installed closure and an unlocked production image is not reproducible. A ratified source may later add such a lock (Track A plan §3.1); A1b updates this expectation and the fixtures in its own PR, so the check is parameterized by the declared lock, never a flat prohibition.
