...
Phase-0 proved **perfect selection** among dense RTH 1m opens at G=10 clears EM1 while
all-take does not. That licenses designing a **trade object** on this universe — not a
selector claim and not an OF ρ-cell.

---

## §3 — Question

Can a pre-registered **independent hard-stopped** construct on dense RTH 1m opens (entry rule
named without lookahead; stop **G=10**; session-complete ≤16:00 ET) clear EM0–EM5 / TNEC N-SHAPE
toward `SHAPE-CLEAR` and, under later GO, TNEC N-ACT…N-SIZE — or does every frozen entry rule fail?
(MNQDTL D1/D2 are preferences only — [ADR 2026-08-08](../adr/2026-08-08-edge-cohort-correction-and-necessity-retarget.md).)

---

## §4 — Falsifiable hypothesis (H-CON-1)

**Hypothesis H-CON-1:** A Stage-0–frozen construct with the §7 geometry attests
`SHAPE-CLEAR` (or typed U only where still unscored) at G0, then under separate GO clears its
pre-registered edge limbs on EXPLORATION without Cap claim unless reserved.

**Falsifier:** If Stage-0 cannot name a causal entry rule distinct from oracle foresight, or
EM1/EM2 remain U after stop/R exist because the entry rule is not tradeable → `FALSIFIED` /
typed VOID; STOP construct catalogue; re-proposal = new entry mechanism (not G retune alone).

---

## §5 — Forbidden moves

- Scoring a selector that uses future path labels (oracle laundering).
- OF ρ retune as substitute for entry+stop.
- Cap claim without Cap-reservation GO; ORB unpark; Pine/deploy without GO.
- Elevating Phase-0 diagnostics G=5/20 into this construct after seeing Phase-0 numbers
  (gate G=10 frozen).
...
|---|---|---|
| `RESOLVED` | Stage-0 freeze + later GO path clears pre-registered edge limbs | INTEGRATE candidate record |
...
## §7 — Construct geometry (Stage-0 freeze — operator 2026-08-08)

| Element | Frozen value |
|---|---|
| Universe | RTH Mon–Fri 09:30–15:59 ET 1m opens `MNQ.v.0` (MNQSEL-2 panel, roll-excluded) |
...
```bash
test -f docs/briefs/Q-MNQDTL-CON-1-dense-1m-em-construct-scoping.md
rg "Hypothesis|Falsifier|falsified" docs/briefs/Q-MNQDTL-CON-1-dense-1m-em-construct-scoping.md
rg "RESOLVED" lab/analysis/c1/mnq_selection_ceiling_allbars_2026-08/RESULTS.md
```
