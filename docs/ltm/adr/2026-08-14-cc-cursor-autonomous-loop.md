<!-- relocated from docs/adr/; relative links rewritten 2026-09-15 -->
...
> narrowing of "no commit/merge without operator go" is withdrawn, not inherited. The body is
> preserved unedited below as the record of a ratified decision; read it as history. Current
> owner: [worker-surface allocation](../../adr/2026-07-14-cc-cursor-surface-allocation.md), Revision
> 2026-09-15 (§Decision, §8 disposition table). Retirement actions are in
> [§11 below](#11--retirement-2026-09-15-proposed).
...
ruled for one campaign as D-B6 on 2026-09-11; generalised repo-wide by the parent ADR's
2026-09-15 revision.

**Why retired in full, not in part.** Every operative clause here is keyed to the Cursor lane:
§2.1 dispatches via `scripts/dispatch_cursor.ps1` (deleted by the same sweep), §2.2 detects
`cursor/*` PR-ready events (a namespace with no branches, local or remote), §2.3 auto-merges
those PRs. Remove the lane and no clause has a referent. There is no surviving fragment to
restate here; the general rules that outlived it are stated by the parent.

| Clause | Disposition |
|---|---|
| §2.1 dispatch without chip approval | **Explicitly retired.** Proactive-dispatch *authority* survives at the parent (routing test 3, read as an act, not a label); the chip mechanism and its exception path do not. |
| §2.2 webhook detection on `cursor/*` PR-ready events | **Explicitly retired.** See the live-integration note below — the webhook itself is account-level and outside this tree. |
| §2.3 + the binary auto-merge gate (a–e) | **Explicitly retired.** Merge authority returns to the operator with no automated exception. Conditions b and c (full gate battery; full `pytest`) survive as ordinary pre-merge discipline under `CLAUDE.md` §Gate composition authority — not as a merge *gate*. |
| Auto-merge-forbidden surface list | **Explicitly retired as a gate input.** It never governed what a worker may *build* — that is the parent's routing test 1, which stands unchanged. Retiring this list therefore widens no build eligibility. |
| §4 limb 1 (auto-merged defect) | **Explicitly retired — unreachable.** No auto-merge can occur, so the predicate can never be measured. Retired rather than left standing, per this repo's own lesson on unreachable gates (`lesson_gate_reachability_preregistration`). |
| §4 limb 2 (surface-list breach) | **Explicitly retired — unreachable**, same reason. |
| §4 limb 3 (silent-failure recurrence in the detection link) | **Explicitly retired — unreachable.** The detection link is gone. |
| §4 limb 4 (parent falsifier, inherited) | **Retained at the parent**, re-scoped to worker PRs on the surviving surfaces. Never this ADR's to carry. |
| §5 forbidden moves | **Explicitly retired** as written — each names the auto-merge gate. The parent's forbidden moves carry the surviving generic bars, including the unnarrowed no-merge-without-operator rule. |
| §6 heartbeat mitigation (`daily-repo-truth-sync` classifying `cursor/*` branches SPENT / CARRIES-WORK / UNKNOWN) | **Explicitly retired.** That scheduled task lives on the operator's machine, outside this repo; its `cursor/*` step is now a no-op. Flagged below — no repo change can reach it. |
| §7 Phase 1 webhook + its same-day addendum | **Explicitly retired as repo doctrine.** The live integration is an operator action, below. |

**Live integrations this repo cannot revoke — operator action required.** Two mechanisms were
created outside this tree and are unaffected by merging this retirement:

1. **The GitHub webhook trigger** recorded in the 2026-08-14 addendum (routine
   `trig_012nvuH7jqmjFUFgoFVpZ6RP`, firing unfiltered on `pull_request: opened`). It is an
   account-level routine, not a repo file. Retiring this ADR removes its *authority*, not the
   trigger. **The operator should delete or disable that routine**; until then it keeps firing a
   routine whose §2 sequence has nothing left to act on.
2. **`daily-repo-truth-sync`**, the scheduled task whose step 2 classifies `cursor/*` branches.
   Harmless — report-only, and the namespace is empty — but stale. The operator's to prune.

Neither blocks this revision; both are recorded so the retirement is not mistaken for complete
at the account level.

**What is deliberately NOT retired.** The 2026-08-29 bar carried by the parent — *this repo's CI
grants Codex no write/push credential without a superseding ADR* — is untouched. A reader should
not infer from "the Cursor automation is gone" that the credential bar lapsed with it.
