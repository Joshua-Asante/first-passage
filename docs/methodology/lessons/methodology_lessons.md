...
- **Rule:** a digest is not a copy. Every private file a public record pins is registered — in a tracked `*SHA256SUMS` / `*MANIFEST.sha256` manifest or in `docs/evidence/PRIVATE_EVIDENCE.sha256` — and archived to the private `first-passage-archive` repository with `python scripts/evidence_archive.py put <file>`, then pushed, in the same step as the pin. Private outputs still go under an ignored root of the primary checkout, never under `.worktrees/*` or `.claude/worktrees/*` (caution, 2026-09-24: root `local_artifacts/` and `inputs/private_overrides/` are **not** gitignored — only campaign-level ones such as `lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/local_artifacts/` are; check with `git check-ignore -v <path>`); worktree sessions read and write them by absolute path. Before any worktree removal, `scripts/repo_hygiene.py` lists the worktree's ignored entries; stop if any is unarchived.
