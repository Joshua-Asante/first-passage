...
- **Fourth firing (2026-09-24 — a checkpoint commit is not preservation):** a blanket add + commit in the primary checkout ("chore: checkpoint readable workspace material", `272028e`, branch `claude/s2-g5-contract-identity`) tracked 1,025 previously untracked `recovery/` evidence files plus scratch. System-wide `core.autocrlf=true` (`C:/Program Files/Git/etc/gitconfig`) converted CRLF→LF on commit, so 252 of 606 SHA256SUMS-pinned files no longer matched their pins as committed; the subsequent switch to `main` then deleted every working-tree original (tracked on the old branch, absent on the new one), leaving the lossy commit as the only copy. Recovery needed a bundle, `git cat-file` extraction and line-ending reconstruction against every in-packet digest (354 files as committed, 311 full CRLF, 16 final-line CRLF, 8 from zip copies, 1 mixed file with CRLF on lines 29 and 33); all 606 pins verify again, the files are archived via `scripts/evidence_archive.py put`, and PR #492 registers the 9 docs-pinned digests and adds ignore rules for the roots.
- **Rule addendum (2026-09-24, preservation and extraction):** committing untracked evidence is not preservation — it is lossy under `autocrlf`, and a later branch switch deletes the originals. The preservation path is `evidence_archive.py put` first, then any commit that must carry evidence bytes runs with `-c core.autocrlf=false`, then verify against the pins (`evidence_archive.py audit`). The extraction path is `git cat-file` (raw blob bytes); `git archive` and `git checkout` also apply `autocrlf` and are unsafe extraction paths for pinned evidence.
