...
- **Rule:** a digest is not a copy. Every private file a public record pins is registered — in a tracked `*SHA256SUMS` / `*MANIFEST.sha256` manifest or in `docs/evidence/PRIVATE_EVIDENCE.sha256` — and archived to the private `first-passage-archive` repository with `python scripts/evidence_archive.py put <file>`, then pushed, in the same step as the pin. Private outputs still go under the primary checkout's ignored roots (`local_artifacts/`, `inputs/private_overrides/`), never under `.worktrees/*` or `.claude/worktrees/*`; worktree sessions read and write them by absolute path. Before any worktree removal, `scripts/repo_hygiene.py` lists the worktree's ignored entries; stop if any is unarchived.
