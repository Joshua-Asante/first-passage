...
- **Rule addendum (2026-09-24, preservation and extraction):** committing untracked evidence is not preservation — it is lossy under `autocrlf`, and a later branch switch deletes the originals. The preservation path is `evidence_archive.py put` first. Any evidence bytes that must also be committed are staged with `git -c core.autocrlf=false add` (the conversion happens when a file is staged, so the override on `commit` alone is too late) or under a `-text` attribute, as the archive's `evidence/.gitattributes` does. Then verify with `evidence_archive.py audit --verify`, which re-hashes the archived blobs; plain `audit` only checks that a blob named for each digest exists. The extraction path is `git cat-file blob <sha>` (raw blob bytes; not `--filters`, which converts); `git archive` and `git checkout` also apply `autocrlf` and are unsafe extraction paths for pinned evidence.
