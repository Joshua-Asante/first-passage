...
- **Rule:** a digest is not a copy. Every private file a public record pins is registered — in a tracked `*SHA256SUMS` / `*MANIFEST.sha256` manifest or in `docs/evidence/PRIVATE_EVIDENCE.sha256` — and archived to the private `first-passage-archive` repository with `python scripts/evidence_archive.py put <file>`, then pushed, in the same step as the pin. Private outputs still go under an ignored root of the primary checkout, never under `.worktrees/*` or `.claude/worktrees/*` (caution, 2026-09-24: root `inputs/private_overrides/` is **not** gitignored — only campaign-level ones such as `lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/inputs/private_overrides/` are; root `/local_artifacts/` has been ignored since 2026-09-24, but an ignored root is not an archive; check with `git check-ignore -v <path>`); worktree sessions read and write them by absolute path. Before any worktree removal, `scripts/repo_hygiene.py` lists the worktree's ignored entries; stop if any is unarchived.
- **Enforcement (2026-09-24):** `make audit` runs `evidence_archive.py audit`, which reports each pinned digest as ARCHIVED, UNPUSHED or MISSING. The three losses were silent because nothing listed what had only one copy; the audit is that list.
- **Fourth firing (2026-09-24 — a checkpoint commit is not preservation):** a blanket add + commit in the primary checkout ("chore: checkpoint readable workspace material", `272028e`, branch `claude/s2-g5-contract-identity`) tracked 1,025 previously untracked `recovery/` evidence files plus scratch. System-wide `core.autocrlf=true` (`C:/Program Files/Git/etc/gitconfig`) converted CRLF→LF when the files were staged, so 252 of 606 SHA256SUMS-pinned files no longer matched their pins as committed; the subsequent switch to `main` then deleted every working-tree original (tracked on the old branch, absent on the new one), leaving the lossy commit as the only copy. Recovery needed a bundle, `git cat-file` extraction and line-ending reconstruction against every in-packet digest. Of the 606 SHA256SUMS-pinned files, 354 matched as committed and the 252 mismatches were restored as 236 full CRLF, 14 final-line CRLF, 1 from a zip copy and 1 mixed file with CRLF on lines 29 and 33; counting every file pinned by any in-packet digest (those 606 plus 265 pinned only outside SHA256SUMS, 871 in all), 535 matched as committed and 336 were restored (311 full CRLF, 16 final-line CRLF, 8 from zip copies, 1 mixed). All 606 SHA256SUMS pins verify again and the files are archived via `scripts/evidence_archive.py put`. Two follow-ups were open PRs when this was written: PR #492 registers the 9 docs-pinned digests, and PR #493 adds the root-anchored ignore rules (`/recovery/`, `/tmp/`, `/tmp-*/`, `/tmp-*.md`, `/tmp-*.py`) and the `staged-debris` commit gate. Both have since merged (#493 at `36a1f08`, #492 at `b7d42ad`), so the tracked `.gitignore` covers those roots and the `staged-debris` gate runs in pre-commit and CI.
- **Rule addendum (2026-09-24, preservation and extraction):** committing untracked evidence is not preservation — it is lossy under `autocrlf`, and a later branch switch deletes the originals. The preservation path is `evidence_archive.py put` first. Any evidence bytes that must also be committed are staged with `git -c core.autocrlf=false add` (the conversion happens when a file is staged, so an override on a commit of already-staged files is too late). That override does not switch off `text`/`eol` attributes: the root `.gitattributes` sets `ops/calendars/evidence/*.json text eol=lf`, for example, and `add` still converts CRLF there. So check `git check-attr text eol -- <path>`, keep such evidence under a `-text` attribute (the archive's `evidence/.gitattributes` is `* -text`), and before committing confirm that the staged blob id from `git ls-files -s <path>` equals `git hash-object --no-filters <path>`. Then verify with `evidence_archive.py audit --verify`, which re-hashes the archived blobs; plain `audit` only checks that a blob named for each digest exists. The extraction path is `git cat-file blob <sha>` (raw blob bytes; not `--filters`, which converts); `git archive` and `git checkout` also apply `autocrlf` and `text`/`eol` attributes and are unsafe extraction paths for pinned evidence.
- **Link:** `core/strategies/PORT_MANIFEST.sha256` loss records; `scripts/evidence_archive.py`; the `repo-hygiene` skill.
