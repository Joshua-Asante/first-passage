...
- **Pending — data-dependent Phase 2 code:** (a) **Call-1 σ-source + harness** (reads `baselines.md` + a live-PF source; applies `decay_breach`/`autonomous_demote`; writes demotions into `lifecycle_state.json`) — σ-source design in flight; (b) the **beta-cohesion diagnostic** (lagged-correlation lead-lag on historical panels; informs the soft-flag interim review) — landed at [`lab/research_utils/beta_cohesion.py`](../../lab/research_utils/beta_cohesion.py) + [`scripts/beta_cohesion_read.py`](../../scripts/beta_cohesion_read.py). Report-only; does not write `lifecycle_state.json`. Call-1 σ-source remains pending.
