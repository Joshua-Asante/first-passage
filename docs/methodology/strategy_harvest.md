...
**Why this lane exists (evidence-contingent — owner ADR §4 is still live; K-reasoning current as of [ADR 2026-08-04](../adr/2026-08-04-family-k-bank-disclosure-not-gate.md), Requirement 3 below):** wide mining's cost is its own `K_intrinsic` — the trial count of the search itself — which for a genuine wide sweep (hundreds to thousands of candidates) drives its own DSR floor high. `K_banked(family)` no longer enters that arithmetic: the 2026-08-04 ADR retired the family term as a gate, so a pre-registered single-hypothesis seed screens at floor 0.65 on *every* family, GC/MGC included (Requirement 3 below). Q-GATECART-1 M-19's DSR floor 2.05 at banked K = 3,177 is a **historical measurement under the pre-amendment rule** — it describes GC/MGC's now-disclosure-only family bank, not a live ceiling on new seeds. What harvest actually buys is a small `K_intrinsic`: the original author already paid their own search cost, so confirming their mechanism on our data enters at K_intrinsic ≤ 3 → floor 0.65–0.98, versus an in-house wide search's own large `K_intrinsic`. Inside this lane the seed's job is to **confirm**, not to mine. That does not make harvest the primary or sole fundable discovery route. D5 (Baltussen et al. 2021 *JFE* intraday momentum) is the only axis ever to PASS the two-clause screen, and it is a harvested public strategy ([`d5_clause_n_rescreen.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/lab/archive/q_kbudget_1_2026-07/d5_clause_n_rescreen.md), verdict RESOLVED 2026-07-15); D5, H-OD-1, and H-TSMOM-1 all closed 2026-07-16 without a successor. Idle guard and limb counts live on the owner ADR §4. Cross-read [`N-2026-08-25-ox-alpha-mechanism-gate-overcorrection.md`](../notes/notice/N-2026-08-25-ox-alpha-mechanism-gate-overcorrection.md) — the mechanism gate's false-negative rate has never been measured (named gap); the stronger "therefore the gate is over-correcting" inference does not survive.
...
- **Tier A — fund-first.** Low-frequency, large-per-event-δ, futures-native: time-series momentum/trend, carry, term-structure/roll-yield, factor-momentum, positioning-extreme reversal. `H-TSMOM-1` (Moskowitz-Ooi-Pedersen TSMOM, ES family) was Tier A's screen-PASS candidate and the intended reference case for what a fundable seed looks like post-cost — ⚠ **Correction 2026-08-31:** it closed 2026-07-16 as a Clause-N FAIL at the scoping stage, before ever reaching Stage-2 cost-law ([`H-TSMOM-1-ES-tsmom-scoping.md`](../briefs/rnd-pipeline/H-TSMOM-1-ES-tsmom-scoping.md), `CLOSED`). Tier A currently has no live occupant.
...
| ~~**1-tie**~~ | ~~**Structural flow census**~~ — **RETIRED 2026-08-24** ([ADR](../adr/2026-08-24-sourcing-phase-channel-retirement.md), superseding [ADR §2-B](../adr/2026-07-26-mechanism-counterparty-constraint-boundaries.md) in part) | Zero admissible seeds across all 4–5 real passes (2026-07-26 → 2026-08-01); no future entry opens under this rank. | Original row preserved below (struck, not deleted, per dated-decision integrity) — do not source under this rank going forward. |
...
### 2.4 Standing radar cadence — zero new code (shape pre-check added 2026-08-24)

Rides the existing quarterly review dates already on the forward board (2026-08-08, 2026-11-08) rather than adding a scheduler — consistent with this ADR's own §3 "automate last" ordering and current low seed volume. Each pass: (1) citation-graph traversal per rank-1, scoped, seeded from every Tier-A axis confirmed since the last pass; (2) class-directed query-family pass — **inherits the frozen Q1–Q6 families from Q-KBUDGET-HARVEST-1 by reference, never edits them**; a pass wanting new families opens its own pre-registration; (3) survey-paper harvest per rank-2; (4) **shape pre-check** — score each staged candidate's predicted payoff profile (win rate, mean-win/mean-loss shape) against the current venue's feasibility region before it clears §2.2's cost sniff test; a candidate outside the region is a shape-kill, recorded and dropped, never staged into a manifest. Current instance: `Tradeify_Select_100K` requires FEASIBLE win rate + large mean win at a floor that is **shape-specific, not one flat ~65–70% band** — ⚠ **Correction 2026-08-31:** read the live per-shape, per-risk floor directly off the [`shape_feasibility_map_2026-08/RESULTS.md`](../../lab/analysis/c1/shape_feasibility_map_2026-08/RESULTS.md) §6.3 table (three shapes × three EM2 risk levels, each cell distinct — do not paraphrase it into one flat number per shape, that reintroduces this same class of error) before scoring it, never the rounded ~65–70% band, which understates every shape's floor at its cheapest risk level — this binds ahead of, not instead of, §2.2; (5) stage the surviving candidate rows, apply §2.2's sniff test before opening any manifest. A thin on-demand Semantic Scholar script is the named automate-later step, gated on seed volume rising enough to justify it — not built now.

**Why added:** §2.1's Tier-A momentum/carry/trend priority list aims sourcing at low-frequency, large-per-event-δ mechanisms on cost grounds alone; it does not check the resulting payoff *shape* against what the ruled venue can actually survive. Phase B of the viable-strategy sequence ([plan](../superpowers/plans/2026-08-23-viable-strategy-phase-b-mechanism-supply.md)) already runs this check per-lane (its B1.4-class step); this amendment folds the same check into the radar's standing cadence so a future pass does not re-source a mechanism-class-fund-first-but-shape-wrong seed the way `H-TSMOM-1` was (screen-PASS, but never checked against a venue shape requirement because none was published yet — the map landed 2026-08-23, after that seed's 2026-07-16 screen).
