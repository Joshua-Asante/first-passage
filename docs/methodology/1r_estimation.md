...
> ⚠ **DORMANT 2026-08-14.** The live-calibration section below asserts a
> present-tense verdict against `ops/accounts.py::calc_multiplier`, a function
> deleted 2026-07-24 (`ops/accounts.py` no longer exists; no `.py` file
> references `calc_multiplier`). This file is a historical record of a
> 2026-04-25 verification, not a live cross-check. Do not cite the
> live-calibration verdict as current.

**Date established:** 2026-04-17
**Status:** Dormant — live-calibration depends on deleted `ops/accounts.py::calc_multiplier` (2026-08-14).
...
peak-drawdown trigger (`DD_TRIGGER`, C2 since the 2026-05-08 relock) which cuts
the day's sizing by 0.40× the moment equity drops 1.5% below peak. So the live system is balance-compounding at weekly
...
intra-week balance drift bounded by `dd_protection`'s 1.5% trigger.
...
multiplier verified directly from the raw `Size (qty)` columns of all 29
layer-1 / layer-2 pairs is internally consistent to 4dp across the whole
panel (no implementation drift) — **exact multiplier value redacted from
the public tree, 2026-08-24, per
[`docs/adr/2026-08-14-repo-public-visibility-transition.md`](../adr/2026-08-14-repo-public-visibility-transition.md);
see the private operational archive**. Multiple pairs (e.g. 2022-08-12,
2026-01-06) are exact to 4dp. Every pair also entered with positive
layer-2-vs-layer-1 entry-price delta, confirming the pyramid trigger is
gated on favorable layer-1 movement.
...
2. **The 17% headline inflation is a measurement artefact** of counting
   ...
   Pyramid layers run at a larger multiple of initial qty and stop out at
   a correspondingly larger fraction of pyramid-layer equity (exact
   multiplier and SL-adjacent figure redacted from the public tree,
   2026-08-24 — see the private operational archive), which by
   construction is larger than initial-layer equity at the moment the
   pyramid was added.
...
Tradeify election owner (this CFD-era fork is not the
...
· [`closure`](../briefs/closures/Q-TRADECAP-2-closure-resolved.md).
...
The program-level prior this specific divergence sits *inside* — expect live to
retain only a fraction (≈30–70%) of backtest edge before any strategy-specific gap —
is the `docs/methodology/backtest_live_shrinkage.md` convention.
