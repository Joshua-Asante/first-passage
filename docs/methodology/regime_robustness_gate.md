...
1. Take the locked panel (e.g. 52-month Pepperstone — ⚠ retired 2026-08-02, bytes deleted, kept below only as the worked-example panel; there is no canonical CFD feed, see [pepperstone-feed-retirement ADR](../adr/2026-08-02-pepperstone-feed-retirement.md). No MC-engine-eligible replacement panel is admitted today: `core/mc/modes.py::PANELS_BY_BROKER`'s only other entry, `"cme"`, is a research-tooling data source scoped to `breadth.py` only — its own admitting [ADR](../adr/2026-08-19-cme-broker-panel-admission-for-breadth-revival.md) states it "does not admit any candidate into any live book" and is not wired into this gate's MC-sweep path; do not substitute it here). Treat as a daily DataFrame with one row per business day.
