...
- **Handoff premises that failed Phase-0 (recorded, not worked around):** `work-decomposition` exists nowhere in this repo or its history and "retirement record pending" appears nowhere, so the task's deliverable 4 and its fold-in option for `cursor-fleet` were both unexecutable; `scripts/repo_hygiene.py` has no `cursor/*` branch handling. Same class as `feedback_web_advisor_handoff_confabulates_repo_state`.
