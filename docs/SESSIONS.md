...
## 2026-08-29j — Codex second look lands as its native GitHub integration, not a repo workflow

**Focus:** operator directive to move the Cursor-PR auto-review-request (2026-08-23 addendum) from Claude to Codex. Built a repo-workflow CLI-based design first; corrected within the same session after the operator asked Codex directly about the required secret and got pointed at Codex's own native, secret-free GitHub review integration instead — verified independently via `WebSearch` before acting on it, then landed as the final state.
**Shipped:** `docs/adr/2026-07-14-cc-cursor-surface-allocation.md` addendum 2026-08-29 (rewritten in place, still unmerged, to record the correction per Rule 14 rather than pile on a second addendum for same-day unmerged content). Net repo diff: **none** beyond the ADR and this entry — `codex-judgment-review.yml` / `check_codex_judgment_review.py` / its test (including the same-session adversarial `fable-judge`-prompt mode built on top of it) were added and then deleted before merge; `notify-cursor.yml` reverted byte-identical to `origin/main`; `claude.yml`'s `allowed_bots` narrowing (drop unused `github-actions`) is the one change that survives, since it's correct regardless of mechanism. Operator connected GitHub to Codex and enabled automatic reviews for open PRs on this repo directly in `chatgpt.com/codex/settings/code-review` — an account-level setting, not a repo file.
**Decisions/defects:** the first design assumed Codex has no GitHub-App listener at all (wrong) and would have needed a `CODEX_ACCESS_TOKEN` secret that was never going to be provisioned (per Codex's own guidance) — so it would have sat permanently red. Caught before merge, not after. Adversarial fable-judge mode dropped too: no per-PR custom-prompt equivalent in the native integration (`AGENTS.md` review rules are repo/path-scoped); revisit later via `openai/codex-action@v1` + `OPENAI_API_KEY` if still wanted. Write-access ruling from the first pass still stands and is unaffected: this repo's tree grants Codex no push credential; `@codex fix it` is the operator's own account-level tool, a different trust boundary entirely.
