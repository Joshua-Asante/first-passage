...
**Open / next:** STATE queue: `#1` [Seven-strategy Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md) — **seven prerequisites open: 1, 2, 3, 6, 7, 8, 9** (the count has risen every review round; the freeze was never as close as earlier entries reported). **THE NEXT DECISIONS ARE THE OPERATOR'S, AND WORK IS BLOCKED ON THEM:** **D30** — the two historical 200K chart states are **UNRECOVERABLE** (operator established, §26), so prerequisite 3 **cannot close by capture**; the ruling is whether the capital-only attribution is re-established by a fresh 200K re-run from the pinned body (a new source, full G1.1–G1.10) or recorded `UNESTABLISHED` — orchestrator recommends `UNESTABLISHED`. **Do not dispatch a session to capture those states.** **D31 (NEW, §27b)** — cross-configuration multiplicity: the Phase 3 contract waived it while the Phase 4+5 row deploys on the per-configuration bound, so a best-of-K winner's bound is optimistic and there is **no post-hoc repair** once the frozen path budget is spent; three repairs named, orchestrator recommends a **fresh independent winner-validation sample `n₃`** on a third disjoint seed stream. **RULED, work owed:** D26 (a) declare the Pine input overrides — values in a **gitignored artifact + tracked digest**, never in a tracked file (⚠ no `.gitignore` rule reaches a non-CSV artifact under `lab/analysis/**/` today; the rule must land before the capture does); D27 Route B (excursion-bounded DD); **D28 = repair (ii)** — the 5.0% bound comes from the **independent stage-2 paths alone**, stage 1 a pure screen — ⚠ **the n₂ sizing figures were WITHDRAWN §27** (they evaluated the bound at the expected bust count, giving 56–74% certification power); `n₂` must be sized from a **frozen certification power**, ~630 paths at a true 3% rate for 80% — three conditions or it is cosmetic: a **disjoint seed stream**, `n₂` power-sized, and multiplicity now carried by D31; **D29** — prerequisite 2 closes by **policy ruling on the residual**, acceptance becomes a bracket (closed-trade ≤ panel ≤ scalar-coincident bound) and G1.4 never verifies the drawdown to the cent. **STILL OWED WITH NO RULING:** Codex's per-leg scaling read (must key on DOLLAR columns), and **prerequisite 7** — the live-state MC engine change: `simulation.py` models a pristine account and passing the live balance as `starting_equity` is invalid, because `preflight.py` scales the target and DD width off it. ⚠ **Phase 3 freezes the PROCEDURE, not the snapshot** (§24c) — the weekly token trade moves the account through Phase 5 and Phase 8, so the criterion is re-captured and re-run immediately before arming. ⚠ **Live account dollar figures were published to this public repo and are redacted forward-only** (§23a/§24a) — they remain in git history. **A redaction record states counts, field roles and coverage only** (§24e); it never carries a value, a pattern containing values, or which field failed — the literal detail stays private. §22 is the dispatch packet, and it is **not dispatchable until D30 and D31 are ruled**. #296 **merged** (main `ad5d1e5`) · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24) — on the campaign's critical path via Phase 8; weekly venue-idle token trade placed 2026-09-03.
