...
- **Open / next:** operator ratifies (or rejects) the ADR revision, the supersession, and the `a6` SUBTRACT — **R6, the three frozen packets "pending dispatch", is left deliberately unresolved and is not closed by a SUBTRACT ruling**. Operator-only, outside this tree: disable webhook routine `trig_012nvuH7jqmjFUFgoFVpZ6RP` and `daily-repo-truth-sync`'s `cursor/*` step; reconfirm the [subscription ledger](pursuits/SUBSCRIPTION_LEDGER.md) Cursor row (cancellation date / final charges still not supplied). Optional: wire `guard_shell_command.py` as a `PreToolUse` hook.
