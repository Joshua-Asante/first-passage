...
- **Second half of the session — delegation review, six operator rulings:** the operator's working org chart (Codex "Astra" executive, Opus coordinator, GLM 5.3 worker) had diverged from the [surface-allocation ADR](adr/2026-07-14-cc-cursor-surface-allocation.md), which still named Codex a worker and no GLM surface. Rulings: (1) revise that ADR; (2) keep the committed-file seat boundary; (3) give every harness the relevant memory via `LESSONS_INDEX.jsonl`; (4) quality first, operator attention second; (5) acceptance tests named in the card; (6) committed-handoff rule + Fable trigger conditions, on this branch. **Shipped:** dated 2026-09-20 revision of the ADR (seat table; reviewer ≠ planner; committed-handoff rule; six-item handoff contract; Fable lane with three named triggers and a budget; operator-minutes-per-accepted-change measure; §8 rows; prior text pinned at `9a55a9c7`); M-26 … M-48 migrated from Claude memory into [`methodology_lessons.md`](methodology/lessons/methodology_lessons.md) in the light format with the index regenerated (memory-name rows become `migrated` pointers; a cp1252 mojibake defect from the 2026-08-26 generation repaired in passing); two pointer lines in `AGENTS.md`. **Judgment:** Fable is a budgeted interrupt, not a seat — its triggers are the two-failure restart, milestone adjudication, and load-bearing contract authoring; the executive's existing PR review is the independent signature, so no second reviewer seat was created. Grok's benchmark/pricing claims about Fable were treated as unverified and non-load-bearing. **Review round 1 (two P2s, both verified and folded in the review-round commit on this branch):** the lightweight-issue exception was re-scoped to the six-item contract with a body-hash freeze (the one permitted non-file card), and an *Ownership and precedence* paragraph resolves the collision between the older "→ coordinator" wording and the Fable triggers — the coordinator keeps ownership and executes the rebuild/authoring on the lane. Producer template (`cc_handoff.md` §6.0) and consumer checklist (`handoff-verify` item 8) carry the same items.
- **Open / next:** pull the main checkout to a `CLAUDE.md`-free commit right after merge (worktrees load instruction files from the parent checkout); confirm in the next session that `AGENTS.md loaded` appears at startup. Fifteen untracked handoff packets sit in the main checkout's `docs/briefs/handoffs/` (2 GLM, 1 Astra, 12 others; `git status --short -- docs/briefs/handoffs/`) — the committed-handoff rule's first application: commit or tombstone each before the next dispatch that relies on it. Research-domain lesson stubs (regime, DSR, MC, instrument findings) remain `external-unmigrated` for a later batch.
...
- **Shipped:** `inspect_environment(…, *, host_config)` with a `docker_client` readiness check (the shared `protected_executable` moved into the environment module; `host.py` re-imports it) and an image observation reduced to the opaque ID, because `evidence/boundary/environment.json` is exported wholesale to a public CI artifact; `role_policy.TREE_BINDINGS` (`qualification_tree_bindings/v1`, table digest retained in the manifest), tree records with `uid`/`gid`/`mode`, creation as record → `mkdir 0700` → `chmod` → `chown` behind an umask/setgid precondition, and cleanup that rejects an edited record even when the filesystem matches and compares owner, group and 12-bit mode before any recursive deletion; a `workflow_dispatch`-only host-evidence workflow (`--host-only` ×2 + one boundary cycle with a nondefault protected client at `/var/lib/fp-docker/bin/docker`).
...
- **Judgment:** (1) **Record shape — the task specified a new ADR; the operator ruled against it after I put the admission rule to them.** `2026-07-14-cc-cursor-surface-allocation.md`'s subject *is* surface allocation, so retiring one of two surfaces is a scope change to that decision, and [ceremony tiering](adr/2026-08-08-adr-ceremony-tiering.md) (rev 2026-09-08) explicitly retired the "new sibling ADR per amendment" rule the Cursor pair already exemplified. Landed as a dated **Revision** (prior text pinned at blob `0bcd6699…`, converted to `Format: concise`, six addenda folded into a §8 disposition table); the sibling is superseded **in full**, not in part, because every clause keys to `cursor/*` events and `dispatch_cursor.ps1`. (2) **`.cursor/hooks/before_shell.py` was the one carrier whose discipline had no surviving owner** — no git hook can guard `--no-verify`, since that flag is what skips git hooks — so it was **migrated** to [`scripts/guard_shell_command.py`](../scripts/guard_shell_command.py) with 28 tests rather than retired, and deliberately **left unwired** (CLAUDE.md §Continuous improvement item 6 governs `.claude/settings.json`). Every other hook and all eight `.cursor/rules/*.mdc` were verified as derived mirrors naming their own surviving owner. (3) Each auto-merge falsifier limb is **explicitly retired as unreachable** rather than left standing, per the gate-reachability lesson.
- **Shipped:** [worker-surface allocation](adr/2026-07-14-cc-cursor-surface-allocation.md) Revision 2026-09-15 (§8 clause dispositions, §9 operator actions this repo cannot perform, §10 audit hooks re-run and corrected) · [autonomous loop](adr/2026-08-14-cc-cursor-autonomous-loop.md) → `Superseded` stub + [cold body](ltm/adr/2026-08-14-cc-cursor-autonomous-loop.md) §11 · [TOMBSTONES 2026-09-15](adr/TOMBSTONES.md#2026-09-15-cursor-agent-retirement) (23 non-ADR artifacts, every blob verified retrievable) · [`a6`](pursuits/a6-cursor-fleet-worker-capability.md) SUBTRACT **proposed** with residuals R1–R6 and re-entry armor · `cursor-fleet` skill deleted at operator instruction; `.cursor/`, both dispatch scripts, `notify-cursor.yml` and `claude.yml`'s `allowed_bots` removed. `gate_manifest --tier check` matches baseline except the first-party module count; `pytest` 4788 passed / 53 skipped (delta fully accounted).
- **Defects found in my own work, both fixed before commit:** an `rg -rn` where `-r` is `--replace` silently rewrote a reference scan I nearly acted on; and a re-targeted test passed **vacuously** on `Request ID must be a UUID` without reaching the copy path it claimed to cover — it now pins the rejection reason. Also corrected the ADR's own §10 hook, which claimed an empty result it does not produce.
- **Phase-0 finding, and my own error in it — CORRECTED same session.** I reported that `work-decomposition` "exists nowhere in this repo or its history" and that the task description was confabulating (`feedback_web_advisor_handoff_confabulates_repo_state`). **That was wrong.** The skill exists on the open [#401](https://github.com/Joshua-Asante/first-passage/pull/401) branch (`claude/keen-carson-i8gou9`) and carries the exact phrase "retirement record pending" at line 19. My search covered `main`, local history and `git log --all -S` in this checkout, which had never fetched that branch — so the negative was an artifact of my search scope, not a fact about the repo. **The lesson is the search, not the handoff:** an unfetched remote branch is invisible to `--all`, and "does not exist" needs `git fetch` plus an open-PR check before it is asserted. `scripts/repo_hygiene.py` having no `cursor/*` branch handling does hold (read directly).
- **In-flight PR interactions (not resolved here):** [#402](https://github.com/Joshua-Asante/first-passage/pull/402) amends `cursor-fleet/SKILL.md`, which this PR deletes — whichever lands second conflicts, and the operator's delete instruction should win. [#401](https://github.com/Joshua-Asante/first-passage/pull/401) adds `work-decomposition` with a "retirement record pending" pointer that this ADR revision now satisfies, and also edits `cursor-fleet`. Neither is editable from this branch; both need a pointer/conflict pass at merge.
- **Same-day follow-up (§9 verified; stacked PR onto #405):** both "operator-only" residues were already gone before this PR merged — routine `trig_012nvuH7jqmjFUFgoFVpZ6RP` returns HTTP 404 on a direct `get` (the `list` endpoint shows only the 20 newest routines and cannot page, so the earlier listing was not evidence either way), and `daily-repo-truth-sync` reports `taskDeleted: true` after 42 runs. What the sweep reported as empty — `cursor/*` on `origin` — holds four spent refs; §10 hook 3 now asks the remote (`git ls-remote`) instead of the local ref list. Evidence in [ADR §9](adr/2026-07-14-cc-cursor-surface-allocation.md).
- **Open / next:** operator ratifies (or rejects) the ADR revision, the supersession, and the `a6` SUBTRACT — **R6, the three frozen packets "pending dispatch", is left deliberately unresolved and is not closed by a SUBTRACT ruling**. Operator-only, outside this tree — **the routine and the scheduled task were verified already gone in the same-day follow-up (ADR §9 items 1–2), so nothing to disable**; still owed: delete the four spent `cursor/*` refs on `origin` (§9 item 3) and supply the Cursor cancellation date to the [subscription ledger](pursuits/SUBSCRIPTION_LEDGER.md) (terminated / no-final-charge recorded 2026-09-15; the date alone is still open). Optional: wire `guard_shell_command.py` as a `PreToolUse` hook.
...
## 2026-09-05i — Admit both long-only MNQ expressions to the candidate grammar

**Open / next:** STATE queue: `#1` [Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md#51--long-only-mnq-co-exposure-approved-2026-09-05) · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24). The operator approved the [long-only co-exposure amendment](adr/2026-09-05-tradeify-select-striker-expression-readmission.md#same-day-amendment--long-only-co-exposure). Source binding, shared-symbol replay/ownership and the statistical freeze remain owed; runtime parity precedes deployment. No search or sample consumed.

---

## 2026-09-05h — Add used-account state to the simulation kernel

**Open / next:** STATE queue: `#1` [Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md#50--used-account-kernel-2026-09-05) · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24). The [kernel specification](superpowers/plans/2026-09-05-tradeify-used-account-kernel.md) supplies the original-basis state interface. Private account binding, export-bound captures, synchronized replay and the statistical freeze remain owed. The operator merged #308/#309; no campaign sample or new attempt was consumed.
...
## 2026-09-05d — Roll STATE weekly idle-trade deadline past 2026-09-04 (queue-exception: CI `skills (3.12)` failed on state-currency)

**Open / next:** STATE queue: `#1` [Seven-strategy Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md) · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24). Off-queue residue carries from `05c`: D26 capture checkpoint still blocks freeze prerequisites 2 and 6; A3 (or CC-solo) still owed for the three live calculator defects #305 merged without A2; §47e scanner panel still open. This stub only rolls the weekly venue-idle row to deadline **2026-09-11** / bucket 09-07→09-11 and bumps `Last curated` to 2026-09-05 so `state-currency` clears.

---

## 2026-09-05c — Codex round 18 on #302 folded twice, merged; first adversarial-panel run; #305 merged without A2; D26 capture checkpoint gate-read (queue-exception: operator asked Cursor to address Codex's review) (queue-exception: operator asked Cursor to address Codex's review)

**Open / next:** STATE queue: `#1` [Seven-strategy Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md) · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24). **No freeze prerequisite is currently dischargeable.** 2 and 6 are blocked by the D26 capture checkpoint ([§48](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md)) — 2 of 5 captures complete, none *bound*, since "complete" is not "pinnable" until each source's re-export matches its pin; 8 and 9 are blocked because [#305](https://github.com/Joshua-Asante/first-passage/pull/305) **merged `2a89348` at the pure-merge head, so fix round A2 never landed** and all three of its items are live defects in the certification-power calculator on `main` (§45) — a fresh packet **A3** off `main` or a CC-solo fix is owed, pointer prepared; 1 and 7 stay owed as before. **Fleet fully dispositioned:** #303 MERGED `6b5bc96`, #304 CLOSED without merge (the §2-B label-allocator judgment stays unclaimed until an operator ruling), #305 MERGED but not converged. ⚠ **Codex round 18 was folded twice independently on this branch** — a Cursor agent and the orchestrator, neither routed through the claim manifest; the merge keeps the Cursor design and records the duplication as the defect (§47d). On the operator's ruling, every pre-push diff now goes through an **internal adversarial panel**; its first run found **29 confirmed bypasses of the disclosure scanner, 18 rated P1**, including a regression in the orchestrator's own round-18 fix. Two root causes are fixed; **fifteen remain open, so the scanner is not yet fit to be §47b step 6b's gate** (§47e).
...
**Shipped:** [fleet brief](briefs/handoffs/2026-09-04-cursor-fleet-scripts-side-three-packets.md) §6 manifest → RETURNED (A, C) / WITHDRAWN-but-built (B), fix rounds A1 and C1 frozen in §6b; campaign-state [§43](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md) (the gate read, the dispatch-revision defect, the #301 confirmation); `main` merged into [#302](https://github.com/Joshua-Asante/first-passage/pull/302); this stub carries the pointer refresh that `sessions-append-only` no longer lets the `04d` entry carry. · **#303 (Packet C) MERGED `6b5bc96` 2026-09-05** — post-merge checklist done on the merged tree (check tier exit 0; `repo_hygiene.py` exit 0 on a gh-less host; the brief's §0.9-C probe on `main` → DONE); manifest row C → MERGED; this branch merged `main` (`0fd2e39`).
**Decisions / defects:** (1) All three workers were fired from the brief as first opened (`af0203f`), before Codex's pre-dispatch review re-froze it (`9914dd8e`…`70f1c47c`) — relay lag, not worker error, but the re-freeze withdrew B and moved C's guard after the builds had started: **a brief's Codex round is part of the freeze and dispatch waits for it**, and the dispatch pointer must carry the brief's commit SHA (proposed for the `cursor-fleet` skill; not applied — CLAUDE.md §Continuous improvement, item 6). (2) Packet C @ `0e7ab25` is **FALSIFIED** on the §6 criterion: it guards `_run` (a §5 forbidden move) and lacks the pinned git-raises test; Codex's P2 is that consequence, reproduced here (git absent → `--json` exits 0 with an empty report). Fix round C1 owed. (3) Packet A @ `7b1418b` is **RESOLVED** — §10 hooks pass here, and `check_repo_map_scripts_table.py --check` passes on a scratch merge with current `main` although the branch was cut from `ba713ee`; Codex's two P2s verified (silent out-of-domain arguments; quadratic CDF recomputation, 6.6 s at `n-max` 4000) — fix round A1 owed. (4) Packet B was built regardless and returned `NEEDS_CONTEXT` with exactly the contradiction §2-B predicts: the withdrawal stands, #304 closes without merge, no re-anchor. (5) #302's required check went red because #297's `04e` stub landed above `04d`, turning this branch's pointer edit into a mutation of a prior entry — fixed by this stub plus restoring `04d` to `main`'s bytes. (6) #301: Codex at `512f7a3` returned zero findings and every job was green on the pure-merge head `f4132a0` — the §42 green light was confirmed and **the operator merged it as `5ab079c` at 22:35Z** (§46).
**Open / next:** STATE queue: `#1` [Seven-strategy Tradeify Select configuration campaign](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md) — #301 **MERGED** `5ab079c` 2026-09-04 (§46): D26/D27 code readiness is on `main`; next, the five current-input captures (§22a; 200K halves struck) and the single re-freeze — **dispatch packet §47** (operator captures first, then one local Codex population job; iteration 6 of ≤8); prerequisites 1 (Codex's per-leg scaling read) and 7 (live-state MC engine change under `core/`, CC solo) still owed · `#2` [B7-REFIRE Stage 1 + M1](adr/2026-07-22-c1-venue-native-monitoring-maturity.md#addendum-2026-08-24--test-strategy-licensed-for-item-5-dated-08-24). Off-queue residue (owner: the fleet brief §6): C1 returned and **converged** (§44 — #303 is the operator's merge); A1 returned and resolved but Codex's next round found three P2, so **fix round A2** (§6b) is the operator's to fire at Cursor and #305 waits on it; #304 to be closed; the orchestrator writes the fleet integration commit after the merges. **Residue 2026-09-05:** the fleet is fully dispositioned — #303 MERGED `6b5bc96`, #304 CLOSED without merge (the §2-B label-allocator judgment stays unclaimed until an operator ruling), #305 MERGED `2a89348`. ⚠ #305 merged at the **pure-merge head**, so Packet A's fix round **A2 never landed** and all three of its items are live defects on `main` — the certification-power calculator must not size the `n₂`/`n₃` freeze (prerequisites 8, 9) until a fresh packet **A3** or a CC-solo fix lands; the pointer is prepared and the choice is the operator's. The §47 Codex loop continues (eighteen rounds folded) and, on the operator's ruling of 2026-09-05, every pre-push diff now goes through an internal adversarial panel first. **D26 capture checkpoint (local Codex, 02:30Z): 2 complete / 3 partial** — gate-read in [§48](briefs/programs/2026-09-03-seven-strategy-select-campaign-state.md); "complete" is not "pinnable" until each source's re-export binding check passes, so no digest enters `phase1_config.json` yet, D26 population and the shared Phase 1 regeneration stay blocked, and with prerequisites 8 and 9 blocked by the calculator defects **no freeze prerequisite is currently dischargeable**.
**Class:** Decision · **Rule 2:** $0 / K=0 / MC=none (fleet builds spend Cursor tokens, not campaign budget)
