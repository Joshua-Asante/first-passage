...
- **Open / next:** pull the main checkout to a `CLAUDE.md`-free commit right after merge (worktrees load instruction files from the parent checkout); confirm in the next session that `AGENTS.md loaded` appears at startup. The untracked GLM closeout packets in the main checkout's `docs/briefs/handoffs/` are the committed-handoff rule's first application — commit or tombstone them before the next GLM dispatch. Research-domain lesson stubs (regime, DSR, MC, instrument findings) remain `external-unmigrated` for a later batch.
