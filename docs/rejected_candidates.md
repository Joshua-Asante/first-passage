...
**Class:** edge-failure (the signal carries no orthogonal information over generic reversal, and sits below the placebo null's own median for both symbols) — NOT a cost-constraint (B2.0 already confirmed both symbols clear the 4× cost floor at full size).
...
**Closure basis:** frozen plan kill criterion (`docs/superpowers/plans/2026-08-23-viable-strategy-phase-b-mechanism-supply.md`, Lane B2 task B2.2): "Kill if the fix dummy adds nothing over generic reversal or sits ≤ placebo 60th percentile." On 2 years of Databento GLBX.MDP3 `ohlcv-1m`/`ohlcv-1h` (continuous `.v.0`, 2024-08-24→2026-08-24; n=469 6E / n=447 6B valid fix-observations), both legs failed independently for both symbols. Orthogonality regression (`target ~ 1 + trailing_vol + prior_hour_return + imp_sign`, adapted from the gamma-family GEX-gate precedent's `partial_out_t`): 6E correctly-signed but far short of significance (t=−0.90 vs the |t|≥2 bar); 6B wrong-signed (t=+1.63, positive = momentum-continuation, not fade). Placebo null (1,000 replicates, hourly-clock family, day-of-week + trailing-vol matched by construction and verified): 6E's real statistic ranks at the 20.9th percentile of the null, 6B at the 3.9th — both below the null's own median, let alone its 60th-percentile bar.
...
<!-- concept-intake-entry
     ...
     rejection_reason="FALSIFIED (B2.2 placebo/orthogonality battery, plan Lane B2): orthogonality regression (target ~ 1 + trailing_vol + prior_hour_return + imp_sign) NOT significant/correctly-signed for either symbol -- 6E t=-0.90 (correct sign, |t|<2), 6B t=+1.63 (wrong sign); placebo null (1000 replicates, hourly-clock family, day-of-week+trailing-vol matched) ranks real statistic at 20.9th pctile (6E) / 3.9th pctile (6B), both below the null's own median. 2yr Databento GLBX.MDP3 ohlcv-1m/1h, continuous .v.0, 2024-08-24->2026-08-24, n=469/447."
     harness_disposition_ref="B2.2 frozen kill criterion (lab/analysis/_inbox/b2_london_fix_wake_2026-08-24/RESULTS.md)"
     date="2026-08-24"
     class="edge-failure"
     role_tested="entry"
     falsifier_failed="orthogonality regression |t|<2 or wrong-signed AND placebo rank <=60th pctile, both legs, both symbols"
...
- **fx-fixing-reversal-dealer-wake on 6E/6B** — rejected 2026-08-24 (edge-failure: B2.2 placebo/orthogonality battery FALSIFIED both symbols — orthogonality regression |t|<2 or wrong-signed (6E t=-0.90, 6B t=+1.63); placebo-null rank 20.9th/3.9th percentile, both below the null's own median; n=469/447 over 2yr Databento GLBX.MDP3); artifact `docs/notes/research/2026-08-24-phase-b-lane-b2-placebo-battery-results.md`.
