...
Primary output: JSON file at `docs/notes/audits/issue_54_ulp_audit.json` (spawn-era path was `data/audits/`; lived at `ops/data/audits/` until relocated 2026-08-03). Secondary output: markdown table at `docs/notes/audits/issue_54_ulp_audit.md`.
...
- `totals.instance_count >= 3` → claude.ai opens Pre-Q (Inquire-phase brief) at `archive/docs/briefs/PreQ-ULP-sweep.md` to gate the sweep with PR #53 ADR's variable-scale precision rule.
- `totals.instance_count in {0, 1, 2}` → claude.ai opens fix-as-found Cursor briefs (one per site) at `archive/docs/briefs/fix_<file>_<line>_ulp.md`.
...
- **Artifact presence**: `ls docs/notes/audits/issue_54_ulp_audit.json` after spawn close. Missing = spawn did not complete; promotion gate check (1) cannot evaluate.
...
- **Schema conformance check**: parent runs `python -c "import json; json.load(open('docs/notes/audits/issue_54_ulp_audit.json'))"` and validates against §8 schema before disposition.
...
3. Execute §2 phases.
4. Emit artifacts at docs/notes/audits/issue_54_ulp_audit.{json,md} (spawn-era: `data/audits/`).
