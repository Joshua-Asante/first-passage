...
C1 every evidence file exists and is non-empty; the three canonicalized paths are distinct and the three digests are distinct (one artifact may not serve two evidence roles); digests computed. C2 both `EvaluationState(original_basis, equity, historical_eod_peak, prior_trade_days, prior_max_day_profit)` constructs **and** the kernel's `_drawdown_outcome` returns `None` for that equity/peak with the Tradeify tier geometry. Derive `starting_equity`, `dd_type`, `static_dd_pct`, `trailing_dd_pct` and `dd_lock_offset_usd` from `mc.preflight.firm_kwargs("Tradeify_Select_100K")` (use `None` for an absent static/trailing/lock argument; these unused branches are not evaluated for this tier). A non-null outcome refuses the seal as C2: a constructor-valid snapshot at or beyond its drawdown floor is already breached. Reuse the kernel predicate, including its six-decimal normalized boundary rounding; do not replace it with a raw `equity > trailing_threshold` comparison. C3 `equity == balance`, `positions_export_shows_flat` is true and `working_orders_count == 0`. C4 relational: `historical_eod_peak ≥ balance`; when `balance == trailing_threshold + width` the account is at its high-water mark (recorded as `at_high_water_mark: true`), otherwise the carried drawdown `peak − balance` is recorded. C5 freshness: every capture timestamp is at or before the seal timestamp (a capture after the seal refuses — mistyped or clock-skewed future evidence must not pass); all capture timestamps lie within `freshness_window_minutes` (frozen default 30) of each other; the seal happens within `seal_within_hours` (frozen default 24) of the latest capture; every capture and the seal fall inside one venue session boundary (`session_boundary_rule`: between a weekday 17:00 ET close and the following 18:00 ET reopen, or between Friday 17:00 ET and Sunday 18:00 ET) so no session activity can intervene. C6 when `balance − original_basis > 0`, `prior_max_day_profit / (balance − original_basis)` agrees with `consistency_display_pct` within one percentage point (the display caps at 100 %; a capped display passes when the ratio is ≥ 1). C7 `profit_target_display` equals the tier constant. C8 `cash_adjustments_total == 0`, where the total is the sum of absolute amounts of every deposit, withdrawal or other adjustment in E3; offsetting transactions must not cancel. Any non-zero value refuses — the kernel's `original_basis`, target and drawdown geometry have no rebasing semantics, so no note can make an adjusted account compatible with them; an evaluation with adjustments is out of scope for this seal. C9 the output path is reported ignored by `git check-ignore` before anything is written; the canonicalized output path differs from every evidence path (the tool must not destroy an evidence artifact by writing the seal over it); the tool prints no account values or evidence paths; stdout is limited to the sealed-file digest, passed check ids, derived `valid_until` and the fixed attestation sentence defined below. C10 consumption expiry: the sealed JSON carries `valid_until` (the reopen that ends the boundary window of C5); the tool refuses to seal when the seal time is at or after it; validity is strictly before the reopen, never inclusive. **Consumer obligation** (not enforceable by the tool, so stated as a frozen gate for TB-E2, TB-B7 and TB-B10): the sole n3 must start, and the arm must occur, before `valid_until`, with no fill, order or adjustment on the account in between; otherwise the seal and its dependent n3 result are void. Stop for the operator under the governing plan S2; this validity rule does not authorize an automatic second outcome-bearing sample. If continuation is explicitly authorized, capture and seal fresh evidence, bind any authorized TB-E2 result to that new seal, and re-establish dependent decision/approval gates before TB-B10. A failed qualification remains terminal. For the initial arm, consumption means effective activation after restart, not merely the earlier config write: TB-I3 repeats fresh boot-bound identity/no-activity/expiry checks and records activation acknowledgment before enabling risk-add admission, as proposed TB-P2 T11 requires. Bind the seal digest to the n3 result and arm evidence; a new seal alone cannot validate an old result (campaign-state §15 item 2: the procedure is frozen, never the numbers).

Output: one private JSON (`values`, `derived` incl. `valid_until`, `evidence` {path basename, sha256, captured_at}, `checks` {id: pass}, `seal_timestamp`, `tool_sha256`, `contract`: this file's path) written atomically under an ignored root (default `lab/analysis/c1/tradeify_seven_strategy_phase1_2026-09/local_artifacts/account_snapshots/`); stdout carries the sealed file's SHA-256, the check ids that passed and `valid_until`. The seal is **attested-by-evidence, not machine-verified against the broker**: no read API exists for the dashboard, and the final stdout line is exactly `Attested by evidence; not machine-verified against the broker.`
