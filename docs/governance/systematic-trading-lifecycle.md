...
|---|---|---|---|---|---|
| 1 | Strategy R&D (concept → analysis → codification → validation) | **Alpha research** / research pipeline; validation sub-stage = **model validation** | Front (quant research) | OUTER (INQHIORI), funded by STRATEGIC | K-trial pre-registration manifests, Pre-Q briefs, kill records, rejected-candidates registry, validation-harness dispositions (CPCV/DSR/PBO/permutation) |
