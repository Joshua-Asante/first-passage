...
A persistent host slice shares MemoryMax and swap=0 across qexec, schedulers, clients, guardians and Docker payloads (the manager never sets memory.oom.group on a slice: a payload OOM is a single-process kill under Docker's delegated scope, so stop-on-OOM is the guardian's memory.events poll plus settlement's retained facts). Memory peak/OOM facts conservatively include other campaigns and survive work completion. Shared platform daemons outside the accounting scope are excluded under the coordinator's explicit interpretation. Fixed Docker requests only inspect/info/create/start/kill/delete installed probe images; no build/archive/exec processing is outsourced.
