...
- [ ] Start the actual service process on loopback with an ephemeral port, a temporary configuration, and synthetic state. Wait for readiness with a bounded deadline; capture diagnostic output and terminate the process in test cleanup, including failure paths. **Port discovery needs a seam:** `serve()` binds `int(cfg["bind_port"])` and then logs `startup_log_line(cfg)`, which reports the configured value, so with `bind_port: 0` the OS-assigned port never appears in the log or anywhere the harness can read (`ops/c1_rail/c1_rail_http_server.py`, `serve()`), and reserving a port by bind-and-release before launch races other processes on a shared CI runner. Land a one-line port-reporting seam first — log or write `server.server_address` after bind (an `ops/c1_rail/*.py` edit that obeys both freeze windows, so it waits for the A5→A8 window to close) — and have the harness read the bound address from it; until that seam exists, run this harness serially (no `xdist`), choose the port by bind-and-release, and retry once on a bind failure, treating the residual race as accepted and documented.
