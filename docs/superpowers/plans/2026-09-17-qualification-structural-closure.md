...
Status: PAUSED at the failed Linux verification checkpoint recorded below;
qualification acceptance remains HELD. Baseline PR415
`bfad2f187e05980369dbf0e8f75801f621ba998d`. Subsequent user handoffs authorized the
implemented increments; the original design-only restriction is historical.
The coordinator retains combined acceptance. PR preparation does not resume
the remaining lifecycle, concurrency or invariant-gate implementation.
...
### Real fault verification follow-up (2026-09-18 02:18 UTC)

- After explicit user approval, published61d7ff2 and dispatched run35298023879 on two fresh Ubuntu hosts. Each collected19:18passed/1failed/0skipped/errors. Both records stable/complete, exit1 and successful owned cleanup. Evidence retained under `.cache/qualification-linux/35298023879`; records host1 `c9e10d9198074361b8709eed3f316518`, host2 `cdc061380f8147f3b7a34d22ab86dd79`.
- Each of the four zero-exit/CPU/wall/memory cases reached its callback marker and passed; archived journal confirms IN_DOUBT, no attestation/assessment, one start, admission/legality contract binding. Checked314 exported object hashes per host and record artifact hashes. These are valid four-fault results inside failed overall records, not full acceptance.
- Remaining stop failure: self-SIGSTOP was ignored by PID-namespace init. Exact container inspection showed worker PID1 exited0; archived journal became ATTESTED from its normal completed replay, so no stopped-worker evidence exists. Linux pid_namespaces(7) documents forcibly delivered SIGSTOP from an ancestor namespace: https://www.man7.org/linux/man-pages/man7/pid_namespaces.7.html.
- Corrected TEST_ONLY injection only: source callback holds at its marker; administrator sends SIGSTOP to the exact Docker container, then requires host `/proc` stateT. Retains sender UID, execution/container identity and stopped process observation before actual supervisor death/restart. Production entrypoint, process topology, validators and state machine unchanged. This closes a fault-injection defect within the requested five-fault outcome.
- Local final fixture selection `./fp.ps1 --workers 2 python -m pytest tests/ops/qualification/execution/test_boundary_fixture.py -q --tb=short`:6passed in53.91s (`20260918T021700Z-0aa653ad055b`). `./fp.ps1 check`:passed (`20260918T021722Z-31310dda4d6e`),72 evidence-store tests/3 existing skips and absent private artifacts advisories. Both completed/exit0/stable/complete/no report errors, launcher Python3.13.2. Fresh real Linux stop/recovery evidence remains required before bounded completion.

### Approved run67b93dd — failed verification checkpoint (2026-09-18)

- User explicitly approved publishing67b93dd and running the jobs. Commit `67b93dd853113f7bb693cae9256ad3b2cb65f356` is pushed on `codex/qualification-structural-closure`. Run https://github.com/Joshua-Asante/first-passage/actions/runs/35299214700 completed failure; no subsequent repair/push/rerun is included in this checkpoint.
- Host1:19collected/18passed/1failed/0skipped/errors, stopped-worker checkpoint failed. Host2:19collected/17passed/2failed/0skipped/errors, stopped-worker checkpoint and memory OOMKilled assertion failed. Both records have stable source, complete capture, no capture errors, exit1 and explicit failed-JUnit report errors. Both owned cleanups succeeded without failures.
- Artifacts retained under `.cache/qualification-linux/35299214700/`. Host1 record `b9bb4a968f9c4e5fa2bd42f65a4674d3`; host2 record `94f358433c234f65bcbe35f1514f5844`. `inspection.json` and `failure-detail.json` contain coordinator read-only inspections;312 object hashes per host and recorder artifact hashes checked. The inspector exits1 as expected for failed overall verification. A missing start-event export for host2 memory is unavailable evidence, not an observed zero-start count.
- Zero-exit/CPU/wall cases passed on both hosts, reaching actual fault markers with IN_DOUBT/no attestation/no assessment and one retained daemon start. Memory passed with OOMKilled=true on host1; host2 reached its marker and exited137 with OOMKilled=false, IN_DOUBT and no attestation/assessment. Exit137 alone does not establish OOM. Host2's test failed before its restart/retry/start-event export; those later assertions are not verified for that case. Earlier run35298023879 retained passing memory evidence on both hosts, but does not erase this new contrary result.
- Stop test's administrator injection depends on seeing worker stderr during execution. `start_and_capture` opens a buffered `stderr.log`, writes without flushing per chunk, and flushes only after stream EOF. Both exported stop logs were empty at observation; no stopped-process receipt was produced. Thus the current injection has no reliable readiness channel, and neither supervisor death at observed SIGSTOP nor no-redraw recovery is established. Both archived stop executions eventually became IN_DOUBT with no attestation and reason `worker did not stop normally`; that is not the requested stopped-worker evidence.
- Next bounded work needs a buffer-independent TEST_ONLY readiness observation tied to the exact worker/container, plus retained kernel/cgroup OOM facts and Docker exit ordering to diagnose the host2 discrepancy. Preserve real validators, actual process topology, no-redraw and strict OOM assertions; do not weaken a failed assertion to exit137-only. No concurrency integration or invariant acceptance work was performed.
- Linux doctor selected each protected host env/bin/python3.12.3,62 locked packages,cryptography50.0.1; the launcher ran `scripts/qualification_boundary_verification.py --test-only --manifest <manifest>`, recording `-m pytest tests/integration/qualification_boundary -q --tb=short --junitxml=<record>/junit.xml`. Local evidence inspection used `./fp.ps1 python .cache/qualification-linux/35299214700/inspect_evidence.py` (expected exit1) and `inspect_failures.py` (exit0), after doctor passed with `C:/Users/joshu/multi_firm_operations/tmp/ops-env/Scripts/python.exe`3.13.2. No additional local test pass is claimed.
- Pin consistency is repaired; complete five-fault verification is NOT achieved. Qualification acceptance remains HELD. Both CI jobs and the local watch process are complete. Existing unused service scheduler/test and unrelated worktrees remain preserved.

### PR relationship and publication checkpoint

- The user requested a separate PR and assessment of relevance to PR415, without resuming implementation. PR415 head051fe1e contains the same bfad2f1 qualification baseline plus a main merge. Merging051fe1e into this branch produced exactly the existing committed tree `df71a211eb2f80af93d17a9260f9af0e66db58dd`; it only establishes an unambiguous stacked review base.
- Open the successor as a draft against `codex/phase3-post413-integration`. Its diff is the structural/protected-N1 increment, not a duplicate of PR415's foundation. PR420 and421 are already merged. Preserve PR415's branch/code and earlier regression evidence.
- PR415's four latest structural review findings map here: retained registry/source admission (`legality.py`, `source_admission.py`); canonical fixed-product basis (`policy_sources.py`, `policy.py`, v2 contract validation); reconstructed role-specific evidence (`evidence.py`, protected G5); canonical output inventory (policy role function and closed v2 result plan). The older execution-completion provenance finding maps to service-owned launch/capture/attestation plus retirement of public v1 authentication/claim/commit/seal and legacy production executor. These are successor implementations, not fixes already present in PR415 or completed acceptance claims.
- Do not cherry-pick isolated validators back into PR415: they depend on the coherent v2 policy, contract, evidence and protected execution interfaces. Update PR415's description to link the successor, correct its stale head/merged dependency references and preserve the explicit acceptance hold. Leave review threads unresolved pending actual review; prior owner replies and current code are evidence, not permission to dismiss threads.
- Latest held work belongs to this successor: reliable stop readiness independent of buffered stderr, host2 OOM cause/exit-order evidence, remaining Task8 lifecycle/writer races and Task9 invariant gate/mutations/combined review. Ordinary host-readiness CI is not the manual boundary=true acceptance run. No runtime code was changed during PR preparation; the unused scheduler helper/test remains local and excluded.

- PR425 opened as a draft stacked on PR415. PR415 description now links it and preserves the acceptance hold; its code head remains051fe1e. The new PR's first hosted pytest check at51121c1 failed during collection because both the boundary and root suites imported `test_lifecycle`. Renamed only the boundary file to `test_boundary_lifecycle.py`; bytes/assertions unchanged. This is a PR integration repair, not resumption of fault/lifecycle implementation. `./fp.ps1 python -m pytest tests/integration/qualification_boundary/test_boundary_lifecycle.py tests/test_lifecycle.py --collect-only -q`:34collected, exit0, record `20260918T024715Z-cc3460e7ce68`, completed/stable/complete/no report errors, operations Python3.13.2. This verifies collection, not execution. The prior Linux failure records keep their original node IDs and remain applicable to unchanged test behavior.

### Bounded Task 9 invariant-gate completion (2026-09-18)

The follow-up [invariant-gate handoff](2026-09-18-qualification-invariant-gate.md) is complete. Canonical inventory: 621 exact critical cases across all nine families; missing, renamed, skipped, failed or invalid-record evidence fails closed. All three planned structural mutations were killed by intended behavioral assertions with passing controls and byte-exact retained patch hashes/restoration. Independent review findings were resolved.

Candidate `f3099bb5a1675e8cee8177656d0fa24502ce6a86` passed [two-host run35358450882](https://github.com/Joshua-Asante/first-passage/actions/runs/35358450882): 621 passed per host, zero failures/errors/skips, completed stable capture and successful owned cleanup. Offline record verification and lifecycle audit passed: 27 executions, four writer orders and two real permission identities per host, with journal/object/runtime identity checks. The [evidence audit](../../notes/audits/2026-09-18-qualification-invariant-gate.md) preserves commands, interpreter/source/configuration identities, the initial local fixture failure and its corrected rerun, mutation records and private-source limitations.

This closes the selected Task 9 gate outcome and supersedes earlier statements that its wiring/mutations/review were pending. It does not claim full-campaign or production execution, malicious-qexec confinement, merge or deployment. Earlier failed records and bounded stopping decisions remain historical evidence.
