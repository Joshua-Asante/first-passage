...
**Windows verification on `bb88b7e`** (code bytes; `b0300e1` differs only by the docs correction above; ops-env Python 3.13.2 through the checkout's `fp.ps1`, doctor 62 packages): §2.6 line 1 (ten execution files, `--workers 2`) **419 passed / 0 skipped**, exit 0, record `20260920T005716Z-154ad561d4a1`; line 2 (lifecycle/artifact/manifest/campaign_host) **79 passed + the pre-existing Windows symlink skip**, exit 0, record `20260920T010558Z-25aadfcbc855`. Line 3 (`tests/ops/qualification` + phase3, `--workers 2`), `check` and `git diff --check` results are appended below. `make audit` and the isolation bridge were not run (the bridge exceeds its 1800 s limit on this box — standing caveat, never claimed).

**Integrated Linux evidence**: run **35482452099** (`pull_request` on `claude/s2-enforcement-gaps-fab5e6` @ `b0300e15c15f6d66838cf5e851c4ff050b2f08a0`; `main` is still the merge base, so the merge-ref tree is the branch head) was dispatched by the push and is the integrated task-2 run. Result appended below.
