...
**AUTHORIZATION:** A1+A2 GO executed 2026-08-23; A3 voided (empty revival list).
The prior `AWAITING GO` / "not yet given" line is history.
...
- [x] **Inputs (read, not re-derived):** `docs/rejected_candidates.md` (now complete — 30-row
  backfill landed 2026-08-23, debt 0); the WHO-track notice
  ([`N-2026-08-14-msl-who-track.md`](https://github.com/Joshua-Asante/first-passage-archive/blob/5d47b4dc5fd20da5e93edfed2f6eafd0d4a6ddd2/docs/notes/notice/N-2026-08-14-msl-who-track.md)) §3 door
  ...
  lesson).
- [x] **Kill-reason classes (frozen before tagging):** `SIZE` (full-institutional-flow /
...
- [x] **Per-row tags:** kill class(es) · cell-demonstrated vs category-inherited ·
  would-revive-under: {impact-persistence size re-aim, threshold-parameterized WHEN, neither}.
- [x] **Output:** `docs/notes/audits/2026-08-XX-kill-register-attribution-audit.md` — the matrix,
  plus the **revival list** (the only cells A3 may touch) and a count of category-inherited kills
  re-tested vs confirmed.
- [x] **Fold-in:** the compelled-abstention arithmetic falsifier (Phase B, candidate B3) — a
...
- [x] **Engine reuse (Rule 0 reads before any code):** `core/mc/simulation.py` (`simulate_path`,
...
- [x] **Parameter axes (synthetic trade-generating process, not a strategy):** win rate
...
- [x] **Firms scored now:** `Tradeify_Select_100K` (deployment venue — model repaired: W1 +
...
- [x] **Statistical discipline (Q-STATVALID-1, binding):** output is a **feasible region with
...
- [x] **Output:** `lab/analysis/c1/shape_feasibility_map_2026-08/` — harness + `RESULTS.md`
  (region table + the one-page reading: "what shape must a mechanism produce"), CATALOG row.
- [x] **First consumers, run immediately:** (i) re-check the reopened Tradeify-native fade
...
**Voided-not-run.** A1's revival list is empty; A3's own gate says it does not need to be
ruled. Checkboxes below are closed as voided, not executed.
...
- [x] **Amendment 1 — size screen re-aim (impact-persistence):** replace "the flow must be a
  ...
  wakes.
- [x] **Amendment 2 — WHEN clause extension (threshold-parameterized triggers):** admit published
...
- [x] **Vehicle:** amend-in-place addendum on the mechanism-boundaries ADR
...
- [x] **Degeneration guard (stated on the ruling card):** this is the `programme-audit`
