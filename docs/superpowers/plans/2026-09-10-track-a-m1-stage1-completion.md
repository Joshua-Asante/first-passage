...
**Owner:** Joshua (operator) · Astra coordinator (current implementation and review owner); original author: Claude Code
...
- [A6 — Astra: daemon deploy, inert](../../briefs/handoffs/2026-09-10-track-a-a6-claude-daemon-deploy-inert.md)
- [A7 — Operator + Astra: attended Stage 1 dry-run](../../briefs/handoffs/2026-09-10-track-a-a7-claude-attended-stage1-dry-run.md)
- [A8 — Astra: signoff, RESOLVED artifact, redeploy](../../briefs/handoffs/2026-09-10-track-a-a8-claude-m1-resolution.md)
...
- [x] **A5** — **DONE 2026-09-12** ([first-passage#352](https://github.com/Joshua-Asante/first-passage/pull/352) `aceec66`: readiness record §A5 — listener at release v8 from `cf75e45`, boot line disarmed, test row present at `cap_alloc 0` / `RETIRED` with the 69/11 residue released under the operator's `RW = True` ruling, preflight refused at cap 0, nine `fixture_hashes` refreshed from in-container reads, status stays `CODE_LANDED`; return `DONE_WITH_CONCERNS`, none blocking). Criterion: listener at the new release; `dry_run=True armed_until=-` by host read; `m1_stage1_test` row present at `cap_alloc=0` / lifecycle `RETIRED`; `fixture_hashes` refreshed in-container; `--status` prints `m1_gate: status='CODE_LANDED' result=FAIL` (expected while unresolved). No arm, no signal.
...
- [x] **A4 (daemon half)** — **DONE 2026-09-12** ([first-passage#354](https://github.com/Joshua-Asante/first-passage/pull/354) `d6f8e84`: readiness record §A4-D — every row GO; the v1-failed-release anomaly explained; `poll_interval_s` 5 → 1 recorded as the pending operator write; journal absent (first-deploy case); import closure re-traced at runtime on the A1b merge; A6 sequence frozen; three Codex rounds folded). Criterion: second dispatch of the A4 brief (Step 2.3b/2.4b): readiness record §A4-D with every row GO; the v1-failed-release anomaly explained from evidence; approved-source keys present but disabled; journal state clean; daemon import closure re-traced on the A1b merge SHA; A6 command sequence frozen.
- [x] **A6** — **DONE 2026-09-12** ([first-passage#355](https://github.com/Joshua-Asante/first-passage/pull/355) `e04fe38`: readiness record §A6 — daemon at release v2 from `d6f8e84`; boot line `emit_enabled=false`; health `poll_interval_s 1`, `NullStrategy`, source disconnected, `DISABLED`, `effective_emit false`; 60 s quiet window clean; journal created fresh at generation 0; listener ledger unchanged at `seq 32`; return `DONE_WITH_CONCERNS`, none blocking — the sftp-put lessons carried into §A7-P). Criterion: daemon at the new release; `GET /` shows `emit_enabled=false`, `effective_emit=false`, `strategy=NullStrategy`, source disconnected; no signal on boot (listener ledger last `seq` unchanged); ceremony state file valid; `status` CLI healthy. STOP before any ceremony.
- [x] **A7** — **DONE 2026-09-14** ([PR #388](https://github.com/Joshua-Asante/first-passage/pull/388); readiness record A7 attempt 4): ceremony `stage1-20260914-4`, listener UUID `7186beef-51b3-4da3-8f3f-b1052bddaaf1`; unique canonical projection and quantity-one dry-run triad, sender never invoked; mandatory close and inert teardown verified. Joshua attended and selected the candle; Codex captured/transcribed, enabled and injected under the accepted 2026-09-14 actor amendment. No live-feed qualification or arm.
...
- [x] **A8** — **DONE 2026-09-14**: signed acceptance [PR #389](https://github.com/Joshua-Asante/first-passage/pull/389), measured deploy/pin record [PR #390](https://github.com/Joshua-Asante/first-passage/pull/390), and verified docs-only re-bake to v11 from `93566fe`. In-container status RESOLVED/PASS while disarmed; embedded JSON matches the merged record and every image-carried pin matches its running file; all 27 source hashes unchanged through re-bake. [Final deployment evidence](../../notes/rail_build/M1_STAGE1_DEPLOYMENT_READINESS.md#a8--signed-acceptance-deployed-re-bake-verified-2026-09-14). The final closeout record removes Track A from STATE. **STOP. No arm.** A9/provider readiness remains separate.
