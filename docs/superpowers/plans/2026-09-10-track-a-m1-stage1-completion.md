...
8. `fixture_hashes` are refreshed only from in-container bytes after a real deploy; a tree-bytes rewrite asserts a deploy that never happened. Owner: acceptance JSON `fixture_hashes_note`.
...
- [ ] **A4 (daemon half)** — readiness record §A4-D; the v1-failed-release anomaly explained from evidence; volume config keys read; GO/NO-GO.
...
- [ ] **A8** — acceptance JSON: `dry_run_strategy_signal_event_id`, `operator_signoff`, `status=RESOLVED`, in-container `fixture_hashes`; validator plain + `--require-resolved` exit 0; PR merged; listener redeployed with the `RESOLVED` artifact; `--status` → `result=PASS`. **STOP. No arm.** STATE row 1 leaves the queue with its record.
