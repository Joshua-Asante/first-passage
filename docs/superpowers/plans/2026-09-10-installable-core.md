...
- [ ] Add `.github/workflows/core-package.yml`, with push-to-main and pull-request filters for `core/**`, `pyproject.toml`, `requirements-ops.lock`, `requirements-build.in`, `requirements-build.lock`, `requirements-tooling.lock`, `scripts/build_core_wheel.py`, `tests/packaging/**`, `LICENSE`, and its own workflow — the builder stages `LICENSE` into the wheel, so a change or deletion to it must also be verified, and `requirements-build.in` is the declared input Task 1 compiles the lock from, so an edit to it without a regenerated lock must not skip the job. The job regenerates the lock from the `.in` with the same pinned `pip-compile` invocation and fails on any diff, so the declared build inputs and the lock CI actually installs cannot diverge unnoticed. `pip-compile` is not in any existing hashed lock and the build lock contains only pip and setuptools, so the regeneration step bootstraps `pip-tools` from a committed, hash-pinned `requirements-tooling.lock` (one line, `pip-tools==<pinned>` with its transitive hashes, installed with `--require-hashes`) and invokes that exact executable — an ad-hoc `pip install pip-tools` would leave the lock generator, which can change the generated lock, unversioned. `requirements-tooling.lock` is itself a trigger path and a row in the file map. Add `workflow_dispatch` with a `ref` input so a wheel can be built and verified for an arbitrary clean `main` commit, not only commits that touch the filtered paths. Run `python -m pytest tests/packaging/test_core_wheel.py -q` after installing the hashed locks. Test both event predicates and unrelated docs/research-lock negative cases.
