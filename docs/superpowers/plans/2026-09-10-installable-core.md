...
- [ ] Add `.github/workflows/core-package.yml`, with push-to-main and pull-request filters for `core/**`, `pyproject.toml`, `requirements-ops.lock`, `requirements-build.lock`, `scripts/build_core_wheel.py`, `tests/packaging/**`, `LICENSE`, and its own workflow — the builder stages `LICENSE` into the wheel, so a change or deletion to it must also be verified. Add `workflow_dispatch` with a `ref` input so a wheel can be built and verified for an arbitrary clean `main` commit, not only commits that touch the filtered paths. Run `python -m pytest tests/packaging/test_core_wheel.py -q` after installing the hashed locks. Test both event predicates and unrelated docs/research-lock negative cases.
