...
If explicitly selected, the separately tagged `tradeify_secondary_early_close/v1` adapter binds a wrapper to the exact bytes of `ops/calendars/cme_holiday_calendar_2022_2026.json`, whose provenance remains `SECONDARY`. It accepts only the verified account-level union of entry dates with any `EARLY_CLOSE` group as 12:59 ET deadline rows, including when another campaign product group such as 6J is `NORMAL`; it never turns secondary evidence into `COMPLETE`. CME-trade-date full closures are retained as inventory rather than wall-date deadline rows. Pre-12:59 closes, unresolved details, inert source URLs, and the source-revision caveat that revision tips were resolved after research rather than captured at read time remain published provenance limitations. The primary calendar schema and its yearly-CME-capture checks remain unchanged.
...
The campaign-wide status is the most severe constituent status. D13(b)'s accepted-unmodeled continuous-roll limitation alone no longer blocks this campaign, but other callers without explicit acceptance retain the roll blocker. D19 accepts the tagged secondary calendar's venue-date membership as complete coverage only; it does not model close times or exchange sessions, and its preserved residuals remain visible. Missing independent scalar summaries retain the `NEEDS_CONTEXT` cap. Generation `tradeify-phase1-normalization-v3` is echoed in manifest/RESULTS; fee/config/calendar/summary digests come from exact parsed byte snapshots, never a post-load reread.
...
The prior Key-stats panels are historical and never rebound to replacement hashes. D17 leaves five independent scalar requirements (count, net, win rate, profit factor, max drawdown); absent fresh panels emit `MISSING_ANCHOR`. Monthly totals are exact exit-month reconstructions from canonical row-level trades and appear only in gitignored local artifacts; manifest/RESULTS/detail outputs retain hashes, basis, bucket count, status and residuals but no monthly map. Independent commission evidence is `AMENDED_OUT` because no independent total exists; derived commission is inventory and fee-audit behavior is unchanged. Present scalar mismatches emit `TV_SUMMARY_MISMATCH` blockers while retaining accounting. `summary_reconciliation_status` can be `COMPLETE` only with every active scalar anchor; the Phase 1 context cap remains `NEEDS_CONTEXT` until fresh panels arrive. Calendar and anchor hashes are frozen from the exact bytes parsed, not a later reread.
