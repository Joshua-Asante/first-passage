...
Supported primary captures remain reviewed yearly JSON extracts with exact `{year, source_url, page_date, rows}` keys and `{date, deadline_local}` rows; raw PDF/HTML needs a separately reviewed extraction step. The D19 accepted-secondary wrapper is the checked-in path for the next generation: it covers 2022-09-01 through 2026-09-02 with 40 verified in-span venue-date rows from the source calendar's 49-date inventory and preserves `SECONDARY` provenance. Calendar `COMPLETE` here means only accepted venue-date membership; it neither establishes primary historical truth nor changes the unresolved product-close and exchange-session limitations. The prior 49-date generated artifacts remain deliberately stale until N6's single new freeze.
...
7. The next N6 freeze will audit all five ledgers against D19's 40-date in-span secondary venue-date membership (from the 49-date source inventory) as calendar `COMPLETE`; full-closure dates and product close-time/exchange-session claims remain excluded. The prior 49-date ledger observations are deliberately stale until then.
