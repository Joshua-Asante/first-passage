...
| 2026-08-21 | **Cross-reference only -- no content in this spec changed.** The spawnable roster this document describes (§5.1-§5.3) is narrowed to Front Office + CEO apex + cross-office CFO by [`docs/adr/2026-08-21-persona-hierarchy-front-office-only.md`](../../adr/2026-08-21-persona-hierarchy-front-office-only.md), ratified same day by direct operator instruction. The six retired Middle/Back-office seats' charters (§5.1-§5.2) and the §6.3 CRO hard-block mechanics remain this spec's own historical record of what was originally designed and ratified 2026-08-18/19; the new ADR is the current authority on which seats are live and how the hard-block is implemented today. Same pointer-tier-registration pattern as the 2026-08-19 row above -- this spec is not rewritten to match. | Claude Code |
