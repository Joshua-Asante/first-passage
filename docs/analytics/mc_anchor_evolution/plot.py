...
Reads data.csv in this directory and emits 4 PNGs:
  pass_trajectory.png       — pass-rate over the anchor trajectory + OANDA overlay
...
# OANDA overlay maps each *canonical* OANDA anchor onto its date-matched
# Pepperstone x position. The 2026-05-23 O9 anchor (3-strategy DJ30 v4.4) is
# omitted: it was superseded same-cycle by the 2026-05-24 O10 panel refresh
# (4-strategy v4.5 DJ30 + v1 NAS100), which is plotted at the A9 position (8).
oanda_xmap = {("2026-05-05", "O4"): 3,
              ("2026-05-08", "O5"): 4,
              ("2026-05-14", "O7"): 6,
              ("2026-05-16", "O8"): 7,
              ("2026-05-24", "O10"): 8}
# OANDA anchors intentionally NOT plotted: superseded same-cycle, or with no
# date-matched Pepperstone anchor to position against. Kept explicit so the
# guard test (tests/test_mc_anchor_chart.py) can assert every data.csv OANDA
# row is accounted for — mapped or deliberately excluded — and a future anchor
# can't silently vanish from the overlay.
oanda_excluded = {("2026-05-23", "O9")}  # superseded by O10 2026-05-24 refresh
oan = oan[[(d, aid) in oanda_xmap
           for d, aid in zip(oan["date"], oan["anchor_id"])]].reset_index(drop=True)
